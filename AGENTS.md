...
- Technical review is AI-first and autonomous. Use the current GPT/Codex session to freeze inputs, run independent model roles in fresh contexts, write hash-bound receipts, and validate them through `scripts/validate_ai_review_receipt.py`; follow `docs/AI_REVIEW_GOVERNANCE.md` and `configs/ai_review_workflows_v1.json`.
- Do not stop a workflow to request human review. Two-pass disagreement triggers a fresh third-model adjudication; abstention or unresolved evidence fails that promotion branch closed while independent work continues.
...
- Participant consent, license/rights grants, physical capture/measurement, credentials, and destructive external actions are source facts or permissions rather than review tasks. Models audit their receipts but never fabricate those facts.
