...
- 问题：YOLO 检测框几何无法稳定表达低矮未知障碍、走廊是否被切断或真实距离。
- 假设：可通行区域、相对深度或设备深度只能作为受控候选证据，需证明在真实连续序列中降低关键漏报且不放大误报和延迟。
- 实施门槛：建立独立数据/评测合同、同设备基线和 fail-closed 晋升门；不得直接替换正式 App 的 `yolo11n_fp16_320.tflite`。
