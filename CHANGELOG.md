...
- 修正入口误解：恢复原首页为 launcher；打开 App 不自动启动硬件采集或避障推理。
- 原首页“开始辅助”启动当前 A+LOCAL 硬件算法，沿用首页语音/震动开关；不再启动旧手机摄像头检测。点击“结束辅助”返回首页并停止会话。
- A+LOCAL 模型、阈值和未标定实验边界不变。

## 10.15.0 — 2026-09-23

- 硬件页面增加默认开启、可切回基础 ToF 的 A+LOCAL 实验模式；冻结 A 几何评分、LOCAL 树模型和阈值，手机离线推理，不重训、不修改研究结果。
- 实物适配使用中心 16:9 裁剪、名义视场和粗朝向、区域中心径向转轴向近似，明确显示未标定；相机失效暂停 LOCAL，ToF 失效停止判断，计算后的过期输入被拒绝。
- LOCAL 单独触发不伪造距离或目标位置，保留 UNKNOWN；留证/回放保存实际模式状态与原判决。

## 10.14.0 — 2026-09-23

- 新增最近最多 20 秒 / 256 帧的本地留证：保存原始区域、判决、流状态、时龄和语音入队/震动请求。默认不保存画面，可开启最多 2 Hz 的 320×240 抽帧。
- App 内支持列表、时间轴、播放/暂停、原判决回放、手动导出和确认删除；回放无语音/震动，页面重建后恢复为暂停回放。记录位于私有不备份目录，不自动上传或删除。
- Wi-Fi 相机和 ToF 分别过期：相机不可用时明确进入仅测距模式；ToF 不可用时只显示新鲜画面，不继续障碍判决。
- 新增背景/中心/左/右四步安装粗检查，每步至少 5 个新帧；只报告声明位置与测距变化的一致性，不标定视场、不修改判决参数。

## 10.13.0 — 2026-09-23

- App 启动优先进入实物前视感知，原首页保留在“更多功能”；离线时可直接重新发现并连接。
- 障碍首次触发立即提醒，持续触发每 12 秒提醒；测距较上次提醒缩短至少 300 mm 且间隔至少 1 秒时追加提示。短暂非触发不重复启动事件，不播报安全或可通行。
- 有效实时数据中断只提醒一次，恢复时明确提示；回放不发语音或震动，离开页面停止反馈。
- 语音和震动独立开关并保存设置。基础 ToF 几何规则、固件、10 Hz 采样与数据过期门槛不变。

## 10.12.1 — 2026-09-23

- ToF 按用户要求固定 8×8 / 10 Hz，改为租约式 UDP 最新样本主动推送，取消高频 HTTP 取数。
- 显示取数跟随屏幕刷新，增加相机采集、传输、解码及复制耗时诊断；VGA 画质和算法不变。
- 热点 45 秒测试 1300/1300 快照有效；ToF 时龄上界中位数/P95 为 71/132 ms，读出至手机接收上界中位数 14 ms。

## 10.12.0 — 2026-09-23

- 相机与 8×8 ToF 可加入手机热点，App 自动发现并直接接收两路数据，电脑退出运行链路。
- 展示页采用石墨黑主视图，相机与测距为主体，原始热图和连接配置默认收起。
- 保留基础 ToF 算法、UNKNOWN 和回放静音；增加无线时龄、重启及失效处理。热点传输是局域网明文演示协议。
