...
def main():
    ...
    if not args.worker:
        ...
        write(folder / 'request.json', dict(command=command, cwd=os.getcwd(),
              ...
              environment={key: value for key, value in os.environ.items() if key.upper() in ['PATH','TEMP','TMP','VIRTUAL_ENV','PYTHONUTF8','OMP_NUM_THREADS','MKL_NUM_THREADS','PIP_CACHE_DIR','HF_HOME','TORCH_HOME','GRADLE_USER_HOME','ANDROID_HOME','ANDROID_USER_HOME','ANDROID_SDK_ROOT','JAVA_HOME','CUDA_PATH','CUPY_CACHE_DIR','NUMBA_CACHE_DIR','XDG_CACHE_HOME','MPLCONFIGDIR','YOLO_CONFIG_DIR','NPM_CONFIG_CACHE'] or key.startswith('BLINDASSIST_')}))
