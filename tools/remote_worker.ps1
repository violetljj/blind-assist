...
$prefix = "`$ErrorActionPreference='Stop'; `$ProgressPreference='SilentlyContinue'; . '$enter'; "
if ($Action -eq 'Exec') {
...
[ordered]@{ host=$env:COMPUTERNAME; source=$source; revision=$head; dirty=@($dirty); artifacts=$env:BLINDASSIST_ARTIFACTS; python=(Get-Command python).Source; freeMemoryMiB=[math]::Round($os.FreePhysicalMemory/1024); freeDiskBytes=(Get-PSDrive -Name ([IO.Path]::GetPathRoot($source).Substring(0,1))).Free; gpu=$gpu; gpuProbeExit=$gpuExit; jobs=$tasks } | ConvertTo-Json -Depth 5
'@
}
$encoded = [Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($prefix + $body))
& ssh -o BatchMode=yes -o StrictHostKeyChecking=yes -o ConnectTimeout=10 -o IdentitiesOnly=yes -i $config.identityFile "$($config.user)@$($config.host)" "$($config.remotePowerShell) -NoProfile -EncodedCommand $encoded"
