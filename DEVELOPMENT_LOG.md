# Development Log
## 2026-07-26
- 时间：2026-07-26；执行者：Codex。按明确授权和设计锁 `3fcc21e2…52bac` 实现唯一 `OBSERVABLE_THREE_FRAME_SUPPORT_MANAGER_R0`，锁定代码、16 项确定性/oracle-firewall/support-hull 负回归测试、Python/OpenCV 环境、receipt schema 与固定输出后，仅在 development seeds `2000–2019` 完整运行一次原 2520-trial matrix。结果 clean 1680/1680、stress 838/840 可评价，clean coverage point/CI lower/worst cell 均 `1.0`，stress 为 `0.997619/0.992857/0.95`；clean error、rotation、FPS、stress、coverage 六个原组件全 PASS，receipt `93b4c924…214e3c` 独立复算 `VALID`。终态 `DEVELOPMENT_GATE_PASS_VALIDATION_REMAINS_UNAUTHORIZED`；未物化 sealed validation `3000–3019`，未接触真实数据或 Phase B。详见 [Development Gate 结果](docs/research/rcle/RCLE_OBSERVABLE_SUPPORT_RECOVERY_R0_DEVELOPMENT_GATE_RESULT_2026-07-26.md)。
- 时间：2026-07-26；执行者：violjjet。按新独立边界冻结 RCLE Observable Support Recovery R0 的 design-only 预注册与机器设计锁；未写算法代码、未物化或运行 formal trials、未抓取真实数据。唯一候选固定为连续三帧的 observable support manager：只用 forward-backward、photometric consistency 与 track lifecycle 判断可观测遮挡，只在原 3×3 cell 的 support/hull 不足时做确定性空间补点；禁止读取 generator occlusion mask、生成伪 correspondence、扩 canvas、降门或多候选择优。旧 seeds `1000–1019` 与 R0/R1 结果永久降为 discovery；development `2000–2019` 与 sealed validation `3000–3019` 各自冻结为原完整 2520-trial matrix。前两版纸面锁因准入和时序歧义被独立审查判 FAIL；只做纸面修订后，最终锁 `3fcc21e2…52bac` 九项复审全 PASS，状态为 `DESIGN_REVIEW_PASS / EXECUTION_NOT_AUTHORIZED`。未来开发或一次性独立验证任一原门失败即关闭，验证 PASS 也不自动开放 Phase B。详见 [设计预注册](docs/research/rcle/RCLE_OBSERVABLE_SUPPORT_RECOVERY_R0_PREREGISTRATION_2026-07-26.md)与[审查结果](docs/research/rcle/RCLE_OBSERVABLE_SUPPORT_RECOVERY_R0_DESIGN_REVIEW_RESULT_2026-07-26.md)。
- 时间：2026-07-26；执行者：violjjet。完成 RCLE-Minimal Phase A 唯一一次 implementation-only coverage revision R1。R0 `rcle_minimal/`、runner/config/tests、2520 trial manifest、20 seeds、全部输入 hash、阈值/gate、101 个负结果与 receipt `14ed23e3...041ca` 保持不变并再次复算 `VALID / REVISE`；R1 以新 package/runner/tests/output 隔离实现 multilevel backward LK cycle 与复用原 `0.75px/frame` 门的 affine consensus，67 tests 与 Python compile 通过。正式 R1 clean 从 `1661/1680` 提高至 `1680/1680`，stress 从 `758/840` 提高至 `810/840`，raw/comp affine residual abstention 降为 `0/0`；但 partial-occlusion pitch 四 cell 仅 `12/20、12/20、13/20、13/20`，worst `0.60 < 0.70`，30 trial 完整保留为 `NOT_EVALUABLE`。R1 receipt `d5edb952...2b15` 独立复算 2520 trials 为 `REVISE / VALID`；按冻结的单次 revision 仍失败即停止语义，研究终态为 `STOP_CURRENT_IMPLEMENTATION / VALID`，禁止第二次 coverage revision、降门、Phase B/Replay/Bonn/Android/安全或生产后继。详见 [Coverage Revision R1 结果](docs/research/rcle/RCLE_MINIMAL_PHASE_A_COVERAGE_REVISION_R1_RESULT_2026-07-26.md)。
- 时间：2026-07-26；执行者：violjjet。预注册、实现并完整运行 RCLE-Minimal Phase A Synthetic Signal Audit R0。结果前冻结 2520 trials、20 seeds、15/30/60 FPS、yaw/pitch/roll、scale up/down、rotation+scale、noise/blur/partial occlusion、trial-level seed-cluster bootstrap 与数值 Kill Gate A，protocol SHA-256 `d20e77f3...1b502`。新增 source-known rotation warp、Sparse LK、3×3 local affine `e=0.5 trace(A)`、RSR/CRR 分母保护、可视化、runtime、不可评价与 hash-bound receipt；16 项 Phase A tests 和 Python compile 通过。正式结果 clean rotation/closing/FPS 与 stress 可评价误差门均通过，但 clean worst-cell coverage `10/20`、partial-occlusion pitch worst-cell `0/20`，总体 `2419/2520` 不得覆盖最差 cell，终态 `REVISE / VALID`；receipt 独立复算 2520 trials，SHA-256 `14ed23e3...041ca`。结果包在 `artifacts.local/{evidence,datasets}/rcle_minimal_r0/formal_run_r0/`；Phase B、Replay Demo、深度、双区域、Android 与生产权限仍关闭。详见 [Phase A R0 结果](docs/research/rcle/RCLE_MINIMAL_PHASE_A_SYNTHETIC_SIGNAL_AUDIT_R0_RESULT_2026-07-26.md)。
## 2026-07-25
