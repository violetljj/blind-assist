...
- [RCLE 研究主线](research/rcle/README.md)：`current`；R1.0 长期能力地图、R1.1 Phase A R0/R1 终态、Observable Support Recovery R0 development + independent sealed validation `PASS / VALID / SYNTHETIC_EVIDENCE_ONLY`、Phase B Bonn metadata-only 入口设计锁和旧 USTRF/Looming 证据关系的唯一仓库入口。
