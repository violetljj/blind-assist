...
- USTRF-SC 已把 route-conditioned、object-agnostic risk field 设为优先研究主线；P0 shared decision、U0 v2 unified runner，以及 `baseline_yolo_geometry` 和 `detector_bbox_explicit_route` 两条真实 Android adapter 执行链现已就绪。第二臂在设备内因果选择显式路线并以固定走廊筛选原始 YOLO bbox；最终内核 APK 的 SM-S9280/API 36 负控仍在相同 encoded sample/RGBA 下使同一 person 从排除/raw `NONE` 变为保留/raw `MEDIUM`，复跑稳定。dense→shared-kernel seam 也已在真机冻结：禁止 bbox/distance/lifecycle 注入，保留统一 temporal/event/feedback，并由 admission 绑定 teacher provenance、LOSO fold 和逐帧 field/归一化算术；但 teacher field generator 与四个 dense/control 真 adapter 尚未实现，所以 seam 不计作第三臂。`120 episode / 60 matched pair` 人类真值仍为 0，设备米制几何 admission 仍为 false，不运行有效 U0、不训练 student、不接入 App、不替换默认模型。详见 [ROUTE_CONDITIONED_OBJECT_AGNOSTIC_RISK_FIELD_PLAN_2026-07-20.md](ROUTE_CONDITIONED_OBJECT_AGNOSTIC_RISK_FIELD_PLAN_2026-07-20.md)。
