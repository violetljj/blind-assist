...
放量 v3 已完成并通过主判定（v1、v2 失败记录保留）。待决：“确认畅通距离”方向（规则修正诊断后的建议：只作辅助可确认性地图，承诺改为 ≥10 cm、ρ≥0.5、最坏摆放；暂不正式检验，见 [CNH_CLEARANCE_RULEFIX_DEV_20260926](../research/active/dtr-r0/nearfield/CNH_CLEARANCE_RULEFIX_DEV_20260926.md)）；L8CH 桌面标定时机；头部近处是否启动相机线。真实传感器计数/串扰与安装参数仍待标定；算法未达标不能推断物理信息上限。RGB线和实机方案另行决定。
