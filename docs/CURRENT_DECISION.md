...
放量 v3 已完成并通过主判定（v1、v2 失败记录保留）。待决：“确认畅通距离”方向（建议：可确认性地图 + 按子区域确认畅通距离，新 audit 批次正式检验）；L8CH 桌面标定时机；头部近处是否启动相机线。真实传感器计数/串扰与安装参数仍待标定；算法未达标不能推断物理信息上限。RGB线和实机方案另行决定。
