# Current decisions: L10 active observation; DTR public and CARLA lines separated

Status: `L10_R0_ACTIVE / DTR_R2_DYNAMIC_RETAINED`

- **L10 core/ordinal line:** the seek-guide-reacquire controller and causal
  handoff guard remain implemented. The later SceneFun3D ordinal search is
  terminal at `SC40_NOT_EVALUABLE_NO_FRESH_DEPTH_VISIBLE_ACTIVE_VIEW`.
- **L10 active observation:** PanoLab passed
...
- **DTR CARLA line:** X24 met its same-source C2 Development gate; X26 and X30
  did not meet their frozen gates. X31 is frozen, C8--C10 are terminal
  `SOURCE_NOT_EVALUABLE` before any X31 metric, and the CARLA C11 source is
  frozen but `NOT_RUN` pending its one fresh solid-body capture.
