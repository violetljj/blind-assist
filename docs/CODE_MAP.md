...
| AG-DUE gap-driven source prescreen、claim-bound evidence、SANPO manifest/result/source-audit lock 与数据角色防火墙 | [`assistive_geometry_data_upgrade/README.md`](../scripts/research/assistive_geometry_data_upgrade/README.md) | `validate_due_r0.py`、`validate_due_sanpo_manifest_lock.py`、`validate_due_sanpo_prescreen_result.py`、`validate_due_sanpo_synthetic_r1_protocol.py` 及 tests；metadata-only `PRESCREEN_ADMIT/PARTIAL` 或 source inventory 不建立 source data support、DCA PASS 或执行权限 |
