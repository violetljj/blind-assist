...
当前专用评测集主要由 150 张 COCO 静态图组成，风险字段来自检测框几何自动标注，仍需要人工逐张复核；连续帧逼近证据不足。设备回归脚本主要负责安装、启动和采集日志/截图，只要 ADB 命令成功就可能写入 `status=passed`，没有充分断言相机预览、有效帧、模型 ready、无 crash/ANR 和真实提醒行为。
