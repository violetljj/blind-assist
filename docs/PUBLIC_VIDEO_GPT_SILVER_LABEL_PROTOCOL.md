...
它解决的是“自动获得可检验信号并开展暂定训练”，不是把模型输出误称为客观传感器事实或真人用户效果。旧 `v1` 银标保持仅对照；新的 `v2` 在公开下载来源、哈希绑定和多帧时序审阅齐全时，可授权 `provisional_model_supervision` 隔离训练，许可和隐私状态作为限制字段保留而非前置门。无论版本，单路银标都不得独立用于校准、blind 评测或替换默认模型；冻结的独立双模型共识/裁决证据可按总治理进入研究和冻结评测。

## 可用来源与前置条件

1. 只要素材能通过不规避认证、付费或访问控制的普通公开渠道下载，就可立即保存并使用；至少记录来源 URL、抓取时间、本地 SHA256，许可或作者信息在页面提供时一并记录。
2. 对含人物的公开 RGB，机器隐私检查与脱敏可以在下载后并行执行；`privacy_audit_required=true` 作为限制披露保留，但不阻止本地隔离筛选、银标或暂定训练。原始素材对外上传、共享/再分发或生产使用前再处理对应边界。
...
非弃权标签还要有风险轮廓；所有标签必须绑定证据帧 SHA256、模型/提示词版本、置信度和不确定性理由。`validate_public_video_silver_labels.py` 会拒绝未知帧、单帧“时序”判断，以及任何放开校准、blind 评测或生产替换的授权。`v2` 只额外要求 hash-bound 公开来源记录和 `provisional_training_authorized=true`；许可证缺失或未核验只写入 limitation，不阻止隔离训练。
...
冻结输入上的独立双模型共识或第三模型裁决，是把候选一致性升级为研究质量指标的自动化依据；它仍不是客观传感器事实、安全认证或真人用户效果证据。
