...
本项目默认由 GPT/VLM 在已取得许可、已机器脱敏的公共连续第一视角视频上完成候选筛选、风险判断和生命周期草案，并将其与 BlindAssist 端侧输出逐 episode 对照；不建立人工复核 fallback。

它解决的是“自动获得可检验信号并开展暂定训练”，不是把模型输出误称为客观传感器事实或真人用户效果。旧 `v1` 银标保持仅对照；新的 `v2` 在 CC-BY/CC0 来源、哈希绑定、多帧时序审阅与机器隐私处理记录齐全时，可授权 `provisional_model_supervision` 训练。无论版本，单路银标都不得独立用于校准、blind 评测或替换默认模型；冻结的独立双模型共识/裁决证据可按总治理进入研究和冻结评测。
