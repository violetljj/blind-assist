...
1. 优先顺序为：合法公开/已授权来源的自动发现与下载、现有设备的自动采集脚本、仿真/合成生成、模型生成与自动补采。不得把“安排人员拍摄、搬动物体、逐张处理或现场记录”设为默认下一步。
2. 采集 Agent 必须保存来源、许可/同意凭证引用、下载或设备 receipt、时间、配置和内容哈希。物理设备实际测得的数据可以作为设备证据；模型生成或推断的数据必须明确标为 proxy/synthetic/model-generated。
3. 自动隐私检测、脱敏、语义筛选和质量检查失败时，样本自动拒收或隔离；不得进入人工待办。
4. 法律同意、许可证/权利授予、账号凭据和真实设备测量不得伪造。缺少这些外部事实时，应换用合法自动来源、缩小证据声明，或记录 unavailable，而不是要求人工补审。
...
| --- | --- | --- | --- |
| `autonomous_source_acquisition_v1` | 来源内容/连续性/隐私风险 | 许可、来源、下载/设备 receipt 与哈希 | 自动采集准入 |
...
参与者同意、许可证或权利授予、签名凭据来自真实主体，物理测量来自真实设备。GPT/Codex 可以通过授权自动化获取或检查收据是否存在、是否一致、是否满足政策，但不能代替主体同意、凭空生成测量或冒充凭据持有人。此限制不会触发任何人工任务；事实缺失时换用合法自动来源，或标为证据缺失并让对应分支失败关闭。
