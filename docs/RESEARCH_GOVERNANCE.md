...
`WILD_LAB` 不是降低科学要求，而是把科学要求放在正确位置：早期实验只需问题/假设、
可信 baseline、一个有意义变化、可观察指标/决策和停止条件；复用外部成果时保留
来源、许可并区分继承部分与新贡献。它不因缺少最终
...
失败不可怕；能排除假设或指出下一机制的负结果就是有效进展。早期负结果只需记录
观察、被削弱的假设和下一动作；只有改变主线判断或形成正式结论的失败才展开完整
learning record。禁止仅记录“失败”后无依据地更换参数或开启组合。

大模型可以高效率地生成和测试多种数据、公式、信号与算法方案，但不能把无目的穷举
当作研究。优先进行能够快速区分路线可行性的单变量、条件交互、消融和反事实实验；
只有成本会影响决策时才额外写资源预算。
...
不会改变下一决策的 canary 不需要另建 round 文档、锁、receipt 或复审层。
...
早期 Development 的最小交付就是问题/假设、可信 baseline、一个有意义变化、可观察
指标/决策和停止条件。只有结果进入 durable candidate/claim 时，再补必要的代码/模型/
主配置/数据身份、来源与许可、限制和下一步。默认不要求每轮表/图/demo、逐文件 SHA、
完整 hash chain、一次性 holdout、底层逐行独立全量复算、
