...
执行范围：本页保留既有 oracle→移动模型基线及其历史决策，不是所有新论文实验的强制
顺序。新的 R4 `THESIS_DEVELOPMENT` 可直接比较明确声明的模型或候选算子；只要不声称
生产晋级，就不要求先闭合本页全部 oracle、INT8 或设备门。
...
盲道占用是已记录但未伪造的缺口：本轮未找到同时满足“连续、可小规模下载、明确盲道被占用”的公开序列。后续只要 VIP-Mobility360 或其他来源可经普通公开渠道取得，就可直接拉取小分片并进入隔离研究；仍必须使用独立 importer 与同样的几何、review/finalize 技术门禁，许可/隐私未知项只作限制披露。
