...
| 分类 | 当前重点 | 入口 |
|---|---|---|
| 算法 | 当前主线是 BlindAssist Assistive Geometry R2 因子化几何；SuperTeacher→learned-factor V2 seam 已落地，Attempt17 与无锚 walking_xyz 负结果保持冻结。跨传感器 F2 calibration-control R0 one-shot 保持 fail-closed consumed。R1 以 ETH3D `/uvc_camera/cam_2` 与 Kalibr same-node `rostopic` 为选择依据，并已补齐先消费的 independent replay receipt、全 failure 重放、完整 hash chain 与 downstream Confirmation binding；69/69 专项门 PASS。真实 R1 archive 尚未访问，科学状态仍 `NOT_RUN`。用户已授权当前唯一动作：提交锁前实现后，各执行一次 R1 producer/replay；不得重跑 R0 或越级访问 session/model/Confirmation。TARO 保持独立并行 WILD_LAB，B1-A0、AG-QSF 与 AG-CBF R0 保持各自终态 | [算法研究入口](ALGORITHM_RESEARCH_CURRENT.md) |
