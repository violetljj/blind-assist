...
| 分类 | 当前重点 | 入口 |
|---|---|---|
| 算法 | 当前主线是 BlindAssist Assistive Geometry R2 因子化几何假设；B1-A0、AG-QSF 与并行 AG-CBF R0 均已到各自负终态，AG-CBF 因 TRAIN grid data support 不足在 oracle 前关闭；DepthART-S 是可替换 encoder/initialization、depth baseline 与部署使能线；YOLO + 语义分割双环是论文次线 | [算法研究入口](ALGORITHM_RESEARCH_CURRENT.md) |
