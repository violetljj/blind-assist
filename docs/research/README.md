...
| 分类 | 当前重点 | 入口 |
|---|---|---|
| 算法 | 当前主线是 BlindAssist Assistive Geometry R2 因子化几何；SuperTeacher→learned-factor V2 seam 已落地，Attempt17 与无锚 walking_xyz 负结果保持冻结。独立跨传感器 F2 的 activation blocker 已由非执行 repair implementation 关闭：schema v2 exact-bind Kalibr YAML/camera node/IMU convention 与控制 evidence，29-file DepthART runtime source manifest 已独立复核，calibration-control-only one-shot 与 producer-free validator 通过 synthetic gates。真实 archive/member、模型与 Confirmation 仍未执行；下一步只能另行授权 calibration archive control lock，不能越级访问三个 session 或运行模型。TARO 保持独立并行 WILD_LAB，B1-A0、AG-QSF 与 AG-CBF R0 保持各自终态 | [算法研究入口](ALGORITHM_RESEARCH_CURRENT.md) |
