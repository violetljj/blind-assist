...
| 分类 | 当前重点 | 入口 |
|---|---|---|
| 算法 | 当前主线是 BlindAssist Assistive Geometry R2 因子化几何；SuperTeacher→learned-factor V2 seam 已落地，Attempt17 与无锚 walking_xyz 负结果保持冻结。跨传感器 calibration-control R0/R1 one-shot 均已 fail-closed consumed。R1 producer 在读取 2 个 YAML 后以 `F2_R1_KALIBR_ROSTOPIC` 停止，independent replay 一次性复现同一失败并完成六文件 hash-chain 验签；discovery/match count 保持 `null/UNKNOWN`。session/model/truth/scoring/Confirmation 均未运行，科学状态仍 `NOT_RUN`，当前无 active successor。TARO 保持独立并行 WILD_LAB，B1-A0、AG-QSF 与 AG-CBF R0 保持各自终态 | [算法研究入口](ALGORITHM_RESEARCH_CURRENT.md) |
