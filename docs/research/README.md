...
| 分类 | 当前重点 | 入口 |
|---|---|---|
| 算法 | 当前主线是 BlindAssist Assistive Geometry R2 因子化几何；SuperTeacher→learned-factor V2 seam 已落地，Attempt17 与无锚 walking_xyz 负结果保持冻结。跨传感器 F2 calibration-control R0 one-shot 保持 fail-closed consumed；其三文件 failure observability gap 已只读确认。R1 已冻结以 ETH3D `/uvc_camera/cam_2` 与 Kalibr same-node `rostopic` 为依据的选择规则及 count-preserving failure evidence，58/58 synthetic gates PASS；真实 R1 archive 未访问，科学状态仍 `NOT_RUN`。下一步只能另行授权 R1 calibration-control one-shot，不能重跑 R0 或越级访问 session/model。TARO 保持独立并行 WILD_LAB，B1-A0、AG-QSF 与 AG-CBF R0 保持各自终态 | [算法研究入口](ALGORITHM_RESEARCH_CURRENT.md) |
