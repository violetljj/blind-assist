...
| Assistive Geometry gap-driven source upgrade | `AG-DUE_R0_SANPO_INITIAL_STATIC_PRESCREEN_COMPLETE_BOTH_PARTIAL / SOURCE_SUPPORT_FALSE / PAYLOAD_NOT_AUTHORIZED` | [AG-DUE current](assistive-geometry-data-upgrade/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_DUE_R1_SANPO_SYNTHETIC_SOURCE_SPECIFIC_INTEGRITY_AND_CAPABILITY_AUDIT_PROTOCOL_LOCK`：只锁 Synthetic 来源审计协议；不得执行审计、刷新 metadata、打开 payload、调用 Teacher 或训练 |
