...
| BA-ADT Goal Episode mining | `REVERSIBLE_EXPLORATION / ADT_0_SAMPLE_MINED_PARTIAL_EVENT_COVERAGE` | [Goal Copilot current](goal-copilot/README.md)；[sample result](goal-copilot/BA_ADT_REAL_EVIDENCE_ADT0_SAMPLE_RESULT.md)；实现见 [`ba_adt_real_evidence`](../../scripts/research/ba_adt_real_evidence/README.md) | 固定 sample miner/门槛，对少量完整 sequence 做 GT-only mining；GT 与未来 RGB estimator 严格分离 |
