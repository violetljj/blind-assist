...
| Assistive Geometry gap-driven source upgrade | `AG-DUE_R0_SANPO_INITIAL_SOURCE_MANIFESTS_LOCKED / NOT_PRESCREENED / NO_PAYLOAD_OR_TEACHER_AUTHORITY` | [AG-DUE current](assistive-geometry-data-upgrade/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_DUE_R0_SANPO_INITIAL_MANIFEST_STATIC_PRESCREEN_EXECUTION`：只执行两份 byte/hash-locked 零计数 manifest 的静态判定；不得刷新 metadata、打开 payload、调用 Teacher 或训练 |
