...
| Assistive Geometry gap-driven source upgrade | `AG-DUE_R0_GAP_PROTOCOL_LOCKED / NO_SOURCE_MANIFEST_LOCKED / NO_PAYLOAD_OR_TEACHER_AUTHORITY` | [AG-DUE current](assistive-geometry-data-upgrade/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_DUE_R0_INITIAL_SOURCE_MANIFEST_LOCK`：只锁 SANPO Real/Synthetic 或明确公开候选源的 metadata manifest；不得打开 payload、调用 Teacher 或训练 |
