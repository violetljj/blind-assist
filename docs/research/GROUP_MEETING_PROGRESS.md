...
当前研究主线：路线条件化、对象无关的连续助盲风险感知与事件闭环
...
工程、实验和文档产出量大致相当于一名熟练硕士生全职 6—9 个人月；扣除平台搭建、自动化生成、代理标签、合成/oracle 证据和未闭合的真实事件真值后，已经形成的论文级研究进展更接近 3—5 个月。当前阶段最需要的不是继续增加实验分支，而是固定问题、补真实连续事件、完成六臂消融和统计验证。
...
这条问题能够解释五月以来的全部工作。早期 App 解决“检测结果怎样转成用户可理解的提醒”；EvalSet 和同设备 A/B 解决“模型是否真的改善助行相关指标”；SANPO 将研究对象从 COCO 类别扩展到自由空间、边界和连续事件；显式路线实验表明风险必须相对于用户打算经过的走廊判断；USTRF-SC 再把检测框、稠密几何、动态接近、不确定性和事件生命周期放进同一证据链。
...
| 次数 | 汇报题目 | 核心证据 | 应形成的结论 |
| --- | --- | --- | --- |
| 1 | 从目标检测到助盲提醒 | 五月原型、风险规则、真机链路 | 检测结果需要风险语义转换 |
| 2 | 多帧稳定与可解释反馈 | 稳定器、SessionTrace、播报策略 | 单帧命中不能直接驱动反馈 |
| 3 | 无障碍与真实使用流程 | TalkBack、大字体、权限与模式 | 算法与交互必须分别验证 |
| 4 | 实时视觉系统可靠性 | 多模块、Hilt、CameraX、故障注入 | 生命周期错误可能成为安全错误 |
| 5 | EvalSet 与检测器 A/B | 150 图、YOLO11n/26n、延迟 | 更快模型不一定更适合助行 |
| 6 | 深度融合为何被否决 | MiDaS A/B、误报和延迟 | 局部漏报改善不足以晋级 |
| 7 | 项目审计与安全语义 | RiskEvidenceState、session token | “未检测到”不能表达成“安全” |
| 8 | SANPO 连续序列与 oracle | 30/90 帧、curb 误报、事件闭环 | 分析单位应从帧转为事件 |
| 9 | SANPO 数据与训练协议 | 14 session、blind、hash、等价门 | 数据治理是可复现实验的一部分 |
| 10 | 多 seed 与 P0/P1/P2 消融 | seed 范围、worst scene、负结果 | 最佳 seed 不代表稳定能力 |
| 11 | 公开银标、反事实与路线意图 | DINO/IMU 负结果、显式 route teacher | 路线是比继续堆视觉 head 更强的变量 |
| 12 | USTRF-SC 风险场与安全内核 | risk field、capsule、fail-closed、V13 | 从类别检测转向对象无关路线风险 |
| 13 | 跨相机连续事件门 | R1.2a/b、性能与事件分离 | 延迟通过不等于事件效果通过 |
| 14 | 小目标检测受控负结果 | R1.2d 三 seed | small recall 提升不等于事件收益 |
| 15 | RGB-D/pose replay 与设备停止门 | R2/R3、ARCore、LILocBench | transport 通过不等于真值准入 |
| 16 | 正式六臂、统计与下一阶段 | H1—H5、LOSO、停止门 | 固定可证伪的论文实验协议 |
...
下一阶段的优先级固定为：完成第三个独立且满足协议的 RGB-D+pose 来源；补齐真实路线正负 matched event；冻结 120 episode/60 pair 与 LOSO inventory；实现四条剩余 dense/control adapter；运行六臂主比较和预注册统计；只有研究门通过后才进入 INT8 和同机连续事件 shadow。

不再把新增 detector、提高分辨率、继续 tiling、单个 best seed 或扩大同一视频帧数作为主线进展。R1.2d 已经提供反证：small-object recall 提高而事件结果不变且假检增加。未来的有效工作量应体现在独立来源、真实事件分母、受控变量、可复算统计和停止规则上。
