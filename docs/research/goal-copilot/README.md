...
状态：`current / PRODUCT_AND_RESEARCH_MAINLINE / PUBLIC_IDENTIFIABLE_REFERENT_C2_SMALL_ROSTER_MATERIALIZABLE / VISIBLE_ONLY_20_FOUND_16_SAME_INSTANCE_4_DISTRACTOR_1_ABSTAIN / DINOV2_LOCAL_13_OF_17 / TWO_REFERENCE_ZERO_RESCUE_THREE_COLLATERAL / TLESS_DINO_BASELINE_27_OF_30 / PDM_RESCUE_1_COLLATERAL_4_CONTROL_RETENTION_4_OF_6 / PDM_UNARY_REJECTED / IDENTITY_FAILURE_AUDIT_NEARID_JUSTIFIED / NEAR_IDENTITY_HARD_NEGATIVE_UNARY_V0_PROTOCOL_FROZEN_NO_OUTCOME / RELIABLE_VERIFIER_NOT_ESTABLISHED / NO_BELIEF / NO_P1 / DEFAULT_APP_UNCHANGED`
...
只读 [`identity failure-layer audit`](PUBLIC_IDENTIFIABLE_REFERENT_IDENTITY_FAILURE_LAYER_AUDIT_V0_RESULT_2026-08-24.md) 未调用模型或生成新图像；12 对为 `REPRESENTATION_COLLAPSE=4 / LOCAL_LAYOUT_LOST=3 / UNKNOWN=5`，唯一终态 `LEARNED_NEAR_IDENTITY_REPRESENTATION_JUSTIFIED`。现已另行授权并冻结 [`NearID-style unary V0 protocol`](NEAR_IDENTITY_HARD_NEGATIVE_UNARY_V0_PROTOCOL_2026-08-24.md)，但本状态点尚未下载数据、训练或产生 outcome；layout 仍不预先 fusion。
