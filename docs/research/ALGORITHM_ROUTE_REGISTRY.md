...
| HFTF 未来可通行场 | 研究连续 clearance/traversability field 与深度/时序候选的可学习性 | `DEVELOPMENT_STANDARD / DEPTHART_R0_FAIL / R1_A3_BLOCKED`；独立支线 | [HFTF README](hftf/README.md) | 新鲜、父事件隔离的数据下，另立一个有明确 causal difference 的候选；否则保持 diagnostic | 不把 teacher、HTP、合成或移动端质量结果升级为主线/App/安全 authority | 否 |
