...
状态：`current`。本表是跨研究域的短入口；当前项目研究中心是 DepthART 及其配套的数据、链路和性能工作。详细协议、结果和历史过程仍由各路线唯一真源维护。

## 当前项目研究中心

`DepthART-centered research program`：以 DepthART-S 为当前算法候选，同时研究支撑它的
数据集/数据角色、通信链路与端到端延迟、CPU/GPU/HTP 性能、内存/热/稳定性和部署可行性。
这些是同一项目研究计划的不同 workstream，不应全部归类为“算法搜索”，也不自动获得默认 App 权限。

| 路线 | 主张 | 当前状态 | 唯一真源 | 下一动作 / 唯一 successor | 禁止动作 | 是否影响默认 App |
|---|---|---|---|---|---|---|
| DepthART 项目研究中心 | 以 DepthART-S 为算法候选，并联合推进数据、链路、性能和部署研究 | `CURRENT_PROGRAM / DEVELOPMENT_STANDARD / DEFAULT_APP_UNCHANGED` | [HFTF README](hftf/README.md) + 本表 | 按 workstream 分别推进：新 parent-disjoint admission、数据合同、链路延迟分解、端到端性能优化 | 不把任一 workstream 单独写成完整算法 admission、生产或安全结论 | 否 |
...
| DepthART 数据集与数据治理 | 建立 parent/session 隔离、truth、coverage、质量和可复用数据角色 | `DEVELOPMENT_DATA_WORKSTREAM` | [HFTF README](hftf/README.md) + 各 DepthART protocol | 先修数据身份/质量/独立性，再消费 admission holdout；形成新的 data contract successor | 不用候选输出反推 truth、不把 candidate pool 当 event truth、不重用被选择污染的数据作 confirmation | 否 |
| DepthART 通信链路与延迟 | 分解外设/网络/拷贝/排队/推理/反馈链路，定位真实端到端瓶颈 | `DEVELOPMENT_PLATFORM_WORKSTREAM` | [HFTF README](hftf/README.md) + 对应 timing result | 先做 bounded smoke 和单变量 attribution，再做链路优化；优化结果只说明 measured path | 不把吞吐、RTT 或 accelerator occupancy 写成准确率、安全或 App 全链路 authority | 否 |
| DepthART 性能与部署优化 | 优化 CPU/GPU/HTP、内存、热、稳定性、模型导出和 runtime 可行性 | `DEVELOPMENT_PLATFORM_WORKSTREAM / A3_BLOCKED_SELECTIVESCAN` | [DepthART R1 result](hftf/DEPTHART_ADMISSION_R1_RESULT_2026-08-07.md) | numerical parity → operator/lowering feasibility → bounded backend benchmark → 再评估设备集成 | 不在科学 admission 前做无边界 quantization/partition 搜索，不把平台 benchmark 伪装成模型质量 | 否 |
