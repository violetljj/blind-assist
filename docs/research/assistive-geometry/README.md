...
状态：`current / RESEARCH_MAINLINE / B1_DUAL_ORIENTATION_PROTOCOL_LOCK_PASS / IMPLEMENTATION_NOT_AUTHORIZED / FORMAL_TRAINING_NOT_AUTHORIZED`
...
Attempt 2 冻结 dual-shape full-FOV、orientation bucket 和重新平衡的 Development split。下一步
只物化 TRAIN target cache、实现 dual-shape shared decoder/head/loss 并关闭 smoke；此前不训练
student、不读取 DEVELOPMENT/CONFIRMATION outcome、不运行双教师，也不改默认 App。
