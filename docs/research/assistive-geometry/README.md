...
状态：`current / RESEARCH_MAINLINE / B1_PROTOCOL_LOCK_PASS / IMPLEMENTATION_NOT_AUTHORIZED / FORMAL_TRAINING_NOT_AUTHORIZED`
...
identity 在 outcome 前拆成 4 calibration / 4 selection。下一步只物化 TRAIN target cache、实现
shared decoder/head/loss 并关闭 forward/backward/flip/resume smoke；implementation lock 前不训练
student、不读取 DEVELOPMENT/CONFIRMATION outcome、不启动 DA3 + Metric3D 双教师，也不改默认 App。
