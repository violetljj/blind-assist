...
| 部署可行性 | ONNX/QNN graph、operator/lowering、backend 和设备约束 | `HTP_V73_AND_AARCH64_PACKAGE_COMPILED / RUNTIME_NOT_EVALUATED`（DepthART A3） | [DepthART A3 result](hftf/DEPTHART_ADMISSION_R1_A3_RESULT_2026-08-07.md) | `NUMERICAL_PARITY_AND_HTP_KERNEL_SUCCESSOR`：先用 primitive oracle 完成算子 parity，再进入 QNN context/device backend 评估；编译成功本身不是 HTP 执行证据 |
