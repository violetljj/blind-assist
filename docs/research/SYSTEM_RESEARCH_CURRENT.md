...
| 部署可行性 | ONNX/QNN graph、operator/lowering、backend 和设备约束 | `STRICT_G4D_NEGATIVE_TERMINAL / D0_NO_ELIGIBLE_PRECISION_ARM / D1_HEADER_80_OF_80_PASS_BODY_PREFLIGHT_FROZEN_NOT_RUN / R2_CANDIDATE_NOT_SELECTED`（DepthART） | [DepthART current](hftf/README.md)；[A3 result](hftf/DEPTHART_ADMISSION_R1_A3_RESULT_2026-08-07.md)；[D0 terminal](hftf/DEPTHART_TASK_PRESERVING_D0_PRECISION_SCREEN_RESULT_2026-08-09.md) | 执行 D1 label-blind 300-frame portrait/pose/RGB-D/K body preflight；通过后才重建 `608×448` fixed-mixed 单候选，不得计算 task outcome、修改 D0 或访问 R2 cohort 选模 |
