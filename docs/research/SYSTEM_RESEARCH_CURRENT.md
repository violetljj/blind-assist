...
| 部署可行性 | ONNX/QNN graph、operator/lowering、backend 和设备约束 | `HTP_REFERENCE_SOURCE_READY / HEXAGON_SDK_AUTH_BLOCKED`（DepthART A3） | [DepthART A3 result](hftf/DEPTHART_ADMISSION_R1_A3_RESULT_2026-08-07.md) | `NUMERICAL_PARITY_AND_HTP_KERNEL_SUCCESSOR`：QPM3 登录后安装 v73 SDK/toolchain，编译 scalar reference kernel 并用 primitive oracle 验证；之后才进入 context/device 评估 |
