...
- D3 已另立为新的 bidirectional error-certificate router，不重跑 D2 direct-state head。它分别学习 `CLEAR release` 与 `OCCUPIED veto`，证据冲突时转 `UNKNOWN_GROUND`，证据不足时保持 baseline；目标是在 fresh identity-disjoint Development 上同时降低 false-clear 与 false-block。pre-outcome 协议与纯 CPU mechanics canary 已冻结并 PASS。metadata planner 随后扫描当前工作区 1,485 个 research JSON/Markdown（含 untracked receipts），排除 394 个既有官方 identity，锁定 48 个全新 Training-fold 候选；与 D1、D2、sealed R2 显式 overlap 均为 0。source scope 已登记，但媒体 HEAD/body、truth、训练与 D3 outcome 仍未激活。
...
`EXPLICIT_D3_PHASE_A_INTRINSICS_TRAJECTORY_HEAD_ONLY_PREFLIGHT_ACTIVATION`。该门只允许对
冻结的 48 身份请求 `lowres_wide_intrinsics.zip` 与 `lowres_wide.traj` 的 HEAD，记录 status、
content-length、ETag/last-modified 与 URL receipt；不授权 GET/body、archive member、pose 内容、
truth、训练、D3 Development 或 R2。D1/D2 consumed outcome 继续不可用于拟合或回救 D3。
