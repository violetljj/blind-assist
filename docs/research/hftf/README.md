...
- D3R2 作为新版本冻结 exact-32 × exact-300、exact-64 的 coverage-only census。用户随后显式激活；activation 经受保护 master 的 PR #34 全部检查后合并，正式 r0 才开始全新下载。前 44 个资产形成连续 checkpoint 前缀；第 45 个 `44796744/lowres_depth.zip` 虽得到 HTTP 200，但流式正文长度与冻结 `Content-Length` 不一致，按 transport hard gate terminal 停止。当前保留 44 bodies / 22 paired identities / `4,223,537,610 bytes`、sealed failure 与 non-resumable marker；没有 manifest/validation，不发布 partial coverage，不运行 missing-source policy/truth/selection。终态为 `D3R2_PHASE_B_COVERAGE_CENSUS_EXECUTION_INVALID_INCOMPLETE`，`scientific_terminal=null / next_gate=null / successor=null`。r0 不可 resume、删 marker 修补、复用 partial assets 或同版本重跑；任何未来恢复须再次授权新版本、新协议和 fresh root。
- D3R3 是独立 transport-recovery version，保留 D3R2 失败但不继承其 execution authority、44 bodies、checkpoints、failure/temp 或 partial coverage。exact-32 identity 顺序、每身份 exact-300 stems、9600-stem plan 与 exact-64 URL 顺序保持不变。fresh HEAD 已 64/64 PASS：声明正文总计 `5,580,879,686` bytes，相对旧 snapshot 的 Content-Length/ETag/Last-Modified 均零漂移，redirect 与 body read 均为0；独立 validator PASS。由此把恢复假设收窄为 transient premature EOF。新 census 只在 HTTP 200 且 headers 仍匹配、实际正文短于 Content-Length 时允许删除 partial、从 byte 0 完整重试，最多3次；15/15 targeted tests PASS，fresh-root exact-64 run 已激活。它只读容器目录/member names，不读 member payload、不解码 pixel、不派生 truth/selection。
...
- [D3R2 Phase-B source-coverage recovery protocol](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-12.md) · [machine protocol](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-12.json) · [source-scope receipt](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_SCOPE_RECEIPT_2026-08-12.json) · [activation](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_ACTIVATION_2026-08-13.json) · [execution stop](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_EXECUTION_STOP_2026-08-13.md) · [machine receipt](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_EXECUTION_STOP_2026-08-13.json)
- [D3R3 Phase-B source-coverage transport recovery](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-13.md) · [machine protocol](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-13.json) · [source-scope receipt](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_SCOPE_RECEIPT_2026-08-13.json)
- [D3R3 fresh-HEAD activation](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_HEAD_ACTIVATION_2026-08-13.json) · [coverage census protocol](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_CENSUS_PROTOCOL_2026-08-13.json) · [census activation](DEPTHART_TASK_PRESERVING_D3R3_PHASE_B_SOURCE_COVERAGE_CENSUS_ACTIVATION_2026-08-13.json)
...
`RUN_D3R3_EXACT64_SOURCE_MEMBER_COVERAGE_CENSUS`。使用 fresh D3R3 root 对64个资产全量重新 GET；
不复用 D3R2 bodies。只有 matching-header premature EOF 可从 byte 0 重试，Range/redirect/member payload/
pixel/truth/selection 均禁止。完整64项与独立 validation PASS 后，才进入 missing-source policy registration。
D3R2 r0 继续保持 `scientific_terminal=null / successor=null`。
