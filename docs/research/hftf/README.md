...
`EXPLICIT_D3R1_PHASE_A_INTRINSICS_TRAJECTORY_BODY_AND_LABEL_BLIND_CONTINUITY_ACTIVATION`。它只允许
在另行冻结 body protocol/activation 后，对已通过 HEAD 的 exact-127 intrinsics/trajectory 执行
GET、全 payload integrity/schema 与 label-blind portrait/pose continuity；不能扩大/reorder pool、降低
300-frame continuity 门、提前选择 32 identities，或读取 RGB/depth/confidence、truth/model/outcome。
