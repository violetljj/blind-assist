...
唯一 successor 是 `EXPLICIT_D2_PHASE_C_EXACT_EIGHT_RGB_HEAD_ONLY`，当前 `execution=false`。
只有用户显式授权后，才允许对 D2R1 已锁的 exact 4 TRAIN + 4 sealed DEVELOPMENT identity 的
`lowres_wide.zip` 发出 8 个 HEAD 请求，以冻结 RGB body 可用性与精确字节数；不得读取 body、
重新请求 depth/confidence/intrinsics、运行模型、训练或打开 Development。HEAD 即使 PASS 也必须
另立带精确 body 上限的 source/training activation，不能自动继承 GET 或训练权限。
