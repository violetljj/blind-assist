...
唯一 successor 是 `EXPLICIT_D2_TRAIN_ONLY_BASE_OUTPUT_AND_HEAD_TRAINING_ACTIVATION`，当前
`execution=false`。只有用户显式授权并连接 fresh `SM-S9280 / SM8650 / HTP v75` 后，才允许解码
4 个 TRAIN identity 的 1,200 帧、派生 TRAIN truth、使用冻结 saved context 生成 TRAIN base output，
并按固定 seed/recipe 训练唯一 277 参数 head、锁 step-500 checkpoint。4 个 DEVELOPMENT identity
必须保持 sealed；训练权限不能自动继承为 Development quality、R2 或性能权限。
