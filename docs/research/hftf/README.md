...
它只允许为已锁定 exact-32 Phase-A identities 登记后续 `lowres_depth.zip` 与 `confidence.zip` 的
source-integrity/support use；不得直接发 HEAD/GET，不得改写 32 身份或 300-frame windows，也不授权
RGB、truth/model、TRAIN/DEVELOPMENT、R2 或 outcome。
