...
`EXPLICIT_D3R1_PHASE_A_INTRINSICS_TRAJECTORY_HEAD_ONLY_PREFLIGHT_ACTIVATION`。它只允许为已冻结的
exact-127 Training metadata roster 对 `lowres_wide_intrinsics.zip` 与 `lowres_wide.traj` 执行
254 个 HEAD，并强制 response body 为 0；不自动授权 GET、continuity selection、Phase-B、训练、
Development 或 R2。
