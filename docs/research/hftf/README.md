...
- 用户已另行授权建立 D3R2 新版本、新协议和新 root 登记，但没有授权打开媒体。官方 RAW 格式说明明确指出，同步的 60FPS family 不保证每个 timestamp 都存在全部资产；因此 D3R2 首先冻结 exact-32 × exact-300、exact-64 的 coverage-only census，而不是替换邻帧或降低门。未来 census 必须全新下载 64 bodies，仅解析 ZIP directory/member names，不打开 member payload、不运行 `testzip`、不验证 CRC、不解码或派生 truth；完成后仍 `scientific_terminal=null`，只进入 missing-source policy registration。当前正式 D3R2 root 不存在，HEAD/GET/Range、archive directory/member、truth、selection、RGB、模型、角色、训练、Development 和 R2 均未打开。
...
- [D3R2 Phase-B source-coverage recovery protocol](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-12.md) · [machine protocol](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-12.json) · [source-scope receipt](DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_SCOPE_RECEIPT_2026-08-12.json)
...
`EXPLICIT_D3R2_PHASE_B_EXACT64_COVERAGE_ONLY_CENSUS_ACTIVATION`。D3R1 Phase-B r0 仍保持
`scientific_terminal=null / successor=null`，不得删除 orphan source 后强行 resume，不得用邻帧、其余
Phase-A eligible identity 或同版本新 root 回救。D3R2 已另立并冻结 coverage recovery protocol，但当前
只登记 scope；在下一次显式激活前，不得创建正式 root、发送 GET 或读取 ZIP directory/member。
