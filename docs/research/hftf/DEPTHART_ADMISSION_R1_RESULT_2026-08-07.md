...
终态：`DIAGNOSTIC_COMPLETE / G3-C_BLOCKED_SELECTIVESCAN / HTP_NOT_EVALUATED`
...
| A3 ONNX/QNN | G3-A PASS；G3-B PARTIAL PASS；G3-C BLOCKED_SELECTIVESCAN | camera 外提与 hygiene/static folding 将图收敛至 2723 nodes，normal frontier 仍为 5 个 `onnx_selectivescan`；dry-run 仅余 Erf/LayerNorm/Resize/SelectiveScan，HTP 未评估 |
...
4. 下一部署动作是完成各图级 numerical parity，再对 5 个 SelectiveScan 做 primitive lowering / HTP custom-op feasibility；在此之前不开始 quantization、partition 性能或真机 latency。
