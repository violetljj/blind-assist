...
| A3 ONNX/QNN | ONNX export PASS；QNN conversion FAIL | 3555-node `image,K→depth` 图含 5 个 SelectiveScan；QAIRT 在等价消除 Einsum 后卡于 Camera Embedder `Acos`，尚未到达 SelectiveScan |
...
4. 下一部署动作是在保持动态 `K` 的前提下处理 Camera Embedder `Acos`，重跑 converter 直到直接评价 SelectiveScan；之后才进入数值 parity 与真机 HTP。
