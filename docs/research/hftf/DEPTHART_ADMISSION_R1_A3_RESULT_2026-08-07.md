...
状态：`G3-A_EXPORT_PASS / G3-B_PARTIAL_PASS / G3-C_PRIMITIVE_REFERENCE_CONVERTIBLE / G4-A_PACKAGE_REGISTRATION_PASS / G4-B_OPERATOR_PARITY_PASS_SM8650_V75 / G4-C_CONTEXT_HOLD_LAYERNORM_REDUCE_FP16`
...
- G4-C 据此签署 `CONTEXT_HOLD_LAYERNORM_REDUCE_FP16`。它证明 5-op canonical DLC 已抵达真实 HTP graph prepare，也证明当前 QAIRT 2.47/SM8650 v75 的 full-context 闭合仍有独立于 SelectiveScan 的归一化 frontier；partition/fallback、full-model parity、latency/RAM/thermal 继续 `NOT_EVALUATED`。

机器可读 receipt：[`a3-onnx-qnn-preflight.json`](../../../artifacts.local/evidence/hftf/depthart-admission-r1/a3-onnx-qnn-preflight.json)、[`selective-scan-converter-mapping-receipt.json`](../../../artifacts.local/evidence/hftf/depthart-admission-r1/qairt/selective-scan-converter-mapping-receipt.json)、[`selective-scan-primitive-lowering-receipt.json`](../../../artifacts.local/evidence/hftf/depthart-admission-r1/qairt/selective-scan-primitive-lowering-receipt.json)、[`selective-scan-htp-kernel-preflight-receipt.json`](../../../artifacts.local/evidence/hftf/depthart-admission-r1/qairt/selective-scan-htp-kernel-preflight-receipt.json)、[`g4c-full-context-receipt.json`](../../../artifacts.local/evidence/hftf/depthart-g4c-full-graph-sm8650-v75-20260808-r1/g4c-full-context-receipt.json)

## 结论与边界

A3 已证明 ONNX static graph 可以生成，并通过 converter-only custom mapping 完成整图 QAIRT conversion；外围 normal-converter blocker 空间已收敛。exact primitive lowering technically feasible，但 25.2× QNN IR op 膨胀与 196 级串行链使它只保留为 parity oracle。correctness-first kernel 已在当前 SM8650/v75 上完成真实 package registration、QNN compose/finalize、HTP execute 与 3/3 primitive parity，因此 G4-A/G4-B PASS。G4-C 已执行但因 LayerNorm/ReduceMean FP16 prepare frontier 进入 HOLD，完整 context 尚未闭合。该 frontier 关闭前不得开始 G4-D/E/F，也不得写成 G4 overall PASS、完整 DepthART HTP PASS 或 DA2 替换。reference 合同继续保持 `image,K→depth`；mobile graph 的 prompt 输入只代表硬件感知分区，不是固定 K 冒充动态 metric conditioning。
