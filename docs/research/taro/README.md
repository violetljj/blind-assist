...
- [R11 selected-top24 FARO Phase B implementation lock](TARO_O1R_R11_SELECTED_TOP24_FARO_PHASE_B_IMPLEMENTATION_LOCK_2026-08-13.md)：
  冻结 24/674/6066 selected-only reader、完整 R11 dual-class gates、single-frame geometry reuse、原子 678-file terminal
  与独立 no-FARO-replay validator；19 个聚焦测试 PASS，正式 FARO 尚未读取；
...
source-only 48→24 one-shot 与独立复核均已 PASS；24 个 parent identities 已不可变封存。selected-only FARO Phase B
implementation lock 也已完成但尚未读取 FARO。当前唯一 successor 是
`TARO_O1R_R11_SELECTED_TOP24_FARO_PHASE_B_ONE_SHOT_EXECUTION_LOCK`；不得读取 24 个 unselected parent 的 FARO，
不得更改 selection、selector、candidate 或 threshold。
