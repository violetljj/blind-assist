...
failure terminal，并以真实总量 `2,960,390,828 bytes` 为硬上限；当前 execution lock 尚未提交，因此 source
body、DepthART 和 FARO 仍为 0，也没有 TARO
...
- R10 正式 `NOT_EVALUABLE` 保持不可改写；R11 development-only abstention、48-parent fresh protocol 与
  exact 数据授权已冻结。zero-body HEAD 已消费，download implementation 已就绪；当前只可另行提交并消费
  精确绑定实现 commit、HEAD evidence、144-row plan 与 `2,960,390,828` bytes 的 source download one-shot；模型与 FARO 仍须依阶段
