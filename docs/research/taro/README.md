...
当前唯一 successor 是 `TARO_O1R_R11_SOURCE_ONLY_TOP24_IMPLEMENTATION_LOCK`：只实现并冻结 48→24 source-only
R9 scoring/selection，正式评分仍须另立 execution lock；不得提前读取 FARO 或计算真实 top-24。
