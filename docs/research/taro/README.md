...
失败而 supersede；GET/source body 为 0、one-shot 未消费。Attempt 02 已绑定新的模块入口 implementation
commit 并提交，唯一 successor 是消费一次锁内固定 argv并形成
`TARO_O1R_R11_FRESH_POOL_SOURCE_DOWNLOAD_INTEGRITY_PASS` 或 sealed invalid terminal。DepthART 和 FARO 仍为 0，也没有 TARO
...
- R10 正式 `NOT_EVALUABLE` 保持不可改写；R11 development-only abstention、48-parent fresh protocol 与
  exact 数据授权已冻结。zero-body HEAD 已消费；download Attempt 01 pre-start superseded，Attempt 02
  `AUTHORIZED_UNCONSUMED` 且只可执行固定 argv 一次；模型与 FARO 仍须依阶段
