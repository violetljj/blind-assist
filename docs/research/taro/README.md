...
- [R11 source inventory one-shot execution lock](TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_ONE_SHOT_EXECUTION_LOCK_2026-08-12.json)：
  绑定已推送 implementation commit `9c659c30`、11 份代码/协议/authorization/download evidence、central-directory-only policy 与预算；状态 `AUTHORIZED_UNCONSUMED`；
...
`testzip/open/read`，声明 CRC 不冒充 member payload CRC 验证。独立 one-shot execution lock 已冻结为
`AUTHORIZED_UNCONSUMED`，content seal `244D5E16...BB10`。唯一 successor 是先提交并确认
`TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_ONE_SHOT_EXECUTION_LOCK` 位于 `origin/master`，再严格按其
module argv 消费一次；在该锁消费前正式 inventory、
DepthART 与 FARO 均为 0，也没有 TARO route/deployment/device/product/safety promotion。
