...
- [R11 zero-body HEAD one-shot execution lock](TARO_O1R_R11_FRESH_48_PARENT_ZERO_BODY_HEAD_ONE_SHOT_EXECUTION_LOCK_2026-08-12.json)：
  ...
  root；当前 `AUTHORIZED_UNCONSUMED`，只允许一次 zero-body HEAD；
...
HEAD implementation、6 项 synthetic/mutation tests与 one-shot execution lock 均已冻结。唯一 successor
是消费一次 exact 144-request HEAD root。HEAD 前不得读取 source body、运行 DepthART 或打开 FARO；也没有
TARO route/deployment/device/product/safety promotion。
