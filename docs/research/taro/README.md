...
失败而 supersede；GET/source body 为 0、one-shot 未消费。唯一 successor 是提交
`TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_ONE_SHOT_EXECUTION_LOCK_ATTEMPT_02`，绑定新的
模块入口 implementation commit 后才可执行。DepthART 和 FARO 仍为 0，也没有 TARO
...
- R10 正式 `NOT_EVALUABLE` 保持不可改写；R11 development-only abstention、48-parent fresh protocol 与
  exact 数据授权已冻结。zero-body HEAD 已消费；download Attempt 01 pre-start superseded，当前只可提交
  模块入口形式的 Attempt 02 one-shot lock；模型与 FARO 仍须依阶段
