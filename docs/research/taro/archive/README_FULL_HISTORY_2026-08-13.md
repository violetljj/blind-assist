...
- [R11 bounded source download Attempt 01 lock](../TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_ONE_SHOT_EXECUTION_LOCK_2026-08-12.json)：
  精确绑定 implementation commit `399b53ec`、11 份代码/HEAD evidence、144-row plan、总字节、共享 deadline、
  transient-only retries 与双 root failure terminal；因 direct-script import pre-start failure 已 supersede；
- [R11 bounded source download Attempt 01 incident](../TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_ATTEMPT_01_PRESTART_INCIDENT_2026-08-12.md)：
  import 在 root/GET 前失败，GET/source body 为 0、one-shot 未消费；唯一修正是模块入口与新 Attempt 02 lock；
- [R11 bounded source download Attempt 02 lock](../TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_ONE_SHOT_EXECUTION_LOCK_ATTEMPT_02_2026-08-12.json)：
  绑定 implementation commit `ad609b35` 与稳定 module-form argv，其余 roster/HEAD/预算/权限不变；当前
  已消费且不得重跑；
- [R11 bounded source download result](../TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_RESULT_2026-08-12.md)：
  144/144 首试下载和逐文件 SHA-256/CRC32 重验 PASS，总量 `2,960,390,828 bytes`；archive members、模型与 FARO 未打开；
- [R11 source inventory implementation lock](../TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_IMPLEMENTATION_LOCK_2026-08-12.md)：
  实现 metadata-only ZIP central-directory index、exact-ns trajectory/frame plan、one-shot failure/result sealing 与 FARO payload firewall；正式 inventory 尚未运行；
- [R11 source inventory one-shot execution lock](../TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_ONE_SHOT_EXECUTION_LOCK_2026-08-12.json)：
...
- [R11 all-48 source-only Phase A implementation lock](../TARO_O1R_R11_FRESH_48_PARENT_SOURCE_ONLY_PHASE_A_IMPLEMENTATION_LOCK_2026-08-12.md)：
  ...
  firewall；该 non-executing lock 原样保留，正式运行状态见下；
- [R11 all-48 source-only Phase A one-shot execution lock](../TARO_O1R_R11_FRESH_48_PARENT_SOURCE_ONLY_PHASE_A_ONE_SHOT_EXECUTION_LOCK_2026-08-12.json)：
...
- [R11 Phase-A independent-validator round-12 representation repair](../TARO_O1R_R11_PHASE_A_INDEPENDENT_VALIDATOR_ROUND12_REPRESENTATION_REPAIR_2026-08-13.json)：
  保持原 validator/lock/5,219-file root 不变，只将独立重建 pose/gravity 按 producer 冻结的 canonical JSON 12 位规则
  规范化后作精确比较；无 epsilon、无模型重跑、无 scoring/FARO 权限，须在推送后对同一 root 只读重验；
...
failure terminal，并以真实总量 `2,960,390,828 bytes` 为硬上限。Attempt 01 在 root/GET 前因入口导入
失败而 supersede；GET/source body 为 0、one-shot 未消费。Attempt 02 随后 144/144 首试完成并通过独立
hash/CRC/manifest 重验，正式终态为 `TARO_O1R_R11_FRESH_POOL_SOURCE_DOWNLOAD_INTEGRITY_PASS`。后继
`TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_IMPLEMENTATION_LOCK` 已完成：正式 runner 只读 ZIP central
...
当前唯一 successor 是
`RUN_READ_ONLY_R11_PHASE_A_ROUND12_REPRESENTATION_REPAIR_AUDIT`：先把新绑定 repair 推送到 master，再对同一
sealed 5,219-file root 保留原 validator 全部检查，仅把独立重建的 pose/gravity round-12 后作精确比较；不重跑
模型、不改写 evidence、不使用 epsilon。该 audit PASS 后才可另立 source-only top-24 implementation/execution lock。
...
- R10 正式 `NOT_EVALUABLE` 保持不可改写；R11 development-only abstention、48-parent fresh protocol 与
  ...
  source-only Phase A one-shot 已消费并封存 producer PASS terminal，Phase A highres/FARO 为 0；CUDA 已在重启后
  恢复，原独立 validator 的 round-12 numeric representation stop 已由只读薄 repair 冻结，当前只可在 repair
  推送后对同一 sealed root 做只读重验。验签 PASS 前不得执行 R9 scoring/top-24 或 FARO；之后二者仍须依阶段
  另立 successor，且 unselected FARO 必须为 0；
