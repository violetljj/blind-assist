...
25/25 synthetic focused tests 通过，其中包含 HEAD transport/write failure、truth root-creation 窗口的 one-shot consumed 注入，以及仓库授权 `artifacts.local` junction containment 回归。该结果不说明 72 个远端 asset 可用、真实 gate 可通过或 O0R 有因果
