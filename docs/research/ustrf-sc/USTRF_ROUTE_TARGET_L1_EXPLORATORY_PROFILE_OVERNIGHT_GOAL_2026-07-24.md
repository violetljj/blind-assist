...
在不新增数据、不改变候选、不选择胜者的前提下，建立一条可恢复、哈希绑定、fail-closed 的独立运行链：严格读取 R2-L1 eligibility mask 的 62,229 帧显式 ledger，让冻结的 C1、C2、C3 各对 41 条 masked sequence ledger 做一次完整 replay，只为当前已获授权的指标生成探索 profile，并用机器收据说明哪些结果有效、条件资格是否兑现、哪些仍不可评。ledger 内冻结的观测断点必须重置候选状态，不能把缺失区间当成连续观察。
...
父 R2-L1 数量合同同时固定为：6,369 个 event/proposal unit、50,952 个 event×metric 分类、62,229 帧、41 条 masked sequence ledger、62,188 个相邻 pair、3,801 个合格负暴露 pair、836 个合并区间及 `297,376,110,945ns` 严格负暴露。新阶段不得因本机可用文件不同而缩小这些集合。
...
- 每条 masked sequence ledger 的状态初始化，以及由 `frame_id` 不连续、时间戳非正或相邻 gap 超过 1 秒冻结出的 discontinuity reset 边界；
...
2. 验证 62,229 帧和 41 条 masked sequence ledger 精确覆盖，无重复或顺序漂移；完整枚举现有 15 个 `frame_id_not_consecutive` 边界及其中 14 个同时超过 1 秒的边界，禁止把它们静默补成连续帧。
...
5. 若 canonical stream 不完整，先新增 R2-L1E 专用、sequence-sharded exporter/decoder 合同，再复用已经冻结的正式 App Canvas/TFLite 路径物化缺失流；旧 exporter 的 selection-authority/单体输出前置条件不能直接沿用。不得改变模型、labels、`.35/.45`、preprocess、NMS、tracker 或候选。
...
- frame 缺失、乱序、跨 sequence 或跨冻结 discontinuity 的状态泄漏、重复运行、候选实现 SHA 漂移均 fail closed；
...
- 按候选名册顺序执行，但三个候选相互隔离；每个候选对每条 masked sequence ledger 只产生一条权威 trace。
- 候选 FSM 及 replay-local association/history state 必须在 sequence 起点和冻结 discontinuity 边界重置；冻结的输入 identity/association receipt 不得改写。不得按评分 event/window 重置，任何指标的观察区间也不得跨 gap。
- truth join 必须发生在候选状态更新和输出落盘之后，仅用于归因与评分。
- 每个 candidate×masked-ledger 写原子 checkpoint、输入 SHA、输出 trace SHA、首尾 frame、frame count、discontinuity reset count、wall time、资源摘要与 attempt ID。
- 进程在 masked sequence ledger 中途崩溃时，该 attempt 标为 incomplete 且没有评估权限；只可从该 ledger 起点恢复。第一个通过完整性验证的 trace 成为唯一权威 trace，禁止为改善结果选择性重跑。
...
- canonical raw tensor 的全量展开估算约 40.9GiB，而 2026-07-24 preflight 的 E 盘可用空间仅约 23.06GiB；禁止生成全量 monolith。每次只允许一个 masked sequence ledger shard、一个设备和一个 host worker。
- 每个 shard 必须按 `device raw -> raw receipt -> host compact detector ledger -> successor receipt` 顺序闭合；successor receipt 验证通过后才删除该 shard 的 raw/device staging。删除失败只做最多两次有界重试，不能先删后验。
- 每个 shard 开始前记录 projected bytes；E 盘 free-space 必须至少为 `2 × projected shard bytes + 5GiB reserve`。host worker 最大 RSS 为 8GiB，系统可用物理内存低于 6GiB即停止。以上值必须写入冻结配置和每个 shard receipt。
- 允许 masked sequence ledger 边界断点恢复；恢复前逐个核验已有 checkpoint 和 trace SHA，不信任仅凭文件存在的完成标记。
- 对设备、GPU、外部进程最多执行一次初始尝试和两次有界重试；超限后写失败收据并结束，不无限等待、不静默重启。
- 默认使用 CPU。若冻结 canonical pipeline 必须使用 Android 设备或 GPU，单设备串行运行，并冻结 Android thermal status `<=2`、最终电池温度 `<=45°C`、相对起始温升 `<=8°C`；任一越界立即安全终止，不得降频后静默续跑。
...
三个候选均有覆盖全部 41 条 masked sequence ledger、62,229 帧和全部冻结 discontinuity reset 的 first-valid-complete trace；所有 profile、收据和 validator 通过，且没有选择、排名或晋级结论。
...
3. 三个候选各自覆盖 41/41 masked sequence ledger、62,229/62,229 frame 与全部冻结 discontinuity reset；
...
先冻结独立探索配置、输入合同、恢复合同和 validator。对 mask 的 62,229 帧、41 条 masked sequence ledger 做 fail-closed preflight，并枚举 15 个 frame-id discontinuity；FSM 与 replay-local history 必须在 sequence 起点及这些冻结断点重置，任何指标不得跨 gap。必须使用 Android Canvas canonical detector raw stream、冻结 T0 association 和 causal input，禁止 host PIL reconstruction、未来 truth 或候选输出进入状态更新。若全量输入不齐，生成精确 gap receipt 并以 FAIL_CLOSED_INPUT_BLOCKED 结束，不运行候选。

输入通过后，让 C1、C2、C3 各对每条 masked sequence ledger 做一次完整 replay；truth 只在输出后归因。每个 candidate×ledger 原子写 checkpoint 和 trace SHA。中途失败的 attempt 无评估权限，只可从 ledger 起点恢复；第一个完整有效 trace 唯一权威，禁止候选特定重跑或 best-of-retries。
...
所有输出写入 artifacts.local/，哈希绑定、可恢复、可复算。canonical raw 必须逐 ledger shard 串行导出、验证 successor receipt 后清理，禁止约 40.9GiB 全量 monolith；冻结 free-space/RSS 门以及 thermal status `<=2`、电池 `<=45°C`/温升 `<=8°C`，每项最多两次重试。输入、资源或完整性失败必须写机器收据并安全终止，不无限等待。完成前运行变异测试、完整 validator 和独立 agent 只读复核，并更新日期化结果、研究索引、模块 README 与 handoff。
