...
| 事件与关键帧 | event-driven slow-loop receipt 合同、reference-free event gate | JVM frame/TTL/event mismatch 回归；SM-S9280 静止 CameraX 30 帧中 3 个 frame-bound event、27 个周期抑制 | 非米制语义源、真实用户查询/目标变化和人工事件真值 | benchmark-only，不驱动生产链 |
...
若以后选择开启任一设备的“受控几何/事件 shadow”，才需要一个经独立复核的 calibration manifest，按 [校准协议](USTRF_SC_CALIBRATION_PROTOCOL.md) 填充，至少包含：固定 `bodyFrame`、外部 artifact SHA-256、不同采集者与复核者、样本/姿态覆盖、内参 reprojection、depth registration 与完整 mount repeatability。没有该包，`UstrfIndependentCalibrationEvidenceVerifier` 必须返回 unavailable。

在此之后，仍需要单独的几何和动态事件集，以及同机时延、热、连续人工事件评测；任何单项绿灯都不允许替代完整 promotion gate。
