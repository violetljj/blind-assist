...
3. 采集数据、原始影像、标定板信息与人工记录放在受控的外部实验存储；仓库和 trace 只保存匿名汇总、manifest 路径标识与 SHA-256。
4. 采集者与复核者必须是不同责任主体；复核者要能访问原始 artifact 而不是只看到摘要。
...
| 独立复核 | 复核者核对 manifest digest、样本数、姿态桶、算法输出与失败样本 | review approval、reviewer ID | 所有几何授权保持 unavailable |
...
1. 记录设备型号、系统版本、相机/支架装配照片编号（不写入仓库）、`cameraFrame`、`bodyFrame`、采集者和时间。
...
6. 由不同复核者抽查原始 artifact 与摘要。只有复核通过后，才可写 `independentReviewApproved=true`。
...
通过本协议后仍需独立验证：逐像素 depth 置信校准、地面/台阶/头部几何、动态 TTC、热/时延、连续人工事件真值和默认反馈链的独立安全门。上述事项全部完成前，USTRF 继续是 production-isolated experiment。
