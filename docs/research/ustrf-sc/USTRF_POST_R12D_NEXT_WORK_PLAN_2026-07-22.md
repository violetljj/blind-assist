...
1. 用来源 Agent、自动设备脚本、仿真/合成和多模型共识，把 route-conditioned event reference 从 `0` 推进到可审计无人化证据链；
2. 判断 SM-S9280 是否能产生同一 frame 绑定的 metric depth + stable pose，而不是重复旧 ARCore 窗口；
3. 只有正式自动多模型事件证据与 device metric geometry 都闭合后，才执行 U0 效果矩阵。

因此后续顺序固定为 `自动获得首个 matched pair → 10-episode autonomous pipeline pilot → 独立 ARCore frame-bound 几何 canary → 正式 120-episode model-reference / geometry admission → U0`。detector 数据诊断保留为冻结 backlog，不进入主动队列。
...
| eligible automated multimodel event evidence=`0`，device metric geometry admission=`false` | 现有模型比较不能回答真实助盲效果 | 主动队列转向自主数据获取、模型共识与 geometry |

现有 YOLOE 继续作为 benchmark-only 外部参考；不替换 App 默认模型，也不把 `5/6` 当作生产通过。

## 工作面 A：自主路线事件证据链（第一优先）

### A1. 首个自动 matched pair canary
...
10-episode pilot 通过后，自动审计计算资源、隐私、模型 review 一致性和失败补采率；满足冻结扩展门即由 Agent 扩到正式 `6 session × 5 scene × 2 episode = 120 episode / 60 matched pair`。正式矩阵和独立双模型/第三模型裁决闭合前，不运行有效 U0。
...
1. 正式 120-episode/60-pair automated multimodel event-reference gate 通过，而非 10-episode pilot；
...
| 顺序 | 动作 | 通过后 | 失败后 |
| ---: | --- | --- | --- |
| 1 | Agent 自动获取/生成 `route_obstacle_pair_01` 两集 | 扩 10-episode pilot | 隔离失败来源，修复自动链或切换合法来源 |
| 2 | 完成 10-episode 自动审计 | 自动扩至正式 120 集 | 保持 model-reference=0，不进 U0 |
...
当前单一推荐下一动作：按 `START_HERE.md` 启动来源 Agent/自动设备脚本，获取首个 `route_obstacle` matched pair，并完成隔离双模型复核。除此之外不新增训练、模型、阈值或 R1.3 来源。
