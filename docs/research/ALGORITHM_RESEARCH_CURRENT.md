...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_BIDIRECTIONAL_ROUTER_MECHANICS_PASS / D3_FRESH_METADATA_POOL_48_LOCKED / D3_PHASE_A_HEAD_96_OF_96_PASS / D3_PHASE_A_BODY_NOT_ACTIVATED / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3_PHASE_A_INTRINSICS_TRAJECTORY_BODY_AND_LABEL_BLIND_CONTINUITY_ACTIVATION`：下载并校验 exact 96 个 intrinsics/trajectory 资产，按冻结顺序锁定前 32 个 portrait/pose-qualified 身份；不读取 RGB/depth/confidence/truth/model | 用已消费 D1/D2 outcome 拟合或回救 D3；未经独立 gate 下载或读取其他资产、进入 Phase-B、训练或 Development；用 outcome 换身份；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
