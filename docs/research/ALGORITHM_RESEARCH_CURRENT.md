...
|---|---|---|---|---|---|---|
| Goal Copilot / Destination Grounding | 在真实多建筑、多门场景中，哪个入口真正属于用户指定的目标建筑 | `PRODUCT_AND_RESEARCH_MAINLINE / GOAL_GROUNDING_P0 / V2_SYSTEM_BLUEPRINT_CURRENT / R5_PERMANENTLY_CLOSED / TARGET_PERSISTENCE_FOUNDATION_RETAINED / SPATIAL_MEMORY_DEFERRED / NO_R6_R7 / NO_SKY / DEFAULT_APP_UNCHANGED` | [Goal Copilot current](goal-copilot/README.md) / [V2 路线图](goal-copilot/BLINDASSIST_GOAL_DRIVEN_VISUAL_COPILOT_V2_ROADMAP_2026-08-21.md) | `BA_DESTINATION_GOAL_GROUNDING_R0`：先设计并冻结真实场景、goal、candidate/truth schema、Development/fresh 角色与 ranking evaluator；不运行 baseline | 继续 tiny-object teacher/R6/R7；另开 carrot VIO/SLAM rescue；先造模块群；让 Sky 决定研究方向；未冻结 evaluator 即运行 baseline；启动 held-out/default App | 否 |
