...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_TRANSFER_FAIL_STOP / ANCHOR_DEVICE_CANARY_PASS / RGB_PAIR_SUPPORT_PASS / RGB_HISTORY_RETENTION_COST_PASS / FRESH_RAW_DEPTH_PAIR_FAIL_STOP / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_RGB_SELECTED_PAYLOAD_DECODE_INTEGRITY_R0`：只对 exact receipt 命中的 reference/selected owned YUV 做延迟解码完整性与成本验证 | 用旧/reprojected depth 回救；把 RGB ownership 写成任务增益；运行模型、接默认 App、风险融合或 guidance | 否 |
