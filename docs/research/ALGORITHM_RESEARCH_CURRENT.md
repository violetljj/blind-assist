...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_PHASE_A_FAIL_21_OF_32_NO_SELECTION / D3R1_FRESH_METADATA_POOL_127_LOCKED_MEDIA_UNOPENED / D3R1_SOURCE_SCOPE_REGISTERED / D3R1_PHASE_A_HEAD_AWAITING / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3R1_PHASE_A_INTRINSICS_TRAJECTORY_HEAD_ONLY_PREFLIGHT_ACTIVATION`：只对 exact-127 的 intrinsics/trajectory 执行 254 个零正文 HEAD，仍不授权 GET | 复用或扩大旧 D3 exact-48/21-qualified pool；降低 continuity 门；把 127-pool planning heuristic 写成成功保证；未单独激活即发 HEAD/GET；进入 Phase-B、训练或 Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
