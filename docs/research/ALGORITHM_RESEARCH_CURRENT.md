...
状态：`current / PRODUCT_MAINLINE=GOAL_DRIVEN_VISUAL_COPILOT / ALGORITHM_MAINLINE=GRAIL_R / R1C_O_OWNER_LOCAL_CEILING_ESTABLISHED / R1C_P_PAIRED_RGB_ORIENTATION_FAILED / R1C_L_TASK_TRAINED_PAIRWISE_OWNER_COORDINATE_AUTHORIZED / FRESH_HOUSE_DISJOINT / ONE_ARCHITECTURE_TWO_SEEDS_MAX / FINAL_TEST_UNOPENED / STOP_BEFORE_DEPTH_GEOMETRY_AND_M2 / DEFAULT_APP_UNCHANGED`
...
当前 referent bottleneck，但不证明视觉系统已获得关系。R1 ablation 将信息收窄到 sibling ordinal；随后 training-free R1A
...
用户现已明确授权 [`R1C-L`](goal-copilot/GRAIL_R1C_L_TASK_TRAINED_PAIRWISE_OWNER_COORDINATE_PROTOCOL_2026-08-25.md) 作为最后一次 RGB-only learned successor：160/20/12 个完全隔离 houses，shared DINOv2-S + mask adapter + 两层双向 cross-attention + 36-bin head，一个架构、最多两个 seeds，直接以 symmetry-marginalized slot permutation loss 训练。validation 相对同 cohort fixed OA-V2 slot uplift 不足 `+8` 就在 final test 前停止；depth geometry 与 M2 均未开放。
...
SAGE-LM V1-B-R2 已在 outcome 前用正确官方 pose 从头物化 24 条新 cohort，24/24 满足 lateral `0.18–0.30 m`、
forward `<=0.45 m` 与第二视图 aperture 投影可见门。B0=`24/24` geometry/confidence/arrival，B1 在 oracle association 下仅
`2/24` geometry、`0/24` confidence/arrival，21/24 为 `BOUNDARY_CANDIDATE_MISSING`；B2=`14/24` geometry、`5/24`
confidence、`3/24` arrival。R3 官方 DeepLSD dense field + fragment fusion 将 true pair `2 -> 15`、geometry `2 -> 13`、
missing `21 -> 9`，证明 sparse representation 是重要瓶颈，但四个成功门均未过。预定失败 successor R4 的 pose-conditioned
top-96 joint-support accumulation 又退化为 true pair `9/24`、geometry `8/24`、missing `15/24`，该 proposal objective
被拒绝。R5 改为完整 pair score + 四投影坐标 diversity 后为 true pair/geometry `12/24`、missing `12/24`；512-pair
diagnostic 可回到 `15/24`，但正式 96-pair arm 仍低于 R3。预定 fallback 的 11-fold source-sequence-disjoint 小型
left/right Conv1D head，held-out top-8 四边界覆盖仅 `5/24`，最终 true pair/geometry=`11/24`、missing=`13/24`，
该小样本 field-summary head 被拒绝。dense row support 与旧 confidence contract 不同，`0/24 confident` 不作全部 geometry
质量否定；coverage 未过，R6 与 B2 不运行、association 仍未裁决。任何
...
V1-C 已按新路线直接学习 RGB left/right boundary heatmap。TartanAir door-mask r1 的 synthetic validation 收敛但真实
true pair/geometry 为 `0/0`；排除评估 source 后从余下 ARKitScenes 自动生成 336 个 strong-line/depth-discontinuity
opening proxy，同域 r2 的 C0/C1 四边界 Recall@8=`1/24`、`4/24`，true pair/geometry=`1/24`、`3/24`，均低于 R3。
当前轻量 CNN + automatic opening-proxy supervision 关闭；这不否定具有独立弱边界真值的新 supervision 家族。
V1-D 随后固定同一 24 条与 R2 geometry，使用 RAFT-Small 双向 flow、pose rotation compensation、FB consistency 与
residual-parallax discontinuity 的 LEFT/RIGHT top-8。结果四边界 Recall@8=`4/24`、true pair/geometry=`4/24`，
R3 missing rescue=`0/9`，并丢失 11 个 R3 已有 pair；因此不做 R3 fusion，当前 active-parallax 实现关闭。该结论只覆盖
当前 endpoint 条件；其中 endpoint relative rotation median 约 `20.1 deg`、`6/24 >30 deg`。当时唯一 successor 是从
ARKitScenes mesh/Faro depth 建立独立弱 aperture boundary teacher 的 V1-E privileged geometry supervision，现已执行。V1-E0 已用
全 24 条可对齐的官方 3DOD mesh raycast metric depth/normal 生成 RGB-independent heatmap、signed depth jump 与 valid mask；
四边界 Recall@8=`10/24`、true pair/geometry=`9/24`、R3 missing rescue=`3/9`、retention=`6/15`，三个 teacher ceiling
继续门全部失败。故停止于 E0，不训练 student；该结论只关闭当前 ARKit mesh + aperture mapping，Faro highres depth 因
固定 pair 同时刻覆盖不足未形成完整 24-case ceiling。V1-F 随后在 24 个 source-frame-ancestry-fresh Development cases 上
改用 anchor 支撑平面后的跨视图 connected free-space field；同批 R3 true pair/geometry=`18/24, 7/24`，V1-F D2=
`0/24, 10/24`，retention=`0/18`、rescue=`0/6`。仅 geometry uplift 过门，且 D2 为同 cohort outcome 后聚合修正，
故 `STOP_BEFORE_STUDENT`。四离散边界恢复作为主线表示整体关闭；R3 只保留为冻结参考，不再扩信息源、top-k、配对、
teacher/student 或 fusion。该结论只拒绝当前 D2 portal teacher，不反证所有目标相关自由空间表示。
...
|---|---|---|---|---|---|---|
| GRAIL-R / relational interaction pose | task-trained pairwise RGB/mask coordinate 能否恢复 R1C-O slot mechanism | `R1C-L AUTHORIZED / TRAIN 160 HOUSES / VALIDATION 20 / FINAL 12 UNOPENED / ONE ARCHITECTURE TWO SEEDS MAX / STOP BEFORE DEPTH AND M2` | [GRAIL-R1C-L protocol](goal-copilot/GRAIL_R1C_L_TASK_TRAINED_PAIRWISE_OWNER_COORDINATE_PROTOCOL_2026-08-25.md) | 先物化 train/validation；validation 相对 fixed OA-V2 slot uplift `>=+8` 才打开一次 final test | R1C-P consumed cohort 调参；backbone/crop/bin/loss/ensemble sweep；提前访问 final；depth/R1C-G；M2；Android/App 晋升 | 否 |
...
| Goal Copilot / SAGE-LM last-mile representation closure | exact identity 后，boundary 或 portal teacher 是否能超过冻结 R3 并保留/救回其成功样本 | `SAME-COHORT R3 PAIR/GEOMETRY=18/7 / V1-F=0/10 / RETENTION=0/18 / RESCUE=0/6 / FOUR_BOUNDARY_MAINLINE_CLOSED / STOP_BEFORE_STUDENT` | [V1-F result](goal-copilot/SAGE_LM_V1_F_ANCHOR_CONDITIONED_PORTAL_INTERIOR_FIELD_RESULT_2026-08-25.md) | 无算法 successor；R3 仅为冻结参考，展示接独立 V2 marker-pose seam | 补 Faro；新 boundary 信息源/top-k/pair/fusion；训练 E0/V1-F student；R6/B2；把 D2 负结果外推为所有 portal field；P1/default App | 否 |
