...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / R10_NOT_EVALUABLE_DUAL_CLASS_COVERAGE / R10_NO_PROMOTION / R11_WEAK_DISTAL_ABSTENTION_DEVELOPMENT_ONLY / R11_HEAD_144_OF_144_PASS_ONE_SHOT_CONSUMED / R11_SOURCE_UNOPENED / R11_SCIENTIFIC_NOT_RUN / DEFAULT_APP_UNCHANGED` | [TARO current](taro/README.md) | `TARO_O1R_R11_FRESH_48_PARENT_BOUNDED_SOURCE_DOWNLOAD_IMPLEMENTATION_LOCK`：以真实 `2,960,390,828 bytes` 实现 no-redirect、HEAD/GET validator-bound 下载与 checkpoint；执行仍须后续 one-shot lock | 用 prior/LM/regularizer 伪造可观测；把 UNKNOWN 当 negative；覆盖、重跑、改门或救活已消费 R10；复用 R10 全 32-source-pool parents 冒充 fresh evidence；让 truth/knownness 进入 source phase；越过 R11 阶段锁读 source/FARO、运行模型、训练、主动提示、Android/HTP 或默认 App；把 WILD_LAB 结果写成部署、产品或安全证明 | 否 |
