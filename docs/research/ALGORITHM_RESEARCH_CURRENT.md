...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：连续视觉几何因子 → deterministic body-swept task geometry | `R2_F0_SYNTHETIC_REDUCER_PASS / F1_SUPERVISION_FRONTDOOR_SATISFIED / AG_ST_DIRECT_TEACHER_TO_AG_REAL_SEAM_PASS / F1_STUDENT_ATTEMPT17_FAIL_NO_PROMOTION / AG_R2_SUPERTEACHER_TO_AG_FINAL_V2_SEAM_PASS / AG_R2_CROSS_SENSOR_FACTOR_ACCURACY_CONFIRMATION_EXECUTOR_IMPLEMENTATION_LOCK_PASS_SYNTHETIC_ONLY / SCIENTIFIC_NOT_RUN / CONFIRMATION_OUTCOMES_UNOPENED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_R2_CROSS_SENSOR_FACTOR_ACCURACY_CONFIRMATION_ONE_SHOT_EXECUTION_LOCK`：只能另行授权后冻结 exact runtime/calibration/IMU bindings；本实现锁未创建或授权该 lock，不能自动执行 | 枚举/解压已封存 archive 或运行模型/Confirmation；在 exact calibration member encoding/keys 与 IMU column/axis/specific-force sign convention 未冻结前猜测解析；改 ETH3D parent/roster/gate；用 reducer/CLEAR/OCCUPIED 反传、校准或选模型；重选/重跑 Attempt17；在已消费 walking_xyz/sitting_rpy 上调参；把 synthetic implementation lock 写成跨传感器 PASS、HTP、默认 App、产品或安全证明 | 否 |
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / R10_NOT_EVALUABLE_DUAL_CLASS_COVERAGE / R10_NO_PROMOTION / R11_WEAK_DISTAL_ABSTENTION_DEVELOPMENT_ONLY / R11_SOURCE_DOWNLOAD_144_OF_144_INTEGRITY_PASS_ONE_SHOT_CONSUMED / R11_SOURCE_ARCHIVES_UNOPENED / R11_SCIENTIFIC_NOT_RUN / DEFAULT_APP_UNCHANGED` | [TARO current](taro/README.md) | `TARO_O1R_R11_FRESH_48_PARENT_SOURCE_INVENTORY_IMPLEMENTATION_LOCK`：只实现并以 fixture/已下载 containers 验证 ZIP inventory、CRC 与 exact frame/pose plan；像素、模型、FARO 继续关闭 | 用 prior/LM/regularizer 伪造可观测；把 UNKNOWN 当 negative；覆盖、重跑、改门或救活已消费 R10；复用 R10 全 32-source-pool parents 冒充 fresh evidence；让 truth/knownness 进入 source phase；越过 R11 阶段锁读 source/FARO、运行模型、训练、主动提示、Android/HTP 或默认 App；把 WILD_LAB 结果写成部署、产品或安全证明 | 否 |
