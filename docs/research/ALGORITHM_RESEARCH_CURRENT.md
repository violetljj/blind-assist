...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_TRANSFER_FAIL_STOP / ANCHOR_DEVICE_CANARY_PASS / RGB_PAIR_SUPPORT_PASS / RGB_HISTORY_RETENTION_COST_PASS / RGB_SELECTED_DECODE_INTEGRITY_PASS / FRESH_RAW_DEPTH_PAIR_FAIL_STOP / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_RGB_PAIR_FROZEN_VISUAL_EVIDENCE_BACKEND_PREFLIGHT_R0`：outcome-blind 审计现有冻结 RGB backend 并预注册唯一同预算 positive-only shadow protocol | 换/训练模型；用 live output 事后定义指标；把 pipeline readiness 写成任务增益；接默认 App、风险融合或 guidance | 否 |
