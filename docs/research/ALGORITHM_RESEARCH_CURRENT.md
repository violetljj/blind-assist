...
当前唯一 active algorithm lane 是 SAGE-LM V0 的 controlled monocular RGB observation successor；任何 prospective goal intake、
fresh confirmation、正式模型选择、新 proposal arm 或 App 集成都需要用户另行授权，不能由旧文档中的 successor 或历史优先级自行恢复。
...
| Goal Copilot / SAGE-LM V0 authority-separated last mile | identity 已由 exact semantic anchor 确认后，active target-aperture geometry 是否优于 bbox center/scale 的 approach 与 completion | `CONTROLLED 36 / TARGET-FRONT ARRIVAL 7->33 / DIRECTION 22.9%->67.9% / LATERAL 0.592->0.094 M / COMPLETION PRECISION 19.4%->93.3% / POSITIVE_MECHANISM_SIGNAL` | [SAGE-LM V0 result](goal-copilot/SAGE_LM_V0_CONTROLLED_GEOMETRY_RESULT_2026-08-24.md) | 保持 identity firewall/baseline，接 controlled short monocular RGB video 的真实 boundary/flow/depth observation | procedural result 冒充 real RGB/navigation/traversability；geometry 重绑 identity；先接 learned head/UI/Android/P1/default App | 否 |
