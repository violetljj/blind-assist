...
SAGE-LM V1-B 已实现 source-pose-assisted two-view line geometry，但官方 pose audit 发现 V1 materializer 把 rotation-vector
列误作 camera positions；冻结 pair 仅 `2/24` 满足 motion gate，正式为 `NOT_EVALUABLE`。B1/B2 raw output 不裁决 boundary
路线；正确 pose 的新 cohort 需要用户另行显式授权，当前无自动 successor。任何
...
| Goal Copilot / SAGE-LM last mile | exact identity 后，真实 RGB observation 能否忠实恢复 active aperture geometry | `V1-A ORACLE=24/24 PASS / V1-B B0 RAW=23/24 / CORRECT-POSE PAIR GATE=2/24 / NOT_EVALUABLE / B1-B2 NOT ADJUDICATED` | [V1-B result](goal-copilot/SAGE_LM_V1_B_SOURCE_POSE_TWO_VIEW_BOUNDARY_GEOMETRY_RESULT_2026-08-24.md) | 无；正确 pose 物化的新 cohort 须另行授权 | 用 raw B1/B2 调 detector/association；替换/挑当前 pair；改 policy/门/baseline/anchor；VO/Android/P1/App | 否 |
