...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D0_NO_ELIGIBLE_PRECISION_ARM / D1_MEDIA_PREFLIGHT_PASS_FINAL_8_SESSION_ROSTER_LOCKED / D1_608X448_SINGLE_CANDIDATE_HOST_CONVERSION_PASS / SM8650_CONTEXT_AND_DEVELOPMENT_OUTCOME_NOT_RUN / R2_CANDIDATE_NOT_SELECTED` | [DepthART current](hftf/README.md) | `DEPTHART_TASK_PRESERVING_D1_SM8650_HTP_CONTEXT_AND_OUTCOME_ACTIVATION_PREFLIGHT`：连接冻结 SM8650/v75 设备，对 exact DLC 建 context 并关闭 synthetic full-graph parity，再做无 outcome activation validation；通过后才可另行激活 Development quality screen | 设备门前计算 D1 task truth/模型 outcome 或性能；修改已锁 candidate recipe；用 host conversion 冒充 HTP execution；访问独立 R2 cohort 选模；事后把 G4-C 塞回 D0 | 否 |
