...
状态：`current / PRODUCT_RESEARCH_MAINLINE=GOAL_DRIVEN_VISUAL_COPILOT / P0_POLICY_DISCOVERY_CLOSED / P1_TARGET_PERSISTENCE / P1_R0_REAL_RGB_HEADROOM_ESTABLISHED_ON_CONSUMED_ADT / P1_A1_CONSERVATIVE_LOCAL_TRACK_VALIDITY / NO_SKY / DEFAULT_APP_UNCHANGED`
...
落在任何 visible ADT instance 都匹配不上的 background。当前唯一 successor 为
`P1_A1_CONSERVATIVE_LOCAL_TRACK_VALIDITY`：只改变 local-track RGB drift rejection，冻结 candidate generator、
reacquisition、state machine、evaluator、cohort 与 truth firewall。
...
|---|---|---|---|---|---|---|
| Goal Copilot / P1 Target Persistence | 已建立 physical referent 的身份连续性、丢失与保守重捕获 | `PRODUCT_AND_RESEARCH_MAINLINE / P1_R0_REAL_RGB_HEADROOM_ESTABLISHED_ON_CONSUMED_ADT / P1_A1_CONSERVATIVE_LOCAL_TRACK_VALIDITY / DEFAULT_APP_UNCHANGED` | [current](goal-copilot/README.md) / [P1-R0 result](goal-copilot/P1_R0_CONSUMED_ADT_BASELINE_RESULT_2026-08-21.md) / [contract](goal-copilot/P1_R0_TARGET_PERSISTENCE_CONTRACT_V1.md) | `P1_A1_CONSERVATIVE_LOCAL_TRACK_VALIDITY`：只研究 local flow 的 RGB contradiction / drift rejection，优先降低 wrong-background 与 wrong-lock | 改 candidate generator/reacquisition/evaluator/cohort；ReID/强 tracker；Sky、fresh/large cohort、Active Grounding、Android | 否 |
