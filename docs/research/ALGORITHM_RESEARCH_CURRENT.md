...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 连续因子 → selective no-regret → body-swept geometry | `CORRECTION_GAIN_LOPO_FAIL_STOP / ANGULAR_BOUNDARY_TASK_INERT / SUPPORT_VALIDITY_FAIL_OPEN / OBSTACLE_POSITIVE_UNDERCOVERAGE / SCIENTIFIC_NOT_RUN` | [Assistive Geometry current](assistive-geometry/README.md) | `AG_OBSTACLE_EVIDENCE_TRISTATE_CALIBRATION_CANARY_R0`：冻结 obstacle logit，在 consumed ARKit/TUM 上 parent-LOO 校准 `POSITIVE / VERIFIED_NEGATIVE / UNKNOWN`，逐 parent 控 false-negative 并要求双侧非零 coverage | 让 support 创建 validity；把低 obstacle probability 当 clear；重开 correction/boundary mapping/fresh3；把 consumed calibration 写成 task success | 否 |
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `PARALLEL_WILD_LAB / R12_THREE_SOURCE_LABEL_SATURATION_LOCALIZED / R13_TASK_EVIDENCE_ORACLE_HEADROOM_PASS / POSE_SCORER_NOT_YET_RUN` | [TARO current](taro/README.md) | `TARO_TASK_EVIDENCE_POSE_SCORER_R0`：只用 static evidence + relative pose 学习 R13 oracle gain；FIT 内选模，已消费 EVAL replay 后仍须 untouched 4-parent confirmation | 回调 R12 label/gate；把 oracle/neighbor depth 泄漏进 scorer input；把 Development oracle 写成 learned policy、Android 或产品成功 | 否 |
