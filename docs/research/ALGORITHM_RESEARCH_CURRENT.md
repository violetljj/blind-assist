...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R27_ARKIT_TUM_FULL_GATE_PASS / R27_BONN_DYNAMIC_MACRO_FAIL / OPENLORIS_HOME_FRESH_SOURCE_LOCKED / SAME_TABLE_TUNING_STOP` | [TARO current](taro/README.md) | `TARO_R27_OPENLORIS_HOME_FRESH_SOURCE_FRONTDOOR_R0`：按冻结身份在 home1-1..5 原样检验 R27 | 在 R23–R30 consumed table 调参；使用未校准 RGB/depth；读取 candidate depth；接默认 App/risk/guidance；夸大为 broad breakthrough、安全或产品成功 | 否 |
