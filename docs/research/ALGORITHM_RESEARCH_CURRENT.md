...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `ROUTE_SELECTED / B1_DUAL_ORIENTATION_PROTOCOL_LOCK_PASS / IMPLEMENTATION_NOT_AUTHORIZED / FORMAL_TRAINING_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B1_DUAL_ORIENTATION_TARGET_AND_MODEL_IMPLEMENTATION_LOCK`：生成 dual-shape TRAIN target cache，实现 orientation bucket、shared decoder/head/loss，并关闭 forward/backward、flip/K 与 resume smoke | implementation lock 前正式训练 student；读取 DEVELOPMENT/CONFIRMATION outcome；跨 orientation 裁剪/旋转凑单一 shape；运行 DA3/Metric3D teacher | 否 |
