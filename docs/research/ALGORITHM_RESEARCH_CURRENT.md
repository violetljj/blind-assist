...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / P0_PASS / O0M_PROTOCOL_FROZEN / O0M_IMPLEMENTATION_LOCK_PASS / O0M_ONE_SHOT_LOCKED_UNCONSUMED / SCIENTIFIC_STATUS_NOT_RUN / O0R_NOT_EVALUABLE_DATA_AND_INTERFACE` | [TARO current](taro/README.md) | `TARO_O0M_ONE_SHOT_EXECUTION`：只按已提交 exact argv 运行一次；root 创建即消费，不覆盖或重跑 | 用 prior/LM/regularizer 伪造可观测；把 K 混入 S/P/B factorial；继承 B1 outcome/threshold；把 UNKNOWN 当 negative；超出 one-shot 执行、读 real outcome、训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App；把 synthetic mechanics 冒充 real headroom | 否 |
