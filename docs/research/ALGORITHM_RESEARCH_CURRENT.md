...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_FAIL_STOP / POSE_DIVERSE_BASELINE_MULTI_SOURCE_PASS / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_POSE_DIVERSE_SELECTOR_ARCORE_VIO_ISOLATED_CANARY_R0`：隔离 canary 连接既有 VIO receipt/pose buffer 与默认关闭 selector，只输出 selection receipt | 放宽 scorer opportunity gate；把 generic selector 写成 task scorer；接默认 App、风险融合或用户 guidance | 否 |
