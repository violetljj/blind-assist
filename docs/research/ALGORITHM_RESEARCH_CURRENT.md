...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R27_OPENLORIS_FRESH_SOURCE_FAIL / R27_FRESH_SOURCE_REJECTED / R31_RELIABILITY_CONSISTENCY_SUCCESSOR` | [TARO current](taro/README.md) | `TARO_R31_MULTI_CANDIDATE_RELIABILITY_AND_OCCLUSION_CONSISTENCY_R0`：在 consumed 四源上开发结构性新机制，再进入新的 source/parent-disjoint confirmation | 回调 R27 threshold/gate；把 OpenLORIS 再写成 fresh；使用未校准 RGB/depth；读取 candidate depth；接默认 App/risk/guidance；夸大为 broad breakthrough、安全或产品成功 | 否 |
