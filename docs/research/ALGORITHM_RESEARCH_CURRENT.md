...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：连续视觉几何因子 → deterministic body-swept task geometry | `B1_A0_PERMANENTLY_CLOSED / FAILURE_ANATOMY_DIAGNOSTIC_COMPLETE_NOT_PROMOTABLE / R2_F0_SYNTHETIC_REDUCER_PASS / F1_EXECUTION_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_R2_F1_TRAIN_ONLY_FACTOR_LEARNABILITY_PROTOCOL_LOCK`：只允许另行冻结 F1 TRAIN-only 协议，不授权数据物化、模型初始化或训练 | 复用 B1 Selection/threshold；修救 A0 或激活 A1–A4；让 learned graph 输出 final task state；把 F0 mechanics 当真实任务收益；提前执行 F1、分配 Development、teacher、HTP、时序或默认 App | 否 |
