...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_PREOUTCOME_PROTOCOL_FROZEN / D3_BIDIRECTIONAL_ROUTER_MECHANICS_PASS / D3_SOURCE_SCOPE_NOT_ACTIVATED / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3_FRESH_SOURCE_SCOPE_AND_PARENT_DISJOINT_METADATA_ROSTER_LOCK`：冻结 D1/D2/R2/其他 outcome-bearing identity 排除集合、48 身份 metadata pool 与 source-use scope；不访问媒体 | 用已消费 D1/D2 outcome 拟合或回救 D3；未经独立 gate 访问媒体、source truth、训练或 Development；用 Development 选阈值/checkpoint；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
