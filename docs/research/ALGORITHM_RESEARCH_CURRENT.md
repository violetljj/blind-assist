...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_FAIL_STOP / ANCHOR_DEVICE_CANARY_PASS / RGB_PAIR_SUPPORT_PASS / FRESH_RAW_DEPTH_PAIR_FAIL_STOP / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_RGB_FRAME_HISTORY_RETENTION_AND_COST_R0`：隔离复制并有界保留 source-bound YUV，验证 selection→payload identity 与内存/延迟 | 用旧/reprojected depth 回救；把 RGB pair support 写成任务增益；接默认 App、风险融合或 guidance | 否 |
