...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_BIDIRECTIONAL_ROUTER_MECHANICS_PASS / D3_SOURCE_SCOPE_REGISTERED / D3_FRESH_METADATA_POOL_48_LOCKED / D3_PHASE_A_NOT_ACTIVATED / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3_PHASE_A_INTRINSICS_TRAJECTORY_HEAD_ONLY_PREFLIGHT_ACTIVATION`：只对冻结 48 身份的 intrinsics/trajectory 发 HEAD 并锁 header receipt；不读取 body | 用已消费 D1/D2 outcome 拟合或回救 D3；未经独立 gate 发 GET、读取 archive/pose/truth、训练或 Development；用 Development 选阈值/checkpoint；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
