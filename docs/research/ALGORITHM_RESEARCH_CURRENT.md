...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `A0_EVALUATION_SYNTHETIC_DRY_RUN_PASS / SEED_17_GUARDED_EXECUTION_RUNNING / SEEDS_29_43_NOT_STARTED / DEVELOPMENT_AND_CONFIRMATION_SEALED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B1_A0_SEED_17_GUARDED_FORMAL_TRAIN_EXECUTION`：完成已启动的 seed 17 TRAIN-only A0；完整终态后依次执行 29/43，三 seed 齐全后才激活真实 Development evaluation | 训练中读取 DEVELOPMENT/CONFIRMATION outcome；best-seed 早停/选择；运行 A1–A4、双教师、HTP 或默认 App | 否 |
