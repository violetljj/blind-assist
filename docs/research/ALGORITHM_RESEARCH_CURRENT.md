...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R27_ARKIT_TUM_FULL_GATE_PASS / R27_BONN_DYNAMIC_MACRO_FAIL / R29_THREE_SOURCE_MACRO_PASS_BREADTH_FAIL / SAME_TABLE_TUNING_STOP` | [TARO current](taro/README.md) | `TARO_R27_FRESH_REGISTERED_RGBD_POSE_SOURCE_FRONTDOOR_R0`：锁定新 source 后原样检验 query-aligned reprojection visibility | 在 R23–R30 consumed table 调参；使用未校准 RGB/depth；读取 candidate depth；接默认 App/risk/guidance；夸大为 broad breakthrough、安全或产品成功 | 否 |
