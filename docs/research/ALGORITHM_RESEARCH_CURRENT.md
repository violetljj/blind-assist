...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_MECHANICS_CANARY_PASS / D2R1_SOURCE_SUPPORT_PASS_16_OF_16 / D2_4_TRAIN_4_DEVELOPMENT_SEALED / D2_PHASE_C_RGB_HEAD_AWAITING_EXPLICIT_SCOPE / D2_MODEL_AND_TRAINING_NOT_ACTIVATED / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D2_PHASE_C_EXACT_EIGHT_RGB_HEAD_ONLY`（execution=false）：只允许对 exact 8 个 D2 role identity 的 `lowres_wide.zip` 发出 HEAD，冻结 RGB body 大小；无 GET、模型、训练、Development 或 R2 | 在已消费 D1 outcome 上回救；修改 D2R1 identity/window/support 门；从 HEAD 自动继承 body/training；提前打开 sealed Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
