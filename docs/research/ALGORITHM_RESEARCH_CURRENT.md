...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / P0_PASS / O0M_SYNTHETIC_ANALYTIC_MECHANICS_PASS / O0M_ONE_SHOT_CONSUMED / O0R_SOURCE_AND_ADAPTER_CONTRACT_LOCK_PASS / O0R_TRUTH_PREFLIGHT_NOT_RUN / O0R_SCIENTIFIC_NOT_RUN / EXECUTION_NOT_AUTHORIZED` | [TARO current](taro/README.md) | `TARO_O0R_ARKITSCENES_SOURCE_ADAPTER_IMPLEMENTATION_LOCK`：只实现并静态测试冻结的 receipt/truth/uncertainty/factor-injection adapter；不得打开 source payload 或执行真实 O0R | 用 prior/LM/regularizer 伪造可观测；把 K 混入 S/P/B factorial；继承 B1 outcome/threshold；把 UNKNOWN 当 negative；覆盖或重跑 O0M；在独立 truth-only lock 前下载/打开新 source、物化 truth、运行 DepthART 或执行 O0R；训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App；把 synthetic mechanics 或静态契约写成 real headroom；跳过 O0R 进入 G0/G1/A0/A1/J0 | 否 |
