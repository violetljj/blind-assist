...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_FAIL_STOP / POSE_DIVERSE_BASELINE_MULTI_SOURCE_PASS / ANCHOR_ADMISSION_ANDROIDTEST_COMPILE_PASS / DEVICE_ENV_BLOCKED / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_POSE_DIVERSE_SELECTOR_ARCORE_ANCHOR_DEVICE_RUN_R0`：在明确 ARCore 真机上运行已实现的 project-owned same-anchor camera-pose canary，只输出 selection receipt | 把 camera-only anchor admission 写成 body-frame/risk-fusion 权限；放宽 scorer gate；接默认 App或 guidance | 否 |
