...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `PARALLEL_WILD_LAB / R10_NOT_EVALUABLE_DUAL_CLASS_COVERAGE / R11_SOURCE_ONLY_TOP24_ONE_SHOT_CONSUMED_PASS / R11_SELECTED_TOP24_FARO_PHASE_B_IMPLEMENTATION_LOCK_PASS_NON_EXECUTING / R11_FARO_PHASE_B_EXECUTION_LOCK_AUTHORIZED_UNCONSUMED_PARKED_NONBLOCKING / TASK_OBSERVABILITY_D0_ACTIVE / R11_SCIENTIFIC_NOT_RUN` | [TARO current](taro/README.md) | `TARO_TASK_DIRECTED_OBSERVABILITY_POSITIVE_ORACLE_CANARY_R0`：换 pose-rich source，同预算比较 static/passive/micro/generic/task oracle，仅开放 `UNKNOWN→OCCUPIED` | 改 selected 24 或读 R11 outcome；在零 pair cohort 训练/放宽窗口；输出 CLEAR、把 UNKNOWN 当 negative 或产品化 | 否 |
