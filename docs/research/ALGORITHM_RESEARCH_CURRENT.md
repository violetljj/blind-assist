...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_PHASE_A_FAIL_21_OF_32_NO_SELECTION / D3R1_FRESH_METADATA_POOL_127_LOCKED_MEDIA_UNOPENED / D3R1_SOURCE_SCOPE_AWAITING / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3R1_SOURCE_SCOPE_REGISTRATION_FOR_EXACT_127_METADATA_ROSTER`：只登记 exact-127 future Phase-A intrinsics/trajectory source-use scope，仍不授权 HEAD/GET | 复用或扩大旧 D3 exact-48/21-qualified pool；降低 continuity 门；把 127-pool planning heuristic 写成成功保证；未登记 source scope 即发 HEAD/GET；进入 Phase-B、训练或 Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
