...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `ROUTE_SELECTED / B0_GEOMETRY_RECEIPT_PASS / DATA_ROSTERS_UNRESOLVED / EXECUTION_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B0_DATA_CAPABILITY_AND_ROSTER_LOCK`：shape 已支持，SM-S9280 benchmark geometry/tensor K 已观测；下一步只做新数据 capability/identity/license/roster lock | blocker 未闭合时训练 student；直接全开 DA3 + Metric3D；把 pseudo-label 当安全 truth；用已消费 cohort 或部署 R2 roster 选模 | 否 |
