...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / P0_PASS / O0M_SYNTHETIC_ANALYTIC_MECHANICS_PASS / O0R_SOURCE_TRUTH_MATERIALIZED / DEPTHART_CANDIDATES_239_SEALED / SOURCE_ANCHORED_FACTOR_CANARY_R1_COMPLETE / DIRECT_APPLE_HYBRID_R4A_COMPLETE / R5_TASK_METRIC_CONFIRMATION_FAIL / R6_PROTOCOL_FROZEN / R6_FACTOR_COMPOSITOR_IMPLEMENTATION_FROZEN / FORMATION_REPLAY_PASS / UNTOUCHED_DATA_AUTHORITY_REQUIRED / NO_ACTIVE_EXECUTION` | [TARO current](taro/README.md) | `TARO_O0R_R6_UNTOUCHED_COHORT_AND_DATA_USE_LOCK`：冻结至少 8 个未参与 R4/R5/R6 形成的新 parents、exact frame counts、source/truth hashes 与明确数据使用权限；仍不执行模型或 truth scoring | 用 prior/LM/regularizer 伪造可观测；把 UNKNOWN 当 negative；覆盖或重跑已消费 root；复用当前 24 个 formation parents 冒充 untouched；让 truth/knownness 参与 factor ownership；改变 R6 exact-copy compositor、factor depth lineage、8-parent floor 或 gates；把 formation replay 写成 confirmation；训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App；跳过完整 query truth admission进入 G0/G1/A0/A1/J0 | 否 |
...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_MECHANICS_CANARY_PASS / D2R1_SOURCE_SUPPORT_PASS_16_OF_16 / D2_4_TRAIN_4_DEVELOPMENT_SEALED / D2_PHASE_C_SOURCE_MATERIALIZATION_PASS / D2_TRAIN_ONLY_ACTIVATION_AWAITING_EXPLICIT_SCOPE / D2_MODEL_AND_TRAINING_NOT_ACTIVATED / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D2_TRAIN_ONLY_BASE_OUTPUT_AND_HEAD_TRAINING_ACTIVATION`（execution=false）：只允许 4 TRAIN / 1,200 帧的 decode、truth、frozen saved-context output 和固定 step-500 head training；4 DEVELOPMENT sealed | 在已消费 D1 outcome 上回救；修改 D2R1 数据/门；提前打开 Development；改变 seed/recipe/features/checkpoint；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
