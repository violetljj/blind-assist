...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `PARALLEL_WILD_LAB / R13_ORACLE_HEADROOM_PASS / R14_R20_SCORER_TRANSFER_NOT_CONFIRMED / LEARNED_RANKER_DEVELOPMENT_ACTIVE` | [TARO current](taro/README.md) | `TARO_CROSS_SOURCE_LEARNED_RANKER_R0`：只用已消费 TUM/Bonn/ARKit task-evidence target，做 leave-one-source-family-out；三源均胜 passive/generic 后才锁新确认源 | 回调 R12/R19/R20 gate；把 neighbor depth 泄漏进 scorer input；把 R18 Development 或确认失败写成 Android/产品成功 | 否 |
