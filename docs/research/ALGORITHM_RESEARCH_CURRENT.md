...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_BIDIRECTIONAL_ROUTER_MECHANICS_PASS / D3_FRESH_METADATA_POOL_48_CONSUMED / D3_PHASE_A_HEAD_96_OF_96_PASS / D3_PHASE_A_FAIL_21_OF_32_NO_SELECTION / D3_NO_ACTIVE_SUCCESSOR / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | 无；当前 D3 evidence version 因 Phase-A 仅 21/48 identity 满足固定 continuity 门而终止。恢复必须另立 pre-outcome fresh roster/source-scope/protocol 版本 | 扩大当前 48 identity pool、降低 continuity 门或晋级 21 个 partial role；用已消费 D1/D2 outcome 拟合或回救；进入 Phase-B、训练或 Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
