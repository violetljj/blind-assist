...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `A0_SEED_17_COMPLETE / SEED_29_GUARDED_EXECUTION_RUNNING / SEED_43_NOT_STARTED / DEVELOPMENT_EVALUATION_IMPLEMENTED_NOT_ACTIVATED / DEVELOPMENT_AND_CONFIRMATION_SEALED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B1_A0_SEED_29_GUARDED_FORMAL_TRAIN_EXECUTION`：完成 seed 29 后执行 43；只有三 seed 齐全才允许物化 Development Selection 并运行已冻结 evaluator v2 | 训练中读取 DEVELOPMENT/CONFIRMATION outcome；best-seed 早停/选择；运行 A1–A4、双教师、HTP 或默认 App | 否 |
| AG-QSF / Queryable Survival Geometry | 并行 WILD_LAB：以 profile-queryable body-swept robust q-contact 生存分布统一 clearance 与 horizon-consistent occupancy | `PARALLEL_ROUTE_PREPARED / H1_IMPLEMENTATION_NOT_STARTED / TRAIN_CANARY_NOT_AUTHORIZED / LEARNABILITY_NOT_ESTABLISHED` | [AG-QSF current](assistive-geometry-qsf/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_QSF_H1_ONLY_IMPLEMENTATION_AND_TRAIN_CANARY_LOCK`：先关闭 H1 target/mask/decoder/loss/head 与隔离测试，再另行冻结 TRAIN-only canary | 读 B1 Development/Confirmation 或 active run；改 B1 successor/seed/gate；共享可变输出；抢占正式 seed 的 GPU/重 I/O；H1/H2 各自过 canary 前训练组合版 | 否 |
