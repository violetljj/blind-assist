...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_TRAIN_ONLY_24_OF_24_COMPLETE / D2_STEP500_HEAD_LOCKED / D2_DEVELOPMENT_QUALITY_24_OF_24_COMPLETE / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / NO_ACTIVE_SUCCESSOR / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | 无；任何新假设必须建立新版本、pre-outcome 合同与 fresh data，并重新取得显式授权 | 在已消费 D1/D2 outcome 上回救；改变已锁 head/recipe/features/checkpoint/阈值/data/postprocess/denominator/gates；用 Development 训练或校准；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
