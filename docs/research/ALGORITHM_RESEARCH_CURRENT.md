...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：连续视觉几何因子 → deterministic body-swept task geometry | `R2_F0_SYNTHETIC_REDUCER_PASS / F1_P_PROTOCOL_FROZEN / FACTORTENSOR_ADAPTER_PROTOCOL_FROZEN / ADAPTER_IMPLEMENTATION_AUTHORIZED_NOT_STARTED / SUPERVISION_FRONTDOOR_UNSATISFIED / F1_EXECUTION_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_R2_F1_FACTORTENSOR_ADAPTER_IMPLEMENTATION_AND_SYNTHETIC_CANARY`：只实现冻结的零参数 adapter 并运行 8-case/A01–A10 synthetic canary；不读 real data/outcome | 改写 F0/F1-P/adapter protocol 冻结字节；更改 17 项 operation；让 learned graph 输出 final task state；提前物化、训练、F2、teacher、HTP、时序或默认 App；把 synthetic seam mechanics 写成 real factor headroom | 否 |
...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D0_NO_ELIGIBLE_PRECISION_ARM / D1_FINAL_8_SESSION_ROSTER_LOCKED / D1_608X448_SM8650_V75_CONTEXT_AND_EXECUTION_PREFLIGHT_PASS / RAW_DEPTH_PARITY_DIAGNOSTIC_FAIL / DEVELOPMENT_QUALITY_SCREEN_ACTIVATED_AND_RUNNING / FIRST_50_FRAME_CHUNK_DEVICE_PASS / R2_CANDIDATE_NOT_SELECTED` | [DepthART current](hftf/README.md) | `COMPLETE_FROZEN_D1_DEVELOPMENT_TASK_QUALITY_SCREEN`：按冻结 48×50-frame chunk 完成 2400 帧 reference/candidate outputs，再一次性运行 task-quality gates；quality PASS 前不得测性能 | 修改 candidate/data/postprocess/known-coverage denominator/gates；用部分块作选择或结论；把 raw-depth diagnostic 当 D1 task result或重开 strict G4-D；访问独立 R2 cohort；事后把 G4-C 塞回 D0 | 否 |
