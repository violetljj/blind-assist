...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / P0_PASS / O0M_PROTOCOL_FROZEN / SCIENTIFIC_STATUS_NOT_RUN / O0M_IMPLEMENTATION_NOT_AUTHORIZED / O0M_EXECUTION_NOT_AUTHORIZED / O0R_NOT_EVALUABLE_DATA_AND_INTERFACE` | [TARO current](taro/README.md) | `TARO_O0M_IMPLEMENTATION_LOCK`：只在独立 runtime Module 创建纯解析 mechanics 与 tests 并冻结 SHA；不得运行 canary 或创建 scientific artifact | 用 prior/LM/regularizer 伪造可观测；把 K 混入 S/P/B factorial；继承 B1 outcome/threshold；把 UNKNOWN 当 negative；提前执行 O0M、读 real outcome、训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App；把 synthetic mechanics 冒充 real headroom | 否 |
