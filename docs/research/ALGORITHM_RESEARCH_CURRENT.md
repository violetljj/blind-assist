...
ARKitScenes mesh/Faro depth 建立独立弱 aperture boundary teacher 的 V1-E privileged geometry supervision。
...
| Goal Copilot / SAGE-LM last mile | exact identity 后，真实 RGB observation 能否忠实恢复 active aperture geometry | `R3 TRUE-PAIR=15 GEOMETRY=13 / V1-C1=3/3, FOUR-BOUNDARY=4/24 / V1-D=4/4, R3-MISSING-RESCUE=0/9 / C AND D CLOSED / R6 NOT RUN / B2 NOT RUN` | [V1-D result](goal-copilot/SAGE_LM_V1_D_ACTIVE_PARALLAX_BOUNDARY_FIELD_RESULT_2026-08-25.md) | V1-E：先从 ARKitScenes mesh/Faro depth 建立与 RGB line strength 独立的 weak-aperture boundary teacher，再训练 RGB(+anchor) student | 继续训练/换 backbone/loss/top-k 或重采样 V1-C proxy；扫 V1-D flow/FB/gradient/pair/endpoint；R3 fusion；运行 R6/B2；Android/P1/App | 否 |
