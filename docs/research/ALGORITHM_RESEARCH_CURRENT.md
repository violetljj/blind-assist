...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_PHASE_A_FAIL_21_OF_32_NO_SELECTION / D3R1_PHASE_A_BODY_127_PROCESSED_53_ELIGIBLE_32_LOCKED / D3R1_PHASE_B_SOURCE_SCOPE_AWAITING / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3R1_PHASE_B_DEPTH_CONFIDENCE_SOURCE_SCOPE_REGISTRATION_FOR_EXACT_32_PHASE_A_SELECTION`：仅登记 exact-32 的 depth/confidence 后续范围，不自动发 HEAD/GET | 复用或扩大旧 D3 exact-48/21-qualified pool；改写已锁 32 身份/windows；降低 continuity 门；未登记 scope 即进入 Phase-B；训练或 Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
