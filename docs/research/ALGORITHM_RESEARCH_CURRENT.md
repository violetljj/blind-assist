...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `ROUTE_SELECTED / B1_DUAL_ORIENTATION_IMPLEMENTATION_LOCK_PASS / A0_EXECUTION_PROTOCOL_NOT_FROZEN / FORMAL_TRAINING_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B1_A0_DEPTH_ONLY_THREE_SEED_TRAIN_EXECUTION_LOCK`：冻结 A0 loader、optimizer/scheduler、checkpoint/resume、三 seed 与 receipt 后，才允许 TRAIN-only depth baseline execution | execution lock 前正式训练；读取 DEVELOPMENT/CONFIRMATION outcome；用带缺失 Autograd-key 警告的部署 operator 训练；运行 A1–A4 或双教师 | 否 |
