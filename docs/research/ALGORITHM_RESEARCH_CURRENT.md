...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：连续视觉几何因子 → deterministic body-swept task geometry | `B1_A0_PERMANENTLY_CLOSED / FAILURE_ANATOMY_DIAGNOSTIC_COMPLETE_NOT_PROMOTABLE / R2_PRE_OUTCOME_HYPOTHESIS_FROZEN / F0_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_R2_F0_SYNTHETIC_FACTOR_GEOMETRY_CANARY_PROTOCOL_AND_FIXTURES`：只冻结 factor schema、deterministic reducer 与 analytic fixtures，当前无 execution authority | 复用 B1 Selection/threshold；修救 A0 或激活 A1–A4；让 learned graph 输出 final task state；提前训练、分配 Development、teacher、HTP、时序或默认 App | 否 |
...
| AG-CBF / Corridor Bottleneck Field | 并行 WILD_LAB：检验 ground-aligned、body-profile inflated、拓扑连通的 corridor bottleneck 表示是否保留三带摘要丢失的信息 | `R0_DATA_SUPPORT_AUDIT_LOCKED_NOT_RUN / ORACLE_NOT_AUTHORIZED / MODEL_AND_TRAINING_NOT_AUTHORIZED` | [AG-CBF current](assistive-geometry-cbf/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_CBF_R0_TRAIN_GRID_DATA_SUPPORT_AUDIT_EXECUTION` | 在数据支撑前计算 oracle outcome 或建模；把 UNKNOWN 当 free/occupied；读取 A0 consumed Development/Confirmation；把并行路线静默升为主线 successor | 否 |
