...
状态：`current / PRODUCT_RESEARCH_MAINLINE=GOAL_DRIVEN_VISUAL_COPILOT / PASSIVE_SINGLE_REFERENCE_RGB_EXACT_INSTANCE_MAINLINE_STOP / SEMANTIC_ANCHOR_V1_CONTROLLED_DEMO / CLOSE_NATURAL_SAGE_R / CONTROLLED_EXACT_ANCHOR_RETAINED / SAGE_LM_V0_CONTROLLED_36_POSITIVE / SAGE_LM_V1_REAL_RGB_24_FAIL_2_VS_7 / V1_A_ALL_ORACLE_24_OF_24_PASS / SOURCE_POSE_TWO_VIEW_NEXT / LIVE_ANDROID_NOT_RUN / NO_P1 / DEFAULT_APP_UNCHANGED`
...
当前唯一 active algorithm lane 是 SAGE-LM V1-A faithfulness audit；24 个 controlled real-RGB episode 未保留 V0 synthetic
uplift，但同 cohort all-oracle 为 `24/24` arrival、原八条标准全过，已建立 frozen downstream policy ceiling。唯一 successor
是 source-pose-assisted two-view boundary geometry，不先修 LK、换 flow/depth 或改门。任何
...
| Goal Copilot / SAGE-LM last mile | exact identity 后，真实 RGB observation 能否忠实恢复 active aperture geometry | `V0 7->33 POSITIVE / V1 RGB: 7 VS 2 FAIL / V1-A ORACLE: 24/24 PASS / DOWNSTREAM_CEILING_ESTABLISHED` | [V1-A result](goal-copilot/SAGE_LM_V1_A_ALL_ORACLE_OBSERVATION_CEILING_RESULT_2026-08-24.md) | source-pose two-view boundary geometry | 修/扫 LK、换 flow/depth、改 policy/门/baseline/cohort/anchor；Android/P1/App | 否 |
