...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / R0_ROUTE_GUIDE_FROZEN / DESIGN_AUTHORIZED / EXECUTION_NOT_AUTHORIZED` | [TARO current](taro/README.md) | `TARO_P0_TASK_QUERY_IDENTIFIABILITY_AND_FACTOR_ORACLE_CANARY_PROTOCOL_LOCK`：只冻结 schema、局部 identifiability、analytic fixture、factor-oracle arms、退化负控、数据角色、指标与停止条件；不授权实现或执行 | 从 RGB-only/纯旋转伪造 metric scale；覆盖有效 K receipt；继承 B1 outcome/threshold；把 UNKNOWN 当 negative；让 learned graph/solver/scorer 输出最终三态；提前运行 oracle、训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App | 否 |
