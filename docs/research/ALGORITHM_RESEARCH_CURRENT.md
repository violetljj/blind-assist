...
状态：`current / PRODUCT_RESEARCH_MAINLINE=GOAL_COPILOT / GOAL_COPILOT_1_PERMANENTLY_CLOSED / GOAL_COPILOT_2A_COMPLETE / GOAL_COPILOT_2B_SEARCH_COMPLETE_NO_HELDOUT_ADMISSION / GC2B_NOISE_ROBUST_SEARCH_SIGNAL_NOT_ESTABLISHED / ACTIVE_ALGORITHM_P0=D_ORACLE_1 / DEFAULT_APP_UNCHANGED`
...
GC2-A perception-uncertainty characterization 已完成并触发 GC2-B protocol-design admission，但尚未
授权任何 GC2-B 模型调用或搜索。D-ORACLE-1 仍是当前 evidence-provider/
...
|---|---|---|---|---|---|---|
| Goal-Driven Visual Copilot / GOAL-COPILOT-2 | 冻结 GC1 winner 在 perception uncertainty 下的 closed-loop robustness | `PRODUCT_AND_RESEARCH_MAINLINE / GOAL_COPILOT_2B_SEARCH_COMPLETE_NO_HELDOUT_ADMISSION / GC2B_NOISE_ROBUST_SEARCH_SIGNAL_NOT_ESTABLISHED / NO_SUCCESSOR_AUTHORIZED` | [Goal Copilot current](goal-copilot/README.md) | 无；只有 materially different uncertainty/state representation 才可另立 pre-outcome successor | 恢复 GC1/GC2-B；打开 held-out；rescue rerun/扩预算/同候选面 GC2-C；夸大为真实感知、安全、产品证据 | 否 |
