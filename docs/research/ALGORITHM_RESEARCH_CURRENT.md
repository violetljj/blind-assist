...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 连续因子 → selective no-regret → body-swept geometry | `FACTORWISE_ORACLE_HEADROOM_PASS / CORRECTION_GAIN_LOPO_FAIL_STOP / ANGULAR_BOUNDARY_FAIL_CLOSED_SAFE_BUT_TASK_INERT / SCIENTIFIC_NOT_RUN` | [Assistive Geometry current](assistive-geometry/README.md) | `AG_POSITIVE_OBSTACLE_SUPPORT_TASK_EFFECT_AUDIT_R0`：在 consumed ICL 上 prediction-first 地执行 learned-support-only、learned-obstacle-only 与二者合取替换臂，定位真正影响 body-swept state 的 positive factor | 重开 correction router/fresh3；继续调 boundary mapping；用 UNKNOWN 当负类或把 consumed audit 写成 task success | 否 |
