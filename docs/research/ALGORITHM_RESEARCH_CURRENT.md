...
| TARO / Task-directed Active Risk Observability | 同预算观测使 body/path query 更早可识别 | `R13_ORACLE_HEADROOM_PASS / R14_R22_TASK_SCORER_TRANSFER_FAIL_STOP / ANCHOR_DEVICE_CANARY_PASS / RGB_PAIR_SUPPORT_PASS / RGB_HISTORY_RETENTION_COST_PASS / RGB_SELECTED_DECODE_INTEGRITY_PASS / YOLO_POSITIVE_EVIDENCE_SHADOW_PASS / FRESH_RAW_DEPTH_PAIR_FAIL_STOP / CORE_SELECTOR_DEFAULT_OFF` | [TARO current](taro/README.md) | `TARO_DEFAULT_OFF_APP_POSITIVE_EVIDENCE_SHADOW_PREFLIGHT_R0`：先锁 App integration seam/cadence/resource/abstention，再允许无 guidance telemetry shadow | 换/训练模型；保存图像/box/身份；把单设备 screen-space positive evidence 写成 detector accuracy、碰撞/安全或产品成功；未经 preflight 接默认 App、风险融合或 guidance | 否 |
