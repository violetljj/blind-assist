...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / R10_NOT_EVALUABLE_DUAL_CLASS_COVERAGE / R10_NO_PROMOTION / R11_WEAK_DISTAL_ABSTENTION_DEVELOPMENT_ONLY / R11_FRESH_48_PARENT_PROTOCOL_LOCKED / R11_SCIENTIFIC_NOT_RUN / R11_EXECUTION_FALSE / DEFAULT_APP_UNCHANGED` | [TARO current](taro/README.md) | 先取得 exact R11 48-parent × 3-asset 数据使用授权；授权后唯一代码 successor 为 `TARO_O1R_R11_FRESH_48_PARENT_ZERO_BODY_HEAD_ONE_SHOT_EXECUTION_LOCK`（仍为 execution=false，直至另行消费）。当前只允许静态 validator 与 metadata-only roster 重算 | 用 prior/LM/regularizer 伪造可观测；把 UNKNOWN 当 negative；覆盖、重跑、改门或救活已消费 R10；复用 R10 全 32-source-pool parents 冒充 fresh evidence；让 truth/knownness 进入 source phase；未获 exact R11 权限即发 HEAD、读 source/FARO、运行模型、训练、主动提示、Android/HTP 或默认 App；把 WILD_LAB 结果写成部署、产品或安全证明 | 否 |
