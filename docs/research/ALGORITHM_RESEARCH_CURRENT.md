...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D0_NO_ELIGIBLE_PRECISION_ARM / D1_LICENSE_SCOPE_EXTENDED / D1_HEADER_80_OF_80_PASS / BODY_PREFLIGHT_FROZEN_NOT_RUN / PRODUCT_ASPECT_GRAPH_NOT_BUILT / R2_CANDIDATE_NOT_SELECTED` | [DepthART current](hftf/README.md) | `DEPTHART_TASK_PRESERVING_D1_LABEL_BLIND_BODY_PREFLIGHT_EXECUTION`：按冻结主备顺序执行 300-frame portrait/pose/RGB-D/K body preflight；通过后才重建 `608×448` fixed-mixed 单候选 | 计算 D1 task truth 或模型 outcome；按内容/结果替换 primary；用 `448×448` canary 替代产品视场；访问独立 R2 cohort 选模；事后把 G4-C 塞回 D0 | 否 |
