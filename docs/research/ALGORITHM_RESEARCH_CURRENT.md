...
| AG-QSF / Queryable Survival Geometry | 并行 WILD_LAB：以 profile-queryable body-swept robust q-contact 生存分布统一 clearance 与 horizon-consistent occupancy | `H1_IMPLEMENTED / ATTEMPT_03_PERFORMANCE_QUALIFIED / H1_NOT_EVALUABLE_EVAL_RIGHT_CENSOR_ZERO / LEARNABILITY_NOT_ESTABLISHED` | [AG-QSF current](assistive-geometry-qsf/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_QSF_H1_PARENT_LEVEL_TRAIN_SUPPORT_AUDIT_AND_RELOCK`：先做无模型 TRAIN target support audit，按预冻结 manifest-order 规则建立两侧分母非零的新 split；通过后才可另立 H1 evidence version | 读 B1 Development/Confirmation 或 active checkpoint/progress；把 support-based roster 当 unbiased evaluation；H1 有效科学终态前实现 H2；H1/H2 各自通过前训练组合版 | 否 |
