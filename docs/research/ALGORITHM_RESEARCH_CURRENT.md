...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / R10_FRESH_32_PARENT_PIPELINE_COMPLETE / R10_POSITIVE_OCCUPANCY_GATES_PASS / R10_NOT_EVALUABLE_DUAL_CLASS_COVERAGE / R10_NO_PROMOTION / DEFAULT_APP_UNCHANGED` | [TARO current](taro/README.md) | `TARO_O1R_R11_POSITIVE_OCCUPANCY_ABSTENTION_AND_FRESH_DUAL_CLASS_CONFIRMATION_PROTOCOL_LOCK`（execution=false）：只允许把 consumed R10 的误报诊断形成 development-only abstention 候选，并在任何新 outcome 前冻结新 fresh parent-disjoint cohort、dual-class coverage 与 one-shot 前门；新数据读取仍需单独授权 | 用 prior/LM/regularizer 伪造可观测；把 UNKNOWN 当 negative；覆盖、重跑、改门或救活已消费 R10；复用已观察的 parents 冒充 fresh promotion evidence；让 truth/knownness 进入 source phase；未获新权限即读 FARO、训练、主动提示、Android/HTP 或默认 App；把 WILD_LAB 结果写成部署、产品或安全证明 | 否 |
...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D1_TASK_QUALITY_FAIL_TERMINAL / D2_DEVELOPMENT_FROZEN_HEAD_QUALITY_FAIL_TERMINAL / D3_PHASE_A_FAIL_21_OF_32_NO_SELECTION / D3R1_PHASE_A_BODY_127_PROCESSED_53_ELIGIBLE_32_LOCKED / D3R1_PHASE_B_HEAD_64_OF_64_PASS_BODY_AWAITING / R2_CANDIDATE_NOT_AUTHORIZED` | [DepthART current](hftf/README.md) | `EXPLICIT_D3R1_PHASE_B_DEPTH_CONFIDENCE_BODY_AND_SOURCE_TRUTH_SUPPORT_ACTIVATION`：另锁 exact-64 body 与全 32 support audit，不自动 GET/选 first-16 | 复用或扩大旧 D3 exact-48/21-qualified pool；改写已锁 32 身份/windows；降低 continuity/support 门；未单独激活即 GET/读 body；训练或 Development；重开 strict G4-D；访问 R2、测性能或接入默认 App | 否 |
