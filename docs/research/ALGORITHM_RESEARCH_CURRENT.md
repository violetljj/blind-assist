...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 连续因子 → selective no-regret → body-swept geometry | `FACTORWISE_ORACLE_HEADROOM_PASS / FRAME_LCB_ZERO_HARM_BUT_PARENT_COVERAGE_FAIL / TUM14_RUNTIME_OBSERVABILITY_FAIL_STOP / SCIENTIFIC_NOT_RUN` | [Assistive Geometry current](assistive-geometry/README.md) | `AG_RUNTIME_CORRECTION_GAIN_OBSERVABILITY_CANARY_R0`：先在 consumed TUM 以 leave-one-parent-out 检验 temporal reprojection / model uncertainty 是否能观测 correction joint gain；冻结非零候选前不读 fresh3 outcome | 用同一 observable 重训 selector；消费 fresh3 去评全 fallback；读 ETH3D；用零伤害或 macro gain 冒充机制成功 | 否 |
