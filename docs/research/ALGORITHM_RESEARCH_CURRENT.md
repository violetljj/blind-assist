...
|---|---|---|---|---|---|---|
| BlindAssist Assistive Geometry | 当前算法主线：学习 Ground / Clearance / Confidence / UNKNOWN / Body-swept Occupancy | `ROUTE_SELECTED / B1_PROTOCOL_LOCK_PASS / IMPLEMENTATION_NOT_AUTHORIZED / FORMAL_TRAINING_NOT_AUTHORIZED` | [Assistive Geometry current](assistive-geometry/README.md) | `BLINDASSIST_ASSISTIVE_GEOMETRY_B1_TARGET_MATERIALIZATION_AND_MODEL_IMPLEMENTATION_LOCK`：只生成 TRAIN target cache，实现 shared decoder/head/loss，并关闭 forward/backward、flip/K 与 resume smoke | implementation lock 前正式训练 student；读取 DEVELOPMENT/CONFIRMATION outcome；用 B0 sensor truth 冒充 human safety truth；运行 DA3/Metric3D teacher | 否 |
