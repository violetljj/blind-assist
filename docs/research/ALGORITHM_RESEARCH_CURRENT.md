...
| TARO / Task-directed Active Risk Observability | 独立并行 WILD_LAB：在声明的米制锚与冻结 factor/reducer 下，以低维 residual gauge posterior、可观测子空间和受限相机微基线，使 body/path-specific clearance query 先于完整场景达到局部可识别 | `PARALLEL_WILD_LAB / P0_PASS / O0M_SYNTHETIC_ANALYTIC_MECHANICS_PASS / O0M_ONE_SHOT_CONSUMED / O0R_SOURCE_AND_ADAPTER_CONTRACT_LOCK_PASS / O0R_SOURCE_ADAPTER_IMPLEMENTATION_LOCK_PASS / O0R_TRUTH_ONLY_PREFLIGHT_LOCK_PASS / HEAD_NOT_RUN / ONE_SHOT_UNCONSUMED / O0R_SCIENTIFIC_NOT_RUN / EXECUTION_NOT_AUTHORIZED` | [TARO current](taro/README.md) | `TARO_O0R_ARKITSCENES_TRUTH_ONLY_MATERIALIZER_IMPLEMENTATION_LOCK`（execution=false）：新 TARO-specific signed 24-parent × 3-asset receipt 后，只实现/静态测试 frozen HEAD/source/truth materializer 与 atomic writer，不执行网络/source/truth | 用 prior/LM/regularizer 伪造可观测；把 K 混入 S/P/B factorial；继承 B1 outcome/threshold；把 UNKNOWN 当 negative；覆盖或重跑 O0M；把 HEAD_NOT_RUN 写成 availability PASS；在独立 truth-only execution authorization 前下载/打开新 source、物化 truth、运行 DepthART 或执行 O0R；训练、主动提示、TwinScene/AC4D、Android/HTP 或默认 App；把 synthetic mechanics、静态契约或 implementation tests 写成 real headroom；跳过 O0R 进入 G0/G1/A0/A1/J0 | 否 |
...
| DepthART-S | Assistive Geometry 的优先 encoder/initialization、depth baseline 与部署使能线，不是算法终点 | `R1_RESEARCH_MAINLINE / STRICT_G4D_NEGATIVE_TERMINAL / D0_NO_ELIGIBLE_PRECISION_ARM / D1_FINAL_8_SESSION_ROSTER_LOCKED / D1_608X448_SM8650_V75_CONTEXT_AND_EXECUTION_PREFLIGHT_PASS / RAW_DEPTH_PARITY_DIAGNOSTIC_FAIL / DEVELOPMENT_QUALITY_SCREEN_RESUMED_AND_RUNNING_FROM_CHUNK_03 / 3_OF_48_DEVICE_CHUNKS_COMPLETE / R2_CANDIDATE_NOT_SELECTED` | [DepthART current](hftf/README.md) | `COMPLETE_FROZEN_D1_DEVELOPMENT_TASK_QUALITY_SCREEN`：同一设备 fresh-check 已通过，chunk-03 正在整块重跑；按冻结顺序完成余下 outputs 后才可一次性运行 task-quality gates，quality PASS 前不得测性能 | 修改 candidate/data/postprocess/known-coverage denominator/gates；复用 chunk-03 半成品或用部分块作选择/结论；把 raw-depth diagnostic 当 D1 task result或重开 strict G4-D；访问独立 R2 cohort；事后把 G4-C 塞回 D0 | 否 |
