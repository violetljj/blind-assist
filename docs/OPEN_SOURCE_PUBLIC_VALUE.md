...
项目的公共利益意图对应联合国可持续发展目标 SDG 10“减少不平等”：通过公开可复核的 Android 无障碍工程、端侧机器学习和评测方法，降低外部学习、审查与参与门槛。该对应关系是项目方向，不是已经取得用户效果、社会影响或安全结果的证据。
...
开源支持将优先用于 issue 分类、pull request 审查、回归测试、依赖与许可证核验、文档同步、安全检查和可复现实验维护。具体输入、输出、权限、密钥和成效指标由 [Codex maintainer automation](CODEX_MAINTAINER_AUTOMATION.md) 约束；Codex 与其他自动化工具产生的修改仍需要通过仓库门禁和证据边界，模型输出本身不构成许可证、用户同意、设备测量或安全事实。
