...
CONTRACT_PATH = (
    ...
    "DISJOINT_GEOMETRY_STRATIFIED_HOLDOUT_R0_CONTRACT_2026-07-27.json"
)
...
def summary(index: int, role: str) -> dict:
    anchor = Decimal("1673419222.281298")
...
class HoldoutRulesTest(unittest.TestCase):
    def test_candidate_grid_is_guard_disjoint(self) -> None:
        contract = load_contract()
        timestamps = [
            Decimal("1673419222.281298") + Decimal(index) / Decimal("30")
        ...
        self.assertGreaterEqual(
            Decimal(windows[0]["start_timestamp_s"]),
            Decimal("1673419252.281298"),
        )

    def test_candidate_grid_rejects_guard_intersection(self) -> None:
        contract = load_contract()
        contract["geometry_only_selection"]["candidate_window_indices"] = [2]
        with self.assertRaisesRegex(
            ValueError, "CANDIDATE_INTERSECTS_INSPECTED_GUARD"
        ):
    ...
    def test_direction_requires_every_positive_to_exceed_every_low(self) -> None:
        ...
        self.assertEqual(
            terminal, "DISJOINT_WINDOW_DIRECTION_REPLICATED / VALID"
        )
    ...
    def test_contract_names_disjoint_not_cross_source(self) -> None:
        contract = load_contract()
        self.assertEqual(
            contract["authority"]["maximum_claim"],
            "SAME_SEQUENCE_DISJOINT_WINDOW_DEVELOPMENT_HOLDOUT_ONLY",
        )
        self.assertFalse(
