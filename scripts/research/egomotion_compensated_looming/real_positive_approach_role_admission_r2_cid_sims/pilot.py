...
This utility refuses floor3_1 and emits benchmark telemetry to stdout only.  It
...
def run_pilot(root: Path, *, pair_limit: int, workers: int = 1) -> dict[str, Any]:
    ...
    if "floor3_1" in lexical:
        raise ValueError("R2_PILOT_SOFA_3_FORBIDDEN")
    source_id = PILOT_SOURCE_IDS.get(root.name.lower())
    if source_id is None:
        raise ValueError("R2_PILOT_REQUIRES_FROZEN_BURNED_ROOT")
    if workers < 1 or pair_limit < 1:
        raise ValueError("R2_PILOT_POSITIVE_LIMITS")
    controls = {
        name: root / name
    ...
    if not all(path.is_file() for path in controls.values()):
        raise ValueError("R2_PILOT_CONTROL_FILE_MISSING")
    intrinsic = (
    ...
    if source_id == "ETH3D_SLAM_CABLES_1_CAPTURE_FAMILY":
        if associated_depth != depth_index:
            raise ValueError("R2_PILOT_DEPTH_IDENTITY")
    ...
    for source_order, (before, after) in enumerate(pairs):
        ...
        if not depth_path.is_file():
            raise ValueError("R2_PILOT_DEPTH_MISSING")
        worker_task = (
            record,
