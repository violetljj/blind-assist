...
def _worker(spec_values: dict[str, Any]) -> tuple[dict[str, Any], dict[str, Any]]:
    protocol = load_protocol()
    return run_trial(TrialSpec(**spec_values), protocol)
...
def _runtime_row_without_progress(row: dict[str, Any]) -> dict[str, Any]:
    return row
...
def formal_run(
    ...
    with partial_metrics.open("w", encoding="utf-8", newline="\n") as metrics_handle:
        with partial_runtime.open(
    ...
    for name, row in representatives.items():
        if row["evaluable"]:
            ...
            render_flow_comparison(
                spec, protocol, figures / f"{name}_flow.png"
            )
            save_representative_sequence(
                spec, protocol, dataset_root / name
            )
    write_markdown_report(
        summary, runtime, output_root / "RCLE_PHASE_A_RESULT_R0.md"
    )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run or validate RCLE-Minimal Phase A synthetic audit R0."
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=Path("artifacts.local/evidence/rcle_minimal_r0/formal_run_r0"),
    )
    parser.add_argument(
        "--dataset-root",
        type=Path,
        default=Path("artifacts.local/datasets/rcle_minimal_r0/formal_run_r0"),
    )
    parser.add_argument(
        "--workers",
        type=int,
