...
def fit_fixed_grid_local_affine(
    ...
    velocities = (current - previous) / dt_seconds
    for row in range(rows):
        for column in range(columns):
            ...
            points = previous[in_cell]
            cell_velocity = velocities[in_cell]
            ...
            if (
                hull_fraction
                < float(parameters["minimum_track_convex_hull_fraction"])
            ):
                results.append(
                    _abstain(
            ...
            a_xy = coefficients[1, 0] / half_height
            a_yx = coefficients[0, 1] / half_width
