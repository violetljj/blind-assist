...
from scripts.research.egomotion_compensated_looming.periodic_self_motion_counterfactual_r2 import (
    generator_geometry_r2 as r2_generator,
)
from scripts.research.egomotion_compensated_looming.periodic_self_motion_counterfactual_r2 import (
    validate_geometry_independent as r0_validator,
)
from scripts.research.egomotion_compensated_looming.periodic_self_motion_counterfactual_r2 import (
    validate_geometry_independent_r2 as r2_validator,
)


class GeometryValidatorRepairR2Test(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.r0_records = r0_validator.load_jsonl(
            r2_validator.R0_EVIDENCE / "all_seed_geometry_manifest.jsonl"
        )
        cls.r2_records = r0_validator.load_jsonl(
            r2_validator.DEFAULT_EVIDENCE
            / "all_seed_geometry_manifest.jsonl"
        )
        cls.trajectories = r0_validator.load_json(
            r2_validator.DEFAULT_EVIDENCE / "trajectory_manifest.json"
        )
        cls.r2_guards = [
            item
    ...
    def test_radial_gate_is_derived_from_unchanged_20_percent(self) -> None:
        amendment = r0_validator.load_json(r2_validator.AMENDMENT_PATH)
        self.assertEqual(
            math.log(1.20),
        ...
        self.assertAlmostEqual(math.log(1.20), r2_validator.LOG_1P20)
    ...
    def test_repaired_g13_passes_all_sixteen_arms(self) -> None:
        gate = r2_validator.gate_g13_r2(self.r2_guards, self.trajectories)
    ...
    def test_target_or_trajectory_mutation_fails_g13(self) -> None:
        ...
        gate = r2_validator.gate_g13_r2(mutated, self.trajectories)
    ...
    def test_guard_layout_or_declared_target_drift_is_invalid(self) -> None:
        ...
        errors: list[str] = []
        r2_validator._validate_guard_scene_contract(mutated, errors)
        self.assertTrue(
    ...
    def test_r2_validator_imports_no_generator_or_rcle(self) -> None:
        source = Path(r2_validator.__file__).read_text(encoding="utf-8")
        imported = []
    ...
    def test_r2_lock_keeps_all_future_authority_closed(self) -> None:
        lock = r0_validator.load_json(r2_validator.DEFAULT_LOCK)
    ...
    def test_r1_failed_receipt_is_immutable(self) -> None:
        path = (
            r2_validator.R1_EVIDENCE
            / "independent_geometry_validation_receipt.json"
        )
        receipt = r0_validator.load_json(path)
        self.assertEqual(
            r2_validator.EXPECTED_R1_FAILED_RECEIPT_SHA256,
    ...
    def test_r2_all_records_are_byte_identical_to_r1(self) -> None:
        r1_path = (
            r2_validator.R1_EVIDENCE / "all_seed_geometry_manifest.jsonl"
        )
        r2_path = (
            r2_validator.DEFAULT_EVIDENCE / "all_seed_geometry_manifest.jsonl"
        )
    ...
    def test_guard_replay_mutation_fails_g14(self) -> None:
        ...
        guard_replay = r0_validator.load_json(
            r2_validator.DEFAULT_EVIDENCE / "guard_scene_replay_ledger.json"
        )
        fixtures = r0_validator.load_json(
            r2_validator.DEFAULT_EVIDENCE / "analytic_fixture_ledger.json"
        )
    ...
    def test_pairing_and_geometry_hash_mutations_fail(self) -> None:
        ...
        self.assertEqual(
            "FAIL", r2_validator.gate_g11_r2(pairing_mutation)["status"]
        )
        hash_mutation = copy.deepcopy(main)
        hash_mutation[0]["arms"][0]["depth_sha256"] = "0" * 64
        self.assertEqual(
            "FAIL",
            r2_validator.gate_g12_r2(
    ...
    def test_reference_hash_mutation_fails_g01(self) -> None:
        ...
        g01, _ = r2_validator.gate_g01_g02_r2([], mutated)
    ...
    def test_r2_producer_replay_is_exact(self) -> None:
        ...
        replay = r2_generator._build_guard_replay(records)
