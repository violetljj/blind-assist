"""Formal two-phase disjoint-window holdout for CID-SIMS floor3_1."""
...
PROTOCOL_ID = (
    "RCLE_RGB_ALGORITHM_CID_SIMS_FLOOR3_1_"
    "DISJOINT_GEOMETRY_STRATIFIED_HOLDOUT_R0"
)
...
def candidate_windows(
    ...
    anchor = Decimal(rules["anchor_timestamp_s"])
    duration = Decimal(rules["window_duration_s"])
    expected_frames = int(rules["expected_frames_per_window"])
    maximum_dt = Decimal(rules["maximum_pair_dt_s"])
    inspected_start = Decimal(rules["inspected_interval"]["start_timestamp_s"])
    inspected_end = Decimal(rules["inspected_interval"]["end_timestamp_s"])
    guard_start = inspected_start - Decimal(rules["guard_band_before_s"])
    guard_end = inspected_end + Decimal(rules["guard_band_after_s"])
    windows: list[dict[str, Any]] = []
    for index in rules["candidate_window_indices"]:
        start = anchor + int(index) * duration
        end = start + duration
        if not (end <= guard_start or start >= guard_end):
            raise ValueError(f"CANDIDATE_INTERSECTS_INSPECTED_GUARD:{index}")
        rows = [timestamp for timestamp in timestamps if start <= timestamp < end]
        pairs = list(zip(rows, rows[1:]))
        identity_ok = len(rows) == expected_frames and all(
        ...
        windows.append(
            {
                ...
                "identity_reason": (
                    None
                    if identity_ok
                    else "EXPECTED_300_FRAMES_AND_BOUNDED_SOURCE_CONSECUTIVE_DT"
                ),
            }
        )
    return windows
...
def summarize_geometry_window(
    ...
    rules = contract["geometry_only_selection"]
    expected_pairs = int(rules["expected_pairs_per_window"])
    ...
    coverage = len(evaluable) / expected_pairs
    positive_fraction = counts["POSITIVE_APPROACH_GEOMETRY"] / expected_pairs
    below_fraction = counts["BELOW_TRIGGER_REFERENCE"] / expected_pairs
...
def selected_member_identity(
    ...
    for summary in selected:
        ...
        for timestamp in window["frame_timestamps_s"]:
            name = f"floor3_1/color/{timestamp}.png"
...
def aggregate_rgb(
    ...
    terminal = (
        ...
        if not evaluable
            ...
            "DISJOINT_WINDOW_DIRECTION_REPLICATED / VALID"
            if direction
            else "DISJOINT_WINDOW_DIRECTION_NOT_REPLICATED / VALID"
        )
    )
    return terminal, {
        "windows": combined,
...
def run_formal(
    ...
    try:
        ...
        from scripts.research.egomotion_compensated_looming.rgb_algorithm_cid_sims_disjoint_holdout_r0 import (
            validator,
        )
        ...
        failure = {
            ...
            "protocol_id": PROTOCOL_ID,
            "terminal": "DISJOINT_HOLDOUT_INVALID / INVALID",
