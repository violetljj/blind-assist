...
def _role_from_rows(
    ...
    rules = contract["geometry_only_selection"]
    denominator = int(rules["expected_pairs_per_window"])
...
def _validate_rgb_identity_and_cache(
    ...
    for item in identity.get("members", []):
        name = str(item.get("archive_member"))
        if name in seen:
            errors.append(f"RGB_IDENTITY_DUPLICATE:{name}")
        seen.add(name)
        source = info.get(name)
        if source is None:
            errors.append(f"RGB_IDENTITY_MISSING:{name}")
            continue
        if (
            item.get("crc32") != f"{source.CRC:08x}"
            or int(item.get("uncompressed_bytes", -1)) != source.file_size
            or int(item.get("compressed_bytes", -1)) != source.compress_size
        ):
            errors.append(f"RGB_IDENTITY_METADATA:{name}")
    ...
    with zipfile.ZipFile(archive_path) as archive:
        ...
        if not selected:
            ...
            _validate_rgb_identity_and_cache(
                archive, selection, identity, cache_dir, errors
            )
