"""Independent replay validator for ETH3D sofa_3 role-admission R1.
...
def object_from(path: Path) -> dict[str, Any]:
    ...
    if not isinstance(value, dict):
        raise ValueError("R1_VALIDATOR_JSON_OBJECT")
    return value
...
def relative_name(value: str, directory: str) -> str:
    ...
    if (
        ...
        raise ValueError("R1_VALIDATOR_INDEX_NAME")
...
def associations(raw: bytes) -> list[VAssociation]:
    ...
    for tokens in rows(raw):
        if len(tokens) != 4:
            raise ValueError("R1_VALIDATOR_ASSOCIATED_COLUMNS")
        result.append(
            VAssociation(
                Decimal(tokens[0]),
    ...
    if len(result) < 2 or any(
        ...
        raise ValueError("R1_VALIDATOR_ASSOCIATED_ORDER")
    if len({item.depth_name for item in result}) != len(result):
        raise ValueError("R1_VALIDATOR_ASSOCIATED_DUPLICATE")
    return result
...
def unit_quaternion(raw: np.ndarray) -> np.ndarray:
    ...
    if value.shape != (4,) or not np.all(np.isfinite(value)) or length <= 0.0:
        raise ValueError("R1_VALIDATOR_QUATERNION")
    return value / length
...
def pose_table(raw: bytes) -> list[VPose]:
    ...
    for tokens in rows(raw):
        if len(tokens) != 8:
            raise ValueError("R1_VALIDATOR_POSE_COLUMNS")
        numeric = np.array([float(value) for value in tokens[1:]], dtype=np.float64)
        if not np.all(np.isfinite(numeric)):
            raise ValueError("R1_VALIDATOR_POSE_FINITE")
    ...
    if not result or any(a.time >= b.time for a, b in zip(result, result[1:])):
        raise ValueError("R1_VALIDATOR_POSE_ORDER")
    return result
...
def camera_matrix(raw: bytes) -> np.ndarray:
    ...
    if len(values) != 4 or not all(math.isfinite(value) for value in values):
        raise ValueError("R1_VALIDATOR_CALIBRATION")
    fx, fy, cx, cy = values
    if min(fx, fy) <= 0.0:
        raise ValueError("R1_VALIDATOR_CALIBRATION")
...
def depth_samples(raw: bytes) -> tuple[np.ndarray, np.ndarray, tuple[int, int]]:
    try:
        ...
        raise ValueError("R1_VALIDATOR_DEPTH_PNG") from error
    if array.ndim != 2 or array.dtype != np.uint16:
        raise ValueError("R1_VALIDATOR_DEPTH_FORMAT")
...
class ValidationArchive:
    def __init__(self, path: Path) -> None:
        ...
        if len(normalized) != len(set(normalized)):
            raise ValueError("R1_VALIDATOR_DUPLICATE_MEMBER")
        for item, name in zip(infos, names):
            ...
            if (
                ...
                raise ValueError("R1_VALIDATOR_UNSAFE_MEMBER")
        self.inventory = [
            {
                "name": item.filename,
        ...
        if not required.issubset(self.files):
            raise ValueError("R1_VALIDATOR_CONTROL_MEMBERS")
        if {PurePosixPath(name).parts[0] for name in self.files} != {SEQUENCE_ID}:
            raise ValueError("R1_VALIDATOR_ROOT")
    ...
    def control(self, name: str) -> bytes:
        if name not in {"calibration.txt", "groundtruth.txt", "associated.txt", "depth.txt"}:
            raise ValueError("R1_VALIDATOR_CONTROL_READ")
    ...
    def depth(self, relative: str) -> bytes:
        ...
        if member not in self.files:
            raise ValueError("R1_VALIDATOR_DEPTH_MISSING")
...
def replay(archive_path: Path) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    with ValidationArchive(archive_path) as archive:
        intrinsic = camera_matrix(archive.control("calibration.txt"))
        poses = pose_table(archive.control("groundtruth.txt"))
        associated = associations(archive.control("associated.txt"))
        indexed = depth_table(archive.control("depth.txt"))
        if [(item.depth_time, item.depth_name) for item in associated] != indexed:
            raise ValueError("R1_VALIDATOR_ASSOCIATED_DEPTH_IDENTITY")
        ...
        for item in associated:
            try:
                ...
                if str(error) not in {
                    "R1_POSE_NOT_BRACKETED",
                    "R1_POSE_BRACKET_TOO_WIDE",
                }:
                    raise
            else:
                start_item = item
        ...
        if start_item is None:
            raise ValueError("R1_VALIDATOR_NO_JOINT_RGBD_POSE_START")
        ...
        for before, after in zip(selected, selected[1:]):
            ...
            if image_size is None:
                ...
                raise ValueError("R1_VALIDATOR_IMAGE_SIZE")
...
def validate_source_hold(
    ...
    if set(bindings) != required:
        raise ValueError("R1_SOURCE_HOLD_BINDING_KEYS")
    ...
    if (
        ...
        not in {
            "R1_OFFICIAL_SOURCE_ACCESS_FAILED",
            "R1_OFFICIAL_CONTAINER_UNREADABLE",
            "R1_OFFICIAL_CONTAINER_IDENTITY_INCOMPLETE",
        }
        or acquisition.get("partial_archive_bytes")
        != bindings["archive"].stat().st_size
        or acquisition.get("partial_archive_sha256") != hashes["archive"]
    ):
        errors.append("SOURCE_HOLD_PARTIAL_IDENTITY")
    allowed_urls = {
        "https://www.eth3d.net/data/slam/datasets/sofa_3_rgbd.zip",
        "https://eth3d.ethz.ch/data/slam/datasets/sofa_3_rgbd.zip",
    }
    if (
...
def validate(formal_dir: Path, bindings: Mapping[str, Path]) -> dict[str, Any]:
    ...
    if set(bindings) != required:
        raise ValueError("R1_VALIDATOR_BINDING_KEYS")
    ...
    if (
        ...
        or result_source.get("associated_row_count") != source["associated_row_count"]
        or result_source.get("depth_index_row_count") != source["depth_index_row_count"]
        or result_source.get("groundtruth_row_count") != source["groundtruth_row_count"]
    ...
    if (
        ...
        or set(manifest.get("r1_candidate_must_not_descend_from", [])) != NON_OVERLAP
