"""Stable formal one-shot orchestration for role-admission R1.
...
def run_formal(
    *,
    ...
    if not all(_within(path, run_dir) for path in outputs):
        raise ValueError("R1_RUN_OUTPUT_OUTSIDE_RUN_DIR")
    if success_path.exists() or failure_path.exists():
        raise FileExistsError("R1_TERMINAL_PATH_ALREADY_EXISTS")
    if claim_path.exists():
        raise FileExistsError("R1_ONE_SHOT_OUTPUT_ALREADY_EXISTS")
    ...
    try:
        ...
        if claim.get("claim_created_by_runner_only") is not True:
            raise ValueError("R1_RUNNER_CLAIM_FLAG")
        ...
        if validation["validation_terminal"] != "VALID":
            raise ValueError("R1_VALIDATOR_INVALID")
        completed = 4
        terminal = {
        ...
        try:
            ...
            failure = {
                ...
                "scientific_terminal": "INVALID_R1_EVIDENCE / INVALID",
            ...
            raise
        if validation["validation_terminal"] != "VALID":
            invalid_error = ValueError("R1_SOURCE_HOLD_VALIDATOR_INVALID")
            failure = {
                "schema_version": "rcle.formal_failure.v1",
                "protocol_id": PROTOCOL_ID,
                "status": "FAILURE",
