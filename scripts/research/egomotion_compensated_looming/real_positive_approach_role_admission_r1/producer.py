"""Geometry-only producer for the frozen ETH3D sofa_3 RGB-D archive.

The archive is never extracted.  RGB bytes and rgb.txt contents are never read.
Only ZIP metadata, four frozen text controls, and depth PNG members referenced
by the first half-open ten-second window are consumed.
"""
...
REQUIRED_BURNED_GROUPS = {
    ...
    "ETH3D_SLAM_CABLES_1_CAPTURE_FAMILY",
    "ICL_NUIM_AND_TARTANAIR_SYNTHETIC_FAMILIES",
    "ALL_PREEXISTING_ARTIFACTS_LOCAL_PAYLOADS",
}
EXPECTED_ANCESTRY = [
    "ETH3D_OFFICIAL",
    "ETH3D_SLAM_TRAINING",
    "ETH3D_SLAM_SOFA_SCENE",
    "ETH3D_SLAM_SOFA_3",
    "ETH3D_SLAM_SOFA_3_RGBD",
    PROTOCOL_ID,
]
INDEPENDENCE_GROUP = "ETH3D_SLAM_SOFA_SCENE_CAPTURE_FAMILY"
REUSE_POLICY = (
    "GEOMETRY_SELECTED_REAL_APPROACH_CANARY_ONLY; NEVER_CONFIRMATION; "
    "MAY_REMAIN_SOURCE_CHARACTERIZATION_COUNTEREXAMPLE_OR_REGRESSION"
)
...
def _safe_relative(value: str, expected_directory: str) -> str:
    ...
    if (
        ...
        raise ValueError("R1_INDEX_UNSAFE_OR_WRONG_DIRECTORY")
...
def _parse_associated(raw: bytes) -> list[AssociatedRow]:
    ...
    for values in _tokens(raw):
        if len(values) != 4:
            raise ValueError("R1_ASSOCIATED_COLUMNS")
        rows.append(
            AssociatedRow(
                Decimal(values[0]),
    ...
    if len(rows) < 2:
        raise ValueError("R1_ASSOCIATED_TOO_SHORT")
    if any(
        left.depth_timestamp >= right.depth_timestamp
        for left, right in zip(rows, rows[1:])
    ):
        raise ValueError("R1_ASSOCIATED_DEPTH_NOT_MONOTONIC")
    if len({row.depth_path for row in rows}) != len(rows):
        raise ValueError("R1_ASSOCIATED_DUPLICATE_DEPTH")
    return rows
...
def _parse_depth_index(raw: bytes) -> list[tuple[Decimal, str]]:
    ...
    for values in _tokens(raw):
        if len(values) != 2:
            raise ValueError("R1_DEPTH_INDEX_COLUMNS")
    ...
    if not rows or any(left[0] >= right[0] for left, right in zip(rows, rows[1:])):
        raise ValueError("R1_DEPTH_INDEX_NOT_MONOTONIC")
    if len({path for _, path in rows}) != len(rows):
        raise ValueError("R1_DEPTH_INDEX_DUPLICATE_PATH")
    return rows
...
def _normalize_quaternion(value: np.ndarray) -> np.ndarray:
    ...
    if (
        ...
        raise ValueError("R1_POSE_QUATERNION")
    return result / norm
...
def _parse_poses(raw: bytes) -> list[PoseRow]:
    ...
    for values in _tokens(raw):
        if len(values) != 8:
            raise ValueError("R1_GROUNDTRUTH_COLUMNS")
        numeric = np.asarray([float(item) for item in values[1:]], dtype=np.float64)
        if not np.all(np.isfinite(numeric)):
            raise ValueError("R1_GROUNDTRUTH_NONFINITE")
        result.append(
            PoseRow(
                Decimal(values[0]),
                numeric[:3],
    ...
    if not result or any(
        ...
        raise ValueError("R1_GROUNDTRUTH_NOT_MONOTONIC")
    return result
...
def _parse_intrinsic(raw: bytes) -> np.ndarray:
    ...
    if len(values) != 4 or not all(math.isfinite(item) for item in values):
        raise ValueError("R1_CALIBRATION_FORMAT")
    fx, fy, cx, cy = values
    if fx <= 0.0 or fy <= 0.0:
        raise ValueError("R1_CALIBRATION_FORMAT")
...
def _decode_depth(raw: bytes) -> tuple[np.ndarray, np.ndarray, tuple[int, int]]:
    try:
        ...
        raise ValueError("R1_DEPTH_PNG_INVALID") from error
    if depth.ndim != 2 or depth.dtype != np.uint16:
        raise ValueError("R1_DEPTH_PNG_FORMAT")
...
class SofaArchive:
    """Read-fenced ZIP wrapper; rgb.txt and RGB pixel members have no read API."""
    ...
    def __init__(self, path: Path) -> None:
        ...
        required = {
            f"{SEQUENCE_ID}/calibration.txt",
            f"{SEQUENCE_ID}/groundtruth.txt",
            f"{SEQUENCE_ID}/associated.txt",
            f"{SEQUENCE_ID}/depth.txt",
            f"{SEQUENCE_ID}/rgb.txt",
        }
        if not required.issubset(self._files):
            raise ValueError("R1_ARCHIVE_REQUIRED_MEMBER")
        roots = {PurePosixPath(name).parts[0] for name in self._files}
        if roots != {SEQUENCE_ID}:
            raise ValueError("R1_ARCHIVE_TOP_LEVEL_IDENTITY")
        if not any(name.startswith(f"{SEQUENCE_ID}/rgb/") for name in self._files):
            raise ValueError("R1_ARCHIVE_RGB_DIRECTORY_IDENTITY")
        if not any(name.startswith(f"{SEQUENCE_ID}/depth/") for name in self._files):
            raise ValueError("R1_ARCHIVE_DEPTH_DIRECTORY_IDENTITY")
        self.depth_members_read: list[str] = []

    def read_control(self, name: str) -> bytes:
        if name not in {"calibration.txt", "groundtruth.txt", "associated.txt", "depth.txt"}:
            raise ValueError("R1_CONTROL_READ_FORBIDDEN")
    ...
    def read_depth(self, relative: str) -> bytes:
        ...
        if member not in self._files:
            raise ValueError("R1_DEPTH_MEMBER_MISSING")
    ...
    def __enter__(self) -> "SofaArchive":
        return self
...
def _binding_state(bindings: Mapping[str, Path]) -> tuple[dict[str, str], dict[str, Any]]:
    ...
    if set(bindings) != required:
        raise ValueError("R1_BINDING_KEYS")
    ...
    if any(
        ...
        raise ValueError("R1_BINDING_PROTOCOL")
    if acquisition.get("archive_sha256") != hashes["archive"]:
        raise ValueError("R1_ARCHIVE_HASH")
    if acquisition.get("claim_sha256") != hashes["claim"]:
        raise ValueError("R1_ACQUISITION_CLAIM_HASH")
    for name in (
        "contract",
        "source_authority",
        "burned_manifest",
        ...
        if claim.get("bindings", {}).get(name, {}).get("sha256") != hashes[name]:
            raise ValueError(f"R1_CLAIM_BINDING:{name}")
        if acquisition.get("preaccess_document_sha256", {}).get(name) != hashes[name]:
            raise ValueError(f"R1_ACQUISITION_BINDING:{name}")
    if (
        ...
        raise ValueError("R1_ACQUISITION_NOT_ONE_SHOT")
    if (
        ...
        raise ValueError("R1_CONTRACT_CANDIDATE")
    ...
    if any(float(gates.get(name, -1)) != float(value) for name, value in expected_gates.items()):
        raise ValueError("R1_CONTRACT_GATE_DRIFT")
    if authority.get("candidate_count") != 1:
        raise ValueError("R1_AUTHORITY_CANDIDATE_COUNT")
    ...
    if (
        ...
        raise ValueError("R1_AUTHORITY_ANCESTRY_OR_REUSE")
    if set(manifest.get("r1_candidate_must_not_descend_from", [])) != REQUIRED_BURNED_GROUPS:
        raise ValueError("R1_BURNED_MANIFEST_SCOPE")
    return hashes, {
...
def _evaluate(
    ...
    if workers < 1:
        raise ValueError("R1_WORKERS_MUST_BE_POSITIVE")
    intrinsic = _parse_intrinsic(archive.read_control("calibration.txt"))
    poses = _parse_poses(archive.read_control("groundtruth.txt"))
    associated = _parse_associated(archive.read_control("associated.txt"))
    depth_index = _parse_depth_index(archive.read_control("depth.txt"))
    if [(row.depth_timestamp, row.depth_path) for row in associated] != depth_index:
        raise ValueError("R1_ASSOCIATED_DEPTH_IDENTITY")
    start_row: AssociatedRow | None = None
    for row in associated:
        try:
            _interpolate_pose(poses, row.depth_timestamp)
        except ValueError as error:
            if str(error) not in {"R1_POSE_NOT_BRACKETED", "R1_POSE_BRACKET_TOO_WIDE"}:
                raise
        else:
            start_row = row
            break
    if start_row is None:
        raise ValueError("R1_NO_JOINT_RGBD_POSE_START")
    ...
    try:
        for record, current_size in evaluated:
            ...
            if image_size is None:
                ...
                raise ValueError("R1_DEPTH_IMAGE_SIZE_DRIFT")
            pair_records.append(record)
    finally:
        if workers != 1:
    ...
    source = {
        "associated_row_count": len(associated),
        "depth_index_row_count": len(depth_index),
        "groundtruth_row_count": len(poses),
...
def run(
    ...
    *,
    ...
    with SofaArchive(bindings["archive"]) as archive:
        if archive.inventory != bound["acquisition"].get("archive_member_inventory"):
            raise ValueError("R1_ARCHIVE_INVENTORY_MISMATCH")
