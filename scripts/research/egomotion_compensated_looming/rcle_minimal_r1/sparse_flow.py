...
def _empty_track_result(requested: int) -> SparseTrackResult:
...
def track_features(
    ...
    if current is None or forward_status is None:
        return _empty_track_result(requested)

    previous_flat = points.reshape(-1, 2)
    current_flat = current.reshape(-1, 2)
    best_error = np.full(requested, np.inf, dtype=np.float64)
    backward_available = np.zeros(requested, dtype=bool)
    for level in range(maximum_level, -1, -1):
        ...
        error = np.linalg.norm(backward_flat - previous_flat, axis=1)
        finite = (
            (backward_status.reshape(-1) > 0)
            & np.isfinite(backward_flat).all(axis=1)
            & np.isfinite(error)
        )
        improve = finite & (error < best_error)
    ...
    valid = (
        ...
        & (best_error <= float(parameters["forward_backward_max_error_pixels"]))
    ...
    return SparseTrackResult(
        ...
        forward_backward_errors=np.ascontiguousarray(
            best_error[valid].astype(np.float32)
        ),
        requested_count=requested,
    )
