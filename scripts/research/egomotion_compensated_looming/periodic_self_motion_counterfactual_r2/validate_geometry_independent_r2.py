"""Independent R2 full G01-G14 source-known geometry validator.

The validator imports only the already-independent R0 geometry math, never a
generator and never RCLE. It revalidates every gate on byte-identical R1
geometry, closes the earlier trusted-manifest paths, recomputes G13 using the
frozen source-known component decomposition, and preserves both failed
...
def _validate_r0_immutability(
    ...
    if base.sha256_file(r0_receipt_path) != EXPECTED_R0_RECEIPT_SHA256:
        ...
        if (
            not isinstance(evidence_hashes, dict)
            or set(evidence_hashes) != expected_r0_evidence_keys
        ):
    ...
    if len(r0_lines) != 88 or len(r1_lines) != 88 or len(r2_lines) != 88:
        errors.append("R2_MANIFEST_LINE_COUNT")
    else:
        if r0_lines[:80] != r2_lines[:80]:
            errors.append("R2_MAIN_RECORD_BYTES_DRIFT_FROM_R0")
        if r1_lines != r2_lines:
            errors.append("R2_RECORD_BYTES_DRIFT_FROM_R1")
    r0_main = {
...
def validate(evidence: Path, lock_path: Path) -> dict[str, Any]:
    ...
    producer = base.load_json(evidence / "generator_r2_receipt.json")
    records = base.load_jsonl(evidence / "all_seed_geometry_manifest.jsonl")
    _validate_r2_lock(lock, evidence, errors)
    _validate_r0_immutability(records, evidence, errors)
    if amendment.get("status") != "FROZEN_BEFORE_R2_IMPLEMENTATION_VALIDATION":
        errors.append("R2_AMENDMENT_STATUS")
    for field in (
        "formal_execution_authorized",
        "quality_calibration_authorized",
        "automatic_p2_authority",
        ...
        if amendment.get(field) is not False:
            errors.append(f"R2_AMENDMENT_AUTHORITY:{field}")
    if amendment.get("machine_gate_lock") != {
        ...
        errors.append("R2_AMENDMENT_MACHINE_GATE_LOCK")
    if (
        ...
        errors.append("R2_RUNTIME_RCLE_FIREWALL")
    expected_package = {
        "main_cluster": (
            "rcle.periodic_self_motion_counterfactual."
            "p1_geometry_manifest.v1"
    ...
    if (
        package_manifest.get("implementation_id") != IMPLEMENTATION_ID
        ...
        errors.append("R2_PACKAGE_MANIFEST")
    for field in (
        ...
        if producer.get(field) is not False:
            errors.append(f"R2_PRODUCER_BOUNDARY:{field}")
    ...
    for field, expected in expected_producer_identity.items():
        if producer.get(field) != expected:
            errors.append(f"R2_PRODUCER_IDENTITY:{field}")
    ...
    if (
        ...
        errors.append("R2_PRODUCER_ARTIFACT_HASH_KEYSET")
        producer_hashes = {}
    for name, expected in producer_hashes.items():
        path = evidence / name
        if not path.is_file() or base.sha256_file(path) != expected:
            errors.append(f"R2_PRODUCER_ARTIFACT_HASH:{name}")
    ...
    if [gate["id"] for gate in gates] != required:
        errors.append("R2_GATE_ORDER_OR_IDENTITY")
    ...
    return {
        "schema": (
            "rcle.periodic_self_motion_counterfactual."
            "p1_independent_geometry_r2_receipt.v1"
        ),
