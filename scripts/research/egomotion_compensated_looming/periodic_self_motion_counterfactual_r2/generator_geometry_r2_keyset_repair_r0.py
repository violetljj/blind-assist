"""Isolated P1 keyset repair materializer after the consumed R2 failure.

This successor changes no seed, scene, camera, trajectory, threshold, gate, or
all-seed manifest byte. It copies the hash-pinned R2 evidence into a new,
write-once directory and regenerates only version-owned envelope artifacts.
R0, R1, and R2 receipts remain immutable predecessors.
"""
...
def _build_guard_replay(records: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema": (
            ...
            "p1_guard_scene_replay_r2_keyset_repair_r0.v1"
        ),
...
def produce(output: Path) -> dict[str, Any]:
    ...
    if amendment.get("formal_execution_authorized") is not False:
        raise ValueError("KEYSET_REPAIR_FORMAL_EXECUTION_MUST_REMAIN_FALSE")
    failed_receipt = R2_EVIDENCE / "independent_geometry_validation_receipt.json"
    if r0.sha256_file(failed_receipt) != EXPECTED_R2_FAILED_RECEIPT_SHA256:
        raise ValueError("R2_FAILED_RECEIPT_HASH_MISMATCH")
    if output.exists():
        raise FileExistsError(f"WRITE_ONCE_OUTPUT_EXISTS:{output}")
    output.mkdir(parents=True, exist_ok=False)
    for name, expected in COPIED_EVIDENCE_SHA256.items():
        source = R2_EVIDENCE / name
        if r0.sha256_file(source) != expected:
            raise ValueError(f"R2_EVIDENCE_HASH_MISMATCH:{name}")
        shutil.copyfile(source, output / name)
        if r0.sha256_file(output / name) != expected:
            raise ValueError(f"KEYSET_REPAIR_COPIED_EVIDENCE_HASH_MISMATCH:{name}")
    ...
    if replay["mismatch_count"] != 0 or len(replay["items"]) != 8:
        raise ValueError("KEYSET_REPAIR_GUARD_REPLAY_MISMATCH")
    package_manifest = {
        "schema": (
            "rcle.periodic_self_motion_counterfactual."
    ...
    receipt = {
        "schema": (
            "rcle.periodic_self_motion_counterfactual."
            "p1_generator_geometry_r2_keyset_repair_r0_receipt.v1"
        ),
        "protocol_id": r0.PROTOCOL_ID,
        "implementation_id": IMPLEMENTATION_ID,
        ...
        "artifact_sha256": {
            ...
            if path.is_file()
            and path.name != "generator_r2_keyset_repair_r0_receipt.json"
        },
    ...
    r0.write_json(
        output / "generator_r2_keyset_repair_r0_receipt.json", receipt
    )
    return receipt
