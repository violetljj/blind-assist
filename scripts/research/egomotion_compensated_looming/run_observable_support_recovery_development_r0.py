...
DEVELOPMENT_SEEDS = tuple(range(2000, 2020))
OUTPUT_ROOT = (
    REPO_ROOT
    / "artifacts.local"
    / "evidence"
...
def development_protocol() -> dict[str, Any]:
    ...
    protocol["trials"]["seeds"] = list(DEVELOPMENT_SEEDS)
    trials = enumerate_trials(protocol)
    if len(trials) != 2520:
        raise ValueError("DEVELOPMENT_MATRIX_COUNT_DRIFT")
    if sum(trial.split == "clean" for trial in trials) != 1680:
        raise ValueError("DEVELOPMENT_CLEAN_DENOMINATOR_DRIFT")
    if sum(trial.split == "stress" for trial in trials) != 840:
        raise ValueError("DEVELOPMENT_STRESS_DENOMINATOR_DRIFT")
    if {trial.seed for trial in trials} != set(DEVELOPMENT_SEEDS):
        raise ValueError("DEVELOPMENT_SEED_ROLE_DRIFT")
    return protocol
...
def _worker(
    ...
    return run_trial(TrialSpec(**spec_values), development_protocol())
...
def formal_run(workers: int, command: list[str]) -> dict[str, Any]:
    ...
    if OUTPUT_ROOT.exists():
        raise FileExistsError(
            "development output location already exists; the one-run matrix "
            f"will not overwrite or resume it: {OUTPUT_ROOT}"
        )
    OUTPUT_ROOT.mkdir(parents=True, exist_ok=False)
    started_at = _iso_now()
    state = {
        "schema_version": "rcle.observable_support_recovery.run_state.v1",
        "status": "RUNNING",
        "candidate_id": IMPLEMENTATION_REVISION,
        "design_lock_sha256": DESIGN_LOCK_SHA256,
        "protocol_sha256": PROTOCOL_SHA256,
        "seed_role": "DEVELOPMENT_ONLY",
    ...
    terminal = (
        ...
        else "CLOSE_OBSERVABLE_THREE_FRAME_SUPPORT_MANAGER_R0"
    )
    write_json(
        OUTPUT_ROOT / "formal_run_state.json",
        {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the single locked development matrix for "
