"""Verify and finalize the outcome-blind R3 CID-SIMS byte cache.

This program treats the ZIP as opaque bytes. It never opens the archive.
"""
...
FILE_ID = "c595882daafe788a29d687872cc1fc2a"
...
def main() -> int:
    ...
    if args.final.exists() or args.receipt.exists():
        raise FileExistsError("R3_FINAL_OR_RECEIPT_ALREADY_EXISTS")
    size, md5, sha256 = hashes(args.part)
    if size != EXPECTED_BYTES:
        raise ValueError(f"R3_BYTE_COUNT:{size}")
    if md5 != EXPECTED_MD5:
        raise ValueError(f"R3_MD5:{md5}")
    source_lock_sha = hashlib.sha256(args.source_lock.read_bytes()).hexdigest()
    args.part.replace(args.final)
    receipt = {
        "schema_version": "rcle.r3.opaque_transport_receipt.v1",
