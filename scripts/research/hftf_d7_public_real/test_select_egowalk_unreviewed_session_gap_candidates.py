...
from select_egowalk_unreviewed_session_gap_candidates import run


class SelectEgoWalkReviewCandidatesTest(unittest.TestCase):
    def _candidate(self, candidate_id: str, session: str, start: int, end: int) -> dict[str, object]:
        return {
    ...
    def test_excludes_identity_and_temporal_neighbors_and_keeps_one_per_session(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            ...
            (bundle / "bundle_manifest.json").write_text(json.dumps({"candidate_ids": ["old"]}), encoding="utf-8")
    ...
    def test_rejects_empty_selection(self) -> None:
        with tempfile.TemporaryDirectory() as temp:
            root = Path(temp)
            candidate_path = root / "candidate_index.jsonl"
            candidate_path.write_text(json.dumps(self._candidate("only", "s", 1, 4)) + "\n", encoding="utf-8")
            bundle_root = root / "reviews" / "input_bundles"
            bundle_root.mkdir(parents=True)
            (bundle_root / "old" ).mkdir()
            (bundle_root / "old" / "bundle_manifest.json").write_text(json.dumps({"candidate_ids": ["only"]}), encoding="utf-8")
