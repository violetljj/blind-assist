# spatial_calibration_head_r1

状态：$status

## 研究问题与版本

空间校准头 R1 历史负结果与复用诊断。协议版本 R0；遵循 docs/RESEARCH_GOVERNANCE.md。本 Module 只产生声明范围内的诊断或开发证据。
...
只写入 artifacts.local/evidence/spatial_calibration_head_r1/ 或调用方显式指定的 artifacts.local/ 子目录；不写仓库根目录和正式 App 资产。
