#!/usr/bin/env python3
"""Independent D0-R1 execution validator.
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.write_results:
        validation_path = args.run_root.resolve() / "execution_validation.json"
        failure_path = args.run_root.resolve() / "failure_receipt.json"
        try:
        ...
        if result["execution_valid"]:
            receipt = {
                ...
                "claim_ceiling": (
                    "BURNED_SINGLE_CAPTURE_OPERATIONAL_CANARY_PRIORITY_ONLY; "
                    "NO CAUSAL DOMINANCE, EFFECTIVENESS, GENERALIZATION, "
                    "CONFIRMATION, PRODUCT OR SAFETY CLAIM"
                ),
            }
            _atomic_replace(
                args.run_root.resolve() / "progress.json",
                {
                    "protocol_id": PROTOCOL_ID,
                    "state": "VALID",
                    "scientific_exit": result["scientific_exit"],
            ...
            try:
                _exclusive_write(
                    args.run_root.resolve() / "execution_receipt.json",
                    receipt,
                )
            except BaseException as error:
                if not failure_path.exists():
