#!/usr/bin/env python3
"""Evaluate D39 symmetric scene confirm release of the bounded veto latch."""
...
from evaluate_stage_c_d38_thor_magni_bounded_temporal_veto_event import (
    ...
    EXPECTED_SAMPLES,
    EXPECTED_SESSIONS,
    build_gate as build_d38_gate,
    load_kernel_rows as load_d38_kernel_rows,
    summarize,
    write_json_atomic,
)
...
SCHEMA = (
    "blindassist_hftf_stage_c_d39_thor_magni_"
    "confirm_release_veto_event_v0"
)
SUPPORTED_STATUS = (
    "D39_THOR_MAGNI_CONFIRM_RELEASE_VETO_EVENT_"
...
def load_kernel_rows(path: Path) -> list[dict[str, Any]]:
    parsed = load_d38_kernel_rows(path)
    with path.open("r", encoding="utf-8", newline="") as handle:
        raw = list(csv.DictReader(handle, delimiter="\t"))
    if len(raw) != len(parsed):
        raise ValueError("D39 raw/parsed kernel row count drift")
    for target, source in zip(parsed, raw, strict=True):
        if str(target["sample_id"]) != str(source["sample_id"]):
            raise ValueError("D39 raw/parsed kernel row identity drift")
        target["confirm_release_frames"] = int(
            source["confirm_release_frames"]
        )
    return parsed
...
def build_gate(
    ...
    evaluability, support, _ = build_d38_gate(
        pooled,
        rows,
        by_fold,
        producer_receipt,
    ...
    confirmed = [
        row for row in rows if int(row["admitted_confirm_frames"]) > 0
    ]
    released = [
        row for row in rows if int(row["confirm_release_frames"]) > 0
    ]
    evaluability["confirm_opportunity"] = (
        len(confirmed) >= 10
        and len({row["source_session_id"] for row in confirmed}) >= 5
    )
    evaluability["confirm_releases_live_latch"] = (
        len(released) >= 10
        and len({row["source_session_id"] for row in released}) >= 5
    )
...
def main() -> int:
    ...
    for row in kernel_rows:
        ...
        if (
            ...
            raise ValueError("D39 kernel source identity drift")
        joined.append(
            {
                **row,
                "positive": bool(
    ...
    if (
        ...
        raise ValueError("D39 frozen label census drift")
    ...
    for fold in range(5):
        ...
        summary["confirm_release_frames"] = sum(
            int(row["confirm_release_frames"]) for row in fold_rows
        )
    ...
    for source in sorted(
        ...
        summary["confirm_release_frames"] = sum(
            int(row["confirm_release_frames"]) for row in source_rows
        )
    ...
    confirmed_rows = [
        row for row in joined if int(row["admitted_confirm_frames"]) > 0
    ]
    released_rows = [
        row for row in joined if int(row["confirm_release_frames"]) > 0
    ]
    latched_rows = [
        row
        for row in joined
    ...
    payload = {
        ...
        "cohort": {
            ...
            "post_d38_adaptive_outcome_open_development": True,
        },
        "pooled": pooled,
        "by_fold": by_fold,
        "by_source": by_source,
        ...
        "diagnostics": {
            "confirm_opportunity_anchors": len(confirmed_rows),
            "confirm_opportunity_sessions": len(
                {
                    str(row["source_session_id"])
                    for row in confirmed_rows
                }
            ),
            "confirm_release_anchors": len(released_rows),
            "confirm_release_sessions": len(
                {
                    str(row["source_session_id"])
            ...
            "confirm_threshold_per_s": 0.05,
            "contradict_threshold_per_s": -0.05,
            "threshold_or_duration_search_performed": False,
        },
        "bindings": {
        ...
        "claims": {
            ...
            "production_bidirectional_scene_source": True,
            "explicit_confirm_release": True,
