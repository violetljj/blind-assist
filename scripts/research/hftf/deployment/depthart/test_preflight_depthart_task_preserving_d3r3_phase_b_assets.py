...
def fixtures() -> tuple[dict, dict]:
    ...
    scope = {
        "schema": (
            "blindassist_depthart_task_preserving_d3r3_phase_b_source_coverage_"
            "scope_receipt_v1"
        ),
        "status": "D3R3_PHASE_B_SOURCE_COVERAGE_TRANSPORT_RECOVERY_SCOPE_REGISTERED_MEDIA_UNOPENED",
        "next_gate": "EXPLICIT_D3R3_PHASE_B_EXACT64_FRESH_HEAD_ONLY_PREFLIGHT_ACTIVATION",
        "exact_phase_a_selection": {
            "identities": identities,
            "selection_sha256": hashlib.sha256(encoded.encode("ascii")).hexdigest().upper(),
        },
        "registered_asset_scope": {
            "assets": list(ASSETS),
            "fresh_head_request_count": 64,
...
class D3R3PhaseBHeadPreflightTest(unittest.TestCase):
    ...
    def test_success_never_reads_response_body(self) -> None:
        scope, result = fixtures()
        row = with_prior(requests_for(scope, result, "https://example.invalid/raw")[0])
        ...
        self.assertEqual(
            {"content_length": False, "etag": False, "last_modified": False},
            observed["header_drift_from_prior"],
        )

    def test_fresh_header_drift_is_recorded_but_fresh_row_remains_available(self) -> None:
        scope, result = fixtures()
        row = with_prior(requests_for(scope, result, "https://example.invalid/raw")[0])

        def opener(request, timeout):
            response = Response(row["url"])
            response.headers.replace_header("Content-Length", "124")
            response.headers.replace_header("ETag", '"fresh"')
            return response

        observed = head(row, 1.0, 3, "test-agent", opener)
        self.assertFalse(observed["unresolved_error"])
        self.assertEqual(
            {"content_length": True, "etag": True, "last_modified": False},
            observed["header_drift_from_prior"],
        )
