#!/usr/bin/env python3
"""Compare qnn-net-run one-op outputs with the frozen host oracle."""
...
def main() -> int:
    ...
    for index, case in enumerate(source["cases"]):
        ...
        result_dir = args.qnn_output_root / f"Result_{index}"
        candidates = sorted(result_dir.glob("*.raw"))
        if len(candidates) != 1:
            raise ValueError(f"expected one raw output in {result_dir}, found {len(candidates)}")
        ...
        passed = finite and bool(np.allclose(actual, expected, rtol=args.rtol, atol=args.atol))
        all_pass &= passed
        records.append(
            {
                "case": case["name"],
                ...
                "max_rel": float((absolute / denominator).max()),
                "p50_abs": float(np.percentile(absolute, 50)),
                "p95_abs": float(np.percentile(absolute, 95)),
                "p99_abs": float(np.percentile(absolute, 99)),
                "per_step_max_abs": absolute.max(axis=(0, 1)).tolist(),
            }
        )
    receipt = {
        "schema": "blindassist_depthart_selective_scan_g4_operator_parity",
        "schema_version": 1,
        "status": "PASS" if all_pass else "FAIL_NUMERICAL_PARITY",
        "rtol": args.rtol,
        "atol": args.atol,
        "cases": records,
        "authority": "Operator-level parity only; full graph, partition purity, latency, thermal, Android default, safety, and production remain unevaluated.",
    }
