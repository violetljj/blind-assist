#!/usr/bin/env python3
"""Materialize D3R1 Phase-A intrinsics/trajectory and freeze portrait continuity."""
...
from scripts.research.hftf.deployment.depthart.preflight_depthart_task_preserving_d3r1_phase_a_assets import (  # noqa: E402
    PASS_TERMINAL as HEAD_PASS_TERMINAL,
    requests_for as head_requests_for,
    roster_rows as head_roster_rows,
    row_available,
)
PROTOCOL_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_body_protocol_v1"
ROSTER_SCHEMA = "blindassist_depthart_task_preserving_d3r1_fresh_metadata_roster_lock_v1"
HEAD_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_asset_header_preflight_v1"
SCOPE_SCHEMA = "blindassist_depthart_task_preserving_d3r1_source_scope_receipt_v1"
ACTIVATION_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_body_activation_v1"
MANIFEST_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_manifest_v1"
ATTEMPT_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_attempt_v1"
CHECKPOINT_SCHEMA = "blindassist_depthart_task_preserving_d3r1_phase_a_identity_checkpoint_v1"
...
def download_file(
    row: dict[str, Any], output: Path, retries: int = 3, opener: Any | None = None
) -> dict[str, Any]:
    url = str(row["url"])
    ...
    for attempt in range(1, retries + 1):
        ...
        try:
            request = urllib.request.Request(
                ...
                method="GET",
                headers={
                    "User-Agent": "BlindAssist-DepthART-D3R1-phase-a-body",
            ...
            history.append({"attempt": attempt, "method": "GET", "http_status": status, "error": None})
            return {
                "url": url,
                "bytes": size,
            ...
            history.append({"attempt": attempt, "method": "GET", "http_status": getattr(error, "code", None), "error": message})
            if partial.exists():
                partial.unlink()
            if isinstance(error, ValueError):
                break
            if isinstance(error, urllib.error.HTTPError):
                if 300 <= error.code < 500 and error.code not in {408, 429}:
                    break
...
def lookup_assets(
    preflight: dict[str, Any], expected_count: int = 254, roster: dict[str, Any] | None = None
) -> dict[tuple[str, str], dict[str, Any]]:
    if roster is not None:
        expected = head_requests_for(
            roster,
            "https://docs-assets.developer.apple.com/ml-research/datasets/arkitscenes/v1/raw",
        )
        require(len(expected) == expected_count, "HEAD plan count drift")
        require(
            all(
                all(observed[key] == planned[key] for key in ("pool_order", "visit_id", "video_id", "fold", "role", "asset", "url"))
                and row_available(observed)
                for observed, planned in zip(preflight["assets"], expected, strict=True)
            ),
            "HEAD row plan or availability drift",
        )
    ...
    for row in preflight["assets"]:
        ...
        lookup[key] = row
    require(len(lookup) == expected_count, "HEAD row count drift")
    return lookup
...
def expected_attempt(
    ...
    return {
        ...
        "authority": "D3R1_PHASE_A_BODY_AND_LABEL_BLIND_CONTINUITY_ONLY",
    }
...
def read_checkpoint(
    ...
    require(len(source_assets) == 2 and {entry["asset"] for entry in source_assets} == set(ASSETS), "checkpoint source asset inventory drift")
    if output_root is not None:
        expected_dir = (
            output_root / "source" / "Training" / str(expected_identity["video_id"])
        ).resolve()
        for entry in source_assets:
            require(
                Path(entry["path"]).resolve() == (expected_dir / entry["asset"]).resolve(),
                "checkpoint retained source path drift",
            )
        require(
            Path(value["trajectory"]["path"]).resolve()
            == (expected_dir / "lowres_wide.traj").resolve(),
            "checkpoint trajectory path drift",
        )
    for entry in source_assets:
...
def validate_resume_inventory(
    ...
    if not receipts_root.exists():
        require(not source_root.exists(), "orphan source tree without receipts")
        return 0
    receipt_names = {path.name for path in receipts_root.iterdir()}
    completed = 0
    seen_gap = False
    expected_names: set[str] = set()
    for identity in pool:
        video_id = str(identity["video_id"])
        checkpoint = receipts_root / f"{int(identity['pool_order']):03d}-{video_id}.json"
        sidecar = checkpoint.with_suffix(".sha256.json")
        if checkpoint.exists() or sidecar.exists():
            require(not seen_gap, "resume checkpoint sequence is not a continuous prefix")
            require(checkpoint.is_file() and sidecar.is_file(), "partial checkpoint seal")
            read_checkpoint(checkpoint, identity, attempt_sha256, output_root)
            expected_names.update((checkpoint.name, sidecar.name))
            completed += 1
        else:
            seen_gap = True
    require(receipt_names == expected_names, "extra or unplanned receipt inventory")
    expected_source_dirs = {str(pool[index]["video_id"]) for index in range(completed)}
    source_entries = list(source_root.iterdir()) if source_root.exists() else []
    require(all(path.is_dir() for path in source_entries), "extra file in retained source root")
    actual_source_dirs = (
        {path.name for path in source_entries}
        if source_root.exists()
        else set()
    )
    require(actual_source_dirs == expected_source_dirs, "orphan or sparse retained source inventory")
    for video_id in actual_source_dirs:
        require(
            {path.name for path in (source_root / video_id).iterdir()} == set(ASSETS)
            and all(path.is_file() for path in (source_root / video_id).iterdir()),
            "retained identity source inventory drift",
        )
    return completed
...
def validate_bindings(
    ...
    for name in ("head_protocol", "head_activation", "head_governed_result", "head_validation"):
        verify_frozen_file(protocol[name], name.replace("_", " "))
    require(
    ...
    require(
        source_scope["status"] == "D3R1_EXACT_127_PHASE_A_SOURCE_SCOPE_REGISTERED_MEDIA_UNOPENED"
        and source_scope["registered_future_asset_scope"]["phase_a_assets"] == list(ASSETS)
        and source_scope["registered_future_asset_scope"]["future_body_asset_count"] == 254
        and source_scope["registered_future_asset_scope"]["phase_b_depth_confidence_registered"] is False
        and source_scope["registered_future_asset_scope"]["phase_c_rgb_registered"] is False
        and source_scope["registered_future_asset_scope"]["identity_extension_registered"] is False
        and source_scope["redistribution_authorized"] is False,
        "source scope authority drift",
    )
    require(
        activation["authority"]["label_blind_continuity_selection"] is True
        and activation["authority"]["archive_member_read"] is True
        and activation["authority"]["pose_content_read"] is True,
    ...
    require(protocol["expected_identity_count"] == 127, "identity count drift")
    require(protocol["expected_asset_count"] == 254, "asset count drift")
    require(protocol["expected_total_content_length_bytes"] == 133734849, "body byte budget drift")
    require(protocol["pass_successor"] == "EXPLICIT_D3R1_PHASE_B_DEPTH_CONFIDENCE_SOURCE_SCOPE_REGISTRATION_FOR_EXACT_32_PHASE_A_SELECTION", "PASS successor drift")
...
def main() -> int:
    ...
    pool = head_roster_rows(roster)
    require(
        len(pool) == 127
        and [int(row["pool_order"]) for row in pool] == list(range(1, 128))
        and len({row["visit_id"] for row in pool}) == 127
        and len({row["video_id"] for row in pool}) == 127,
        "pool drift",
    )
    lookup = lookup_assets(head_result, 254, roster)
    ...
    require(args.output_root.resolve() == Path(protocol["output"]["root"]).resolve(), "output root drift")
    artifact_root = (REPO_ROOT / "artifacts.local").resolve()
    require(
        args.output_root.resolve().is_relative_to(artifact_root),
        "output root must remain inside artifacts.local",
    )
    ...
    require(
        ...
        >= expected_total + maximum_video_bytes * 3 + int(protocol["minimum_free_space_margin_bytes"]),
    ...
    if args.resume:
        validate_resume_inventory(args.output_root, pool, attempt_sha256)
    ...
    try:
        for parent in pool:
            ...
            checkpoint_path = receipts_root / f"{int(parent['pool_order']):03d}-{video_id}.json"
            if checkpoint_path.exists():
                value = read_checkpoint(checkpoint_path, parent, attempt_sha256, args.output_root)
    ...
    manifest = {
        ...
        "terminal": (
            ...
            if passed
            else "D3R1_PHASE_A_FAIL_FEWER_THAN_32_ELIGIBLE_IDENTITIES"
        ),
    }
