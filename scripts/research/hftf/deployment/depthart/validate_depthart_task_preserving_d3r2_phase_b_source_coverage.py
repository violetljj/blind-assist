#!/usr/bin/env python3
"""Independently replay the D3R2 Phase-B member-coverage census."""
...
from scripts.research.hftf.deployment.depthart.census_depthart_task_preserving_d3r2_phase_b_source_coverage import (  # noqa: E402
    ASSETS,
...
VALIDATION_SCHEMA = "blindassist_depthart_task_preserving_d3r2_phase_b_source_coverage_validation_v1"
...
def validate_activation(activation: dict[str, Any], protocol_path: Path) -> None:
    ...
    require(activation.get("status") == "D3R2_PHASE_B_SOURCE_MEMBER_COVERAGE_CENSUS_ACTIVATED", "activation status drift")
...
def expected_attempt_receipt(
    ...
    return {
        ...
        "bindings": {
            ...
            "head_governed_result_sha256": frozen["head_governed_result"]["sha256"],
...
def main() -> int:
    ...
    require(current_authority["source_scope_registration"] is True and current_authority["protocol_design"] is True, "source scope design authority drift")
    for key in set(SOURCE_SCOPE_AUTHORITY_FIELDS) - {"source_scope_registration", "protocol_design", "r2_access"}:
    ...
    for label, path in {
        ...
        "head_governed_result": args.head_governed_result,
    ...
    activation_labels = (
        "source_scope", "phase_a_result", "phase_a_manifest", "head_governed_result",
        "head_machine_result", "d3r1_execution_stop",
    )
    ...
    for planned in request_plan(selected):
        ...
        require(checkpoint["transport_response_body_bytes_read"] == int(receipt["bytes"]), "checkpoint transport-body count drift")
    ...
    require(manifest["transport_response_body_bytes_read"] == downloaded_bytes, "manifest transport-body count drift")
    ...
    result = {
        ...
        "status": "D3R2_PHASE_B_SOURCE_MEMBER_COVERAGE_VALIDATION_PASS",
        "protocol_sha256": sha256_file(args.protocol),
        "activation_sha256": sha256_file(args.activation),
        "manifest_sha256": sha256_file(args.manifest),
        "processed_identity_count": 32,
        "source_asset_count": 64,
        "transport_response_body_bytes_read": downloaded_bytes,
