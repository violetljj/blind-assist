...
from scripts.research.hftf.deployment.depthart.test_preflight_depthart_task_preserving_d3r3_phase_b_assets import (
    fixtures,
)
from scripts.research.hftf.deployment.depthart.validate_depthart_task_preserving_d3r3_phase_b_head import (
    validate_result,
)


def fixture_result() -> tuple[dict, dict, dict, dict, dict]:
    ...
    prior_head_result = {
        "schema": "blindassist_depthart_task_preserving_d3r1_phase_b_asset_header_preflight_v1",
        "assets": prior_assets,
    }
    result = {
        "schema": RESULT_SCHEMA,
        "protocol_id": PROTOCOL_ID,
        "head_terminal": PASS_TERMINAL,
        ...
        "fresh_head_authority": True,
        "total_content_length_bytes": 64 * 123,
        "authority": "HEAD availability and declared-size evidence only; no body authority.",
        "assets": assets,
    }
    return result, protocol, scope, phase_a_result, prior_head_result


class D3R3PhaseBHeadValidatorTest(unittest.TestCase):
    def test_exact_pass_fixture(self) -> None:
        result, protocol, scope, phase_a, prior = fixture_result()
        summary = validate_result(result, protocol, scope, phase_a, prior)
        self.assertEqual(PASS_TERMINAL, summary["head_terminal"])
    ...
    def test_legitimate_unavailable_terminal_validates(self) -> None:
        ...
        result["assets"][0]["header_drift_from_prior"]["etag"] = None
        result["assets"][0]["unresolved_error"] = True
        result["available_asset_count"] = 63
        result["head_terminal"] = UNAVAILABLE_TERMINAL
        result["fresh_head_authority"] = False
        result["next_gate"] = None
        summary = validate_result(result, protocol, scope, phase_a, prior)
        self.assertEqual(UNAVAILABLE_TERMINAL, summary["head_terminal"])
    ...
    def test_legitimate_incomplete_terminal_validates(self) -> None:
        ...
        result["head_terminal"] = INCOMPLETE_TERMINAL
        result["fresh_head_authority"] = False
        result["next_gate"] = None
        row["header_drift_from_prior"] = {
            "content_length": None,
            "etag": None,
            "last_modified": None,
        }
        summary = validate_result(result, protocol, scope, phase_a, prior)
        self.assertEqual(INCOMPLETE_TERMINAL, summary["head_terminal"])
    ...
    def test_non_transient_retry_is_rejected(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "non-transient attempt was retried"):
            validate_result(result, protocol, scope, phase_a, prior)
    ...
    def test_request_order_drift_fails(self) -> None:
        ...
        with self.assertRaisesRegex(ValueError, "request plan drift"):
            validate_result(mutated, protocol, scope, phase_a, prior)

    def test_prior_header_and_drift_receipt_tamper_fails(self) -> None:
        result, protocol, scope, phase_a, prior = fixture_result()
        result["assets"][0]["header_drift_from_prior"]["etag"] = True
        with self.assertRaisesRegex(ValueError, "header drift evidence mismatch"):
            validate_result(result, protocol, scope, phase_a, prior)
        result, protocol, scope, phase_a, prior = fixture_result()
        result["assets"][0]["prior_head"]["etag"] = '"tampered"'
        with self.assertRaisesRegex(ValueError, "prior HEAD binding drift"):
            validate_result(result, protocol, scope, phase_a, prior)

    def test_get_attempt_fails(self) -> None:
        result, protocol, scope, phase_a, prior = fixture_result()
        result["assets"][0]["attempt_history"][0]["method"] = "GET"
        with self.assertRaisesRegex(ValueError, "non-HEAD attempt"):
            validate_result(result, protocol, scope, phase_a, prior)
    ...
    def test_scientific_terminal_mismatch_fails(self) -> None:
        ...
        result["head_terminal"] = UNAVAILABLE_TERMINAL
        result["next_gate"] = None
        with self.assertRaisesRegex(ValueError, "HEAD terminal drift"):
            validate_result(result, protocol, scope, phase_a, prior)
