#!/usr/bin/env python3
"""HEAD-only preflight for the frozen DepthART D2 Phase-A support pool."""
...
PROTOCOL_SCHEMA = "blindassist_depthart_task_preserving_d2_phase_a_head_protocol_v1"
...
RESULT_SCHEMA = "blindassist_depthart_task_preserving_d2_phase_a_asset_header_preflight_v1"
...
def roster_rows(roster: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    require(len(rows) == 32, "expected exactly 32 support-pool identities")
    require([row["pool_order"] for row in rows] == list(range(1, 33)), "pool order drift")
    ...
    require(all(row["role"] == "D2_SOURCE_SUPPORT_POOL_ONLY" for row in rows), "role drift")
    return rows
...
def head(row: dict[str, Any], timeout: float, retries: int) -> dict[str, Any]:
    ...
    for attempt in range(1, retries + 1):
        request = urllib.request.Request(
            str(row["url"]),
            method="HEAD",
            headers={"User-Agent": "BlindAssist-DepthART-D2-phase-a-preflight"},
        )
        try:
...
def disposition(rows: list[dict[str, Any]]) -> str:
    if any(row["http_status"] is None for row in rows):
        return "D2_PHASE_A_HEADER_PREFLIGHT_INCOMPLETE_MEDIA_BODY_UNOPENED"
    if any(row["http_status"] != 200 or not row["content_length_bytes"] for row in rows):
        return "D2_PHASE_A_ASSETS_NOT_AVAILABLE_MEDIA_BODY_UNOPENED"
    return "D2_PHASE_A_ASSET_HEADERS_AVAILABLE_MEDIA_BODY_UNOPENED"
...
def main() -> int:
    ...
    require(license_receipt["authority"]["phase_a_head"] is True, "Phase-A HEAD not authorized")
    require(
        license_receipt["bindings"]["metadata_pool"]["sha256"] == protocol["roster"]["sha256"],
    ...
    return 0 if terminal == "D2_PHASE_A_ASSET_HEADERS_AVAILABLE_MEDIA_BODY_UNOPENED" else 1
