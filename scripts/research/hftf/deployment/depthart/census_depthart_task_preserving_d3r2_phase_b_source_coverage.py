#!/usr/bin/env python3
"""Materialize the D3R2 Phase-B source-member coverage census.
...
PROTOCOL_PATH = REPO_ROOT / "docs/research/hftf/DEPTHART_TASK_PRESERVING_D3R2_PHASE_B_SOURCE_COVERAGE_RECOVERY_PROTOCOL_2026-08-12.json"
...
SOURCE_SCOPE_SCHEMA = "blindassist_depthart_task_preserving_d3r2_phase_b_source_coverage_scope_receipt_v1"
...
MANIFEST_SCHEMA = "blindassist_depthart_task_preserving_d3r2_phase_b_source_coverage_manifest_v1"
PASS_TERMINAL = "D3R2_PHASE_B_EXACT64_MEMBER_COVERAGE_CENSUS_COMPLETE_NO_MEMBER_PAYLOAD_OR_TRUTH_READ"
PASS_NEXT_GATE = "EXPLICIT_D3R2_PHASE_B_MISSING_SOURCE_POLICY_REGISTRATION"
...
ACTIVATION_FORBIDDEN = (
    "reuse any D3R1 r0 body, checkpoint truth, partial qualification or operator probe",
...
ACTIVATION_NEXT_ACTION = "RUN_FROZEN_D3R2_PHASE_B_SOURCE_COVERAGE_PRODUCER_THEN_INDEPENDENT_OFFLINE_VALIDATOR"
SOURCE_SCOPE_AUTHORITY_FIELDS = (
    "source_scope_registration", "protocol_design", "media_head", "media_get",
...
def selection_rows(
    ...
    require(frame_plan_sha256(selected) == source_scope["registered_future_asset_scope"]["selected_frame_plan_sha256"], "frame plan digest drift")
    return selected
...
def head_lookup(head_result: dict[str, Any], selected: list[dict[str, Any]]) -> dict[tuple[str, str], dict[str, Any]]:
    require(head_result.get("schema") == "blindassist_depthart_task_preserving_d3r1_phase_b_asset_header_preflight_v1", "HEAD schema drift")
    require(head_result.get("terminal") == "D3R1_PHASE_B_ASSET_HEADERS_64_OF_64_AVAILABLE_MEDIA_BODY_UNOPENED", "HEAD terminal drift")
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        try:
            request = urllib.request.Request(url, method="GET", headers={
                "User-Agent": "BlindAssist-DepthART-D3R2-phase-b-coverage",
            ...
            require(size == expected_length, "download length mismatch")
            os.rename(partial, output)
            history.append({"attempt": attempt, "method": "GET", "http_status": status, "error": None, "error_type": None, "retry_class": None})
            return {
            ...
            retry_class = "TRANSIENT_HTTP" if transient_http else "TRANSIENT_TRANSPORT" if transient_transport else "TERMINAL"
            history.append({"attempt": attempt, "method": "GET", "http_status": status, "error": f"{type(error).__name__}: {error}", "error_type": type(error).__name__, "retry_class": retry_class})
            if partial.exists():
                partial.unlink()
...
def expected_attempt(
    *,
    ...
    return {
        ...
        "selection_evaluated": False,
        "authority": "D3R2_PHASE_B_SOURCE_TRANSPORT_CONTAINER_AND_MEMBER_NAME_COVERAGE_ONLY",
        "rgb_access": False,
...
def validate_activation(activation: dict[str, Any], protocol_path: Path) -> None:
    ...
    require(
        activation.get("status") == "D3R2_PHASE_B_SOURCE_MEMBER_COVERAGE_CENSUS_ACTIVATED",
...
def validate_protocol_activation_contract(protocol: dict[str, Any]) -> None:
    ...
    require(contract["required_status"] == "D3R2_PHASE_B_SOURCE_MEMBER_COVERAGE_CENSUS_ACTIVATED", "protocol activation status drift")
...
def validate_bindings(
    *,
    ...
    require(protocol.get("status") == "D3R2_PHASE_B_COVERAGE_RECOVERY_PROTOCOL_FROZEN_MEDIA_UNOPENED", "protocol status drift")
    ...
    require(source_scope.get("status") == "D3R2_PHASE_B_SOURCE_COVERAGE_SCOPE_REGISTERED_MEDIA_UNOPENED", "source scope status drift")
    ...
    require(registered["identity_count"] == 32 and registered["future_get_asset_count"] == 64, "source scope count drift")
    ...
    require(int(registered["declared_download_bytes"]) == int(protocol["declared_download_bytes"]), "source scope declared bytes drift")
    require(source_scope["next_gate"] == protocol["next_gate"], "source scope/protocol gate drift")
    ...
    require(source_scope["registered_future_asset_scope"]["future_output_root"] == protocol["output_root"], "scope/protocol output root drift")
...
def validate_transport_receipt(
    ...
    for index, event in enumerate(history, start=1):
        require(set(event) == {"attempt", "method", "http_status", "error", "error_type", "retry_class"}, "source attempt field set drift")
        ...
        if index < attempts:
            ...
            require(event["retry_class"] in {"TRANSIENT_HTTP", "TRANSIENT_TRANSPORT"}, "terminal attempt was retried")
            ...
            if event["retry_class"] == "TRANSIENT_HTTP":
                ...
                require(status is None, "transport retry status drift")
...
def verify_asset_checkpoint(
    ...
    require(checkpoint["transport_response_body_bytes_read"] == int(receipt["bytes"]), "transport-body count drift")
...
def main() -> int:
    ...
    for planned in plan[len(completed):]:
        ...
        try:
            ...
            checkpoint = {
                ...
                **coverage,
                "transport_response_body_bytes_read": int(receipt["bytes"]),
                "scientific_terminal": None,
                "pixel_decode": False,
                "source_truth": False,
                "selection_evaluated": False,
    ...
    manifest = {
        ...
        "transport_response_body_bytes_read": sum(int(row["source_asset"]["bytes"]) for row in completed),
