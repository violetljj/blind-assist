...
function Parse-Report([string[]]$Lines) {
    $match = [regex]::Match(($Lines -join "`n"), "geometry_equivalent_optimization_r0_report=(\{.*\})")
    if (-not $match.Success) { throw "geometry report is missing" }
...
try {
    ...
    $instrument = Invoke-Native $AdbPath @(
        ...
        "-e", "class", "com.linnan.blindassist.hftf.GeometryOptimizationDeviceTest#frozenGeometryReferenceVsReusableWorkspace",
    ...
    [ordered]@{ schema = "blindassist_geometry_equivalent_optimization_r0_bundle"; generated_at = (Get-Date).ToString("o"); device_serial = $DeviceSerial; transport = "usb"; depth_path = $DepthPath; instrumentation_exit_code = $instrument.ExitCode; report = $report } |
        ConvertTo-Json -Depth 20 | Set-Content -LiteralPath (Join-Path $artifactRoot "result.json") -Encoding utf8
    "artifact_root=$artifactRoot"; "gate_pass=$($report.gate_pass)"; "optimized_p50_ms=$($report.optimized.wall_ms.p50)"
