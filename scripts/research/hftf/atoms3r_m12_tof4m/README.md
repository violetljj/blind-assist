...
R4 通过 session-only API 仅关闭自动曝光并固定到实际值 490。用 R1 的同一
`36.320 ms` 阈值复算，slow fraction 从 17.71% 降至 8.84%，capture→framebuffer
...
设备 R6 对 MJPEG stream 的具体 socket 启用并回读 `TCP_NODELAY`。相邻五分钟
单变量 A/B 中，host JPEG read P95/P99 从 `62.48/72.02 ms` 降至
`32.86/37.23 ms`，capture→feedback P95/P99 从 `128.94/164.42 ms` 降至
`121.37/151.24 ms`，且 off 基线的约 1.1 秒 write/read 尖峰在 on 运行中未复现；
代价是 device response write P50 从 `22.13 ms` 增至 `26.46 ms`。实时路线按尾延迟
优先原则晋升 `TCP_NODELAY`，但单次 A/B 不证明未来不会再有极端阻塞。状态 API 和
逐帧 header 均绑定配置/回读值，主机缺失该字段时拒绝验收。

R7 将每帧 boundary 与 metadata header 从两个 HTTP chunk 合并为一个。相邻五分钟
A/B 中，response write P50/P95 只改善 `1.34/2.11 ms`、P99 仅改善 `0.26 ms`，
候选却观测到一次 `1.564 s` 写出阻塞和 `1.621 s` capture→feedback。无法证明
