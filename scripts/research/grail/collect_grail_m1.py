...
def collect(dataset: Path, manifest_path: Path, role: str, output: Path) -> dict[str, Any]:
    ...
    try:
        for roster_row in roster:
            ...
            for obj in objects:
                ...
                for query_position in ranked_query_positions(reachable, obj["position"], sample_key)[:32]:
                    ...
                    query_pose = {**query_position, "rotation": rotations[0], "horizon": 0.0, "standing": True}
        ...
        if controller is not None:
            controller.stop()
    report = {
        "schema": "blindassist_grail_m1_collection_v2b_single_hash_ranked_visible_yaw", "role": role,
        "manifest_sha256": manifest_hash, "dataset_sha256": dataset_hash,
