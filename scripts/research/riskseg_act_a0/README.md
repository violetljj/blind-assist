...
行动标签质量与 actionability 诊断，历史结果保留。协议版本 R0；遵循 docs/RESEARCH_GOVERNANCE.md。本 Module 只产生声明范围内的诊断或开发证据。
...
只写入 artifacts.local/evidence/riskseg_act_a0/ 或调用方显式指定的 artifacts.local/ 子目录；不写仓库根目录和正式 App 资产。
