#!/usr/bin/env python3
"""Run sealed TARO R10 candidate inference and source-only Phase A."""
...
from scripts.research.taro_o1r_r10_clear_runtime import run_pool_inventory


REPO_ROOT = Path(__file__).resolve().parents[3]
LOCK_SCHEMA = "blindassist.taro.o1r.r10_fresh_pool_phase_a_execution_lock.v1"
LOCK_ID = "TARO_O1R_R10_FRESH_POOL_SOURCE_ONLY_PHASE_A_ONE_SHOT_EXECUTION_LOCK"
INVENTORY_PATH = "artifacts.local/evidence/taro/o1r-r10-fresh-pool-inventory-r0/exact-frame-plan.json"
OUTPUT_ROOT = "artifacts.local/evidence/taro/o1r-r10-fresh-pool-phase-a-r0"
PASS_TERMINAL = "TARO_O1R_R10_FRESH_POOL_PHASE_A_SOURCE_ONLY_SEALED_PASS"
FAIL_TERMINAL = "TARO_O1R_R10_FRESH_POOL_PHASE_A_EXECUTION_INVALID"
FRAME_COUNT = 710
QUERY_COUNT = FRAME_COUNT * 9
PARENT_COUNT = 32
FROZEN_FRAME_COUNTS = [8, 87, 15, 15, 9, 26, 21, 14, 55, 9, 13, 5, 16, 7, 23, 22, 15, 7, 27, 20, 13, 32, 18, 13, 20, 36, 21, 25, 17, 63, 16, 22]
FROZEN_MATERIALIZED_BYTES = 2_388_664_781
...
EXPECTED_BINDINGS = {
    ...
    "R10_DOWNLOAD_RECEIPTS": "artifacts.local/evidence/taro/o1r-r10-fresh-pool-source-r0/download-receipts.json",
    "R10_DOWNLOAD_RESULT": "artifacts.local/evidence/taro/o1r-r10-fresh-pool-source-r0/result.json",
    "R10_INVENTORY_PLAN": INVENTORY_PATH,
    "R10_INVENTORY_RESULT": "artifacts.local/evidence/taro/o1r-r10-fresh-pool-inventory-r0/result.json",
    "R10_INVENTORY_MANIFEST": "artifacts.local/evidence/taro/o1r-r10-fresh-pool-inventory-r0/manifest.json",
    ...
    "SOURCE_FEATURE_RUNTIME": "scripts/research/taro_o1r_r7_canary_runtime/r7_canary.py",
    "EVIDENCE_WRITER": "scripts/research/taro_o0r_factor_headroom_runtime/evidence.py",
    "R10_PHASE_A_RUNNER": "scripts/research/taro_o1r_r10_clear_runtime/run_pool_phase_a.py",
    "R10_PHASE_A_TEST": "scripts/research/taro_o1r_r10_clear_runtime/test_run_pool_phase_a.py",
}
EXPECTED_AUTHORITY = {
...
def _seal(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    require("content_sha256" not in record, "R10_PHASE_A_SEAL_COLLISION", "caller supplied a content seal")
    record["content_sha256"] = adapter.canonical_sha256(record)
    return record


def _validate_seal(value: Any, schema: str) -> dict[str, Any]:
    require(isinstance(value, dict), "R10_PHASE_A_RECORD_INVALID", "sealed record must be an object")
...
def _load_frames(inventory_path: Path) -> list[r6io.R6FrameRef]:
    ...
    for parent, expected, expected_count in zip(
        ...
        require(identity == expected[:2], "R10_PHASE_A_ROSTER_DRIFT", "R10 inventory roster drift")
        ...
        require(len(tokens) == expected_count, "R10_PHASE_A_FRAME_COUNT_DRIFT", "R10 parent frame count drift")
        for token in tokens:
            require(
                all(token in up_index[role] for role in up_index) and token in intr_index,
                "R10_PHASE_A_MEMBER_MISSING",
                "R10 exact member is absent",
                token=token,
            )
    ...
    require(len(frames) == FRAME_COUNT, "R10_PHASE_A_COHORT_DRIFT", "R10 cohort is not exact 32/710")
    require(
        len({(row.parent_id, row.video_id, row.timestamp_token) for row in frames}) == FRAME_COUNT,
        "R10_PHASE_A_KEY_DUPLICATE",
        "R10 frame key duplicated",
    )
    return frames
...
def _read_candidate_input(
    ...
    def observed(role: str, _: str) -> None:
        require(role in {"color", "intrinsics"}, "R10_CANDIDATE_PAYLOAD_FIREWALL", "candidate read a forbidden payload", role=role)
        reads[role] += 1
    ...
    gravity = adapter._normalize_vector(transform[2, :3], "R10_FRESH_GRAVITY_INVALID")
    record = _seal(
        {
            "schema": "blindassist.taro.o1r.r10_fresh_pool_candidate_input.v1",
            "analysis_role": "R10_FRESH_CONFIRMATION",
...
def _run_candidate(
    ...
    require(native_batch.shape == (1, *depthart_runner.NATIVE_SHAPE_HW), "R10_NATIVE_DEPTH_INVALID", "DepthART native shape drift")
    native = np.ascontiguousarray(native_batch[0], dtype=np.float32)
    require(bool(np.all(np.isfinite(native))), "R10_NATIVE_DEPTH_INVALID", "DepthART output contains non-finite values")
    highres = depthart_runner.upsample_native_depth(native)
    inference = _seal(
        {
            "schema": "blindassist.taro.o1r.r10_fresh_pool_depthart_inference.v1",
            "analysis_role": "R10_FRESH_CONFIRMATION",
...
def _load_candidate(root: Path, frame: r6io.R6FrameRef) -> tuple[dict[str, Any], dict[str, Any], np.ndarray, np.ndarray]:
    candidate_input = _validate_seal(_load_json(root / _candidate_input_relative(frame)), "blindassist.taro.o1r.r10_fresh_pool_candidate_input.v1")
    record = _validate_seal(_load_json(root / _candidate_record_relative(frame)), "blindassist.taro.o1r.r10_fresh_pool_candidate_frame.v1")
    require(record["candidate_input_sha256"] == candidate_input["content_sha256"] and record["physical_frame_id"] == frame.physical_frame_id, "R10_CANDIDATE_LINEAGE_DRIFT", "candidate lineage drift")
    blob = record["native_depth_blob"]
    payload = (root / blob["path"]).read_bytes()
    require(len(payload) == blob["bytes"] and materializer.sha256_bytes(payload) == blob["sha256"], "R10_CANDIDATE_BLOB_DRIFT", "candidate blob hash drift")
    native = np.ascontiguousarray(depthart_runner.decode_npy_gzip_bytes(payload), dtype=np.float32)
    require(native.shape == depthart_runner.NATIVE_SHAPE_HW and adapter.canonical_sha256(native) == blob["array_sha256"], "R10_CANDIDATE_ARRAY_DRIFT", "candidate native array drift")
    highres = depthart_runner.upsample_native_depth(native)
    require(adapter.canonical_sha256(highres) == record["inference_receipt"]["highres_depth_sha256"], "R10_CANDIDATE_HIGHRES_DRIFT", "candidate highres replay drift")
...
def _verify_inventory_evidence() -> dict[str, Any]:
    manifest = _load_json(_repo_path(EXPECTED_BINDINGS["R10_INVENTORY_MANIFEST"]))
    require(
        manifest.get("schema") == "blindassist.taro.o1r.r10_fresh_pool_inventory_manifest.v1"
        and manifest.get("one_shot_consumed") is True,
        "R10_PHASE_A_INVENTORY_MANIFEST",
        "R10 inventory manifest identity drift",
    )
    ...
    require(
        ...
        "R10_PHASE_A_INVENTORY_MANIFEST",
        "R10 inventory manifest file set drift",
    )
    ...
    for relative, receipt in files.items():
        ...
        require(
            ...
            "R10_PHASE_A_INVENTORY_FILE",
            f"R10 inventory artifact drift: {relative}",
        )
    inventory = run_pool_inventory.validate_inventory(
        _load_json(_repo_path(INVENTORY_PATH)),
        expected_frame_counts=FROZEN_FRAME_COUNTS,
        expected_materialized_bytes=FROZEN_MATERIALIZED_BYTES,
    )
    result = _load_json(_repo_path(EXPECTED_BINDINGS["R10_INVENTORY_RESULT"]))
    require(
        result.get("schema") == "blindassist.taro.o1r.r10_fresh_pool_inventory_result.v1"
        and result.get("execution_valid") is True
        and result.get("passed") is True
        and result.get("terminal") == run_pool_inventory.PASS_TERMINAL
        ...
        and result.get("materialized_bytes") == FROZEN_MATERIALIZED_BYTES
        ...
        "R10_PHASE_A_INVENTORY_RESULT",
        "R10 inventory result not admitted",
    )
    return inventory
...
def validate_execution_lock(path: Path) -> dict[str, Any]:
    ...
    require(
        ...
        "R10_PHASE_A_LOCK_IDENTITY",
        "R10 Phase-A execution lock identity drift",
    )
    ...
    require(
        ...
        "R10_PHASE_A_USER_AUTHORITY",
        "R10 Phase-A user authority drift",
    )
    actual_argv = [
    ...
    require(lock.get("argv") == actual_argv, "R10_PHASE_A_ARGV_DRIFT", "R10 Phase-A argv drift")
    require(
        lock.get("inventory_path") == INVENTORY_PATH
        and lock.get("output_root") == OUTPUT_ROOT
        and lock.get("overwrite") is False
        and lock.get("rerun") is False,
        "R10_PHASE_A_ROOT_DRIFT",
        "R10 Phase-A root policy drift",
    )
    bindings = lock.get("bindings")
    require(
        isinstance(bindings, list) and len(bindings) == len(EXPECTED_BINDINGS),
        "R10_PHASE_A_BINDINGS",
        "R10 Phase-A binding count drift",
    )
    ...
    for row in bindings:
        ...
        require(
            ...
            "R10_PHASE_A_BINDING_ROW",
            "R10 Phase-A binding row drift",
        )
        ...
        require(
            ...
            "R10_PHASE_A_BINDING_HASH",
            f"R10 Phase-A binding drift: {relative}",
        )
        verified[role] = row
    ...
    require(
        ...
        == {
            ...
            "maximum_peak_rss_bytes": 17179869184,
            "maximum_cuda_allocated_bytes": 8500000000,
            "maximum_evidence_bytes": 2147483648,
        },
        "R10_PHASE_A_BUDGET_DRIFT",
        "R10 Phase-A resource budget drift",
    )
    ...
    require(
        ...
        "R10_PHASE_A_CANDIDATE_IDENTITY",
        "R10 candidate identity drift",
    )
    require(
        source.is_dir()
        ...
        "R10_PHASE_A_CANDIDATE_ASSET",
        "R10 candidate source/checkpoint drift",
    )
    ...
    require(
        commit == identity["source_commit"] and not dirty,
        "R10_PHASE_A_CANDIDATE_SOURCE_DRIFT",
        "R10 candidate source tree drift",
    )
    require(not _repo_path(OUTPUT_ROOT).exists(), "R10_PHASE_A_ROOT_COLLISION", "R10 Phase-A evidence root exists")
...
def _write_failure(writer: FactorEvidenceWriter, error: Exception) -> None:
    try:
        writer.write_json("failure.json", {"schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_failure.v1", "terminal": FAIL_TERMINAL, "execution_valid": False, "failure_code": str(getattr(error, "code", type(error).__name__)), "message": str(error), "one_shot_consumed": True})
        writer.write_json("manifest.json", {"schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_manifest.v1", "terminal": FAIL_TERMINAL, "files": dict(sorted(writer.file_receipts.items())), "file_count_before_manifest": len(writer.file_receipts), "bytes_before_manifest": writer.bytes_written})
    except Exception:
        pass
...
def execute(lock_path: Path) -> dict[str, Any]:
    ...
    def guard() -> None:
        require(time.monotonic() - started <= float(budget["maximum_wall_seconds"]), "R10_PHASE_A_TIMEOUT", "Phase-A wall budget exceeded")
        require(process.memory_info().rss <= int(budget["maximum_peak_rss_bytes"]), "R10_PHASE_A_RSS_EXCEEDED", "Phase-A RSS budget exceeded")

    writer.activate(
        {
            "schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_execution_receipt.v1",
    ...
    try:
        ...
        for parent_key, parent_frames_iter in groupby(frames, key=lambda row: (row.parent_id, row.video_id)):
            ...
            with zipfile.ZipFile(parent_frames[0].upsampling_archive) as up_bundle, zipfile.ZipFile(parent_frames[0].intrinsics_archive) as intr_bundle:
                for frame in parent_frames:
                    ...
                    candidate = _seal(
                        {
                            "schema": "blindassist.taro.o1r.r10_fresh_pool_candidate_frame.v1",
                            "analysis_role": "R10_FRESH_CONFIRMATION",
                            "parent_id": frame.parent_id,
                            "video_id": frame.video_id,
                            "timestamp_token": frame.timestamp_token,
                    ...
                    if completed % 10 == 0 or completed == FRAME_COUNT:
                        print(json.dumps({"phase": "R10_FRESH_CANDIDATE", "completed": completed, "total": FRAME_COUNT, "physical_frame_id": frame.physical_frame_id}, sort_keys=True), flush=True)
        require(candidate_reads == Counter({"color": FRAME_COUNT, "intrinsics": FRAME_COUNT}), "R10_CANDIDATE_READ_COUNT_DRIFT", "candidate read counts drift", reads=dict(candidate_reads))
        writer.write_json(
            "candidate-completion.json",
            _seal(
                {
                    "schema": "blindassist.taro.o1r.r10_fresh_pool_candidate_completion.v1",
        ...
        try:
            ...
            cuda_peak = 0
        require(cuda_peak <= int(budget["maximum_cuda_allocated_bytes"]), "R10_PHASE_A_CUDA_EXCEEDED", "candidate CUDA budget exceeded")
        ...
        def observed_source(role: str, _: str) -> None:
            require(role in {"lowres_depth", "confidence"}, "R10_SOURCE_PAYLOAD_FIREWALL", "source feature phase read a forbidden payload", role=role)
            source_reads[role] += 1
        ...
        for parent_key, parent_frames_iter in groupby(frames, key=lambda row: (row.parent_id, row.video_id)):
            ...
            with zipfile.ZipFile(parent_frames[0].upsampling_archive) as up_bundle:
                for frame in parent_frames:
                    ...
                    source = _seal(
                        {
                            "schema": "blindassist.taro.o1r.r10_fresh_pool_source_frame_receipt.v1",
                            "analysis_role": "R10_FRESH_CONFIRMATION",
                            "parent_id": frame.parent_id,
                            "video_id": frame.video_id,
                            "timestamp_token": frame.timestamp_token,
                    ...
                    guard()
                    if completed % 10 == 0 or completed == FRAME_COUNT:
                        print(json.dumps({"phase": "R10_FRESH_SOURCE_FEATURES", "completed": completed, "total": FRAME_COUNT, "physical_frame_id": frame.physical_frame_id}, sort_keys=True), flush=True)
        require(source_reads == Counter({"lowres_depth": FRAME_COUNT, "confidence": FRAME_COUNT}), "R10_SOURCE_READ_COUNT_DRIFT", "source feature read counts drift", reads=dict(source_reads))
        require(sum(state_counts.values()) == QUERY_COUNT and state_counts["CLEAR_OBSERVED"] == 0, "R10_SOURCE_STATE_DRIFT", "fresh source candidate emitted CLEAR or lost queries")
        completion = _seal(
            {
                "schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_completion.v1",
        ...
        reloaded = _validate_seal(_load_json(output / "phase-a-completion.json"), "blindassist.taro.o1r.r10_fresh_pool_phase_a_completion.v1")
        require(reloaded == completion and reloaded["faro_reads"] == 0, "R10_PHASE_A_COMPLETION_RELOAD_DRIFT", "Phase-A completion reload drift")
        result = {
            "schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_result.v1",
            ...
            "claim_ceiling": "Sealed R10 source-only positive-occupancy features; no FARO label, effectiveness, deployment, product, or safety evidence.",
        }
        writer.write_json("result.json", result)
        require(
            len(writer.file_receipts) == 5 * FRAME_COUNT + 4,
            "R10_PHASE_A_MANIFEST_COUNT_DRIFT",
            "R10 Phase-A file count before manifest drift",
            file_count=len(writer.file_receipts),
        )
        writer.write_json("manifest.json", {"schema": "blindassist.taro.o1r.r10_fresh_pool_phase_a_manifest.v1", "terminal": PASS_TERMINAL, "files": dict(sorted(writer.file_receipts.items())), "file_count_before_manifest": len(writer.file_receipts), "bytes_before_manifest": writer.bytes_written})
        return result
