"""Run SAGE-LM V1-B-R5 anchor-conditioned aperture-pair coverage."""
...
SCHEMA_VERSION = "sage_lm_v1b_r5_anchor_conditioned_aperture_pair_coverage"


def run(cohort_path: Path, extractor: DeepLsdDenseFieldExtractor) -> dict:
    cohort = json.loads(cohort_path.read_text(encoding="utf-8"))
    if len(cohort["episodes"]) != 24:
        raise ValueError("V1-B-R5 requires the frozen 24-episode R2 cohort")
    rows = []
    controls_retained = 0
    for materialized in cohort["episodes"]:
        ...
        provider = AnchorPairOracleBoundaryProvider(episode_input, truth, pose_a, pose_b, extractor)
    ...
    for row in rows:
        ...
        if provider_diagnostics.get("failure") == "APERTURE_PAIR_HYPOTHESIS_MISSING":
            candidate_missing += 1
    diagnostics.update(
        {
    ...
    return {
        ...
        "experiment_label": "V1_B_R5_ANCHOR_CONDITIONED_APERTURE_PAIR_COVERAGE_B1",
        "cohort": {
        ...
        "rows": rows,
        "claim_ceiling": "CURATED_R2_DEVELOPMENT_COHORT_ANCHOR_CONDITIONED_PAIR_COVERAGE_ONLY",
    }
