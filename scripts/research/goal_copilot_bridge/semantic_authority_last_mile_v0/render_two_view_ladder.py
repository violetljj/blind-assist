...
def render(v1: dict, v1a: dict, v1b: dict, cohort: dict, output: Path) -> None:
    evaluable = bool(v1b.get("evaluable"))
    canvas = np.full((1000, 1800, 3), 248, dtype=np.uint8)
    _text(canvas, "SAGE-LM observation ladder", (45, 65), 1.15, thickness=3)
    subtitle = (
        "Correct-pose R2 cohort | same frozen downstream policy"
        if evaluable
        else "Same frozen 24 episodes and downstream policy"
    )
    _text(canvas, subtitle, (48, 105), 0.58, (95, 95, 95), 1)
    ...
    baseline = v1b["metrics"]["bbox_center_scale"] if evaluable else v1["metrics"]["bbox_center_scale"]
    ...
    if evaluable:
        rows = [
            ...
            metric_row("B0 oracle pixels + pose", v1b["metrics"]["b0"], (65, 175, 70)),
            metric_row("B1 RGB + oracle assoc", v1b["metrics"]["b1"], (35, 35, 190)),
            metric_row("B2 RGB + auto assoc", v1b["metrics"]["b2"], (30, 145, 210)),
        ]
    else:
        rows = [
            metric_row("bbox baseline", baseline, (75, 100, 235)),
    ...
    footer = (
        "R2 input contract valid: 24/24 pairs pass motion and projected-aperture visibility gates."
        if evaluable
        else "V1-B N/E: only 2/24 frozen pairs satisfy the intended source-pose motion gate."
    )
    _text(canvas, footer, (65, 650), 0.48, (35, 35, 180), 1)

    valid_rows = [row for row in v1b["rows"] if row["source_pose_audit"]["intended_active_pair_gate_pass"]]
    selected = next(
        (row for row in valid_rows if row["b2"]["observation"]["center_x_m"] is not None),
        valid_rows[0],
    )
    ...
    if evaluable:
        _text(canvas, "B0 passes 24/24; B1 has 2/24 geometry outputs and 0/24 confident arrivals.", (70, 840), 0.63, (35, 35, 150), 2)
        _text(canvas, "Primary bottleneck: RGB boundary extraction; B2 association remains confounded.", (70, 890), 0.63, (35, 35, 150), 2)
        _text(canvas, "No LK, metric depth, policy, baseline, criteria, or post-outcome pair changes.", (70, 930), 0.52, (75, 75, 75), 1)
    else:
