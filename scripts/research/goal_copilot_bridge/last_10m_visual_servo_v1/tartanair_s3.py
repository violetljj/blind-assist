#!/usr/bin/env python3
"""Run and evaluate frozen stateless TartanAir S3 geometric door completion."""
...
from scripts.research.goal_copilot_bridge.last_10m_visual_servo_v1.ade20k_functional_aperture_dev import region_depth_percentile
...
PROTOCOL_ID = "BLINDASSIST_TARTANAIR_CURRENT_FRAME_COMPLETION_S3_V1"
HEIGHT_FRACTION_MIN = 0.40
SENSOR_DEPTH_MAX_M = 1.80
DINO_IOU_MIN = 0.85
TARGET_IOU_MIN = 0.30
AUTH_SCHEMA = "blindassist_tartanair_current_frame_completion_s3_authorization_v1"
RUN_SCHEMA = "blindassist_tartanair_current_frame_completion_s3_run_v1"
EVAL_SCHEMA = "blindassist_tartanair_current_frame_completion_s3_evaluation_v1"
PROVIDER_EXTRA: dict[str, Any] = {}


def select_geometric_candidate(candidates: Sequence[Mapping[str, Any]], dino_candidates: Sequence[Mapping[str, Any]], width: int, height: int) -> dict[str, Any] | None:
    eligible = []
    for candidate in candidates:
        ...
        if not box[0] <= width / 2.0 <= box[2]:
            continue
        if height_fraction < HEIGHT_FRACTION_MIN or float(candidate["sensor_region_depth_m"]) > SENSOR_DEPTH_MAX_M:
            continue
        ...
        if best_iou >= DINO_IOU_MIN:
            eligible.append(dict(candidate) | {"height_fraction": height_fraction, "dino_consensus_iou": best_iou, "dino_candidate": dict(dino_candidates[best_index])})
...
def authorize(manifest_path: Path, public_path: Path, private_path: Path, run_path: Path, journal_path: Path, output_path: Path) -> dict[str, Any]:
    _require(not output_path.exists() and not run_path.exists() and not journal_path.exists(), "S3 formal output already exists")
    manifest, public, private = _read(manifest_path), _read(public_path), _read(private_path)
    _require(manifest.get("protocol_id") == PROTOCOL_ID and manifest.get("created_before_provider_calls") is True, "S3 manifest drift")
    _require(public.get("schema_version") == PUBLIC_SCHEMA and private.get("schema_version") == PRIVATE_SCHEMA, "S3 input schema drift")
    _require(public.get("protocol_id") == private.get("protocol_id") == PROTOCOL_ID, "S3 input protocol drift")
    _require(private.get("public_input_sha256") == sha256(public_path), "S3 public/private binding mismatch")
    public_ids = {case["case_id"] for case in public["cases"]}
    private_cases = {case["case_id"]: case for case in private["cases"]}
    _require(public_ids == set(private_cases), "S3 roster mismatch")
    _require(all("range_sensor" in case for case in public["cases"]), "S3 public range sensor absent")
    near = sum(case["stratum"] == "NEAR" for case in private_cases.values())
    far = sum(case["stratum"] == "FAR" for case in private_cases.values())
    _require(near == 24 and far == 24, "S3 denominator drift")
...
def run(manifest_path: Path, public_path: Path, authorization_path: Path, yoloe_model_path: Path, text_encoder_path: Path, dino_model_dir: Path, journal_path: Path, output_path: Path, device: str) -> dict[str, Any]:
    _require(not journal_path.exists() and not output_path.exists(), "S3 formal run already exists")
    manifest, public, authorization = _read(manifest_path), _read(public_path), _read(authorization_path)
    _require(manifest.get("protocol_id") == PROTOCOL_ID and manifest.get("provider", {}).get("stateless_current_frame_only") is True, "S3 provider drift")
    _require(authorization.get("schema_version") == AUTH_SCHEMA and authorization.get("execution_authorized") is True, "S3 not authorized")
    _require(authorization.get("manifest_sha256") == sha256(manifest_path) and authorization.get("public_sha256") == sha256(public_path), "S3 authorization binding mismatch")
    _require(authorization.get("run_output_path") == str(output_path.resolve()) and authorization.get("journal_output_path") == str(journal_path.resolve()), "S3 output binding mismatch")
    ...
    journal = {"schema_version": "blindassist_tartanair_current_frame_completion_s3_journal_v1", "protocol_id": PROTOCOL_ID, "status": "STARTED", "started_at_utc": datetime.now(timezone.utc).isoformat(), "cases_dispatched": 0, "cases_completed": 0, "yoloe_calls_completed": 0, "dino_calls_completed": 0, "retry_or_replay_authorized": False}
    ...
    try:
        for case in public["cases"]:
            ...
            _require(image_path.is_file() and sha256(image_path) == case["query"]["image_sha256"], "S3 image drift")
            _require(depth_path.is_file() and sha256(depth_path) == case["range_sensor"]["depth_sha256"], "S3 depth drift")
            ...
            candidates = [{"provider_rank": rank, "bbox_xyxy": [float(value) for value in box], "proposal_score": float(score), "sensor_region_depth_m": region_depth_median(depth, box, width, height), "sensor_region_depth_p20_m": region_depth_percentile(depth, box, width, height, 20.0)} for rank, (score, box) in enumerate(ranked, start=1)]
            ...
            selected = select_geometric_candidate(candidates, dino_candidates, width, height)
            rows.append({"case_id": case["case_id"], "image_width": width, "image_height": height, "yoloe_candidates": candidates, "dino_candidates": dino_candidates, "selected_candidate": selected, "completion": selected is not None, "latency_ms": {"yoloe": yolo_latency, "dino": dino_latency}})
            journal.update({"status": "ACTIVE", "active_case_id": None, "cases_completed": journal["cases_completed"] + 1, "yoloe_calls_completed": journal["yoloe_calls_completed"] + 1, "dino_calls_completed": journal["dino_calls_completed"] + 1})
            _atomic_json(journal_path, journal)
            print(f"S3 {journal['cases_completed']}/{len(public['cases'])} case={case['case_id']} completion={selected is not None}", flush=True)
        payload = {"schema_version": RUN_SCHEMA, "protocol_id": PROTOCOL_ID, "manifest_sha256": sha256(manifest_path), "public_sha256": sha256(public_path), "authorization_sha256": sha256(authorization_path), "private_truth_access": False, "stateless_current_frame_only": True, "provider": {"yoloe_sha256": sha256(yoloe_model_path), "text_encoder_sha256": sha256(text_encoder_path), "dino_repository": MODEL_REPOSITORY, "dino_revision": MODEL_REVISION, "dino_weights_sha256": WEIGHTS_SHA256, "dino_prompt": PROMPT, "height_fraction_min": HEIGHT_FRACTION_MIN, "sensor_depth_max_m": SENSOR_DEPTH_MAX_M, "dino_iou_min": DINO_IOU_MIN, "ultralytics": ultralytics.__version__, "transformers": transformers.__version__, "python": platform.python_version(), "device": device, **PROVIDER_EXTRA}, "cases": rows, "claim_ceiling": manifest["claim_ceiling"]}
...
def evaluate(manifest_path: Path, public_path: Path, private_path: Path, run_path: Path, journal_path: Path, output_path: Path) -> dict[str, Any]:
    _require(not output_path.exists(), "S3 evaluation already exists")
    ...
    _require(run_payload.get("schema_version") == RUN_SCHEMA and run_payload.get("private_truth_access") is False, "S3 run boundary mismatch")
    _require(run_payload.get("manifest_sha256") == sha256(manifest_path) and run_payload.get("public_sha256") == sha256(public_path), "S3 run binding mismatch")
    _require(journal.get("status") == "COMPLETED" and journal.get("run_sha256") == sha256(run_path), "S3 journal incomplete")
    _require(journal["cases_completed"] == journal["yoloe_calls_completed"] == journal["dino_calls_completed"] == len(public["cases"]), "S3 call accounting mismatch")
    truths = {case["case_id"]: case for case in private["cases"]}
    runs = {case["case_id"]: case for case in run_payload["cases"]}
    _require(set(truths) == set(runs), "S3 evaluation roster mismatch")
    rows = []
    for case_id in sorted(truths):
        ...
        available = any(iou(validated_box(candidate["bbox_xyxy"], f"{case_id} candidate"), target) >= TARGET_IOU_MIN for candidate in observed["yoloe_candidates"] for target in targets)
        ...
        if selected is not None:
            ...
            matched_index = int(np.argmax(overlaps)) if overlaps and max(overlaps) >= TARGET_IOU_MIN else None
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    for name in ("manifest", "public", "authorization", "yoloe-model", "text-encoder", "dino-model-dir", "journal", "output"):
    ...
    if args.command == "authorize":
        ...
        run(args.manifest, args.public, args.authorization, args.yoloe_model, args.text_encoder, args.dino_model_dir, args.journal, args.output, args.device)
    else:
