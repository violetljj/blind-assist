#!/usr/bin/env python3
"""Fuse frozen functional-region output with the frozen RGB door verifier."""
...
def run(public_path: Path, functional_prediction_path: Path, weights_path: Path, training_receipt_path: Path, output_path: Path, device: str) -> dict[str, Any]:
    ...
    _require(functional.get("private_truth_access") is False and functional.get("public_sha256") == sha256(public_path), "functional/RGB public boundary mismatch")
    _require(receipt.get("decision_threshold") == DOOR_PROBABILITY_MIN and receipt.get("best_weights_sha256") == sha256(weights_path), "frozen RGB verifier drift")
    import torch
    ...
    for case_id in sorted(public_cases):
        ...
        if candidate is not None:
            ...
            _require(sha256(image_path) == public_cases[case_id]["query"]["image_sha256"], "functional/RGB image drift")
            with Image.open(image_path) as opened:
                tensor = transform(expanded_crop(opened.convert("RGB"), candidate["bbox_xyxy"])).unsqueeze(0).to(device)
            with torch.inference_mode():
                probability = float(torch.softmax(model(tensor), dim=1)[0, 1].cpu())
        selected = candidate | {"door_probability": probability} if candidate is not None and probability >= DOOR_PROBABILITY_MIN else None
        rows.append({"case_id": case_id, "functional_candidate_present": candidate is not None, "door_probability": probability, "selected_candidate": selected, "completion": selected is not None})
    payload = {"schema_version": "blindassist_functional_region_rgb_fusion_development_prediction_v1", "created_at_utc": datetime.now(timezone.utc).isoformat(), "role": "DEVELOPMENT_ONLY", "public_sha256": sha256(public_path), "functional_prediction_sha256": sha256(functional_prediction_path), "training_receipt_sha256": sha256(training_receipt_path), "weights_sha256": sha256(weights_path), "private_truth_access": False, "door_probability_min": DOOR_PROBABILITY_MIN, "threshold_or_rule_sweep": False, "cases": rows}
...
def evaluate(private_path: Path, prediction_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    _require(prediction.get("private_truth_access") is False and prediction.get("threshold_or_rule_sweep") is False, "functional/RGB evaluation boundary mismatch")
