#!/usr/bin/env python3
"""Private evaluator for fresh HRG2 globally reranked local proposals."""
...
from scripts.research.goal_copilot_bridge.p1_proposal_availability.run_global_local_reranking import BOUNDED_POOL_SIZE, GLOBAL_NMS_IOU, LOCAL_CANDIDATES_PER_PARENT, PARENT_REGION_POOL_SIZE, PREDICTION_SCHEMA, PROTOCOL_ID
...
def evaluate(public_path: Path, private_path: Path, prediction_path: Path, prompt_map_path: Path) -> dict[str, Any]:
    ...
    _require(private.get("public_input_sha256") == public_hash, "HRG2 private/public binding mismatch")
    _require(prediction.get("schema_version") == PREDICTION_SCHEMA and prediction.get("protocol_id") == PROTOCOL_ID, "HRG2 prediction contract mismatch")
    _require(prediction.get("public_input_sha256") == public_hash and prediction.get("private_truth_access") is False, "HRG2 isolation mismatch")
    ...
    _require(all(provider.get(key) == value for key, value in expected.items()), "HRG2 frozen provider drift")
    ...
    for case in public["cases"]:
        ...
        _require(receipt["private_truth_body_sha256"] == truth_body_hash, "HRG2 precedence/truth binding mismatch")
    ...
    _require(set(case_ids) == set(truth) == set(predicted), "HRG2 case identity mismatch")
    rows = []
    for case_id in case_ids:
        item = truth[case_id]
        visibility = item.get("target_visibility", "VISIBLE")
        _require(visibility in TARGET_VISIBILITY_STATES, "HRG2 visibility drift")
        candidates = predicted[case_id]["candidates"]
        _require(len(candidates) <= BOUNDED_POOL_SIZE, "HRG2 bounded pool exceeded")
        _require([row["rank"] for row in candidates] == list(range(1, len(candidates) + 1)), "HRG2 rank drift")
    ...
    terminal = "P1_HRG2_NOT_EVALUABLE_NO_VISIBLE_CASES" if not evaluable else "P1_HRG2_GLOBAL_LOCAL_TARGET_AVAILABILITY_NOT_OBSERVED" if recall["recall_at_10"] == 0.0 else "P1_HRG2_FULL_GLOBAL_LOCAL_TARGET_AVAILABILITY_ON_FRESH_COHORT" if recall["recall_at_10"] == 1.0 else "P1_HRG2_PARTIAL_GLOBAL_LOCAL_TARGET_AVAILABILITY_ON_FRESH_COHORT"
    return {
        "schema_version": "blindassist_p1_hrg2_evaluation_v1", "protocol_id": PROTOCOL_ID,
        ...
        "identity_selection": "NOT_EVALUATED", "claim_ceiling": "FRESH_GLOBAL_LOCAL_PROPOSAL_AVAILABILITY_ONLY_NO_IDENTITY_GENERALIZATION_PRODUCT_OR_SAFETY_CLAIM",
    }
...
def main() -> int:
    ...
    if args.output.exists():
        raise ValueError("HRG2 evaluation already exists")
    output = evaluate(args.public, args.private, args.prediction, args.prompt_map)
