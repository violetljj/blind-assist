...
def validate_public(public: Mapping[str, Any], prompt_map: Mapping[str, Any], base_dir: Path) -> list[dict[str, Any]]:
    ...
    _require(contract.get("public_spatial_context", False) in {True, False}, "public spatial context declaration is invalid")
    ...
    for raw in cases:
        ...
        spatial = goal.get("public_spatial_context")
        if spatial is not None:
            _require(contract.get("public_spatial_context") is True, f"{case_id} undeclared public spatial context")
            _require(isinstance(spatial, Mapping), f"{case_id} public spatial context must be an object")
            _require(spatial.get("schema_version") in {"blindassist_p1_pa3_public_spatial_goal_contract_v2", "blindassist_p1_pa3_public_spatial_goal_contract_v3"}, f"{case_id} public spatial schema mismatch")
            _require(spatial.get("spatial_goal_role") == "PRODUCT_NAVIGATION_ROUTE_ENDPOINT_CANDIDATE", f"{case_id} public spatial role drift")
            _require(spatial.get("source_authority") == "OPENSTREETMAP_PRETRUTH", f"{case_id} public spatial source drift")
            route_endpoints = spatial.get("route_endpoint_candidates")
            _require(isinstance(route_endpoints, list) and bool(route_endpoints), f"{case_id} route endpoint candidates are required")
            candidate_ids = set()
            for route_endpoint in route_endpoints:
                _require(isinstance(route_endpoint, Mapping), f"{case_id} route endpoint candidate is invalid")
                _require(isinstance(route_endpoint.get("lat"), (int, float)) and isinstance(route_endpoint.get("lon"), (int, float)), f"{case_id} route endpoint coordinates are invalid")
                candidate_ids.add(_text(route_endpoint.get("candidate_id"), f"{case_id} route endpoint candidate id"))
            _require(len(candidate_ids) == len(route_endpoints), f"{case_id} duplicate route endpoint candidate")
            _require(spatial.get("observation_selected_candidate_id") in candidate_ids, f"{case_id} selected public spatial candidate drift")
