...
def validate_public(public: Mapping[str, Any], prompt_map: Mapping[str, Any], base_dir: Path) -> list[dict[str, Any]]:
    ...
    for raw in cases:
        ...
        if spatial is not None:
            ...
            _require(spatial.get("schema_version") == "blindassist_p1_pa3_public_spatial_goal_contract_v2", f"{case_id} public spatial schema mismatch")
            ...
            route_endpoint = spatial.get("route_endpoint_candidate")
            _require(isinstance(route_endpoint, Mapping), f"{case_id} route endpoint candidate is required")
