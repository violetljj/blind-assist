...
状态：`current / WILD_LAB / R0_DATA_SUPPORT_AUDIT_LOCKED_NOT_RUN / ORACLE_NOT_AUTHORIZED / MODEL_AND_TRAINING_NOT_AUTHORIZED`
...
oracle。它不继承 A0、AG-QSF 或合成 H3 的科学 authority。
...
- 后续 oracle 输出必须另立协议和新 namespace；当前不得创建 model/checkpoint。
...
- 数据支撑 gate 通过：只授权下一份 oracle/representation-value 协议锁，不自动授权执行、模型或训练；
