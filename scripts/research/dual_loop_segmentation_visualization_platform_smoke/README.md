# dual_loop_segmentation_visualization_platform_smoke

状态：$status

## 研究问题与版本

分割可视化与平台 smoke，不产生模型 utility 结论。协议版本 R0；遵循 docs/RESEARCH_GOVERNANCE.md。本 Module 只产生声明范围内的诊断或开发证据。
...
只写入 artifacts.local/evidence/dual_loop_segmentation_visualization_platform_smoke/ 或调用方显式指定的 artifacts.local/ 子目录；不写仓库根目录和正式 App 资产。
