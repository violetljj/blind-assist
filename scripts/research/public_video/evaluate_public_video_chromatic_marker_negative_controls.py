#!/usr/bin/env python3
"""Prospectively evaluate the frozen chromatic marker expert on nuisance controls."""
...
SCHEMA = "blindassist_public_video_chromatic_marker_negative_controls_v1"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if features.get("schema") != "blindassist_public_video_traffic_cone_detection_features_v1":
        raise ValueError("unexpected construction-marker feature schema")
    if features.get("prospective_contract") != contract_attestation:
        raise ValueError("feature report is not bound to the supplied chromatic marker contract")
    sources = features.get("sources")
    if not isinstance(sources, list) or not sources:
        raise ValueError("chromatic marker negative-control report has no sources")
    results = []
    for source in sources:
        filtered = dict(source)
        filtered["samples"] = chromatic.apply_policy(source["samples"], policy)
