#!/usr/bin/env python3
"""Run the fixed r7.55 dense future-to-current ego-trace teacher."""
...
SCHEMA = "blindassist_public_video_dense_future_ego_trace_probe_v1"


def map_anchor_with_flow(flow_future_to_current: np.ndarray, anchor_xy_norm: list[float]) -> tuple[float, float] | None:
    flow = np.asarray(flow_future_to_current, dtype=np.float32)
    if flow.ndim != 3 or flow.shape[2] != 2 or not np.isfinite(flow).all():
        return None
    height, width = flow.shape[:2]
    x = min(width - 1, max(0, int(round(float(anchor_xy_norm[0]) * (width - 1)))))
    y = min(height - 1, max(0, int(round(float(anchor_xy_norm[1]) * (height - 1)))))
    delta = flow[y, x]
    return (x + float(delta[0])) / width, (y + float(delta[1])) / height
...
def evaluate_event(features: dict[str, Any], source_id: str, window: tuple[int, int], policy: dict[str, Any]) -> dict[str, Any]:
    ...
    for timestamp in event_times:
        ...
        for horizon in horizons:
            ...
            point = map_anchor_with_flow(flow, policy["future_anchor_xy_norm"])
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    for path in (args.contract, args.event_set_contract, args.output):
        mil.reject_independent_direction(path)
    if args.output.exists() or Path(str(args.output) + ".sha256").exists():
        raise ValueError(f"refusing to overwrite output: {args.output}")
    contract = json.loads(args.contract.read_text(encoding="utf-8"))
    event_contract = json.loads(args.event_set_contract.read_text(encoding="utf-8"))
    if common.sha256_file(args.event_set_contract) != contract["event_set_contract_sha256"]:
        raise ValueError("event-set contract hash mismatch")
    reports = {}
    ...
    for event in event_contract["events"]:
        diagnostic = evaluate_event(
            reports[event["feature_key"]], event["source_id"], tuple(map(int, event["window_ms"])), contract["teacher"]
        )
        rows.append({
