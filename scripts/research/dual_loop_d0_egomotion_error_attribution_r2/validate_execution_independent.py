#!/usr/bin/env python3
"""Independent D0-R2 execution validator.
...
try:
    ...
    from scripts.research.dual_loop_d0_egomotion_error_attribution_r2.runtime_environment import (
        validate_runtime_manifest,
    )


PROTOCOL_ID = "D0_EGOMOTION_ERROR_ATTRIBUTION_R2"
PROTOCOL_SHA256 = "39627821c3da18bd896cae0458294c9d825830435692984f6f0c401211283dfe"
...
def independently_recompute_event_table(repo_root: Path, protocol: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    with paths["revel_calibration"].open("r", encoding="utf-8") as handle:
        calibration = np.asarray(yaml.safe_load(handle)["T_v_c"], dtype=np.float64)
    topics = {
...
def validate_formal_identities(
    ...
    *,
    ...
    if "runtime_environment" in protocol:
        ...
        if (
            formal_start.get("vicon_bag_messages_opened") is not True
    ...
    if "r1_failure_gate" in protocol:
        if (
            ...
            raise ValidationError(
                "implementation-lock R2 recovery binding mismatch"
            )
        ...
        runtime_manifest_path = _bound_file(
            ...
            label="R2 runtime environment manifest",
        )
        if (
            sha256_file(runtime_manifest_path)
            != runtime_specification["manifest"]["sha256"]
        ):
            raise ValidationError("R2 runtime manifest hash mismatch")
        ...
        if (
            ...
            raise ValidationError("R2 live runtime validation mismatch")
    ...
    *,
    ...
    try:
        ...
        if run_root.resolve() != resolve_expected_run_root(
            ...
            raise ValidationError("run root differs from frozen R2 namespace")
            ...
            canonical_json_bytes(
                {
                    "status": "PRESTART_INVALID",
                    "formal_execution_authorized": False,
                    "error": "run root differs from frozen R2 namespace",
                }
            ).decode("utf-8"),
            end="",
        )
            ...
            canonical_json_bytes(
                {
                    ...
                    "formal_execution_authorized": False,
                    "error": (
                        "frozen R2 namespace and formal_start.json must "
                        "already exist"
                    ),
                }
            ).decode("utf-8"),
            ...
            try:
                ...
                _atomic_replace(
                    run_root / "progress.json",
                    {
                        ...
                        "scientific_exit": None,
                        "errors": ["VALID_RECEIPT_PUBLICATION_FAILED"],
                    },
                )
                raise
        else:
