...
状态：`current / TARO_RESEARCH_MODULE / R11_WEAK_DISTAL_ABSTENTION_DEVELOPMENT_ONLY / R11_PROTOCOL_LOCKED / R11_EXACT_DATA_USE_AUTHORIZED / R11_HEAD_PASS_ONE_SHOT_CONSUMED / R11_DOWNLOAD_ATTEMPT_01_PRESTART_SUPERSEDED / R11_SOURCE_DOWNLOAD_144_OF_144_INTEGRITY_PASS_ONE_SHOT_CONSUMED / R11_SOURCE_ARCHIVES_UNOPENED / R11_SCIENTIFIC_NOT_RUN / DEFAULT_APP_UNCHANGED`
...
协议、授权 receipt、development 小型结果和 validator 写入 Git；HEAD evidence 与 Attempt 02 download evidence 均封存在 consumed `artifacts.local/` roots。144 个 source blobs 已完整下载并重验，但 ZIP member 和 source frame 仍未打开。

## 安全边界

R10 只作为 consumed development evidence，不能被改门、重跑或写成 confirmation。R11 候选只有 `OCCUPIED/UNKNOWN`；`UNKNOWN` 永远不是 negative。download Attempt 02 已 144/144 PASS 并消费；后续只能另锁本地 archive inventory，像素解码、模型与 FARO 仍未打开。
