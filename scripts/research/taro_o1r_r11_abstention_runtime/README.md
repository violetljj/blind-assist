...
状态：`current / TARO_RESEARCH_MODULE / R11_WEAK_DISTAL_ABSTENTION_DEVELOPMENT_ONLY / R11_PROTOCOL_LOCKED / R11_EXACT_DATA_USE_AUTHORIZED / R11_HEAD_PASS_ONE_SHOT_CONSUMED / R11_DOWNLOAD_ATTEMPT_01_PRESTART_SUPERSEDED / R11_DOWNLOAD_ATTEMPT_02_AUTHORIZED_UNCONSUMED / R11_SOURCE_UNOPENED / R11_SCIENTIFIC_NOT_RUN / DEFAULT_APP_UNCHANGED`
...
协议、授权 receipt、development 小型结果和 validator 写入 Git；HEAD evidence 已封存在 consumed `artifacts.local/` exclusive root。Attempt 01 因 direct-script import 在 root/GET 前停止；Attempt 02 已绑定稳定 `python -m` 入口并处于 `AUTHORIZED_UNCONSUMED`，source/evidence root 仍不存在。

## 安全边界

R10 只作为 consumed development evidence，不能被改门、重跑或写成 confirmation。R11 候选只有 `OCCUPIED/UNKNOWN`；`UNKNOWN` 永远不是 negative。HEAD 已以 144/144 PASS 消费；download Attempt 01 未消费但已 supersede，Attempt 02 只可消费一次，source body、模型与 FARO 仍未打开。
