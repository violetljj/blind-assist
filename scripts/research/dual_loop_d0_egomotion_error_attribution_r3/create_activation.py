#!/usr/bin/env python3
"""Create D0-R3 activation only after a hash-bound independent review passes."""
...
def main() -> int:
    ...
    activation = {
        ...
        "runtime_execution": {
            "manifest": dict(lock["runtime_environment"]["manifest"]),
            "python_executable": dict(
                lock["runtime_environment"]["python_executable"]
            ),
            "argv_contract": {
                "adapter": (
                    "scripts/"
                    "run_dual_loop_d0_egomotion_error_attribution_r3.py"
                ),
                "command": "produce",
                "activation_path": _relative(root, output),
                "implementation_lock_path": _relative(root, lock_path),
            },
        },
