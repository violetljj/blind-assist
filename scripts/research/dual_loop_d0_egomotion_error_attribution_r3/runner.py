...
FORMAL_OUTPUT_RELATIVE_PATH = Path(
    ...
    "d0-egomotion-error-attribution-r3/run-r3"
)
...
def _load_independent_calibration_parser() -> Any:
    from validate_execution_independent import (
        read_camera_from_marker_independent,
    )

    return read_camera_from_marker_independent
...
def run_producer(
    *,
    ...
    if implementation_validation.get("status") != "VALID":
        raise RunnerError(
            "implementation prestart envelope is invalid: "
    ...
    dependency_smoke = prestart_dependency_smoke()
    synthetic_calibration_path = (
        Path(__file__).resolve().parent / "synthetic_calibration.yaml"
    )
    producer_calibration = science["read_camera_from_marker"](
        synthetic_calibration_path
    )
    independent_calibration = _load_independent_calibration_parser()(
        synthetic_calibration_path
    )
    if (
        producer_calibration.shape != (4, 4)
        or independent_calibration.shape != (4, 4)
        or not bool((producer_calibration == independent_calibration).all())
    ):
        raise RunnerError("independent calibration parser smoke mismatch")
    fixture_relative = (
        "scripts/research/"
        "dual_loop_d0_egomotion_error_attribution_r3/"
        "synthetic_calibration.yaml"
    )
    dependency_smoke["producer_calibration_parser"] = {
        "fixture": fixture_relative,
        "shape": [4, 4],
        "finite": True,
        "values_retained": False,
    }
    dependency_smoke["independent_calibration_parser"] = {
        "fixture": fixture_relative,
        "shape": [4, 4],
        "finite": True,
        "values_retained": False,
    }
    del producer_calibration
    del independent_calibration
    ...
    formal_start = {
        ...
        "vicon_bag_messages_opened": probe.get(
            "vicon_bag_messages_opened", True
        ),
        "prestart_dependency_smoke": dependency_smoke,
    ...
    try:
        ...
        if full_implementation_validation.get("status") != "VALID":
            raise RunnerError(
                "post-marker scientific input validation failed: "
                f"{full_implementation_validation.get('failures', [])}"
            )
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the activated one-shot D0-R3 producer and analysis."
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Must equal the frozen run-r3 output root.",
    )
    return parser
