#!/usr/bin/env python3
"""Run the frozen r10 factor recipe once on checkpoint-unseen real TUM data."""
...
import run_ag_r2_icl_fresh_confirmation_seam as common  # noqa: E402


hybrid = common.hybrid
DEFAULT_FROZEN_RECIPE_RESULT = common.DEFAULT_FROZEN_RECIPE_RESULT
EXPECTED_FROZEN_RECIPE_RESULT_SHA256 = common.EXPECTED_FROZEN_RECIPE_RESULT_SHA256
...
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT
    / "artifacts.local/experiments/ag-r2-tum-real-fresh-confirmation-seam-r0"
)
EXPECTED_PARENT_ID = "rgbd_dataset_freiburg3_sitting_static"
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    hybrid.require(
        ...
        == EXPECTED_FRESH_LABEL_RESULT_SHA256,
        "TUM real fresh label result drift",
    )
    hybrid.require(
    ...
    hybrid.require(
        ...
        and all(frozen["gates"].values()),
        "frozen r10 recipe prerequisite drift",
    )
    hybrid.require(
        fresh["passed"]
        and fresh["status"] == "AG_R2_TUM_REAL_FRESH_CONFIRMATION_LABELS_PASS"
        and fresh["frame_count"] == EXPECTED_FRAME_COUNT,
        "TUM real fresh label frontdoor drift",
    )
    hybrid.require(
        fresh["source"][
            "checkpoint_unseen_by_current_factor_and_metric_students"
        ]
        is True
        and fresh["source"]["real_world"] is True,
        "TUM source identity is not checkpoint-unseen real data",
    )
    hybrid.require(
        fresh["decision"][
        ...
        is False,
        "TUM labels were materialized from current model or reducer output",
    )

    frozen_parents = common.frozen_training_parent_ids(factor_result, metric_result)
    hybrid.require(EXPECTED_PARENT_ID not in frozen_parents, "TUM parent leaked into training")
    rows = sorted(
        [{**dict(row), "role": "FRESH_REAL_CONFIRMATION"} for row in fresh["frames"]],
    ...
    del factor_model
    metric_model, metric_checkpoint = common.load_metric_depth_student(
        metric_result, device
    )
    ...
    for sample in samples:
        ...
        state_counts, reason_counts = common.state_and_reason_counts(reduced_first)
    ...
    gates = {
        "TUMSEAM_C01_EXACT_FROZEN_RECIPE_AND_SOURCE_RECEIPTS": bool(
            len(samples) == EXPECTED_FRAME_COUNT
            and factor_checkpoint["sha256"]
            == frozen["factor_checkpoint"]["sha256"]
        ),
        "TUMSEAM_C02_CHECKPOINTS_EXCLUDE_REAL_PARENT": bool(
            EXPECTED_PARENT_ID not in frozen_parents
        ),
        "TUMSEAM_C03_NO_FRESH_FIT_SELECTION_OR_RECALIBRATION": bool(
        ...
        "TUMSEAM_C04_FACTOR_TENSORS_ROUNDTRIP_12_OF_12": bool(
            len(factors) == EXPECTED_FRAME_COUNT
            and all(Path(row["path"]).is_file() for row in factors)
        ),
        "TUMSEAM_C05_ADAPTER_HAS_VALID_REAL_FRAMES": bool(
            len(structurally_valid) >= 6
        ),
        "TUMSEAM_C06_REDUCER_DETERMINISTIC_12_OF_12": bool(
            all(row["deterministic_repeat_equal"] for row in seam_rows)
        ),
        "TUMSEAM_C07_NONTRIVIAL_FAIL_CLOSED_GEOMETRY_STATE": bool(
            state_set - {"UNKNOWN"} and "UNKNOWN" in state_set
        ),
        "TUMSEAM_C08_FACTOR_ONLY_AND_LABEL_FIREWALL": bool(
    ...
    result = {
        "schema": "blindassist_ag_r2_tum_real_fresh_confirmation_seam_result_v1",
        "status": "AG_R2_TUM_REAL_FRESH_CONFIRMATION_SEAM_PASS"
        if passed
        else "AG_R2_TUM_REAL_FRESH_CONFIRMATION_SEAM_FAIL_NO_RETUNING",
        "passed": passed,
        ...
        "fresh_label_result": {
            ...
            "real_world": True,
            "support_boundary_supervision_tier": "B_GEOMETRY_ANCHORED_TEACHER",
        },
        ...
        "frames": seam_rows,
        "decision": {
            "checkpoint_unseen_real_factor_student_to_ag_mechanics_complete": passed,
            "ag_research_pipeline_mechanics_landed": passed,
            "fresh_outcomes_used_for_fit_selection_or_recalibration": False,
            "cross_sensor_or_cross_dataset_generalization_claim": False,
            "mobile_or_htp_claim": False,
            "product_or_safety_claim": False,
            "default_app_changed": False,
            "next_action_if_pass": "Freeze the AG research-pipeline result and proceed to export/runtime integration without widening the evidence claim.",
            "next_action_if_fail": "Freeze this one-shot negative result and diagnose without retuning on this source.",
            "claim_ceiling": "Checkpoint-unseen real TUM parent with source-native metric depth and geometry-anchored tier-B support/boundary through the unchanged deterministic AG seam; not cross-sensor, mobile, product, or safety proof.",
        },
    }
