#!/usr/bin/env python3
"""Materialize the four AG-F1-fresh Attempt-05 held parents.

The source archives were previously characterized by the read-only AG-ST data
line, but none of these parent identities has been used by the AG R2 F1 factor
model for optimization, checkpoint selection, or canary metrics.
"""
...
DEFAULT_LOCK = REPO_ROOT / (
    ...
    "BLINDASSIST_ASSISTIVE_GEOMETRY_R2_F1_ATTEMPT05_FRESH_AG_HELD_SOURCE_LOCK_2026-08-11.json"
)
...
DEFAULT_OUTPUT_DIR = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt05-fresh-ag-held-labels-r0"


def _locked_rows(lock: dict[str, Any]) -> list[dict[str, Any]]:
    token = str(lock["assignment"]["token"])
    rows: list[dict[str, Any]] = []
    for role in ("CHECKPOINT_SELECTION", "TRAIN_CANARY"):
        role_rows = lock["roles"][role]
        require(len(role_rows) == 2, f"Attempt-05 {role} parent count drift")
        for row in role_rows:
            expected = hashlib.sha256(
                f"{token}:{row['parent_id']}".encode("utf-8")
            ).hexdigest().upper()
            require(expected == row["assignment_sha256"], "Attempt-05 assignment SHA drift")
            source_path = (REPO_ROOT / row["source_path"]).resolve()
            require(source_path.is_file(), f"Attempt-05 source archive missing: {source_path}")
            require(source_path.stat().st_size == int(row["source_bytes"]), "Attempt-05 source length drift")
            require(sha256_file(source_path) == row["source_sha256"], "Attempt-05 source SHA drift")
            rows.append(
                {
                    **row,
                    "role": role,
                    "content_length": int(row["source_bytes"]),
                    "_source_path": source_path,
                }
            )
    require(len(rows) == 4 and len({row["parent_id"] for row in rows}) == 4, "Attempt-05 roster drift")
    return rows


def run(lock_path: Path, source_contract_path: Path, output_dir: Path) -> dict[str, Any]:
    require(not output_dir.exists(), f"output directory exists: {output_dir}")
    lock = json.loads(lock_path.read_text(encoding="utf-8"))
    require(
        lock["status"] == "ATTEMPT05_FRESH_AG_F1_HELD_LABEL_MATERIALIZATION_AUTHORIZED",
        "Attempt-05 fresh-held lock status invalid",
    )
    source_contract_sha = sha256_file(source_contract_path)
    require(
        source_contract_sha == lock["source_and_label_contract"]["sha256"],
        "Attempt-05 source contract binding drift",
    )
    base_contract = json.loads(source_contract_path.read_text(encoding="utf-8"))
    source = base_contract["source_contract"]
    token = str(lock["assignment"]["token"])
    rows = _locked_rows(lock)
    ...
    for row in rows:
        ...
        frames, receipt = load_parent_frames(row, source_path.parent, source, token)
        require(len(frames) == 3, "Attempt-05 selected frame count drift")
        require(receipt["source_archive_sha256"] == row["source_sha256"], "Attempt-05 source receipt SHA drift")
    ...
    require(len(all_frames) == 12, "Attempt-05 total frame count drift")
    ...
    gates = {
        ...
        "A5_S02_ROLE_FRAME_AND_ORIENTATION_EXACT": (
        ...
        "A5_S03_ALL_FOUR_PARENTS_JOINT_PARENT_LEVEL": all(parent_joint.values()),
        "A5_S04_TASK_FIREWALL": True,
        "A5_S05_AG_F1_MODEL_METRICS_UNOPENED": True,
    }
    passed = all(gates.values())
    result = {
        "schema": "blindassist_assistive_geometry_r2_f1_attempt05_fresh_ag_held_labels_v1",
        "status": (
            "ATTEMPT05_FRESH_AG_HELD_LABEL_FRONTDOOR_PASS_EXECUTION_LOCK_REQUIRED"
            if passed
            else "ATTEMPT05_FRESH_AG_HELD_LABEL_FRONTDOOR_FAIL_NO_MODEL"
        ),
        "passed": passed,
        ...
        "decision": {
            "attempt05_model_or_optimizer_authorized": False,
            "selection_or_canary_ag_f1_model_metrics_opened": False,
            "next_action_if_pass": "Calibrate depth and boundary uncertainty on consumed evidence, then freeze the Attempt-05 execution lock before reading any model metric on these labels.",
        },
    }
...
def main() -> int:
    ...
    result = run(args.lock.resolve(), args.source_contract.resolve(), args.output_dir.resolve())
    print(
        json.dumps(
            {
