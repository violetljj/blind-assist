#!/usr/bin/env python3
"""Train a factor-only metric depth student from the expanded SuperTeacher corpus."""
...
DEFAULT_OUTPUT_DIR = (
    REPO_ROOT / "artifacts.local/experiments/ag-r2-metric-depth-student-r0"
)
TRAINING_SEED = 20260812
LOCAL_LOG_CORRECTION_RANGE = 1.25
GLOBAL_LOG_CORRECTION_RANGE = 1.50


class MetricDepthStudentHead(nn.Module):
    """Predict bounded local shape and global metric-scale corrections."""

    def __init__(self, hidden: int = 64, global_hidden: int = 128) -> None:
        super().__init__()
        inputs = DEPTHART_PYRAMID_CHANNELS + 1
        self.local = SpatialBlock(inputs, hidden, 1)
        self.global_scale = nn.Sequential(
            nn.Linear(2 * inputs, global_hidden),
            nn.GELU(),
            nn.Linear(global_hidden, 1),
        )
        nn.init.zeros_(self.global_scale[-1].weight)
        nn.init.zeros_(self.global_scale[-1].bias)
    ...
    def forward(
        ...
        raw_local = LOCAL_LOG_CORRECTION_RANGE * torch.tanh(self.local(value))
        local = raw_local - raw_local.mean(dim=(-2, -1), keepdim=True)
        pooled = torch.cat(
            [
                value.mean(dim=(-2, -1)),
                value.std(dim=(-2, -1), unbiased=False),
            ],
            dim=1,
        )
        global_correction = GLOBAL_LOG_CORRECTION_RANGE * torch.tanh(
            self.global_scale(pooled)
        )[:, :, None, None]
        predicted = base_log + local + global_correction
        return {
            "predicted_log_depth": predicted,
            "log_shape_correction": local,
            "global_log_scale_correction": global_correction[:, 0, 0, 0],
            "removed_log_correction_center": raw_local.mean(dim=(-2, -1)),
        }
...
def metric_depth_loss(
    ...
    predicted_scale = masked_mean(predicted, valid)
    truth_scale = masked_mean(truth, valid)
    predicted_shape = predicted - predicted_scale
    truth_shape = truth - truth_scale
    point = masked_mean(
        F.smooth_l1_loss(predicted, truth, reduction="none", beta=0.10), valid
    )
    scale = F.smooth_l1_loss(predicted_scale, truth_scale, beta=0.05)
    shape = masked_mean(
        F.smooth_l1_loss(predicted_shape, truth_shape, reduction="none", beta=0.05),
        valid,
    )
    ...
    local_regularizer = outputs["log_shape_correction"].square().mean()
    global_regularizer = outputs["global_log_scale_correction"].square().mean()
    total = (
        point
        + 1.50 * scale
        + 0.50 * shape
        + 0.25 * (horizontal + vertical)
        + 0.005 * local_regularizer
        + 0.001 * global_regularizer
    )
    return {
        "total": total,
        "point_log_huber": point,
        "metric_scale_log_huber": scale,
        "shape_log_huber": shape,
        "horizontal_log_gradient": horizontal,
        "vertical_log_gradient": vertical,
        "local_correction_l2": local_regularizer,
        "global_correction_l2": global_regularizer,
    }
...
def frame_metrics(
    ...
    selected = valid.bool()
    predicted_residual = (predicted_log - truth_log)[selected]
    base_residual = (base_log - truth_log)[selected]
    return {
        "depth_abs_log_error": float(predicted_residual.abs().mean()),
        "depth_log_rmse": float(torch.sqrt(predicted_residual.square().mean())),
        "depth_abs_log_error_q90": float(torch.quantile(predicted_residual.abs(), 0.90)),
        "depth_mae_m": float((predicted_log.exp() - truth_depth).abs()[selected].mean()),
        "depth_scale_abs_log_error": abs(float(predicted_residual.mean())),
        "base_depth_abs_log_error": float(base_residual.abs().mean()),
        "base_depth_log_rmse": float(torch.sqrt(base_residual.square().mean())),
        "base_depth_abs_log_error_q90": float(torch.quantile(base_residual.abs(), 0.90)),
        "base_depth_mae_m": float((base_log.exp() - truth_depth).abs()[selected].mean()),
        "base_depth_scale_abs_log_error": abs(float(base_residual.mean())),
        "global_log_scale_correction": float(
            (predicted_log - base_log).mean()
        ),
    }
...
def evaluate(
    ...
    require(bool(samples), "evaluation roster empty")
    model.eval()
    rows = []
    with torch.no_grad():
    ...
    parent_macro = {
        name: float(np.mean([metrics[name] for metrics in parent_metrics.values()]))
        for name in metric_names
    }
    return {
...
def save_checkpoint(
    ...
    torch.save(
        {
            "schema": "blindassist_ag_r2_metric_depth_student_checkpoint_v1",
            "step": step,
            "seed": TRAINING_SEED,
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(corpus["passed"] and corpus["frame_count"] == 156, "distillation corpus invalid")
    unsupported = [row for row in corpus["frames"] if int(row["metric_depth_valid_pixels"]) == 0]
    rows = sorted(
    ...
    require(
        ...
        "metric student role split drift",
    )
    role_parents = {
        role: {sample.parent_id for sample in values} for role, values in by_role.items()
    }
    require(
        role_parents["FIT"].isdisjoint(role_parents["CHECKPOINT_SELECTION"])
        and role_parents["FIT"].isdisjoint(role_parents["TRAIN_CANARY"])
        and role_parents["CHECKPOINT_SELECTION"].isdisjoint(role_parents["TRAIN_CANARY"]),
        "metric student parent overlap",
    )
    ...
    model = MetricDepthStudentHead(args.hidden_channels, args.global_hidden_channels).to(device)
    ...
    def capture(step: int) -> None:
        ...
        checkpoint = save_checkpoint(args.output_dir / f"metric-depth-step-{step}.pt", model, step)
        candidates.append(
            {
                "step": step,
            ...
            json.dumps(
                {
                    ...
                    "selection_depth_scale_abs_log_error": selection["parent_macro_metrics"]["depth_scale_abs_log_error"],
                }
            ),
            flush=True,
        )
    ...
    for step in range(1, args.optimizer_steps + 1):
        ...
        losses = metric_depth_loss(outputs, target)
        require(bool(torch.isfinite(losses["total"])), "metric depth objective non-finite")
    ...
    final_checkpoint = save_checkpoint(
        args.output_dir / "metric-depth-student.pt", model, selected["step"]
    )
    ...
    gates = {
        "METRIC_DEPTH_C01_EXACT_CORPUS_AND_DEPTHART_RECEIPTS": True,
        "METRIC_DEPTH_C02_PARENT_DISJOINT_107_24_24": True,
        "METRIC_DEPTH_C03_SELECTION_ONLY_CHECKPOINT_CHOICE": True,
        "METRIC_DEPTH_C04_TRAIN_CANARY_OPENED_ONCE_AFTER_SELECTION": True,
        "METRIC_DEPTH_C05_FINITE_FACTOR_METRICS_AND_CHECKPOINT": finite_metrics,
        "METRIC_DEPTH_C06_FACTOR_ONLY_NO_TASK_OR_REDUCER_OUTPUT": True,
    }
    passed = all(gates.values())
    result = {
        "schema": "blindassist_ag_r2_metric_depth_student_result_v1",
        "status": "AG_R2_METRIC_DEPTH_STUDENT_TRAINING_PASS_READY_FOR_CONSUMED_SEAM"
        if passed
        else "AG_R2_METRIC_DEPTH_STUDENT_TRAINING_FAIL",
        "passed": passed,
        ...
        "corpus": {
            ...
            "unsupported_unknown_sample_ids": [row["sample_id"] for row in unsupported],
        },
        ...
        "architecture": {
            ...
            "local_log_correction_range": LOCAL_LOG_CORRECTION_RANGE,
            "global_log_correction_range": GLOBAL_LOG_CORRECTION_RANGE,
        ...
        "decision": {
            ...
            "quality_claim_deferred_to_ag_seam": True,
            "next_action": "Compose this factor-only metric depth output with the existing support/boundary factors and run the unchanged deterministic AG seam.",
        },
    }
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--optimizer-steps", type=int, default=3600)
