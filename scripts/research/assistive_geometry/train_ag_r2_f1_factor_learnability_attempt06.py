#!/usr/bin/env python3
"""Attempt 06: fresh selection, preserved canary, and three calibrated sigmas."""
...
from train_ag_r2_f1_attempt05_uncertainty_calibrators import calibration_features  # noqa: E402
from train_ag_r2_f1_attempt06_uncertainty_calibrators import (  # noqa: E402
    load_attempt06_calibrators,
    support_features,
)
from train_ag_r2_f1_factor_learnability import frame_metrics  # noqa: E402
...
from train_ag_r2_f1_factor_learnability_attempt04 import GEOMETRY_CONFIG, load_caches  # noqa: E402
...
DEFAULT_SELECTION_LABEL_RESULT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt06-fresh-selection-labels-r0/result.json"
DEFAULT_PRESERVED_LABEL_RESULT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt05-fresh-ag-held-labels-r0/result.json"
DEFAULT_CALIBRATION_RESULT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt06-uncertainty-calibration-r0/result.json"
DEFAULT_CALIBRATOR_CHECKPOINT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt06-uncertainty-calibration-r0/uncertainty_calibrators.pt"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-factor-learnability-attempt06-r0"
        ...
        for frame_index, prepared_row in enumerate(prepared):
            ...
            for key in members[0]:
                if key not in {"depth_log_sigma", "boundary_sigma_px"}:
                    require(torch.equal(result[key], members[0][key]), f"point factor changed: {key}")
            rows.append(result)
    return rows


def apply_support_calibration(
    prepared_row: dict[str, Any],
    members: list[dict[str, torch.Tensor]],
    raw: dict[str, torch.Tensor],
    baseline: dict[str, Any],
    parent_context: dict[str, Any],
    support_calibrator: torch.nn.Module,
    device: torch.device,
) -> tuple[dict[str, torch.Tensor], dict[str, Any]]:
    sample = prepared_row["sample"]
    factors, receipt = apply_geometry(raw, sample, GEOMETRY_CONFIG, baseline, device, parent_context)
    base_depth = F.interpolate(
        sample.base_depth_feature[None].to(device=device, dtype=torch.float32),
        sample.native_hw,
        mode="bilinear",
    ...
    features = support_features(members, base_depth, factors, receipt, sample.orientation)
    calibrated_sigma = support_calibrator(features).exp()
    factors["support_residual_sigma_m"] = calibrated_sigma
    receipt = {
        **receipt,
        "attempt04_support_sigma_m": float(receipt["support_sigma_m"]),
        "support_sigma_m": float(calibrated_sigma[0]),
        "support_sigma_calibrated": True,
    }
    return factors, receipt


def evaluate_attempt06(
    prepared: list[dict[str, Any]],
    ...
    with torch.no_grad():
        for index, prepared_row in enumerate(prepared):
            ...
            factors, receipt = apply_support_calibration(
                prepared_row,
                [cache[index] for cache in caches],
                outputs[index],
                baseline,
                context,
                support_calibrator,
            ...
            depth_residual = (factors["predicted_log_depth"] - target["depth"].clamp_min(0.01).log()).abs()
            depth_pairs.append((factors["depth_log_sigma"].exp()[depth_valid].cpu().numpy(), depth_residual[depth_valid].cpu().numpy()))
            evidence_valid = target["evidence_valid"]
            boundary_distance = (-3.0 * factors["boundary_probability"].clamp_min(1.0e-8).log()).clamp_max(32.0)
            boundary_residual = (boundary_distance - target["boundary_distance"]).abs()
            boundary_pairs.append((factors["boundary_sigma_px"][evidence_valid].cpu().numpy(), boundary_residual[evidence_valid].cpu().numpy()))
            if bool(target["plane_valid"]):
                support_residual = target["support_residual"][target["support_valid"]].abs()
                support_pairs.append((np.asarray([float(factors["support_residual_sigma_m"][0])]), np.asarray([float(support_residual.mean())])))
...
def serialize_predictions(
    ...
    with torch.no_grad():
        for index, prepared_row in enumerate(prepared):
            sample = prepared_row["sample"]
            factors, geometry_receipt = apply_support_calibration(
                prepared_row,
                [cache[index] for cache in caches],
                outputs[index],
                baseline,
            ...
            payload = {
                ...
                "factor_identity": np.asarray("AG_R2_F1_FACTORS_ATTEMPT06_THREE_UNCERTAINTY_CALIBRATORS"),
            ...
            receipts.append({"sample_id": sample.sample_id, "path": str(path.resolve()), "sha256": sha256_file(path), "bytes": path.stat().st_size, "camera_geometry_receipt_sha256": camera_receipt, "uncertainty_calibrator_sha256": calibrator_sha256, "geometry": geometry_receipt})
    return receipts
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(lock["status"] == "ATTEMPT06_UNCERTAINTY_RECALIBRATION_AND_FRESH_SELECTION_EXECUTION_AUTHORIZED", "Attempt-06 lock invalid")
    for name, path in (
        ("selection_label_result", args.selection_label_result),
        ("preserved_label_result", args.preserved_label_result),
        ("calibration_result", args.calibration_result),
        ("calibrator_checkpoint", args.calibrator_checkpoint),
    ):
        require(sha256_file(path) == lock["bindings"][name]["sha256"], f"{name} drift")
    ...
    selection_labels = json.loads(args.selection_label_result.read_text(encoding="utf-8"))
    preserved_labels = json.loads(args.preserved_label_result.read_text(encoding="utf-8"))
    calibration = json.loads(args.calibration_result.read_text(encoding="utf-8"))
    require(bool(selection_labels["passed"]) and bool(preserved_labels["passed"]) and bool(calibration["passed"]), "Attempt-06 prerequisite not passed")
    selection_rows = [{**row, "role": "CHECKPOINT_SELECTION"} for row in selection_labels["frames"]]
    canary_rows = [{**row, "role": "TRAIN_CANARY"} for row in preserved_labels["frames"] if row["role"] == "TRAIN_CANARY"]
    require(len(selection_rows) == len(canary_rows) == 6, "Attempt-06 role count drift")
    require(
        set(calibration["data_receipt"]["attempt06_selection_excluded"]["parents"])
        == {row["parent_id"] for row in selection_rows}
        and set(calibration["data_receipt"]["preserved_canary_excluded"]["parents"])
        == {row["parent_id"] for row in canary_rows},
        "Attempt-06 held firewall drift",
    )
    args.output_dir.mkdir(parents=True)
    device = torch.device(args.device)
    depth_calibrator, boundary_calibrator, support_calibrator, metadata = load_attempt06_calibrators(args.calibrator_checkpoint, device)
    require(metadata["data_receipt"]["attempt06_selection_excluded"]["sha256"] == lock["bindings"]["selection_label_result"]["sha256"], "checkpoint selection firewall drift")
    ...
    selection_samples, selection_feature_receipt = extract_features(sorted(selection_rows, key=lambda row: row["sample_id"]), args.depthart_source, args.depthart_checkpoint, args.depthart_extension, device)
    ...
    selection_outputs = calibrated_outputs(selection_caches, selection_prepared, depth_calibrator, boundary_calibrator, device)
    selection_evaluation = evaluate_attempt06(selection_prepared, selection_outputs, selection_caches, baseline, support_calibrator, device)
    selection_gate = gate(selection_evaluation, selection_baseline, 97)
    selection_passed = bool(selection_gate["all_primary_metrics_passed"] and selection_gate["all_uncertainty_families_passed"])
    ...
    canary_caches: list[list[dict[str, torch.Tensor]]] = []
    if selection_passed:
        canary_samples, canary_feature_receipt = extract_features(sorted(canary_rows, key=lambda row: row["sample_id"]), args.depthart_source, args.depthart_checkpoint, args.depthart_extension, device)
        ...
        canary_outputs = calibrated_outputs(canary_caches, canary_prepared, depth_calibrator, boundary_calibrator, device)
        canary_evaluation = evaluate_attempt06(canary_prepared, canary_outputs, canary_caches, baseline, support_calibrator, device)
        canary_gate_result = gate(canary_evaluation, canary_baseline, 97)
    ...
    predictions = serialize_predictions(canary_prepared, canary_outputs, canary_caches, baseline, support_calibrator, args.output_dir, calibrator_sha, device) if passed else []
    result = {
        "schema": "blindassist_assistive_geometry_r2_f1_factor_learnability_attempt06_result_v1",
        "status": "R2_F1_FACTOR_LEARNABILITY_ATTEMPT06_PASS_FACTOR_TENSORS_SERIALIZED" if passed else "R2_F1_FACTOR_LEARNABILITY_ATTEMPT06_FAIL_STOP",
        "passed": passed,
        "execution_lock": str(args.lock.resolve()),
        "execution_lock_sha256": sha256_file(args.lock),
        "geometry_config": GEOMETRY_CONFIG,
        "calibration_result": {"path": str(args.calibration_result.resolve()), "sha256": lock["bindings"]["calibration_result"]["sha256"]},
        "calibrator_checkpoint": {"path": str(args.calibrator_checkpoint.resolve()), "sha256": calibrator_sha},
        ...
        "decision": {"point_factor_parameters_changed": False, "optimizer_steps_in_formal_execution": 0, "reducer_or_task_outcome_read": False, "next_action_if_pass": "Run FactorTensorAdapter on the six serialized real factor tensors, then execute the deterministic reducer seam canary."},
    }
...
def main() -> int:
    ...
    parser.add_argument("--selection-label-result", type=Path, default=DEFAULT_SELECTION_LABEL_RESULT)
    parser.add_argument("--preserved-label-result", type=Path, default=DEFAULT_PRESERVED_LABEL_RESULT)
    ...
    result = run(args)
    print(json.dumps({"status": result["status"], "passed": result["passed"], "selection_passed_before_canary_open": result["selection_passed_before_canary_open"], "selection_gate": result["selection_gate"], "canary_gate": result["canary_gate"], "prediction_count": len(result["prediction_receipts"])}, indent=2, sort_keys=True))
