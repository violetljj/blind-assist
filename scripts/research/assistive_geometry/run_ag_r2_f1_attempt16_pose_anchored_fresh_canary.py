#!/usr/bin/env python3
"""Execute the one-shot four-parent fresh factor canary and serialize on pass."""
...
DEFAULT_LOCK = REPO_ROOT / "docs/research/assistive-geometry/BLINDASSIST_ASSISTIVE_GEOMETRY_R2_F1_ATTEMPT16_POSE_ANCHORED_FRESH_CANARY_EXECUTION_LOCK_2026-08-12.json"
DEFAULT_FRESH_LABEL_RESULT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt12-pose-anchored-fresh-canary-labels-r1/result.json"
DEFAULT_CALIBRATION_RESULT = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt15-geometry-uncertainty-calibration-r1/result.json"
DEFAULT_OUTPUT_DIR = REPO_ROOT / "artifacts.local/experiments/ag-r2-f1-attempt16-pose-anchored-fresh-canary-r0"
CORE_POINT_METRICS = (
...
def load_model(
    ...
    require(sha256_file(checkpoint) == expected_sha, "complete factor checkpoint drift")
...
def calibrated_outputs(
    ...
    with torch.no_grad():
        for sample in samples:
            ...
            outputs["depth_log_sigma"] = outputs["depth_log_sigma"] + math.log(
                float(calibration["depth"]["multiplier"])
            )
            support_proxy = str(calibration["support"]["proxy"])
            outputs["support_residual_sigma_m"] = torch.tensor(
                [proxies[support_proxy] * float(calibration["support"]["multiplier"])],
                device=device,
            )
            outputs["boundary_sigma_px"] = outputs["boundary_sigma_px"] * float(
                calibration["boundary"]["multiplier"]
            )
            with np.load(sample.label_path, allow_pickle=False) as payload:
                gravity = torch.from_numpy(
                    np.asarray(payload["gravity_up_camera_xyz"], dtype=np.float32)
                ).to(device)
            ...
            receipts.append(
                {
                    ...
                    "support_sigma_proxy_value": proxies[support_proxy],
                    "support_sigma_m": float(outputs["support_residual_sigma_m"][0]),
...
def point_metric_gates(
    ...
    for metric in CORE_POINT_METRICS:
        ...
        overall = float(baseline["overall_metrics"][metric]) - float(
            evaluation["overall_metrics"][metric]
        )
...
def serialize_factors(
    ...
    for sample, factors in zip(samples, outputs):
        ...
        payload = {
            ...
            "factor_identity": np.asarray("AG_R2_F1_ATTEMPT16_COMPLETE_SUPERTEACHER_FACTORS"),
            "model_checkpoint_sha256": np.asarray(identity["checkpoint_sha256"]),
            "uncertainty_calibration_sha256": np.asarray(identity["calibration_sha256"]),
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(lock["status"] == "ATTEMPT16_POSE_ANCHORED_FRESH_CANARY_EXECUTION_AUTHORIZED_ONCE", "Attempt16 lock invalid")
    require(sha256_file(Path(__file__)) == lock["bindings"]["implementation"]["sha256"], "Attempt16 implementation drift")
    ...
    require(fresh["passed"] and calibration_result["passed"], "Attempt16 prerequisite failed")
    ...
    checkpoint = Path(calibration_result["checkpoint"]["path"])
    checkpoint_sha = str(calibration_result["checkpoint"]["sha256"])
    model = load_model(checkpoint, checkpoint_sha, baseline_parameters, device)
    outputs, geometry_receipts = calibrated_outputs(
        samples, model, calibration_result["calibration"], device
    )
    ...
    height_max_parent = max(
        float(row["camera_height_abs_log_error"])
        for row in evaluation["parent_metrics"].values()
    )
    normal_max_parent = max(
        float(row["support_plane_angular_error_rad"])
        for row in evaluation["parent_metrics"].values()
    )
    ...
    gates = {
        "A16_C01_ALL_CORE_POINT_FACTORS_NONTRIVIAL": all(row["passed"] for row in point_gates.values()),
        "A16_C02_HEIGHT_BOUNDED": height_overall <= 0.25 and height_max_parent <= 0.50,
        "A16_C03_RUNTIME_GRAVITY_NORMAL_EXACT": normal_max_parent <= 0.01 and all(row["normal_gravity_dot"] >= 0.999 for row in validity),
        "A16_C04_ALL_UNCERTAINTY_FAMILIES_PROPER_AND_ORDERED": uncertainty_pass,
        "A16_C05_VALIDITY_NOT_COLLAPSED": all(row["metric_scale_valid"] and row["support_valid"] and row["evidence_valid_pixels"] > 0 for row in validity),
        "A16_C06_EVIDENCE_VALIDITY_NO_MATERIAL_REGRET": evidence_regret <= 0.02,
        "A16_C07_ALL_PARENT_METRICS_FINITE": finite_metrics,
        "A16_C08_TASK_AND_REDUCER_FIREWALL": True,
    }
    passed = all(gates.values())
    identity = {
        "checkpoint_sha256": checkpoint_sha,
        "calibration_sha256": lock["bindings"]["calibration_result"]["sha256"],
    }
    factors = serialize_factors(samples, outputs, args.output_dir, identity) if passed else []
    result = {
        "schema": "blindassist_ag_r2_f1_attempt16_pose_anchored_fresh_canary_result_v1",
        "status": "ATTEMPT16_FRESH_CANARY_PASS_FACTORS_SERIALIZED_ADAPTER_REQUIRED" if passed else "ATTEMPT16_FRESH_CANARY_FAIL_NO_PROMOTION",
        "passed": passed,
        ...
        "decision": {
            ...
            "next_action_if_pass": "Run FactorTensorAdapter and the deterministic reducer seam on all twelve serialized factors.",
        },
    }
...
def main() -> int:
    ...
    result = run(args)
    print(json.dumps({"status": result["status"], "passed": result["passed"], "gates": result["gates"], "factor_tensor_count": len(result["factor_tensors"])}, indent=2, sort_keys=True))
