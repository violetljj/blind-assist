# data_contamination_audit_r0

状态：$status

## 研究问题与版本

数据角色、来源冲突和 ledger 一致性审计。协议版本 R0；遵循 docs/RESEARCH_GOVERNANCE.md。本 Module 只产生声明范围内的诊断或开发证据。
...
只写入 artifacts.local/evidence/data_contamination_audit_r0/ 或调用方显式指定的 artifacts.local/ 子目录；不写仓库根目录和正式 App 资产。
