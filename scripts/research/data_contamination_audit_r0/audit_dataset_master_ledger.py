...
Default invocation (from the repository root):
    ...
    E:\\codex-tools\\bin\\blindassist-python.cmd \\
        scripts\\audit_dataset_master_ledger.py

The generated files are written at the repository root unless --output-dir is
provided.  The default scan root is artifacts.local plus the legacy outer
E:\\linnan\\artifacts.local when it exists.  Junction aliases inside the
checkout are not followed, so .downloads and test-artifacts.local are not
double-counted.
"""
