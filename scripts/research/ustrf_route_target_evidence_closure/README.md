...
状态：R1 `DATA_BLOCKED / STOP_SOURCE_SEARCH` / THÖR source-native recovery R1 `INDEPENDENT_PERSON_TRAJECTORY_TRUTH_AUTHORITY_ABSENT / VALID` / independent person trajectory truth R0 `INDEPENDENT_PERSON_TRAJECTORY_TRUTH_AUTHORITY_ABSENT / VALID` / elastic evidence R1 active / JRDB far-range denominator replication R0 `FAR_RANGE_SUPPORT_DECLINE_REPLICATED / VALID` / JRDB cross-sequence support/bias R0 `CROSS_SEQUENCE_PROFILE_AVAILABLE_WITH_PARTIAL_REPLICATION / VALID` / JRDB sensor support/bias R0 `SENSOR_SUPPORT_AND_BIAS_PROFILE_AVAILABLE_WITH_ABSTENTION / VALID` / JRDB native multisensor P1B `NATIVE_POSE_IMU_TIME_AUTHORITY_PRESENT / VALID` / P2 R0 history `FAIL_CLOSED_LABEL_JOIN / VALID` / P2 R1 `ANNOTATION_DERIVED_PERSON_GEOMETRY_AVAILABLE_WITH_ABSTENTION / VALID` / candidate replay R2 `CANDIDATE_REPLAY_COMPLETE / VALID` / R2-L1 metric profiles `METRIC_PROFILES_COMPLETE / VALID` / route-invalid + reset-scoped lifecycle `MECHANISM_DIAGNOSTIC_COMPLETE / VALID / overall gate false` / eligible-attribution ordered isolated opening `ORACLE_MECHANISM_REPAIR_DIAGNOSTIC_COMPLETE / VALID` / truth-blind causal per-track token R0 `HOLD_FOR_POLICY_GATE / VALID` / causal token policy/risk R1 `POLICY_COVERAGE_REJECT / VALID` / policy failure attribution R1 `POLICY_FAILURE_ATTRIBUTION_CLOSED / VALID` / current-input policy feasibility bound R0 `CURRENT_INPUT_POLICY_FAMILY_NOT_FEASIBLE / VALID` / causal route-intrusion signal R0 `SIGNAL_REJECT / VALID` / route-conditioned scale-growth R0 `FAIL_CLOSED_INPUT_OR_CONTRACT_BLOCKED / VALID` / canonical observation authority R0 `SOURCE_AUTHORITY_ABSENT / VALID` / L2+L3 prereg frozen but not authorized

## THÖR source-native ID / time / transform authority recovery R1

只读审计官方 THÖR records、论文、Qualisys 格式文档和唯一 moving-robot paired-bag canary。6DOF TSV 的毫米单位与 Helmet marker-centre reference point 已恢复；raw QTM/recovery mask、冻结 run 2 paired clock measurement、Velodyne marker→LiDAR measurement-frame extrinsic/error 仍缺，终态 `INDEPENDENT_PERSON_TRAJECTORY_TRUTH_AUTHORITY_ABSENT / VALID`。未读取候选输出，不比较 centroid/tracker/deskew。

```powershell
E:\codex-tools\projects\blindassist\toolchain\python311\python.exe scripts/research/ustrf_route_target_evidence_closure/acquire_thor_source_native_id_time_transform_authority_recovery_r1.py --repo . --config configs/ustrf_thor_source_native_id_time_transform_authority_recovery_r1.json
E:\codex-tools\projects\blindassist\toolchain\python311\python.exe scripts/research/ustrf_route_target_evidence_closure/audit_thor_source_native_id_time_transform_authority_recovery_r1.py --repo . --config configs/ustrf_thor_source_native_id_time_transform_authority_recovery_r1.json
E:\codex-tools\projects\blindassist\toolchain\python311\python.exe scripts/research/ustrf_route_target_evidence_closure/test_thor_source_native_id_time_transform_authority_recovery_r1.py -v
E:\codex-tools\projects\blindassist\toolchain\python311\python.exe scripts/research/ustrf_route_target_evidence_closure/validate_thor_source_native_id_time_transform_authority_recovery_r1.py --repo . --config configs/ustrf_thor_source_native_id_time_transform_authority_recovery_r1.json
```

## Independent person trajectory truth source authority and admission R0

JRDB annotation-derived geometry 因 box/ROI/center 同源循环论证直接拒绝。REveL 与 THÖR 只作为独立 mocap source candidates；冻结 THÖR `Exp_2_run_2_6D.tsv` 整文件、9 条 person rigid body、`Citi_1` reference、五个距离带与 missing 守恒后，`25,912` frame / `233,208` opportunity 完整复算，但 stable-ID postprocessing provenance、payload unit、测得同步误差、sensor measurement-frame 外参与不确定度均未闭合。终态 `INDEPENDENT_PERSON_TRAJECTORY_TRUTH_AUTHORITY_ABSENT / VALID`，不读取候选输出、不比较算法。

```powershell
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/acquire_independent_person_trajectory_truth_source_authority_and_admission_r0.py --repo . --config configs/ustrf_independent_person_trajectory_truth_source_authority_and_admission_r0.json
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/audit_independent_person_trajectory_truth_source_authority_and_admission_r0.py --repo . --config configs/ustrf_independent_person_trajectory_truth_source_authority_and_admission_r0.json
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/test_independent_person_trajectory_truth_source_authority_and_admission_r0.py -v
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/validate_independent_person_trajectory_truth_source_authority_and_admission_r0.py --repo . --config configs/ustrf_independent_person_trajectory_truth_source_authority_and_admission_r0.json
```

## JRDB person 3D trajectory far-range denominator adequacy metadata-blind replication R0

先以 timestamp/member/bag metadata 冻结 8 条未见 sequence × 360 帧，再以 label-only `>=100` 个 `40m+` object-frame、至少 3 条 sequence 的门决定是否读取 PCD。4 条通过；不变 support kernel 的远距支持率下降在 `4/4` 条同向复现，3D-only support 也在 `4/4` 更低，但 residual median 仅 `3/4` 更差。独立 validator `22/22 VALID`；详见[日期化结果](../../../docs/research/ustrf-sc/USTRF_JRDB_PERSON_3D_TRAJECTORY_FAR_RANGE_DENOMINATOR_ADEQUACY_METADATA_BLIND_REPLICATION_R0_RESULT_2026-07-25.md)。
...
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/freeze_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.py --repo . --config configs/ustrf_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.json
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/audit_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.py --repo . --config configs/ustrf_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.json
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/run_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.py --repo . --config configs/ustrf_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.json --phase materialize --sequence <adequate-sequence>
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/run_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.py --repo . --config configs/ustrf_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.json --phase aggregate
.\.python311\python.exe scripts/research/ustrf_route_target_evidence_closure/validate_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.py --repo . --config configs/ustrf_jrdb_person_3d_trajectory_far_range_denominator_adequacy_metadata_blind_replication_r0.json
```
