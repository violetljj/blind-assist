...
- [`research/ustrf_route_target_evidence_closure/`](research/ustrf_route_target_evidence_closure/)：route-target 候选盲真值、指标资格、receipt-aware replay、逐指标 L1 profile、L2 fresh-selection 预注册和 L3 non-executable lockbox 合同；canonical input 和 C1–C3 `123/123` trace 已闭合，R2-L1 profile validator 又重算 `186,687` candidate-frame 为 `VALID`。当前三个 profile 都有 unknown/stale active-alert 硬 veto 且 clearance 点估计失败；没有比较、selection、shadow/H2 或生产权限。
