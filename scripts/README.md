...
- `validate_android_research_preview_admission.py`：Android research-preview v1 收据的 fail-closed 合同验证器；重算 identity/backend/fallback/parity、parent/session denominator、冻结阈值和 `FAIL > UNKNOWN > PASS` 终态，配套测试拒绝 misleading PASS。
