...
- [`research/dual_loop_segmentation_conditional_gating/`](research/dual_loop_segmentation_conditional_gating/)：Atlas `GATING_PARTIAL` 的独立 Development successor；R0 primary 一次执行与 85,235 项检查已 `VALID`，但 FP reduction `0.092572`、最低 session recall retention `0.774580` 未过门。历史 machine terminal 不变，scope 纠正为 primary-only；前向 R0.1 冻结两个 diagnostic-only shadows，等待一次全量 closeout，不救援 primary、不选优。held-out 只作 burned Development stress，不接 Android、risk、alerts 或默认 App。
