...
- [`research/dual_loop_segmentation_conditional_gating/`](research/dual_loop_segmentation_conditional_gating/)：Atlas `GATING_PARTIAL` 的独立 Development successor；在结果前冻结一个类别条件多重负证据门、纯几何 upper membership、同类别 raw-history causal 2-of-3、520-frame/10-source-session membership、五项停止门及独立聚合 validator。一次执行与 85,235 项独立检查已 `VALID`，但 FP reduction `0.092572`、最低 session recall retention `0.774580` 未过门，终态 `CONDITIONAL_GATING_NO_ROBUST_INCREMENT_STOP_GATING_ROUTE`；held-out 只作 burned Development stress，不接 Android、risk、alerts 或默认 App。
