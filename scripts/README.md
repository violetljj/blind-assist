...
- [`research/dual_loop_semantic_refresh_q0/`](research/dual_loop_semantic_refresh_q0/)：固定模型全频参考下的事件保持型语义刷新调度 Q0 离线 R0；每个 arm 隔离 cache/feedback/event state，仅支持 Development-only 反事实筛查，不接 Android、能效、产品或安全路径。
