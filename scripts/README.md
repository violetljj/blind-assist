...
- `research/grail/freeze_grail_pairwise_owner_coordinate_r1cl.py` / `collect_grail_pairwise_owner_coordinate_r1cl.py` / `train_grail_pairwise_owner_coordinate_r1cl.py`：R1C-L house-disjoint RGB/mask pair collection 与唯一 36-bin task-trained model；validation `+8` 门前禁止 final test。
...
- `research/grail/run_grail_relational_observability_r1.py --dataset ... --collection ... --features ... --checkpoint ... --development-result ... --r0-result ... --output ...`：在同一 consumed 78-case 上对 R0 relation field 做 staircase、leave-one-out 与最小组合诊断，并报告 referent/complete oracle uplift recovery；不训练 student，也不建立 RGB/text obtainability、formal test 或 M2 authority。
- `research/grail/run_grail_grouping_r1a.py --dataset ... --collection ... --features ... --checkpoint ... --development-result ... --r0-result ... --output ...`：training-free R1A，用现有 RGB、oracle bbox proposal、semantic type 与 frozen DINO context 检验 same-root grouping、deterministic sibling ordinal 和端到端 oracle uplift recovery；输入没有持久化 pixel mask，结果不得写成 mask-based grouping 或自然场景能力。
- `research/grail/materialize_grail_reference_r1b.py ...` / `run_grail_bilateral_grouping_r1b.py ...`：R1B reference full-scene RGB/proposal/mask replay 与双侧 grouping probe；冻结 R1A affinity 和下游，只审计 target ownership、view-local ordinal、跨视角 agreement 与端到端 rescue。结果证明 ownership 高但 image-space ordinal 不可直接跨视角对齐，不得用该入口调 matcher。
- `research/grail/materialize_grail_canonical_coordinates_r1c.py ...` / `run_grail_canonical_coordinates_r1c.py ...`：R1C-O privileged ceiling；从 AI2-THOR native part position、owner 与 owner yaw 建立 camera-independent sibling slot，并冻结 R1 的最小 selector/downstream。只建立 synthetic owner-local coordinate 机制上界，不建立 RGB orientation；下一步仅允许另立 R1C-V 协议。
- `research/grail/materialize_grail_visual_orientation_oracle_r1cv.py ...` / `run_grail_visual_orientation_r1cv.py ...`：冻结的 R1C-V deterministic RGB/proposal axis×sign probe；native supplement 仅供 evaluator 归因。当前 final slot=`39/78`、referent=`38/78`，axis 前门与 sign 均失败，当前 estimator 已关闭且无自动 successor。
