...
- [`research/dual_loop_segmentation_failure_atlas/`](research/dual_loop_segmentation_failure_atlas/)：当前科学主线的 200-frame consumed Development Atlas；输出组件级失败机制、非组合式空间/因果时序/置信 gating probe、三态 residual 可标注性与定向扩展判据，不训练模型、不访问 fresh holdout、不接可视化 sidecar、Android、risk 或默认 App。
