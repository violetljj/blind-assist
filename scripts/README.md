...
- `generate_ustrf_sc_u0_dense_teacher_loso_artifact.py`：第三臂 WIP 的 label-free、fold-local dense teacher 校准入口；只接受训练 fold 的许可视频与帧清单并输出 auxiliary-only artifact/receipt，不读取 human event truth、blind、future 或 held-out 输入，也不授权 Android/App/生产使用。当前 field/admission Interface 尚未闭合，不代表第三臂完成。
