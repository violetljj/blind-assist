...
- [`research/dual_loop_segmentation_conditional_gating/`](research/dual_loop_segmentation_conditional_gating/)：Atlas `GATING_PARTIAL` 的独立 Development successor；R0 primary 一次执行与 85,235 项检查已 `VALID`，但 FP reduction `0.092572`、最低 session recall retention `0.774580` 未过门。历史 machine terminal 不变，scope 纠正为 primary-only；前向 R0.1 两个 diagnostic-only shadows 又通过 167,327 项检查，但均无 material 或 session heterogeneity，故只关闭这个精确三臂静态手工门家族。下一主边界为 residual-aware DDRNet Development；held-out 只作 burned Development stress，不接 Android、risk、alerts 或默认 App。
- [`research/dual_loop_segmentation_residual_aware_ddrnet/`](research/dual_loop_segmentation_residual_aware_ddrnet/)：静态手工门家族关闭后的单一 FP-aware DDRNet Development successor；保持 backbone、四类真值、loss、70% hazard-guided crop、三 seed 和训练预算不变，只把 30% unguided full-frame 分支改为同 seed train-only baseline FP-pixel 加权的 full-frame draw。preflight 已通过，候选尚未训练；终态只用 consumed 320 帧，不接 INT8、runtime、Android、risk、alerts 或默认 App。
