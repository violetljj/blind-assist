...
- [`research/candidate_event_mining/`](research/candidate_event_mining/)：长视频/公开数据的模型无关候选事件发现 Module；输出 candidate pool/quarantine 和可复核的 hash-bound bundle，不接事件真值、训练、Android 或生产路径。
...
- [`research/failure_case_atlas/`](research/failure_case_atlas/)：Failure Atlas 的批量图册入口；`batch_album.py` 生成 12-panel failure figure、分类 contact sheet、`index.html`、`cases.jsonl` 和 provenance。类别规则维护在 [`category_rules.json`](research/failure_case_atlas/category_rules.json)，可通过 `--category-config` 注入自定义规则；缺失 risk/event/appearance 证据保持 `NOT_AVAILABLE` / `NOT_EVALUABLE`，不驱动提醒。
...
- [`research/information_ceiling_three_arm_d0/`](research/information_ceiling_three_arm_d0/)：同一 90-frame / 3-parent-event SANPO consumed Development cohort 上的 current YOLO、mask-derived truth boxes 与 source-mask current-adapter 三臂设备审计；独立 validator 复算逐帧输入和事件终态。结果只支持当前链的路线诊断，不把 mask adapter/source-policy 增益冒充纯 bbox 上限。
  - [`research/hftf/`](research/hftf/)：独立 HFTF 候选支线的 source/teacher feasibility Module；通用 H0 只准入静态 metric projection，source-specific verifier 进一步认证 official loader/GCS pose-frame binding、metric-depth transform 与 per-frame local-ground proxy，并由 cohort aggregator 要求至少三个独立 session 复现；[`atoms3r_m12_tof4m/`](research/hftf/atoms3r_m12_tof4m/) 提供 AtomS3R-M12 的可调 MJPEG + 中央单区 VL53L1X 距离网页、设备状态、浏览器 JPEG/JSON 抓拍，以及 JSONL 联调、采集和验证入口，明确不冒充 VL53L8CX 多区输入。当前只授权 H1 geometry teacher canary 和隔离的设备 Development 采集，不训练 student、不修改主线或默认 App。
