...
- [`research/hftf/`](research/hftf/)：独立 HFTF 候选支线的 source/teacher feasibility Module；通用 H0 只可准入静态 metric projection，并对多高度身体包络/future teacher 的结构合同与 student-effect 缺口做 fail-closed 裁决；后续层需 source-specific verifier 或独立 parent-event ledger，不训练模型、不修改主线或默认 App。
