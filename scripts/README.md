...
- [`research/dual_loop_segmentation_conditional_gating/`](research/dual_loop_segmentation_conditional_gating/)：Atlas `GATING_PARTIAL` 的独立 Development successor；在任何结果前冻结一个类别条件多重负证据门、纯几何 upper membership、同类别 raw-history causal 2-of-3、520-frame/10-source-session membership、五项停止门及独立聚合 validator。当前 `FROZEN_NOT_RUN`，不做候选/阈值选择，held-out 只作 burned Development stress，不接 Android、risk、alerts 或默认 App。
