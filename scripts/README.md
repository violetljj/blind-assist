...
- `run_guarded_host_research.ps1`：超过 3 分钟或正式 one-shot 的统一电脑端入口；先校验 hash 绑定性能收据和当前 RAM/VRAM，再启动 runner、注入已标定 worker、附加监控并核对 progress/success/failure 终态。
...
- [`research/egomotion_compensated_looming/`](research/egomotion_compensated_looming/)：RCLE canonical Module。旧 Looming/Phase A/B1 R5 证据保持不可回写；CID-SIMS `floor3_1` 同序列 holdout 为 `7 positive / 0 below-reference`，不同 official run 的 `floor3_2` development holdout 为 `17 positive / 0 below-reference / 1 ambiguous`，均未形成冻结 2+2，RGB 未运行。`floor3_2` formal `INVALID` 与 bounded post-hoc `VALID` 同时留痕；无自动 `floor3_3`、confirmation、Replay 或 Android 权限。
