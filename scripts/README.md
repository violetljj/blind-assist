...
- `evaluate_ustrf_sc_u0_teacher_upper_bound.py`：USTRF U0 六臂（四个正式比较臂 + uniform/shuffled route 负控）LOSO 事件评价；会重算完整 route-conditioned 人类真值门并拒绝 blind/future/hash 漂移。
...
- `validate_ustrf_sc_capture_frame_ledger.py`、`validate_ustrf_sc_independent_human_review.py`：正式 full-matrix truth 与 pilot 共用的原子证据验证 Implementation；由上述稳定入口调用。
...
- `run_research_tool.py <domain> <tool.py> [args...]`：统一研究 Adapter；当前支持 `public-video` 历史归档和 `ustrf-crosscam-codex` 代理评测域。
...
- [`research/ustrf_crosscam_codex/`](research/ustrf_crosscam_codex/)：公开头戴视角视频上的 Codex provisional silver / causal comparator，以及显式 route-projection receipt、polygon bottom-center 三档不确定性审计；不产生人工真值、设备米制几何或 U0/生产授权。
