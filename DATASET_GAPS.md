...
本次扫描发现 `7411` 条 session/package 记录，其中 `3275` 条有可定位媒体，`4136` 条为 manifest/structured-only；物理文件 `242360` 个，累计字节 `153,920,526,814`。`consumed/burned` 只表示历史使用证据，不等于全局封存；`fresh/reserved` 只有本地路径或 metadata 明确出现时才标记，未知不被解释为不存在。
...
- 文件发现快照：`2026-08-05T22:25:16+08:00`；profile 修复时间见 ledger 的 `profile_refreshed_at`；发现后消失的路径保留为 `not_readable`。
...
| `replay` | 985 | 278 | 707 | FOUND |
| `canonical_input` | 2 | 1 | 1 | FOUND |
| `segmentation_520` | 54 | 52 | 2 | FOUND |
...
- 文件级 profile 状态：`readable=241081`，`readable_probe=128`，`corrupt_or_unreadable=284`，`not_readable=0`，`not_evaluable=269`，`not_checked=556`。`not_readable` 表示发现后路径消失/权限失败，不等于内容损坏。
- 文件名/frame-key 级缺口：识别到 missing frame keys `305885`，duplicate frame keys `10944`；无法建立 frame key 的记录保持 `not_evaluable`。
...
- 没有可解析 timestamp：`5619` 条；没有可解析 fps：`5865` 条。很多 image sequence 只有 frame index，不能从文件名安全推导真实时间。
- 发现内容重复组：`26407`；其中涉及排他角色冲突：`41`。重复内容不自动合并，因为不同 evidence role 仍需保留。
