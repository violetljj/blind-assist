...
随后[R1规则读出](nearfield/CNH_R1_READOUT_RESULTS_20260925.md)主集已完成：train选定K4补偿dev AP0.3183，对应TP46/FP62/FN362/TN2410，未过0.42；同K不补偿0.2636，K8补偿0.3455仅描述。无噪声串扰扣除0.8654通过，ambient×2/signal×0.5均胜同条件R0；整体门槛仍失败。完整1920帧Street仅描述：R0 AP0.8211、同一K4为0.8828，显示明显场景依赖。新seed族signal×2达到0.5799，不能把原条件失败当信息上限。R2使用仿真内部128bin、轨迹只有名义时间，未建立现有H3硬件可行性或更早提醒；不启动学习器、新采集、City/test，优先明确未标定计数尺度。
用户随后授权的[Track A v1.1](nearfield/CNH_NEARFIELD_ACCEPTANCE_PLAN_V1_1_20260925.md)已按先冻结后生成执行；[12单位回执](nearfield/CNH_TRACK_A_PILOT12_RESULTS_20260925.md)为2880几何帧、1625主帧。视场规则选M1=-10°；G0通过，但精确1cm指纹有3对重复，G1失败；12单位G2均未全过（11单位多对象帧不足、9单位合格组合不足、1单位phi超限）。因此停止传感合成、G5和B0/B1评估，均NOT_RUN；无放量。H3-only B1-R及几何17项合成单测通过，但不等于算法收益。下一版需重新规划整段主子集的组合/多对象停留配额与指纹去重，保留本版失败，不换seed补样；City/test/UE与硬件仍暂停。
