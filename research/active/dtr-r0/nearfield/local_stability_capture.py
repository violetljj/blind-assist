"""Unreal-only576-frame local-stability capture (48 clips, 12 posed frames each).
...
OUT = Path(os.environ['BA_LOCAL_STABILITY_OUTPUT'])
SPEC = Path(os.environ['BA_LOCAL_STABILITY_SPEC'])
SCRIPT = Path(os.environ['BA_LOCAL_STABILITY_SCRIPT'])
PROTOCOL_SHA = os.environ['BA_LOCAL_STABILITY_PROTOCOL_SHA']
SPEC_SHA = os.environ['BA_LOCAL_STABILITY_SPEC_SHA']
SCRIPT_SHA = os.environ['BA_LOCAL_STABILITY_SCRIPT_SHA']
...
def component(source, fmt):
    ...
    actor.set_actor_label('BA local stability capture')
...
def prepare(case):
    if not frames or frames[-1]['clip_id'] != case['clip_id']:
        ...
        for obj in case['objects']:
            ...
            actor.set_actor_label('BA local stability '+case['clip_id']+' '+obj['name'])
...
def tick(delta):
    ...
    try:
        if stage==0:
            ...
            assert spec['schema'] == 'local-stability-source-v1'
        ...
        if stage==4:
            ...
            if index==len(cases):
                ...
                write(OUT/'observations/manifest.json',dict(schema='ba-local-stability-rgb-v1',
