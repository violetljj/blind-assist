...
def run(cache,out):
    ...
    assert sha(Path(__file__).with_name('body_query_expanded_train.py')) == protocol['source_sha256']
    assert sha(Path(__file__).with_name('BODY_QUERY_EXPANDED_B_PROTOCOL_20260909.md')) == protocol['brief_sha256']
