"""Adapt accepted 10000-frame expanded collection labels to the RGB-only QueryRGB cache.
...
EXPECTED = {'train': 5000, 'dev': 2000, 'eval': 3000}
...
def build(dataset, output):
    ...
    require(len({r['frame_id'] for r in frames}) == 10000, 'Frame IDs must be unique')
    ...
    try:
        ...
        manifest.update(status='PASS', backend='CPU', backend_reason='TASK_NOT_GPU_SUITABLE',
                        ...
                        integrity_checks=dict(rgb_hashes=10000, label_hashes=10000, label_semantics=10000,
