"""NF-G9 grouped-intervention acquisition; settled poses with evaluator-only native depth."""
import array
import hashlib
import json
import math
...
def tick(delta):
    ...
    try:
        ...
        if stage == 2:
            ...
            settling = (32 if index == 0 else cases[index].get('settling_frames',8))
            if os.environ.get('BA_UE_CADENCE') == 'burst':
            ...
            if index == len(cases):
                write(OUT / 'model/dataset.json', {
                    'schema': 'nf-g9-grounding-dataset-v1',
