"""One user-authorized BODY-only witness-objective fit; unchanged MZ16 HIGH_DETAIL inference."""
import argparse
...
def main(root,cache,output):
    ...
    write(output/'start.json',dict(status='STARTED',steps_per_arm=1200,inputs=inputs,
        ...
        code_sha256={p.name:sha(p) for p in [Path(__file__),Path(__file__).with_name('mz16_detail_readout.py'),Path(__file__).with_name('mz16_local_metrics.py'),Path(__file__).with_name('mz18_body_loss.py'),Path(__file__).with_name('mz15_train.py'),Path(__file__).with_name('mz15_shared_support.py'),Path(__file__).with_name('mz15_evaluator.py')]},
        protocol_sha256=sha(Path(__file__).with_name('MZ18_BODY_OBJECTIVE_PROTOCOL_20260910.md'))))
    result=dict(metrics={},local={},fits={},cutoffs={},groups={});arrays={};local_arrays={}
    for arm in ['BODY_WITNESS']:
        ...
        for step,ids in enumerate(batches):
            ...
            qloss=body_witness_query_loss(out['field'],q,k,out['eligible'],gt,out['logits'],out['support'])
    ...
    result['witness_local_ap_gain']=all(result['local']['BODY_WITNESS'][n]['macro_ap']>reference['local']['HIGH_DETAIL'][n]['macro_ap'] for n in ['relation10000','distance5000'])
