...
reuse_actor_slots = spec.get('reuse_actor_slots', False)
if type(reuse_actor_slots) is not bool:
    raise ValueError('reuse_actor_slots must be boolean')
if spec.get('profile_frames') is not True:
    raise ValueError('MZ65 requires identical phase profiling in all arms')
slot_pool = {}
slot_created = slot_reused = 0
frame_timing = {}
...
def slot_key(obj):
    if obj.get('placement', 'actor_origin') != 'actor_origin':
        raise ValueError('MZ65 pooling supports actor_origin only')
    return json.dumps({k:v for k,v in obj.items() if k not in
                      ('center_m', 'scale', 'size_m', 'rotation_deg')}, sort_keys=True)


def actor_flags(actor):
    mesh = actor.static_mesh_component
    return dict(hidden=bool(actor.get_editor_property('hidden')),
                editor_hidden=bool(actor.is_temporarily_hidden_in_editor()),
                visible=bool(mesh.get_editor_property('visible')),
                component_hidden=bool(mesh.get_editor_property('hidden_in_game')),
                shadow=bool(mesh.get_editor_property('cast_shadow')),
                collision=bool(actor.get_actor_enable_collision()))


def set_actor_flags(actor, flags):
    mesh = actor.static_mesh_component
    actor.set_actor_hidden_in_game(flags['hidden'])
    actor.set_is_temporarily_hidden_in_editor(flags['editor_hidden'])
    mesh.set_visibility(flags['visible'], True)
    mesh.set_hidden_in_game(flags['component_hidden'], True)
    mesh.set_editor_property('cast_shadow', flags['shadow'])
    actor.set_actor_enable_collision(flags['collision'])
    if actor_flags(actor) != flags:
        raise RuntimeError('Pool visibility/collision flag restoration failed')


INACTIVE_FLAGS = dict(hidden=True, editor_hidden=True, visible=False,
                      component_hidden=True, shadow=False, collision=False)


def prepare_slots(case):
    keys = [slot_key(obj) for obj in case.get('objects', [])]
    if len(keys) != len(set(keys)):
        raise ValueError('Duplicate immutable slot in current case')
    active = set(keys)
    for key, row in slot_pool.items():
        if key not in active:
            set_actor_flags(row['actor'], INACTIVE_FLAGS)
        else:
            set_actor_flags(row['actor'], row['flags'])
    objects.clear()


def clear_objects():
    pending = [row['actor'] for row in slot_pool.values()] if reuse_actor_slots else list(objects)
    errors, failed = [], []
    for actor in pending:
        try:
            ...
            failed.append(actor)
    objects[:] = failed
    for key in list(slot_pool):
        if not any(slot_pool[key]['actor'] is actor for actor in failed):
            del slot_pool[key]
    if errors:
        raise RuntimeError('Pool/actor cleanup failed: ' + '; '.join(errors))
...
def prepare(case):
    global slot_created, slot_reused
    import math
    before_created, before_reused = slot_created, slot_reused
    ...
    for obj in case.get('objects', []):
        ...
        key = slot_key(obj) if reuse_actor_slots else None
        cached = slot_pool.get(key) if reuse_actor_slots else None
        mesh = cached['actor'].static_mesh_component.static_mesh if cached else u.load_asset(asset_path)
        ...
        if cached:
            ...
            if actor is None:
                raise RuntimeError('Could not create controlled actor')
            slot_created += 1
            if reuse_actor_slots:
                # Register immediately so a later material/transform error still cleans it.
                slot_pool[key] = dict(actor=actor, flags=actor_flags(actor))
        objects.append(actor)
        actor.set_actor_label('BA controlled ' + obj['name'])
        if not cached:
            actor.static_mesh_component.set_static_mesh(mesh)
        if obj.get('material_asset') and not cached:
        ...
        if placement != 'actor_origin':
            ...
            actor.set_actor_location(u.Vector(*(v*100 for v in origin)), False, False)
        if reuse_actor_slots and not cached:
            slot_pool[key]['flags'] = actor_flags(actor)
    ...
    if reuse_actor_slots:
        current = {slot_key(obj) for obj in case.get('objects', [])}
        for key, row in slot_pool.items():
            expected = row['flags'] if key in current else INACTIVE_FLAGS
            if actor_flags(row['actor']) != expected:
                raise RuntimeError('Unexpected active/inactive pool flags')
    report.setdefault('object_preparation', []).append(dict(
        case=case['name'], pool_enabled=reuse_actor_slots,
        created=slot_created-before_created, reused=slot_reused-before_reused,
        active=len(objects), inactive=len(slot_pool)-len(objects) if reuse_actor_slots else 0,
        pool_slots=len(slot_pool), flags_verified=True))
    pose = case['camera']
...
def finish(error=None):
    ...
    try:
        ...
        report['pair_drain_s'] = time.monotonic()-drain_started
    except Exception:
        report.update(status='FAIL', pair_cleanup_error=traceback.format_exc())
    try:
        clear_objects()
    except Exception:
        report.update(status='FAIL', object_cleanup_error=traceback.format_exc())
    for actor in captures:
        try:
            ...
            report.setdefault('capture_cleanup_errors', []).append(traceback.format_exc())
            report['status'] = 'FAIL'
    ...
    report['actor_pool'] = dict(enabled=reuse_actor_slots, created=slot_created, reused=slot_reused, slots_remaining=len(slot_pool), cleanup_complete=not slot_pool and not objects)
...
def tick(dt):
    global stage, index, warm, warm_target, progress_written, after, world, rgb, depth, beauty, background_settings, isolated_actor, frame_timing
    ...
    try:
        ...
        if stage == 2:
            if not readiness.poll():
                ...
                return
            frame_timing['readiness_s'] = time.monotonic()-frame_timing['readiness_started']
            frame_timing['post_ready_started'] = time.monotonic()
            stage = 3
        ...
        if warm == 0 and stage == 1:
            frame_timing = dict(index=index, case=spec['cases'][index]['name'], started=time.monotonic(), rgb_render_calls=0)
            prepare(spec['cases'][index])
            frame_timing['prepare_s'] = time.monotonic()-frame_timing['started']
            frame_timing['pre_ready_started'] = time.monotonic()
        rgb.capture_component2d.capture_scene()
        frame_timing['rgb_render_calls'] += 1
        ...
        if warm < warm_target:
            ...
            return
        if stage == 1:
            frame_timing['pre_ready_settling_s'] = time.monotonic()-frame_timing['pre_ready_started']
            frame_timing['readiness_started'] = time.monotonic()
            readiness.begin(world, rgb.capture_component2d)
            stage = 2
            after = 0.
            return
        frame_timing['post_ready_settling_s'] = time.monotonic()-frame_timing['post_ready_started']
        frame_timing['exports_started'] = time.monotonic()
        ...
        if spec.get('native_targets'):
            ...
            if spec.get('export_controlled_targets'):
                ...
                report.setdefault('controlled_target_bindings', []).append(dict(
                    ...
                    lifecycle=('Stable configured assembly identities; actor slots reused and inactive slots hidden per settled view'
                               if reuse_actor_slots else 'Stable configured assembly identities; actors recreated per settled view')))
        ...
        frame_timing['exports_s'] = ended-frame_timing['exports_started']
        frame_timing['total_s'] = ended-frame_timing['started']
        report.setdefault('frame_timings', []).append({k:v for k,v in frame_timing.items()
            if k != 'started' and not k.endswith('_started')})
        index += 1
        warm = 0
        warm_target = settling_ticks
        stage = 1
