"""MZ72 native labels: unchanged kernels, physical assemblies and subset intent."""
import argparse
import hashlib
import itertools
import json
...
from mz72_source_contract import admit_spec, geometry_metadata, intent_matches, shared_objects, source_binding_fields, verify_map_file
...
def build(capture, output, runtime, dependencies, manifest, specs):
    ...
    with torch.inference_mode():
        for i, (case, row) in enumerate(zip(cases, verified_world['rows'])):
            ...
            targets, assembly, support_bounds = [], [], []
            for obj, actor in zip(case['objects'], actors):
                ...
                if not obj['optional_context']:
                    ...
                    assembly.append({k:v for k,v in actor.items() if k!='case'})
                    if obj['target_part']:
                        targets.append({k:v for k,v in actor.items() if k!='case'})
                    else:
                        support_bounds.append(dict(name=obj['name'], min_m=low.tolist(), max_m=high.tolist(),
                            physical_role=obj['physical_role'], mandatory_mount=obj['mandatory_mount'],
                            optional_context=False, outside_queries=bool(high[1]<-.28 or low[1]>.28)))
                else:
                    outside = bool(high[1]<-.28 or low[1]>.28)
                    assert outside, (case['name'], obj['name'], low, high)
                    support_bounds.append(dict(name=obj['name'], min_m=low.tolist(), max_m=high.tolist(),
                        physical_role=obj['physical_role'], mandatory_mount=False, optional_context=True, outside_queries=True))
            ...
            record = dict(index=i, frame_id=case['name'], pair_id=case['pair_id'], site_id=case['site_id'],
                ...
                intent_matches=intent_matches(case, truth),
                ...
                target_actors=targets, shared_assembly_actors=assembly, ancillary_bounds=support_bounds, **geometry_metadata(case))
            ...
            if case['support_context']=='unsupported':
                previous = (case, depth, masks, targets, assembly)
            else:
                before, olddepth, oldmasks, oldtargets, oldassembly = previous
                assert before['pair_id']==case['pair_id'] and oldtargets==targets
                assert before['objects']==shared_objects(case) and oldassembly==assembly
                ...
                pairs.append(dict(pair_id=case['pair_id'], indices=[i-1,i], role=role, target_dictionary_equal=True,
                    actual_target_receipt_equal=True, actual_shared_assembly_receipt_equal=True, changed_event_mask_pixels=difference,
    ...
    write(package/'model/predictor.json', dict(schema='mz72-observable-input-v1', frames=len(cases),
    ...
    write(package/'evaluator/metadata.json', dict(schema='mz72-training-source-v1', records=records, pairs=pairs,
        ...
        expected_relation_authority='Required positive event subset only; OFF_PATH requires no actual query positives; extra actual bits retained, never labels from intent', role_authority='Bound physical recipe-group split; evaluator only'))
    if previews:
