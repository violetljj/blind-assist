"""Independent MZ24 decisions, contributor retention and selected inference audit."""
import argparse
...
def scalar_objective(availability,known,eligible,query,teacher):
    """NumPy float64 loss and analytic angular-output gradient, including ties."""
    a=availability.astype(np.float64);sigmoid=np.exp(-np.logaddexp(0.,-a))
    dense_classes=[];dense_gradients=[]
    for label in [True,False]:
        mask=known==label;counts=mask.reshape(len(a),-1).sum(1);valid=counts>0
        if valid.any():
            values=np.logaddexp(0.,-a) if label else np.logaddexp(0.,a)
            perframe=(values*mask).reshape(len(a),-1).sum(1)/np.maximum(counts,1)
            dense_classes.append(float(perframe[valid].mean()))
            scale=np.where(valid,1./(np.maximum(counts,1)*valid.sum()),0.)[:,None,None]
            dense_gradients.append(mask*(sigmoid-float(label))*scale)
    dense=float(np.mean(dense_classes)) if dense_classes else 0.
    dense_gradient=np.mean(dense_gradients,axis=0) if dense_gradients else np.zeros_like(a)
    negatives=[];positives=[]
    for b in range(len(a)):
        for q in range(teacher.shape[-1]):
            coords=np.argwhere(eligible[b,...,q]&~known[b,:,None,:])
            if len(coords):
                values=a[b,coords[:,0],coords[:,2]];maximum=values.max()
                tied=coords[values==maximum]
                negatives.append((b,float(np.logaddexp(0.,maximum)),tied))
            coords=np.argwhere(eligible[b,...,q]&query[b,...,q])
            if len(coords):
                values=teacher[b,coords[:,0],coords[:,1],coords[:,2],q]
                z,echo,c=coords[int(np.argmax(values))]
                positives.append((b,int(z),int(c),float(np.logaddexp(0.,-a[b,z,c]))))
    nloss=float(np.mean([v[1] for v in negatives])) if negatives else 0.
    ploss=float(np.mean([v[3] for v in positives])) if positives else 0.
    ng=np.zeros_like(a);pg=np.zeros_like(a)
    for b,loss,coords in negatives:
        weights=sigmoid[b,coords[:,0],coords[:,2]]/(len(negatives)*len(coords))
        np.add.at(ng[b],(coords[:,0],coords[:,2]),weights)
    for b,z,c,loss in positives:pg[b,z,c]+=(sigmoid[b,z,c]-1)/len(positives)
    parts=dict(dense=dense,negative=nloss,positive=ploss,negative_bags=len(negatives),positive_bags=len(positives))
    return dense+.25*nloss+.25*ploss,parts,dense_gradient+.25*ng+.25*pg
...
def main(root,run):
    ...
    assert sha(Path(__file__).with_name('MZ24_DECISIVE_AVAILABILITY_PROTOCOL_20260910.md'))==start['protocol_sha256']
    ...
    previous_initial=work/'mz23-availability-20260910/run-v1/initial.pt'
    previous_state=torch.load(previous_initial,map_location='cpu',weights_only=True)
    assert set(previous_state)==set(initial)
    for name,value in previous_state.items():torch.testing.assert_close(initial[name],value,rtol=0,atol=0)
    ...
    loss_data=load_npz(run/'loss_samples.npz');loss_checks={}
    for step in [1,1200]:
        prefix=f'step{step}/';ii=loss_data[prefix+'indices']
        np.testing.assert_array_equal(ii,batches[step-1])
        v={name:loss_data[prefix+name] for name in ['availability','known','eligible','query','teacher','gradient']}
        ...
        assert not (v['query']&~v['known'][:,:,None,:,None]).any()
        total,parts,gradient=scalar_objective(v['availability'],v['known'],v['eligible'],v['query'],v['teacher'])
        np.testing.assert_allclose(total,float(loss_data[prefix+'total']),atol=2e-6,rtol=1e-6)
        for name,value in parts.items():
            np.testing.assert_allclose(value,float(loss_data[prefix+name]),atol=2e-6,rtol=1e-6)
        np.testing.assert_allclose(gradient,v['gradient'],atol=2e-8,rtol=2e-5)
        history=[row for row in result['fit']['history'] if row['step']==step]
        assert len(history)==1
        np.testing.assert_allclose(history[0]['loss'],float(loss_data[prefix+'total']),rtol=0,atol=0)
        for name in parts:assert history[0][name]==float(loss_data[prefix+name])
        loss_checks[str(step)]=dict(total=total,parts=parts,loss_abs_error=abs(total-float(loss_data[prefix+'total'])),
            availability_gradient_max_abs_error=float(np.abs(gradient-v['gradient']).max()),gradient_elements=int(gradient.size),
            method='Independent NumPy per-frame class means, per-valid-query maxima with echo-tie sharing, teacher argmax and analytic sigmoid derivatives')
    ...
    audit=dict(status='PASS',code_sha256=sha(Path(__file__)),scalar_bits=bits,geometric_candidate_bits=support_bits,
        frozen_cutoff_and_batches='byte-identical to MZ20',initialization='exact seed123 CPU parameter reconstruction and exact MZ23 initial state',
        mz23_initial_sha256=sha(previous_initial),
        objective_scalar_and_gradient=loss_checks,
