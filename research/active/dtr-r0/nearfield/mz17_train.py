"""One admitted witness-objective fit; unchanged MZ16 HIGH_DETAIL inference."""
import argparse
...
def main(root,cache,output):
    ...
    admission=read(probe/'admission.json');assert admission['proceed'] is True
    ...
    write(output/'start.json',dict(status='STARTED',steps_per_arm=1200,inputs=inputs,
        ...
        code_sha256={p.name:sha(p) for p in [Path(__file__),Path(__file__).with_name('mz16_detail_readout.py'),Path(__file__).with_name('mz16_local_metrics.py'),Path(__file__).with_name('mz17_witness_loss.py'),Path(__file__).with_name('mz15_train.py'),Path(__file__).with_name('mz15_shared_support.py'),Path(__file__).with_name('mz15_evaluator.py')]},
        protocol_sha256=sha(Path(__file__).with_name('MZ17_WITNESS_OBJECTIVE_PROTOCOL_20260910.md'))))
    ...
    for arm in ['WITNESS']:
        ...
        for step,ids in enumerate(batches):
            ...
            qloss=witness_query_loss(out['field'],q,k,out['eligible'],gt)
    ...
    result['witness_local_ap_gain']=all(result['local']['WITNESS'][n]['macro_ap']>reference['local']['HIGH_DETAIL'][n]['macro_ap'] for n in ['relation10000','distance5000'])
