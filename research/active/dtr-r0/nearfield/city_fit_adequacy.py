"""One fixed2000-step frozen-B fit adequacy check; final-only DEV."""
import argparse
...
def run(a):
    ...
    immutable = {str(p):sha(p) for p in (a.initial,a.old_cache/'manifest.json',a.new_cache/'manifest.json',a.dev_cache/'manifest.json',a.previous/'result.json',a.previous/'selection.json',a.short_run/'result.json',short_checkpoint)}
    ...
    write(out/'protocol.json',dict(seed=17,steps=2000,batch=32,train_frames=len(train),new_frames=len(new),
        backbone='FROZEN',BN='FROZEN_RUNNING_BUFFERS',head_lr=1e-5,weight_decay=1e-4,support_weight=.25,
        ...
        protocol_sha256=sha(Path(__file__).with_name('CITY_FIT_ADEQUACY_20260908.md')),
        ...
        scope='One budget-extension frozen-B Development fit; final2000 only DEV; no fresh TEST or promotion'))
    steps = 0
    try:
        ...
        optimizer = torch.optim.AdamW([p for p in model.parameters() if id(p) not in backbone_ids],lr=1e-5,weight_decay=1e-4)
        ...
        reference=torch.load(short_checkpoint,map_location='cpu',weights_only=True)
        ...
        for step,indices in enumerate(schedule,1):
            ...
            if step in (200,500,1000,2000):
                if step==200 and any(not torch.equal(v.detach().cpu(),reference[k]) for k,v in model.state_dict().items()):
                    raise ValueError('Step200 model prefix differs from baseline')
                snapshot=out/f'B-step{step}.pt';torch.save(model.state_dict(),snapshot)
                snapshots[step]=snapshot
            if step%100==0:
                history.append(dict(step=step,loss=float(loss.detach())))
                write(out/'progress.json',history[-1])
        torch.cuda.synchronize()
        if tensor_digest(model.backbone)!=backbone_hash or tensor_digest(model,buffers_only=True)!=buffers_hash:
            raise ValueError('Frozen backbone/BN changed')
        checkpoint=snapshots[2000]
        fit=dict(steps=steps,seconds=time.perf_counter()-fit_started,history=history,checkpoint_sha256=sha(checkpoint),backbone_unchanged=True,BN_unchanged=True,step200_tensor_parity=True)
        ...
        result=dict(status='PASS',fit=fit,selection=selection,
            ...
            previous_B_DEV=previous_result['DEV_selected']['B'],baseline200_DEV=short_result['DEV_selected'],TRAIN={})
        ...
        for step,path in snapshots.items():
            ...
            for name,cache in (('old750',a.old_cache),('new384',a.new_cache)):
                ...
                if step==200:
                    with np.load(a.short_run/f'{name}-predictions.npz',allow_pickle=False) as z:
                ...
                row[name]=dict(fixed05=evaluate(pn,pm,y,m,g),curves={h:curve(pn[:,j].astype(float).tolist(),y[:,j].astype(int).tolist()) for j,h in enumerate(('BODY','HEAD'))})
        ...
        write(out/'receipt.json',dict(status='PASS',new_fit_count=1,optimizer_steps=steps,backend='CUDA',device=torch.cuda.get_device_name(),
            ...
            scope='Fixed-budget fit adequacy comparison; final2000 consumed DEV only; no fresh TEST or Willow regression, no promotion'))
...
if __name__=='__main__':
    ...
    for name in ('pilot','previous','initial','pretrained','old-cache','new-cache','dev-cache','short-run','output'):
