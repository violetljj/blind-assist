"""One fixed4800-step optimization contrast with verified MZ24 prefix."""
import argparse
...
def main(root,output):
    ...
    for name in ['initial.pt','availability.pt','batches.npy','receipt.json','audit.json']:bind(control/name)
    ...
    base_batches=np.load(control/'batches.npy');np.testing.assert_array_equal(base_batches,np.load(rank/'batches.npy'))
    batches=np.tile(base_batches,(4,1));np.save(output/'batches.npy',batches)
    ...
    write(output/'start.json',dict(status='STARTED',inputs=inputs,steps=4800,seed=123,batch_cycles=4,prefix_tolerance=dict(atol=1e-4,rtol=1e-4),unique_train=len(np.unique(batches)),draws=int(batches.size),
        objective_weights=dict(dense=1.,negative=.25,positive=.25),
        code_sha256={p.name:sha(p) for p in [Path(__file__),Path(__file__).with_name('mz23_availability.py'),Path(__file__).with_name('mz24_decisive_loss.py')]},
        protocol_sha256=sha(Path(__file__).with_name('MZ25_CONVERGENCE_PROTOCOL_20260910.md'))))
    ...
    for step,ii in enumerate(batches):
        ...
        if step==1199:
            ...
            torch.save(state,output/'availability-step1200.pt')
            expected=torch.load(control/'availability.pt',map_location='cpu',weights_only=True)
            errors={k:float((v-expected[k]).abs().max()) for k,v in state.items()}
            matches={k:bool(torch.allclose(v,expected[k],atol=1e-4,rtol=1e-4)) for k,v in state.items()}
            write(output/'prefix-parity.json',dict(status='PASS' if all(matches.values()) else 'FAIL',steps=1200,atol=1e-4,rtol=1e-4,max_abs_error=max(errors.values()),per_tensor_error=errors,per_tensor_match=matches,reference_sha256=sha(control/'availability.pt')))
            assert all(matches.values()),'Step1200 prefix parity failed; do not continue optimization contrast'
            print('PREFIX_PARITY_PASS',max(errors.values()),flush=True)
    ...
    result=dict(metrics={},availability={},thin={},groups={},fit=dict(steps=4800,seconds=fitseconds,parameters=sum(p.numel() for p in head.parameters()),history=history));arrays={}
    ...
    np.savez_compressed(output/'predictions.npz',**arrays);np.savez_compressed(output/'loss_samples.npz',**loss_samples);write(output/'result.json',result)
