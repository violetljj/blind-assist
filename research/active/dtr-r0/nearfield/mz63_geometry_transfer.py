"""Candidate fixed MZ61 transfer; no execution until registered/root GPU GO.

The runner reads observable compact members only. Evaluator labels and source
grouping are opened separately by candidate_score.py after predictions are sealed.
"""
...
METHODS = ('MZ37', 'OLD_NEG/UNION') + tuple(k + '/candidate' for k in HEADS) + UNIONS
...
def validate_index_header(index):
    ...
    assert index['manifest']['sha256'] == '2a84f622d516ae7503571b3d42e7bf8f3c1ff47e3c57f27989f44ab8572aa2cb'
...
def load_predictor(source, bind, expected_index_sha256):
    ...
    for relative, digest in sorted(actual):
        ...
        with CompactSource(path) as archive:
            ...
            assert declaration['schema'] == 'mz61-observable-input-v1'
...
def load_models(root, bind, out, design):
    ...
    anchors, states = {}, {}
    for key in HEADS:
        checkpoint = state('GLOBAL_ANCHOR' if key == HEADS[0] else key)
...
def run(root, task, source, index_sha, design_path, protocol):
    ...
    assert not task.is_relative_to(design_path.parent) and not design_path.parent.is_relative_to(task)
    ...
    try:
        ...
        assert design['schema'] == 'fixed-transfer-design-inputs-v1' and design['status'] == 'DESIGN_ONLY'
        bind = DesignBindings(design)
        bind(design_path); bind(protocol); bind(Path(__file__))
        bind(design_path.parent / 'proposal.md')
        ...
        receipt = dict(status='PASS', inputs=bind.inputs, source_index_sha256=sha(index_path),
            ...
            existing_cutoff_vectors=5, source_calibration_rows_used=0, old_cohort_replay_frames=0,
...
if __name__ == '__main__':
    ...
    run(args.root.resolve(strict=True), args.task.resolve(), args.source.resolve(strict=True),
        args.source_index_sha256, args.design.resolve(strict=True), args.protocol.resolve(strict=True))
