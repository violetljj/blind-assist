"""Fixed1200/4800 budget endpoints sharing one deterministic trajectory."""
import os
...
def main(root,output):
    ...
    control=work/'mz24-decisive-availability-20260910/run-v1'
    for name in ['initial.pt','batches.npy','receipt.json','audit.json']:bind(control/name)
    ...
    base_batches=np.load(rank/'batches.npy');batches=np.tile(base_batches,(4,1));np.save(output/'batches.npy',batches)
    ...
    torch.manual_seed(123);head=StableAngularAvailability(frozen.grid).cuda();torch.save(head.cpu().state_dict(),output/'initial.pt');head.cuda()
    expected=torch.load(control/'initial.pt',weights_only=True)
    for key,value in head.state_dict().items():torch.testing.assert_close(value.cpu(),expected[key].cpu(),rtol=0,atol=0)
    write(output/'start.json',dict(status='STARTED',inputs=inputs,steps=4800,endpoints=[1200,4800],seed=123,batch_cycles=4,unique_train=len(np.unique(batches)),draws=int(batches.size),
        objective_weights=dict(dense=1.,negative=.25,positive=.25),
        runtime=dict(torch=torch.__version__,cuda=torch.version.cuda,cudnn=torch.backends.cudnn.version(),device=torch.cuda.get_device_name(),deterministic=torch.are_deterministic_algorithms_enabled(),cudnn_deterministic=torch.backends.cudnn.deterministic,cudnn_benchmark=torch.backends.cudnn.benchmark,matmul_tf32=torch.backends.cuda.matmul.allow_tf32,cudnn_tf32=torch.backends.cudnn.allow_tf32,cublas_workspace=os.environ['CUBLAS_WORKSPACE_CONFIG']),
        code_sha256={p.name:sha(p) for p in [Path(__file__),Path(__file__).with_name('mz23_availability.py'),Path(__file__).with_name('mz24_decisive_loss.py'),Path(__file__).with_name('mz25_fixed_sampler.py')]},
        protocol_sha256=sha(Path(__file__).with_name('MZ26_DETERMINISTIC_CONVERGENCE_PROTOCOL_20260910.md'))))
    ...
    head.train();opt=torch.optim.Adam(head.parameters(),lr=.001);history=[];loss_samples={};fitstart=time.perf_counter()
    for step,ii in enumerate(batches):
        x,r,v=get(ii);a=head(x,r,v);target=torch.from_numpy(known[ii]).cuda()
        with torch.no_grad():teacher=frozen.inspect(x,r,v)
        contributors=torch.from_numpy(query[ii]).cuda()
        assert not (contributors & ~target[:,:,None,:,None]).any()
        capture=step in [0,1199,4799]
        if capture:a.retain_grad()
        loss,parts=availability_objective(a,target,teacher['eligible'],contributors,teacher['candidate_logits'])
        assert torch.isfinite(loss);opt.zero_grad();loss.backward()
        if capture:
            prefix='step'+str(step+1)+'/'
            for name,value in dict(availability=a,gradient=a.grad,known=target,eligible=teacher['eligible'],query=contributors,teacher=teacher['candidate_logits']).items():
                loss_samples[prefix+name]=value.detach().cpu().numpy()
            loss_samples[prefix+'indices']=ii.copy();loss_samples[prefix+'total']=np.array(float(loss.detach()))
            for name,value in parts.items():loss_samples[prefix+name]=np.array(value)
        opt.step()
        if step in [1199,4799]:
            torch.save({k:v.detach().cpu().clone() for k,v in head.state_dict().items()},output/('availability-step'+str(step+1)+'.pt'))
        if step%400==0 or step in [1199,4799]:
            row=dict(step=step+1,loss=float(loss.detach()),**parts);history.append(row);print(row,flush=True);write(output/'progress.json',row)
    torch.cuda.synchronize();fitseconds=time.perf_counter()-fitstart;head.eval()
    fit=dict(steps=4800,seconds=fitseconds,parameters=sum(p.numel() for p in head.parameters()),history=history)
    results={}
    for endpoint in [1200,4800]:
        head.load_state_dict(torch.load(output/('availability-step'+str(endpoint)+'.pt'),weights_only=True))
        result=dict(metrics={},availability={},thin={},groups={},fit=dict(steps=endpoint,parameters=sum(p.numel() for p in head.parameters())));arrays={}
        ...
        np.savez_compressed(output/('step'+str(endpoint)+'-predictions.npz'),**arrays)
        results['step'+str(endpoint)]=result
        print('ENDPOINT',endpoint,gates,retained,flush=True)
    np.savez_compressed(output/'loss_samples.npz',**loss_samples)
    write(output/'result.json',dict(fit=fit,arms=results))
    write(output/'receipt.json',dict(status='PASS',backend='CUDA strict deterministic',device=torch.cuda.get_device_name(),seconds=time.perf_counter()-started,outputs={p.name:sha(p) for p in output.iterdir() if p.is_file()}));print('PASS',flush=True)
