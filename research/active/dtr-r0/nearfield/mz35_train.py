"""One fixed4800-step trajectory after exact MZ32 prefix reproduction."""
import os
...
def main(root,run):
    ...
    protocol=Path(__file__).with_name('MZ35_RESPONSIBILITY_CONVERGENCE_PROTOCOL_20260910.md');bind(protocol)
    code={n:sha(Path(__file__).with_name(n)) for n in ['mz35_train.py','mz30_select.py','mz30_train.py']}
    parent=work/'mz32-expanded-responsibility-20260910/run-v1';parent_receipt=read(parent/'receipt.json');assert parent_receipt['status']=='PASS';bind(parent/'receipt.json')
    for name in ['selector.pt','initial.pt','training-loss.npy','predictions.npz','supervision.npz']:
        bind(parent/name,parent_receipt['outputs'][name])
    bind(parent/'audit.json');assert read(parent/'audit.json')['status']=='PASS'
    diag=work/'mz33-query-gradient-20260910/run-v1';bind(diag/'receipt.json');bind(diag/'result.json',read(diag/'receipt.json')['outputs']['result.json'])
    ...
    batches=np.tile(batches,(4,1));assert batches.shape==(4800,16)
    ...
    write(run/'start.json',dict(status='STARTED',inputs=inputs,code_sha256=code,seed=123,steps=4800,batch_size=16,parameters=sum(p.numel() for p in model.parameters()),
        class_counts_rgb_tof=counts,class_weights_rgb_tof=weights,initial_state_exact=True,normalization_exact=True,schedule='Original1200x16 stream repeated4times',supervision='All original TRAIN RGB/ToF sign disagreements',runtime_mask='Unchanged MZ30 base-positive unsupported disagreement',
    ...
    try:
        for step,batch in enumerate(batches):
            ...
            if step%1200==0 or step in [1199,4799]:print('FIT',losses[-1],flush=True)
            if step==1199:
                prefix={k:v.detach().cpu().clone() for k,v in model.state_dict().items()}
                torch.save(prefix,run/'prefix1200.pt');torch.save(opt.state_dict(),run/'optimizer1200.pt');np.save(run/'prefix1200-loss.npy',np.array(losses))
                reference=torch.load(parent/'selector.pt',map_location='cpu',weights_only=True)
                errors={k:float((v-reference[k]).abs().max()) for k,v in prefix.items()}
                state_equal=all(torch.equal(v,reference[k]) for k,v in prefix.items());loss_equal=np.array_equal(np.array(losses),np.load(parent/'training-loss.npy'))
                parity=dict(status='PASS' if state_equal and loss_equal else 'INVALID_FOR_REQUESTED_COMPARISON',tensor_bit_exact=state_equal,loss_rows_bit_exact=loss_equal,max_abs_by_tensor=errors)
                write(run/'prefix-parity.json',parity)
                if not state_equal or not loss_equal:
                    write(run/'receipt.json',dict(status='INVALID_FOR_REQUESTED_COMPARISON',training_steps=1200,inputs=inputs,code_sha256=code,outputs={p.name:sha(p) for p in run.iterdir() if p.is_file()}))
                    print('INVALID_PREFIX',parity,flush=True);return
        torch.cuda.synchronize();fitseconds=time.perf_counter()-tick;torch.save(opt.state_dict(),run/'optimizer4800.pt');model.eval();torch.save(model.cpu().state_dict(),run/'selector.pt');model.cuda();np.save(run/'training-loss.npy',np.array(losses))
        scored={}
        ...
        p28=load_npz(oldpath/'predictions.npz');p30=load_npz(previous/'predictions.npz');p32=load_npz(parent/'predictions.npz');rows=read(metadata/'selected.json')
        result=dict(metrics={},thin={},groups={},calibration=calibration,cutoff=cut.tolist(),fit=dict(steps=4800,active_batches=active,eligible_query_exposures=exposures,seconds=fitseconds,parameters=sum(p.numel() for p in model.parameters())),class_counts_rgb_tof=counts,class_weights_rgb_tof=weights);arrays={}
        train_logits=scored['normal/logits'][ti];train_target=target[ti];train_mask=supervision[ti]
        train_weights=np.where(train_target,weights[0],weights[1]);train_bce=(np.logaddexp(0,train_logits.astype(float))-train_target*train_logits)*train_weights
        train_errors=((train_logits>=0)!=train_target)&train_mask
        result['train']=dict(examples=int(train_mask.sum()),responsibility_errors=int(train_errors.sum()),errors_by_query=train_errors.sum(0).tolist(),
            weighted_bce=float(train_bce[train_mask].mean()),weighted_bce_by_query=[float(train_bce[:,q][train_mask[:,q]].mean()) for q in range(4)])
        np.savez_compressed(run/'train-scores.npz',global_ids=train,logits=train_logits,target=train_target,mask=train_mask)
        for name in ['DEV','clean','stress','relation10000','distance5000']:
            ...
            for wrong in ([False] if name=='DEV' else [False,True]):
                key=name+('_wrong' if wrong else '');base=p28[key+'/baseline'];truth=p28[key+'/truth'];oldscore=p28[key+'/candidate'];score30=p30[key+'/candidate'];score32=p32[key+'/candidate'];candidate=np.where(remove,negative,oldscore)
                assert not (remove&~(base>=0)).any();assert not ((candidate>=0)&(oldscore<0)).any()
                m=dict(baseline=metrics(base,truth),mz28=metrics(oldscore,truth),mz30=metrics(score30,truth),mz32=metrics(score32,truth),candidate=metrics(candidate,truth),
                ...
                m['versus_mz32']=dict(tp_gained=((candidate>=0)&(score32<0)&truth).sum(0).tolist(),tp_lost=((candidate<0)&(score32>=0)&truth).sum(0).tolist(),fp_removed=((candidate<0)&(score32>=0)&~truth).sum(0).tolist(),fp_added=((candidate>=0)&(score32<0)&~truth).sum(0).tolist())
                result['metrics'][key]=m
                if name in ['clean','stress']:result['thin'][key]=metrics(candidate[100:125],truth[100:125])
                for k,v in dict(global_ids=ii,baseline=base,truth=truth,mz28=oldscore,mz30=score30,mz32=score32,candidate=candidate,eligible=eligible,remove=remove,confidence=confidence,
        ...
        for name in ['relation10000','distance5000']:
            ...
            for field in ['family','group','site']:
                ...
                result['groups'][name][field]={u:dict(frames=len(ix),mz32=metrics(arrays[name+'/mz32'][ix],arrays[name+'/truth'][ix]),candidate=metrics(arrays[name+'/candidate'][ix],arrays[name+'/truth'][ix])) for u,ix in units.items()}
        ...
        write(run/'receipt.json',dict(status='PASS',backend='CUDA shared selector fixed4800-step trajectory and cached feature inference',training_steps=4800,seconds=time.perf_counter()-started,inputs=inputs,code_sha256=code,outputs={p.name:sha(p) for p in run.iterdir() if p.is_file()}))
