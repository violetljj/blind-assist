...
def main(root,run):
    ...
    assert sha(Path(__file__).with_name('MZ17_WITNESS_OBJECTIVE_PROTOCOL_20260910.md'))==start['protocol_sha256']
    ...
    init=[torch.load(run/'WITNESS-initial.pt',weights_only=True),torch.load(root/'artifacts.local/work/mz16-visual-detail-20260910/run-v1/HIGH_DETAIL-initial.pt',weights_only=True)]
    ...
    local_gain=all(result['local']['WITNESS'][n]['macro_ap']>reference['local']['HIGH_DETAIL'][n]['macro_ap'] for n in ['relation10000','distance5000'])
