"""Independent AP implementation and scalar replay of the resolution contrast."""
import argparse
...
def main(root,run):
    ...
    assert sha(Path(__file__).with_name('MZ16_VISUAL_DETAIL_PROTOCOL_20260910.md'))==start['protocol_sha256']
    ...
    init=[torch.load(run/(arm+'-initial.pt'),weights_only=True) for arm in ['LOW_DETAIL','HIGH_DETAIL']]
    ...
    local_gain=all(result['local']['HIGH_DETAIL'][n]['macro_ap']>result['local']['LOW_DETAIL'][n]['macro_ap'] for n in ['relation10000','distance5000'])
    assert local_gain==result['higher_detail_local_ap_gain']
