...
def metrics(ids, evaluator, values, unknown):
    ...
    for name,pred in values.items():
        ...
        rows[name]=row
    comparison={}
    for name in ('mz92_control','mz93_supported'):
    ...
    b,c=rows['matched_hold'],rows['mz93_supported']; paired=comparison['mz93_supported']
...
def provenance(obs, values, truth, origins):
    ...
    for i in np.flatnonzero(values['matched_hold']):
        ...
        rows.append(dict(frame=int(i),category=category,outcome='TP' if truth[i] else 'FP',
                         control_retained=bool(values['mz92_control'][i]),calibrated_retained=bool(values['mz93_supported'][i])))
    result={}
...
def run(root,out):
    ...
    control_root=previous.ROOT/'artifacts.local/work/mz92-cross-sensor-calibration-20260912/run-v1'
    ...
    for name in ('mz93_supported_prediction.py','run_mz93_supported_prediction.py','test_mz93_supported_prediction.py','MZ93_SUPPORTED_PREDICTION_20260912.md'):
    ...
    for regime in ('ideal','sensor_proxy'):
        ...
        np.testing.assert_array_equal(values['mz92_control'],np.load(control_root/regime/'predictions.npz')['mz92_calibrated'])
    ...
    for regime in ('ideal','sensor_proxy'):
        ...
        names=('matched_hold','mz92_control','mz93_supported')
    ...
    result['decision']='SUPPORTED_PREDICTION_DEVELOPMENT_GAIN' if result['sensor_proxy']['pass_all'] else 'SUPPORTED_PREDICTION_GATE_NOT_MET'
