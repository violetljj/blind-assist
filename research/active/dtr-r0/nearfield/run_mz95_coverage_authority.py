...
def metrics(ids, evaluator, values, unknown):
    ...
    for name,pred in values.items():
        ...
        rows[name]=row
    comparison={}
    for name in ('unchanged_control','coverage_B'):
    ...
    b,c=rows['matched_hold'],rows['coverage_B']; paired=comparison['coverage_B']
...
def provenance(obs, values, truth, origins):
    ...
    for i in np.flatnonzero(values['matched_hold']):
        ...
        rows.append(dict(frame=int(i),category=category,outcome='TP' if truth[i] else 'FP',
                         control_retained=bool(values['unchanged_control'][i]),calibrated_retained=bool(values['coverage_B'][i])))
    result={}
...
def run(root,out):
    ...
    for name in ('mz95_coverage_authority.py','run_mz95_coverage_authority.py','test_mz95_coverage_authority.py','MZ95_COVERAGE_AUTHORITY_20260912.md'):
    ...
    for regime in ('ideal','sensor_proxy'):
        ...
        names=('matched_hold','unchanged_control','coverage_B')
    ...
    for regime in ('ideal','sensor_proxy'):
        audit_rows=json.loads((previous.ROOT/f'artifacts.local/work/mz90-final-decision-audit-20260912/run-v2/{regime}-rows.json').read_text())
        payload=np.load(out/regime/'predictions.npz')
        conflicts={outcome:[r['frame'] for r in audit_rows if r['outcome']==outcome and r['tof_known'] and not r['tof_branch'] and r['radar_branch']] for outcome in ('FN','TN')}
        result[regime]['fixed_conflicts']={key:dict(total=len(ix),**{name:int(payload[name][ix].sum()) for name in names}) for key,ix in conflicts.items()}
    result['decision']='COVERAGE_DEVELOPMENT_GAIN' if result['sensor_proxy']['pass_all'] else 'COVERAGE_GATE_NOT_MET'
