"""Bound MZ61 metadata admission. No depth, image or model inputs."""
from collections import Counter
import hashlib
import json
from pathlib import Path

MANIFEST_SHA = '2a84f622d516ae7503571b3d42e7bf8f3c1ff47e3c57f27989f44ab8572aa2cb'
...
def geometry_metadata(case):
    ...
    assert wi in range(4) and di in range(2) and ti in range(4)
    fold = (wi + di + ti) % 4
    assert case['training_role'] == ROLES[0 if fold < 2 else fold - 1]
    assert case['geometry_recipe_id'] == case['condition']['family'] + f'-g{wi+4*di+8*ti:02d}'
...
def admit_spec(manifest, specs, captured):
    ...
    assert sha(manifest) == MANIFEST_SHA, 'Not frozen MZ61 spec-v2 manifest'
    m = read(manifest)
    assert captured['map_sha256'] == MAP_SHA
