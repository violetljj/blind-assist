...
SEED = 202609235
TRAJECTORIES = {
    'approach_return': (4.25,3.65,3.08,2.82,2.38,1.87,1.32,.93,1.67,2.72,3.26,4.03),
    'approach_dwell_return': (4.25,3.65,3.08,2.82,2.38,2.38,2.38,2.38,2.38,2.82,3.26,4.03)}
FAMILIES = (
    ('body_protruding_plane','BODY','L_plate',(.15,.40,.36),(.48,.94,.76),1.28),
    ('body_suspended_solid','BODY','depth_step',(.26,.40,.32),(.56,.90,.66),1.28),
    ('head_hanging_plane','HEAD','inverted_L_plate',(.11,.40,.26),(.31,.96,.58),1.83),
    ('head_horizontal','HEAD','T_bar',(.11,.54,.08),(.31,1.06,.24),1.81))
...
def specification():
    ...
    for fi,(kind,layer,shape,small,large,height) in enumerate(FAMILIES):
        for si,(level,size) in enumerate((('small',small),('large',large))):
            group = f'local_rescue_fresh_{kind}_{level}'
            side = -1 if (fi+si)%2 else 1
            world_x, camera_y = 35.+fi*.43+si*.13, (-.045 if fi%2 else .045)
            ...
            for trajectory, fronts in TRAJECTORIES.items():
                for relation, overlap in (('INSIDE',.17),('BOUNDARY',.018),('OUTSIDE',-.075)):
    ...
    return dict(schema='local-rescue-fresh-source-v1',seed=SEED,frames=FRAMES,frames_per_clip=12,
...
def freeze(plan):
    ...
    from local_stability_spec import specification as old_stability
    from launch_local_rescue_fresh import MANDATORY_CODE,MANDATORY_INPUTS,verify_protocol
    repo = Path(__file__).resolve().parents[4]
    assert plan.resolve().is_relative_to((repo/'artifacts.local').resolve()) and not plan.exists()
    spec = specification(); check = check_spec(spec,[old_query(),old_coverage(),old_transfer(),old_stability()])
    write(plan/'spec.json',spec);write(plan/'source-check.json',check)
    write(plan/'protocol.json',dict(schema='local-rescue-fresh-capture-protocol-v1',frames=576,clips=48,
