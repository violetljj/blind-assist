"""MZ55 native source labels; derived from frozen MZ48 with collapsed intent and explicit roles."""
import argparse
import hashlib
import itertools
import json
...
HOLDOUT = {'mz36_dense_candidate_05_site_002', 'mz36_dense_candidate_06_site_004'}
...
def build(capture, output, runtime, dependencies):
    ...
    cases = spec['cases']; assert len(cases) in (32, 312)
    ...
    with torch.inference_mode():
        for i, (case, row) in enumerate(zip(cases, verified_world['rows'])):
            ...
            assert role in ('TRAIN_CANDIDATE', 'CALIBRATION', 'HELDOUT_SITE')
            record = dict(index=i, frame_id=case['name'], pair_id=case['pair_id'], site_id=case['site_id'],
                ...
                target_actors=targets, ancillary_bounds=support_bounds)
    ...
    write(package/'model/predictor.json', dict(schema='mz55-observable-input-v1', frames=len(cases),
    ...
    write(package/'evaluator/metadata.json', dict(schema='mz55-training-source-v1', records=records, pairs=pairs,
        ...
        expected_relation_authority='Intent only; not labels', role_authority='Future fit split by held-out site, pairs intact'))
    ...
    result=dict(status='PASS_SOURCE_CHECKS',frames=len(cases),source_valid_frames=sum(r['source_valid'] for r in records),
        ...
        roles={role:sum(r['role']==role for r in records) for role in ('TRAIN_CANDIDATE','CALIBRATION','HELDOUT_SITE')},
    ...
    write(package/'evaluator/source-bindings.json',dict(inputs=inputs,source_spec_sha256=sha(capture/'source/spec.json'),
        code_sha256=sha(__file__),dependencies={str(p):sha(p) for p in dependencies.glob('*.py')},
