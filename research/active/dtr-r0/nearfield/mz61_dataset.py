"""MZ61 native labels: frozen MZ55 kernels with bound geometry-role/shard admission."""
import argparse
import hashlib
import itertools
import json
...
from mz61_source_contract import admit_spec, geometry_metadata, source_binding_fields, verify_map_file
...
def build(capture, output, runtime, dependencies, manifest, specs):
    ...
    write(package/'model/predictor.json', dict(schema='mz61-observable-input-v1', frames=len(cases),
    ...
    write(package/'evaluator/metadata.json', dict(schema='mz61-training-source-v1', records=records, pairs=pairs,
