"""Frozen public RGB/ToF prediction on composite-shape stability transfer.
...
def validate_identities(ids, old_groups):
    ...
    if len(groups) != 8 or groups & old_groups or any(not g.startswith('local_stability_') for g in groups):
        raise ValueError('Need eight new local_stability_ groups, disjoint from all old model groups')
...
def predict(args):
    ...
    select_backend('model-inference', cpu=BackendCandidate('frozen-hgb-stability-cpu', 'cpu',
...
def synthetic_checks():
    ...
    for family, shape in SHAPES.items():
        for size in ('small', 'large'):
            group = 'local_stability_'+family+'_'+size
