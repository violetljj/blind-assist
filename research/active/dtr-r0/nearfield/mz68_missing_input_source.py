"""MZ68 identical MZ64 frame sequence; inherit MZ66 negative-control outputs."""
...
TASK = 'mz68-missing-input-coverage-20260911'
...
def feature_index(bundle):
    ...
    plan.update(scratch_owner=TASK, fit_arm='NULL_COVERAGE', target_source='mz61',
...
def prepare(root, bind):
    ...
    run66 = Path(root) / 'artifacts.local/work/mz66-replay-projection-20260911/run-v1'
    r66 = read(bind(run66/'receipt.json', '0e562511c0396baa3f2e3fa87e6d5c0e3ef9f34a61b9ed8e36ca0dee5c8ebec4'))
    assert r66['status'] == 'PASS'
    inherited = load_npz(bind(run66/'predictions.npz', r66['outputs']['predictions.npz']))
    for key,value in saved.items(): exact(inherited[key],value,key)
    assert len(inherited) > len(saved)
    for name in ('groups.json','schedule.npz'):
        bind(run66/name,r66['outputs'][name])
        assert r66['outputs'][name] == receipt['outputs'][name]
    bundle['predictions'] = inherited
    ...
    bundle['source_info'].update(adapter=Path(__file__).name, scratch_owner=TASK,
        ...
        mz66_arrays_preserved=len(inherited), mz66_predictions_sha256=r66['outputs']['predictions.npz'],
