"""Independent owner-local audit of the predeclared MZ67 native cases.
...
from mz67_source_contract import MANIFEST_SHA, admit_spec, read, sha
...
def run(manifest,specs,parts,output,host):
    ...
    assert sha(Path(__file__).with_name('mz67_source_contract.py'))=='7f2235b1d36a7a426cd308e50834c05ccf7679d23b622105870d5e905423b65f'
    m=read(manifest);assert (m['frames'],m['canary_frames'],m['main_frames'],m['native_audit_frames'])==(4096,64,4032,80)
    inputs={str(p):sha(p) for p in (manifest,parts,Path(__file__),Path(__file__).with_name('mz67_source_contract.py'))}
    ...
    for entry in selected:
        ...
        assert ds['code_sha256']=='d86c300079a2badd3bb44a7885acf2fb3f5b8fa5ea24f7a40e6bf7ba350500be'
        ...
        assert fr['code_sha256']=='b66a96ec0119757e29feeaed76c0c36f1c61aed8e10ea04316d111d4361818c4'
    ...
    expected=40  # Frozen MZ67: each owner has32 canary plus8 main audit frames.
