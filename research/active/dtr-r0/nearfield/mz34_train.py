"""One matched fit with independently parameterized query selectors."""
import os
...
from mz30_select import BranchSelector as SharedBranchSelector,negative_branch_confidence
from mz34_query_selector import QuerySeparatedSelector
...
def main(root,run):
    ...
    protocol=Path(__file__).with_name('MZ34_QUERY_SEPARATION_PROTOCOL_20260910.md');bind(protocol)
    code={n:sha(Path(__file__).with_name(n)) for n in ['mz34_train.py','mz34_query_selector.py','mz30_select.py','mz30_train.py']}
    parent=work/'mz32-expanded-responsibility-20260910/run-v1';diagnostic=work/'mz33-query-gradient-20260910/run-v1'
    parent_receipt=read(parent/'receipt.json');assert parent_receipt['status']=='PASS';bind(parent/'receipt.json')
    for name in ['predictions.npz','result.json','initial.pt','supervision.npz']:
        bind(parent/name,parent_receipt['outputs'][name])
    bind(parent/'audit.json');assert read(parent/'audit.json')['status']=='PASS'
    diagnostic_receipt=read(diagnostic/'receipt.json');assert diagnostic_receipt['status']=='PASS';bind(diagnostic/'receipt.json')
    bind(diagnostic/'result.json',diagnostic_receipt['outputs']['result.json']);signal=read(diagnostic/'result.json')['summary']
    admitted=signal['final_min_pair_cosine']<=-.1 and signal['final_responsibility_errors_total']>0
    write(run/'admission.json',dict(passed=admitted,diagnostic_summary=signal,rule='final min pair cosine<=-0.1 and remaining TRAIN responsibility error'))
    if not admitted:
        write(run/'receipt.json',dict(status='PASS',disposition='NOT_ADMITTED_NO_FIT',training_steps=0,inputs=inputs,code_sha256=code,outputs={'admission.json':sha(run/'admission.json')}))
        print('NOT_ADMITTED_NO_FIT',signal);return
    ...
    initial=torch.load(previous/'initial.pt',map_location='cpu',weights_only=True)
    parent_initial=torch.load(parent/'initial.pt',map_location='cpu',weights_only=True)
    assert all(torch.equal(v,parent_initial[k]) for k,v in initial.items())
    model=QuerySeparatedSelector(initial)
    assert sum(p.numel() for p in model.parameters())==34564
    assert all(torch.equal(v,initial['net.'+k]) for head in model.heads for k,v in head.state_dict().items())
    assert len({p.data_ptr() for p in model.parameters()})==len(list(model.parameters()))
    torch.save(model.state_dict(),run/'initial.pt');model.cuda()
    shared=SharedBranchSelector();shared.load_state_dict(initial);shared.cuda().eval();initial_error=0.
    with torch.inference_mode():
        for start in range(0,len(ti),256):
            feature=torch.from_numpy((x[ti[start:start+256]]-mean)/std).cuda()
            a=model(feature);b=shared(feature);initial_error=max(initial_error,float((a-b).abs().max()))
            np.testing.assert_array_equal(a.cpu().numpy()>=0,b.cpu().numpy()>=0)
    assert initial_error<=1e-6;shared.cpu();del shared
    parent_supervision=load_npz(parent/'supervision.npz')
    np.testing.assert_array_equal(parent_supervision['mask'],supervision[ti]);np.testing.assert_array_equal(parent_supervision['batches'],batches)
    ...
    write(run/'start.json',dict(status='STARTED',inputs=inputs,code_sha256=code,seed=123,steps=1200,batch_size=16,parameters=sum(p.numel() for p in model.parameters()),
        class_counts_rgb_tof=counts,class_weights_rgb_tof=weights,initial_copies_exact=True,initial_function_max_abs=initial_error,normalization_exact=True,supervision='All original TRAIN RGB/ToF sign disagreements',runtime_mask='Unchanged MZ30 base-positive unsupported disagreement',
    ...
    try:
        ...
        write(run/'receipt.json',dict(status='PASS',backend='CUDA four independent query-selector fit and cached feature inference',training_steps=1200,seconds=time.perf_counter()-started,inputs=inputs,code_sha256=code,outputs={p.name:sha(p) for p in run.iterdir() if p.is_file()}))
