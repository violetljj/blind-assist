"""Frozen MZ23/MZ24 TRAIN-versus-DEV availability-extrema diagnostic; no fit."""
import argparse
...
def main(root,output):
    ...
    bind(normpath)
    runs={tag:work/folder/'run-v1' for tag,folder in [('MZ23','mz23-availability-20260910'),('MZ24','mz24-decisive-availability-20260910')]}
    for tag,run in runs.items():
        receipt=read(run/'receipt.json');assert receipt['status']=='PASS'
        for name in ['availability.pt','initial.pt','batches.npy','start.json']:bind(run/name,receipt['outputs'][name])
        assert sha(run/'batches.npy')==sha(rank/'batches.npy')
    tail_result_path=work/'mz24-decisive-availability-20260910/tail-v1/result.json'
    bind(tail_result_path)
    tail_inputs=read(tail_result_path)['inputs']
    assert str(runs['MZ24']/'receipt.json') in tail_inputs
    assert tail_inputs[str(runs['MZ24']/'receipt.json')]==sha(runs['MZ24']/'receipt.json')
    tailnote='The tail-v1 result was produced with MZ24 run-v1; its inherited literal interpretation says MZ23. This diagnostic identifies each checkpoint by its own hash and does not edit that artifact.'
    ...
    write(output/'start.json',dict(status='STARTED',training_steps=0,input_unique_frames=len(allids),cohort_counts={n:len(v) for n,v in slices.items()},
        inputs=inputs,code_sha256=sha(Path(__file__)),source_label='Consumed Development',tail_provenance_note=tailnote))
    ...
    heads={}
    for tag,run in runs.items():
        head=AngularAvailability(frozen.grid.cpu());head.load_state_dict(torch.load(run/'availability.pt',map_location='cpu',weights_only=True))
    ...
    result=dict(results=results,inputs=inputs,code_sha256=sha(Path(__file__)),event_order=ORDER,training_steps=0,
        ...
        tail_provenance_note=tailnote,causal_limit='Residual TRAIN tails can indicate unresolved objective weighting, representation or optimization; this diagnostic cannot isolate those causes. No threshold was selected.')
    write(output/'result.json',result);np.savez_compressed(output/'extrema.npz',**arrays)
    write(output/'receipt.json',dict(status='PASS',training_steps=0,backend='CUDA frozen MZ20 and final MZ23/MZ24 heads; independent NumPy extrema reduction',
