"""Owned one-editor read-only material inspection; no map or frame capture."""
...
def launch(args):
    ...
    shutil.copy2(HERE/'cnh_alley_effective_material_probe.py',source/'cnh_alley_effective_material_probe.py')
    ...
    env=dict(os.environ,TEMP=str(temp),TMP=str(temp),PYTHONDONTWRITEBYTECODE='1',
             BA_CNH_EFFECTIVE_SPEC=str(source/'spec.json'),BA_CNH_EFFECTIVE_OUTPUT=str(out))
    env['UE-LocalDataCachePath']=str(cache)
    command=[str(engine/'Engine/Binaries/Win64/UnrealEditor.exe'),str(project),
        '-ExecCmds=py '+(source/'cnh_alley_effective_material_probe.py').as_posix(),
        ...
        '-EnablePlugins=CitySamplePCG,PythonScriptPlugin,ProceduralMeshComponent',
    ...
    write(out/'launch.json',dict(command=command,source_hashes={p.name:file_hash(p) for p in source.iterdir()},
        protected_file_hashes=before,scope='READ_ONLY_NO_MAP_OR_FRAME_CAPTURE'))
    error=None
    try:
        run_owned(command,env,out,args.timeout)
        raw=json.loads((out/'native-materials.json').read_text(encoding='utf-8'))
        if raw.get('status')!='NATIVE_SOURCE_LOD0_MATERIAL_INSPECTION_COMPLETE':
            raise RuntimeError('Native material probe failed: '+str(raw.get('error')))
        if len(raw['assets'])!=15:raise ValueError('Native asset count differs')
        after={str(p):file_hash(p) for p in protected}
        if after!=before:raise RuntimeError('Project, map, or source mesh bytes changed')
        terminal=dict(status='PASS_READ_ONLY_NATIVE_MATERIAL_INSPECTION',
                      map_or_frame_capture='NOT_RUN',protected_file_count=len(protected),
                      protected_file_hashes_unchanged=True,native_receipt_sha256=file_hash(out/'native-materials.json'),
...
if __name__=='__main__':
    ...
    for name in ('project','engine','spec','output','result'):
