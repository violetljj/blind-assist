"""One frozen-B fit on old750 plus relational384; unchanged DEV selection."""
import argparse
...
def run(a):
    ...
    immutable = {str(p):sha(p) for p in (a.initial,a.old_cache/'manifest.json',a.new_cache/'manifest.json',a.dev_cache/'manifest.json',a.previous/'result.json',a.previous/'selection.json')}
    schedule = np.random.default_rng(17).integers(0,len(train),size=(200,32))
    np.save(out/'training_indices.npy',schedule,allow_pickle=False)
    all_near = np.concatenate([old.near,new.near])
    write(out/'protocol.json',dict(seed=17,steps=200,batch=32,train_frames=len(train),new_frames=len(new),
        ...
        protocol_sha256=sha(Path(__file__).with_name('CITY_RELATIONAL_TRAIN_20260908.md')),
        source_sha256={p.name:sha(p) for p in (Path(__file__),Path(__file__).with_name('city_dev_baseline.py'),Path(__file__).with_name('city_dev_selection.py'))},
        scope='One data-only frozen-B Development fit; no fresh TEST or promotion'))
    steps = 0
    try:
        ...
        history=[]; fit_started=time.perf_counter()
        ...
        checkpoint=out/'B-relational-step200.pt';torch.save(model.state_dict(),checkpoint)
        fit=dict(steps=steps,seconds=time.perf_counter()-fit_started,history=history,checkpoint_sha256=sha(checkpoint),backbone_unchanged=True,BN_unchanged=True)
        write(out/'fit-complete.json',fit)
        del optimizer,rgb,near,support,n,s,loss,samples
        # DEV is opened only after the sole fit has completed.
        ...
        result=dict(status='PASS',fit=fit,selection=selection,
            ...
            previous_B_DEV=previous_result['DEV_selected']['B'],TRAIN={})
        ...
        write(out/'receipt.json',dict(status='PASS',new_fit_count=1,optimizer_steps=steps,backend='CUDA',device=torch.cuda.get_device_name(),
            ...
            scope='Consumed DEV data-recipe comparison; no fresh TEST or Willow regression, no promotion'))
...
if __name__=='__main__':
    ...
    for name in ('pilot','previous','initial','pretrained','old-cache','new-cache','dev-cache','output'):
