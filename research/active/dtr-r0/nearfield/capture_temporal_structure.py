"""NF-G6 posed static camera windows. No real-time cadence or map saving."""
import array
import hashlib
import json
import math
    ...
    try:
        ...
        if stage == 2:
            ...
            frames.append({'sample_index': index, 'clip_id': cases[index]['clip_id'], 'frame_in_clip': cases[index]['frame_in_clip'],
                           ...
                           'wearer_transform': dict(pose, z=.12, pitch=0., roll=0.)})
            index += 1
            warm = 0
            if index == len(cases):
                write(OUT / 'model/sensor_manifest.json', {
                    'schema': 'nearfield-temporal-geometric-probe-v1',
                    'calibration': {'width': 640, 'height': 360, 'horizontal_fov_degrees': 100., 'depth_max_m': 100.},
                    'sampling': 'POSED_STATIC_THREE_FRAME_WINDOWS_NO_ACTUAL_SENSOR_CADENCE',
                    ...
                    'authority': 'RGB_CALIBRATION_IDEAL_EGO_POSES_ONLY_NO_NATIVE_DEPTH_OR_OBJECTS', 'frames': frames})
                finish()
            else:
                after = 0
    except Exception:
