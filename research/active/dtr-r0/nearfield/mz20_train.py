"""One cross-bag BODY ranking fit; unchanged MZ16 HIGH_DETAIL inference."""
import argparse
...
def main(root,cache,output):
    ...
    probe=work/'mz17-witness-objective-20260910/probe-v1'
    ...
    bind(probe/'admission.json');bind(probe/'receipt.json')
    bind(work/'mz19-margin-diagnostic-20260910/receipt.json')
    bind(work/'mz18-body-objective-20260910/run-v1/receipt.json')
    comparator=work/'mz16-visual-detail-20260910/run-v1'
    for name in ['result.json','HIGH_DETAIL-initial.pt','HIGH_DETAIL.pt']:
        bind(comparator/name,read(comparator/'receipt.json')['outputs'][name])
    ...
    write(output/'start.json',dict(status='STARTED',steps_per_arm=1200,inputs=inputs,
        ...
        code_sha256={p.name:sha(p) for p in [Path(__file__),Path(__file__).with_name('mz16_detail_readout.py'),Path(__file__).with_name('mz16_local_metrics.py'),Path(__file__).with_name('mz20_rank_loss.py'),Path(__file__).with_name('mz15_train.py'),Path(__file__).with_name('mz15_shared_support.py'),Path(__file__).with_name('mz15_evaluator.py')]},
        protocol_sha256=sha(Path(__file__).with_name('MZ20_RANK_OBJECTIVE_PROTOCOL_20260910.md'))))
    ...
    for arm in ['BODY_RANK']:
        ...
        model=make_model(initial).cuda()
        reference_initial=torch.load(comparator/'HIGH_DETAIL-initial.pt',weights_only=True)
        for key,value in model.state_dict().items():torch.testing.assert_close(value.cpu(),reference_initial[key],rtol=0,atol=0)
        ...
        for step,ids in enumerate(batches):
            ...
            qloss=body_rank_query_loss(out['field'],q,k,out['eligible'],gt,out['logits'],out['support'])
    ...
    reference=read(comparator/'result.json')
    result['comparator']={k:reference[k]['HIGH_DETAIL'] for k in ['metrics','local','thin','gates']}
    result['witness_local_ap_gain']=all(result['local']['BODY_RANK'][n]['macro_ap']>reference['local']['HIGH_DETAIL'][n]['macro_ap'] for n in ['relation10000','distance5000'])
    ...
    print('PASS',result['gates'],'LOCAL_AP_GAIN',result['witness_local_ap_gain'],flush=True)
