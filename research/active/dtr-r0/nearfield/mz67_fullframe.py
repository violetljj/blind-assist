"""MZ67 owner-side binding adapter for the frozen MZ52 CPU cell derivation."""
import argparse
...
from mz67_source_contract import admit_spec, verify_map_file, source_binding_fields
...
def run(manifest,specs,capture,dataset,core,output,host):
    ...
    for i,(row,case) in enumerate(zip(rows,captured['cases'])):
        ...
        request_rows.append(dict(global_index=ids[row['frame_id']],frame_id=row['frame_id'],native_path=str(path.resolve()),
            ...
            known=[row['source_valid']]*4,geometry_id=row['geometry_id'],geometry_recipe_id=row['geometry_recipe_id']))
    ...
    schema=dict(fullframe_event_counts='uint8[N,45,80,4]',valid_counts='uint8[N,45,80]',
        ...
        authority='MZ67 native training/evaluator only; unchanged45degree predictor packet')
    ...
    final=dict(status='PASS',inputs=inputs,code_sha256=sha(Path(__file__)),frozen_derivation_sha256=CORE_SHA,
        request_sha256=sha(req),geometry_contract=contract,**source_binding_fields(contract,capture/'source/spec.json'),frames=len(rows),global_frame_order='Frozen MZ67 manifest shard order then case order',
