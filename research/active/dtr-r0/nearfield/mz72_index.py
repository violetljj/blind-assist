"""Merge the frozen4096 MZ72 source; owner-local80 native audits, no raw transfer.
...
canary_gate:{result,receipt}; reference/reference_receipt: sealed MZ72 gate
old48+55+61+67 TRAIN reference; main_go: root-owned expansion authorization.
...
import mz72_source_contract as contract
import mz72_canary_gate as reference_gate

# Current code hashes must also match root-owned deployment snapshots and GO.
DATASET_SHA=contract.sha(Path(__file__).with_name('mz72_dataset.py'))
FULLFRAME_SHA=contract.sha(Path(__file__).with_name('mz72_fullframe.py'))
CORE_SHA='5fb19e543e1b6c841a2c61c8a4e4ee4c918985a3883d1c061a2ce673423aeaa3'
NATIVE_SHA=contract.sha(Path(__file__).with_name('mz72_native_audit.py'))
GATE_CODE_SHA=contract.sha(Path(reference_gate.__file__))
CONTRACT_SHA=contract.sha(Path(contract.__file__))
FROZEN_MANIFEST_SHA='6687a5434a07c0afcdbb8e99a9afdbdc6718e1a298d207d5d150f2a6e2a09233'
CODE_NAMES=('mz72_index.py','mz72_native_audit.py','mz72_source_contract.py',
            'mz72_dataset.py','mz72_fullframe.py','mz72_canary_gate.py','data_lightweight.py')
...
SCHEMA={'index':'mz72-physical-source-v1','frames':4096,'shards':10,
        ...
        'required_shard_pointers':list(REQUIRED),'required_top_pointers':['visual','releases','native_audits','canary_gate','reference','reference_receipt','main_go']}
...
def validate_geometry_case(case):
    """Metadata and assembly integrity against exact manifest-bound frozen spec.

    admit_spec verifies all cases and permits only map_file host remapping.
    This does not reuse MZ67's incompatible camera-relative target hash formula.
    """
    meta=contract.geometry_metadata(case)
    assert contract.shared_objects(case)
    return meta
...
def tensor_collisions(records,counts,truth,known):
    ...
    return dict(schema='mz72-native-tensor-collision-v1',tables=output,
...
def old_train_collisions(records,counts,truth,known,reference):
    """Existing MZ72 gate reference: all four frozen old TRAIN sources."""
    assert reference['status']=='PASS' and len(reference['rows'])==reference['frames']
    assert set(reference['source_frames'])=={'mz48','mz55','mz61','mz67'}
    summary={};details={}
    for mode in ('counts','presence'):
        old={r[mode] for r in reference['rows'] if r['positive']}
        grouped=defaultdict(list)
        for i,r in enumerate(records):
            x=counts[i] if mode=='counts' else counts[i]>0
            sig=hashlib.sha256(np.ascontiguousarray(x).tobytes()).hexdigest()
            positive=bool(truth[i].any() and known[i].all())
            grouped[r['geometry_id']].append(dict(frame_id=r['frame_id'],signature=sig,
                positive=positive,seen_in_old_train=positive and sig in old))
        rows=[]
        for geo,rr in grouped.items():
            r=next(r for r in records if r['geometry_id']==geo)
            positive={v['signature'] for v in rr if v['positive']}
            hits=positive & old
            rows.append(dict(geometry_id=geo,role=r['role'],family=r['family'],
                positive_tensor_variants=len(positive),matching_positive_variants=len(hits),frames=rr))
        summary[mode]={}
        for role in contract.ROLES:
            for family in ('ALL',)+tuple(sorted({r['family'] for r in records})):
                rr=[r for r in rows if r['role']==role and (family=='ALL' or r['family']==family)]
                pp=[r for r in rr if r['positive_tensor_variants']]
                summary[mode][role+'/'+family]=dict(configurations=len(rr),positive_configurations=len(pp),
                    positive_any_variant_seen_in_old_train=sum(r['matching_positive_variants']>0 for r in pp),
                    positive_all_variants_seen_in_old_train=sum(r['matching_positive_variants']==r['positive_tensor_variants'] for r in pp),
                    positive_with_a_novel_variant=sum(r['matching_positive_variants']<r['positive_tensor_variants'] for r in pp))
        details[mode]=rows
    return dict(schema='mz72-old-train-tensor-collision-v1',reference_train_frames=reference['source_frames'],
        summary=summary,configurations=details,unknown_query_bits=int((~known).sum()),
        scope='Descriptive exact whole native counts/presence against unchanged gate TRAIN reference; no novelty admission or row selection.')


def validate_go(value,parts_sha256,code_sha256):
    assert value['status']=='GO'
    assert contract.MANIFEST_SHA==FROZEN_MANIFEST_SHA, 'Different frozen manifest'
...
def validate_canary(value,manifest_sha):
    assert value['status']=='PASS' and value['manifest_sha256']==manifest_sha
    assert set(value['checks'])=={'source_health','all_required_intents','native_pairs','visual_physical_support'}
    assert all(v is True for v in value['checks'].values())
    assert value['frames']==64 and value['allowed_next_frames']==4032
...
def build(task,parts,output,admission):
    ...
    assert task.name=='mz72-physical-source-20260911' and output.is_relative_to(task)
    ...
    try:
        ...
        bind(Path(__file__).with_name('mz72_native_audit.py'),NATIVE_SHA)
        bind(Path(__file__).with_name('MZ72_PHYSICAL_SOURCE_20260911.md'))
        go=read(bind(admission));validate_go(go,sha(parts),sha(__file__))
        assert set(go['code_sha256'])==set(CODE_NAMES)
        for name in CODE_NAMES:
            bind(Path(__file__).with_name(name),go['code_sha256'][name])
            if name not in ('mz72_index.py','mz72_native_audit.py','mz72_canary_gate.py'):
                bind(task/'deployment-v1'/name,go['code_sha256'][name])
        ...
        refpath=bound(bundle['reference'])
        reference=read(refpath);assert reference['status']=='PASS'
        refreceipt=read(bound(bundle['reference_receipt']))
        assert refreceipt['status']=='PASS' and refreceipt['outputs']['reference.json']==sha(refpath)
        assert refreceipt['code_sha256']==GATE_CODE_SHA
        ...
        assert manifest['schema']=='mz72-physical-manifest-v1' and manifest['inputs']
        for p,h in manifest['inputs'].items():bind(p,h)
        assert sha(Path(__file__).with_name('mz72_physical_spec.py')) in manifest['inputs'].values()
        geometry_validation=dict(cases=0,authority='Complete frozen manifest-bound spec/assembly and captured equality except map_file; IDs not redefined')
        gate_receipt=read(bound(bundle['canary_gate']['receipt']))
        gate=read(bound(bundle['canary_gate']['result']))
        assert gate_receipt['status']=='PASS_AUDIT_EXECUTED' and gate_receipt['outputs']['result.json']==bundle['canary_gate']['result']['sha256']
        assert gate_receipt['code_sha256']==GATE_CODE_SHA
        validate_canary(gate,contract.MANIFEST_SHA)
        for p,h in gate_receipt['inputs'].items():bind(p,h)
        assert go['canary_gate_result_sha256']==bundle['canary_gate']['result']['sha256']
        main_go=read(bound(bundle['main_go']))
        assert main_go['status']=='GO' and main_go['manifest_sha256']==contract.MANIFEST_SHA
        assert main_go['canary_gate_result_sha256']==bundle['canary_gate']['result']['sha256']
        assert main_go['allowed_next_frames']==4032
        ...
        assert all(r['accepted'] and r['rgb_native_agreement'] and r['physical_support_checked']
        ...
        for frozen in manifest['shards']:
            ...
            with CompactSource(paths['archive']) as source:
                ...
                assert meta['schema']=='mz72-training-source-v1' and predictor['schema']=='mz72-observable-input-v1'
                ...
                for i,(row,case,native) in enumerate(zip(meta['records'],spec['cases'],binding['native_files'])):
                    ...
                    checked=validate_geometry_case(case)
                    ...
                    assert row['intent_matches']==contract.intent_matches(case,labels['truth'][i])
                    assert row['exclusive_anchor_bits_match']==bool(np.array_equal(labels['truth'][i],expected))
                ...
                for pair in meta['pairs']:
                    ...
                    assert ra['shared_assembly_actors']==rb['shared_assembly_actors'] and pair['actual_shared_assembly_receipt_equal']
        ...
        assert all(len(roles)==1 for roles in geometries.values())
        for field in ('geometry_id','scene_geometry_id','recipe_group'):
            groups=defaultdict(set)
            for r in records:groups[r[field]].add(r['role'])
            assert all(len(v)==1 for v in groups.values()),field
        assert Counter(r['regime'] for r in records)==dict(ACTUAL_SIZE=3072,ANGULAR_CONTROL=1024)
        ...
        result=dict(status='PASS',frames=4096,unique_frame_ids=4096,source_valid_frames=int(known.all(1).sum()),roles=manifest['roles'],
            ...
            independent_native_frames=80,independent_raw_pairs=40,visual_reviewed_frames=80,geometry_ids=len(geometries),role_geometry_intersections=0,
            recipe_groups=len({r['recipe_group'] for r in records}),scene_geometry_ids=len({r['scene_geometry_id'] for r in records}),
            regimes=dict(Counter(r['regime'] for r in records)),
            angular_pair_ids=len({r['angular_pair_id'] for r in records if r['angular_pair_id'] is not None}),
            shared_assembly_invariant_pairs=2048,required_subset_intent=True,
        ...
        index=dict(status='COMPLETE',schema=SCHEMA['index'],frames=4096,source_role='CONSUMED_DEVELOPMENT',manifest=dict(path='spec-v1/manifest.json',sha256=contract.MANIFEST_SHA),
            ...
            canary_gate=bundle['canary_gate'],old_train_reference=bundle['reference'],old_train_reference_receipt=bundle['reference_receipt'],main_go=bundle['main_go'],
            merge_admission=dict(path=admission.relative_to(task).as_posix(),sha256=sha(admission)),
            roles=manifest['roles'],pairs=2048,geometry_ids=len(geometries),
...
def self_test():
    ...
    from mz72_source_contract import intent_matches
    case=dict(required_positive_bits=[True,False,False,False],off_path_expected_zero=False)
    assert intent_matches(case,[True,False,True,False])
    assert not intent_matches(case,[False,False,True,False])
    assert not intent_matches(dict(required_positive_bits=[False]*4,off_path_expected_zero=True),[False,True,False,False])
    x=np.zeros((1,45,80,4),np.uint8);x[0,0,0,0]=3
    ref=dict(status='PASS',frames=1,source_frames=dict(mz48=1,mz55=0,mz61=0,mz67=0),rows=[
        dict(counts=hashlib.sha256(x[0].tobytes()).hexdigest(),presence=hashlib.sha256((x[0]>0).tobytes()).hexdigest(),positive=True)])
    a=old_train_collisions([records[2]],x,x.sum((1,2))>=3,np.ones((1,4),bool),ref)
    assert a['summary']['counts']['HELDOUT_GEOMETRY/ALL']['positive_all_variants_seen_in_old_train']==1
    gate=dict(status='PASS',manifest_sha256='fixture',frames=64,allowed_next_frames=4032,
        checks=dict(source_health=True,all_required_intents=True,native_pairs=True,visual_physical_support=True))
    validate_canary(gate,'fixture')
    for key,value in (('status','STOP_NO_EXPANSION'),('frames',32),('allowed_next_frames',0),('manifest_sha256','wrong'),
                      ('checks',dict(gate['checks'],source_health=False))):
        try:validate_canary(dict(gate,**{key:value}),'fixture')
        except AssertionError:pass
        else:raise AssertionError('Partial/unadmitted canary accepted')
    print('PASS synthetic whole/query/zero/roles/variants/UNKNOWN collision and MZ72 subset intent boundaries; no source merge')
