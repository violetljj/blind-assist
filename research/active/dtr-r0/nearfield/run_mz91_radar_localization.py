...
def metrics(ids, evaluator, values, unknown):
    ...
    for name,pred in values.items():
        ...
        rows[name]=row
    comparison={}
    for name in ('single_frame','multi_frame'):
    ...
    b,c=rows['matched_hold'],rows['multi_frame']; paired=comparison['multi_frame']
...
def provenance(obs, values, truth, origins):
    ...
    for i in np.flatnonzero(values['matched_hold']):
        ...
        rows.append(dict(frame=int(i),category=category,outcome='TP' if truth[i] else 'FP',
                         single_retained=bool(values['single_frame'][i]),multi_retained=bool(values['multi_frame'][i])))
    result={}
...
def run(root,out):
    ...
    for name in ('mz91_radar_localization.py','run_mz91_radar_localization.py','test_mz91_radar_localization.py','MZ91_RADAR_LOCALIZATION_20260912.md'):
    ...
    for regime in ('ideal','sensor_proxy'):
        ...
        names=('matched_hold','single_frame','multi_frame')
    ...
    result['decision']='LOCALIZATION_DEVELOPMENT_GAIN' if result['sensor_proxy']['pass_all'] else 'LOCALIZATION_GATE_NOT_MET'
