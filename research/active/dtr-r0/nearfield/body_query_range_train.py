"""One matched range-supervised fit, against expanded-source B."""
import os
...
OLD_SHA = 'c7aef143bcc7ac464097e95219dc5239523df7ce5716fc95926f37592f94e776'
...
def run(a):
    ...
    schedule = np.load(a.old.parent/'schedule.npy',allow_pickle=False)
    assert np.array_equal(schedule,np.random.default_rng(17).integers(0,2500,(2000,32)))
    ...
    assert initial_digest == read(a.old.parent/'fit-complete.json')['initial_digest']
    protocol = dict(seed=17, steps=2000, batch=32, lr=1e-5, weight_decay=1e-4,
                    ...
                    brief_sha256=sha(Path(__file__).with_name('BODY_QUERY_RANGE_PROTOCOL_20260909.md')),
                    dependency_sha256={name:sha(Path(__file__).with_name(name)) for name in
                        ('body_query_range.py','body_query_model.py','body_query_data.py','body_query_train.py','city_data.py','city_dev_selection.py','decoupled_model.py','representation_model.py')},
                    torch=torch.__version__, device=torch.cuda.get_device_name(),
                    trained_frames=2500, expected_presentations_per_frame=25.6, range_weight=.25)
    ...
    try:
        for step, indices in enumerate(schedule, 1):
            ...
            lr = F.binary_cross_entropy_with_logits(range_from_counts(c), (targets['counts'][ids].reshape(-1,2,2,3).sum(-1)>=3).float())
            loss = ln + .25*ls + .25*lc + .25*lr
            ...
            if step % 100 == 0:
                row = dict(step=step, near=float(ln.detach()), support=float(ls.detach()), query=float(lc.detach()), range=float(lr.detach()), loss=float(loss.detach()))
        ...
        del model, optimizer, rgb, targets, n,s,c,ln,ls,lc,lr,loss
