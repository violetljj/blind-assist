"""G13 semantic-feature gating evaluation with common-VAL recall-first primary thresholds."""
import argparse
import json
import math
...
ARMS=('repvit','repvit_detail','decoupled')
...
def normalize(payload,samples):
    ...
    if set(predictions)!={(arm,seed,'normal') for arm in ARMS for seed in SEEDS}:
        raise ValueError('Expected B/C/D semantic-gating arms, three seeds plus ensembles, normal only')
...
def main():
    ...
    parser.add_argument('--phase',choices=('consumed','fresh'),default='consumed')
    ...
    common=dict(schema='nf-g13-decoupled-evaluation-v1',
    ...
    def result_for(predictions,scopes,targets,maps,support,objects,regression):
        return dict(common,primary=dict(headline=(not regression and args.phase=='fresh'),evaluations=score_scopes(predictions,primary,scopes,targets)),
            ...
            claim_scope='Consumed G10 TEST regression, not fresh confirmation; original results preserved.' if regression else
                ('Consumed G12 TEST mechanism diagnostic; hypothesis was chosen after G12 outcomes; not fresh validation.' if args.phase=='consumed' else 'New G13 same-Willow Development check after consumed-screen selection; not natural generalization.'))
