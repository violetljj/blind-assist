"""NF-G7 prescribed 10 Hz trajectory samples; settled rendering, no map saving."""
import array
import hashlib
import json
import math
...
def tick(delta):
    global after, stage, world, rgb, depth, cases, index, warm, spec
    ...
    try:
        ...
        if stage == 2:
            ...
            frames.append({'sample_index': index, 'clip_id': cases[index]['clip_id'], 'frame_in_clip': cases[index]['frame_in_clip'],
                           ...
                           'wearer_transform': cases[index]['wearer'], 'time_s': cases[index]['time_s'], 'speed_m_s': cases[index]['speed_m_s']})
            index += 1
            ...
            if index == len(cases):
                write(OUT / 'model/dataset.json', {
                    'schema': 'nf-g7-contact-retina-dataset-v1',
                    'calibration': {'width': 640, 'height': 360, 'horizontal_fov_degrees': 100., 'depth_max_m': 100., 'pose_translation_unit': 'metres'},
                    'sampling': 'SIMULATED_10_HZ_POSED_TRAJECTORY_NOT_WALLCLOCK_SENSOR',
                    'pose_authority': 'PRIVILEGED_IDEAL_SIMULATOR_EGOPOSE_NOT_DEPLOYABLE_IMU',
                    'camera_convention': 'UE_X_FORWARD_Y_RIGHT_Z_UP_METRES; horizontal_fov; principal_point=(319.5,179.5)',
                    'authority': 'RGB_CALIBRATION_IDEAL_EGO_POSES_ONLY_NO_NATIVE_DEPTH_OR_OBJECTS', 'frames': frames, 'samples': spec['samples']})
                write(OUT / 'evaluator/labels.json', spec['labels'])
                train_ids = {s['sample_id'] for s in spec['samples'] if s['split'] != 'test'}
                write(OUT / 'training/labels.json', {'targets': {k:v for k,v in spec['labels']['targets'].items() if k in train_ids}})
                finish()
            else:
                after = 0
    except Exception:
