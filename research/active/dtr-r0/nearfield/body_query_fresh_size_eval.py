"""Separate native admission and frozen RGB-only fresh-size inference."""
import argparse
import json
import os
...
def admit(a):
    ...
    for region,location in captures.items():
        ...
        for i,c in enumerate(spec['cases']):
            ...
            isolated = np.load(root/f'evaluator/isolated/head_bar/{i:04d}.npy',allow_pickle=False)
            ...
            ray = [r for r in checks if r['sample_index']==i and r['target_id']=='head_bar']
            ...
            assert target_binding['target_id']=='head_bar'
            if len(ray)==1:
            ...
            rows.append(dict(sample_index=len(rows),capture_index=i,region_id=region,
                 pair_id=c['pair_id'],site_id=c['site_id'],arm=c['arm'],endpoint=c['declared_range'],
                 rgb_path=str(image),rgb_sha256=sha(image),
                 native_sha256=sha(root/f'evaluator/native/{i:04d}.npy'),
                 isolated_sha256=sha(root/f'evaluator/isolated/head_bar/{i:04d}.npy'),
    ...
    status = 'PASS' if all(v>=108 for v in coverage.values()) else 'NOT_EVALUABLE'
    write(a.output/'frame_metadata.json',rows)
    np.savez_compressed(a.output/'evaluator_truth.npz',counts=np.stack(counts))
    write(a.output/'admission.json',dict(status=status,planned_pairs=240,planned_per_arm=120,
...
def infer(a):
    ...
    diagnostic = a.diagnostic_source_shortfall
    if diagnostic:
    ...
    for arm in ('ordinary','matched','combined'):
    ...
    spatial_gate=all(results[arm]['JOINT']['pair_correct']['numerator']/results[arm]['JOINT']['pair_correct']['denominator']>=.8
                     for arm in ('ordinary','matched'))
    gate=spatial_gate and parity.all() and not diagnostic
    ...
    write(a.output/'result.json',dict(status='PASS',promotion_gate=bool(gate),metrics=results,
          evaluation_mode='DIAGNOSTIC_AFTER_SOURCE_SHORTFALL' if diagnostic else 'FROZEN_PRIMARY',
