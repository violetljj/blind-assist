...
The engine entry is selected solely by BA_MZ99_SPEC/BA_MZ99_OUT environment.
"""
...
def source(seed=99013):
    ...
    for i in range(128):
        ...
        scenes.append(dict(episode_id=f'ue99-{i:04d}',split=split[i],family=family,
    ...
    return dict(schema='mz99-fresh-ue-native-provenance-v1',authority=AUTHORITY,seed=seed,dt=DT,frames=FRAMES,
...
def retained_slots(radar, provenance):
    """Original stable noisy-triple sort/truncation; provenance never sorts slots."""
    if len(radar)!=len(provenance):raise ValueError('Missing per-return provenance')
    indexes=sorted(range(len(radar)),key=lambda i:radar[i])[:4]
    return [(radar[i],dict(provenance[i],observed_triple=list(radar[i]))) for i in indexes]


def engine_capture():
    import unreal as u
    out=Path(os.environ['BA_MZ99_OUT']); spec_path=Path(os.environ['BA_MZ99_SPEC'])
    spec=json.loads(spec_path.read_text()); count=int(os.environ.get('BA_MZ99_EPISODES','128'))
    ...
    provfile=(out/'provenance.jsonl').open('w',encoding='utf-8')
    ...
    try:
        for scene in spec['scenes'][:count]:
            ...
            for j in range(FRAMES):
                ...
                radar=[]; radarhits=0; provenance=[]
                for actor_index,(a,b) in enumerate(zip(actors,bounds)):
                    ...
                    if measured:
                        radar.append(measured)
                        provenance.append(dict(kind='real_actor',actor_id=f'{scene["episode_id"]}/actor-{actor_index}',exact_angle_deg=az,pre_noise_range_m=r,pre_noise_velocity=rv))
                ghost=ghost_return(scene.get('radar_ghost'),scene['wearer_speed'],t,yaw)
                ghost_detected=False
                if ghost:
                    measured=measure_radar(*ghost,scene['angle_bias'],rng)
                    if measured:
                        radar.append(measured);ghost_detected=True
                        provenance.append(dict(kind='persistent_ghost',actor_id=None,exact_angle_deg=ghost[1],pre_noise_range_m=ghost[0],pre_noise_velocity=ghost[2]))
                if rng.random()<.15:
                    radar.append((round(rng.uniform(.4,5)/.05)*.05,round(rng.uniform(-60,60)/10)*10,round(rng.uniform(-1,1)/.1)*.1))
                    provenance.append(dict(kind='transient',actor_id=None,exact_angle_deg=None,pre_noise_range_m=None,pre_noise_velocity=None))
                slots=[None]*4
                for k,((r,a,v),prov) in enumerate(retained_slots(radar,provenance)):
                ...
                emit(rawfile,row)
                emit(provfile,dict(episode_id=scene['episode_id'],time_s=t,radar_slots=slots,authority='EVALUATOR_ONLY_PROVENANCE_AFTER_ORIGINAL_NOISY_SORT_TRUNCATION'))
                ...
                total+=1
            rawfile.flush();evalfile.flush();geofile.flush();provfile.flush()
        ...
        rawfile.close();evalfile.close();geofile.close();provfile.close()
        for a in actors:api.destroy_actor(a)
        report['seconds']=time.monotonic()-started
        report['hashes']={p.name:sha(p) for p in (out/'raw.jsonl',out/'evaluator.jsonl',out/'native-geometry.jsonl',out/'provenance.jsonl')}
...
def main():
    ...
    prep=sub.add_parser('prepare');prep.add_argument('--output',type=Path,required=True);prep.add_argument('--seed',type=int,default=99013)
    ...
    cache=art/'work/mz99-angle-information-20260912/ddc';cache.mkdir(parents=True,exist_ok=True)
    port=cache_service_port(cache)
    env=dict(os.environ,BA_MZ99_SPEC=str((args.output/'spec.json').resolve()),BA_MZ99_OUT=str(args.output.resolve()),BA_MZ99_EPISODES=str(args.episodes))
...
if os.environ.get('BA_MZ99_SPEC') and 'unreal' in sys.modules:
    engine_capture()
elif __name__=='__main__':
    # Unreal's ExecCmds interpreter may not import unreal until the script does.
    if os.environ.get('BA_MZ99_SPEC'):engine_capture()
    else:main()
