"""One same-initialization 10000-frame B fit, against the accepted expanded B baseline."""
import os
...
def run(a):
    ...
    cache_manifest = read(a.cache/'manifest.json')
    assert cache_manifest['status'] == 'PASS'
    assert cache_manifest['partitions']['train']['frames'] == 5000
    assert cache_manifest['partitions']['dev']['frames'] == 2000
    assert cache_manifest['partitions']['eval']['frames'] == 3000
    ...
    assert len(train.ids) == 5000 and train.ids == record['sample_indices']
    schedule = np.random.default_rng(17).integers(0, 5000, (2000, 32))
    ...
    protocol = dict(seed=17, steps=2000, batch=32, lr=1e-5, weight_decay=1e-4,
                    ...
                    brief_sha256=sha(Path(__file__).with_name('BODY_QUERY_10000_B_PROTOCOL_20260909.md')),
                    ...
                    trained_frames=5000, expected_presentations_per_frame=12.8)
