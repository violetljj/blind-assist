"""Arbitrary manifest inference with frozen MZ5/MZ28/MZ30/MZ35.

Native depth is used only by the ideal multi-surface packet generator. No
current event, query, contributor or knownness labels are read by this runner.
"""
import os
...
from multizone64_observation import observe, EVENT_ORDER
...
def run(root, manifest_path, output):
    ...
    source_names = [Path(__file__).name, 'mz5_ensemble_readout.py', 'mz16_detail_cache.py',
        ...
        'decoupled_model.py', 'whisker_model.py', 'contact_retina_spec.py']
    ...
    try:
        ...
        for row in frames:
            bind(row['rgb_path'], is_frozen=False); bind(row['depth_path'], is_frozen=False)
        ...
        write(output/'start.json', dict(status='STARTED', frames=len(frames), training_steps=0,
            ...
            interpretation='Ideal geometric multi_surface packet; native depth ends at packet synthesis. No event/source/query/known labels from current frames.'))
        pieces = []
        with torch.inference_mode():
            for begin in range(0,len(frames),BATCH_SIZE):
                rows = frames[begin:begin+BATCH_SIZE]; low = []; high = []; depth = []; tick = time.perf_counter()
                for row in rows:
                    ...
                    d = np.load(row['depth_path'], allow_pickle=False)
                    assert d.shape == (360,640) and d.dtype == np.float32
                    depth.append(d)
                low = np.stack(low); high = np.stack(high); depth = np.stack(depth)
                ...
                def upload():
                    return (torch.from_numpy(low).permute(0,3,1,2).cuda().float()/255,
                            ...
                            torch.from_numpy(depth).cuda())
                low_gpu, high_gpu, depth_gpu = timed('upload_cuda',upload)
                packet = timed('packet_cuda',lambda: observe(depth_gpu,readout='multi_surface'))
        ...
        write(output/'result.json',dict(frames=len(frames),training_steps=0,event_order=EVENT_ORDER,
            ...
            positive_bits={name:(arrays[name]>=0).sum(0).tolist() for name in ['MZ5','MZ28','MZ30','MZ35']},
            evaluator_labels_read=False,bank_rebuilt=False,thresholds_calibrated=False,
            limits='No event accuracy is computed. Ideal clean geometric packet is not physical device emulation; absent evidence is not CLEAR.'))
        write(output/'receipt.json',dict(status='PASS',training_steps=0,backend='CUDA frozen raw-RGB and ideal-packet inference',
...
if __name__ == '__main__':
    ...
    args=parser.parse_args();run(args.root,args.manifest,args.output)
