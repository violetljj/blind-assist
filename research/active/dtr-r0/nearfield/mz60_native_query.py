"""MZ60 native-witness positive query objective; unchanged predictor and source."""
import argparse
import gc
...
from mz60_native_query_loss import loss_for
...
def load_heads(root, bind):
    ...
    assert identities['WITNESS'] == prior59['initial_parameter_sha256']['DIVERSE']
    assert identities['WITNESS'] == prior59['initial_parameter_sha256']['CONTROL']
...
def fit(model, arm, bundle, features, mean, std, out):
    ...
    return dict(steps=600, seconds=seconds, history=history, native_presentations=4800,
                mz55_presentations=2400, old_negative_presentations=4800)
            ...
            for begin in range(0, len(data['ranges']), 16):
                ...
                for profile in CONDITIONS:
                    ...
                    for arm, model in models.items():
                        prefix = cohort + '/' + profile + '/MZ60/' + arm + '/'
...
def run(root, task):
    from mz60_source import prepare, FeatureStore
    out = task / 'run-v1'
    ...
    try:
        ...
        for name in ('MZ60_NATIVE_QUERY_20260911.md', 'mz60_native_query.py', 'mz60_source.py', 'mz60_native_query_loss.py',
        ...
        for eid in ('mz56-global-anchor-20260911', 'mz57-complementary-scale-union-20260911',
                    'mz58-diverse-transfer-20260911', 'mz59-training-diversity-20260911'):
        ...
        write(out / 'start.json', dict(status='STARTED', arms=ARMS, steps_per_arm=600, total_steps=600,
        ...
        for cohort in COHORTS:
            for profile in CONDITIONS:
                ...
                if old_key in p:
                    ...
                    p[old_key] = old_union
                for arm in ARMS:
                    p[prefix+'MZ60/'+arm+'/UNION'] = np.maximum(old_union, p[prefix+'MZ60/'+arm+'/candidate'])
        for name in ('crop_mask', 'sensor_coverage', 'rays'):
            p['mz60/' + name] = getattr(models['WITNESS'], name).cpu().numpy()
        np.savez_compressed(out / 'predictions.npz', **p)
        bind.check()
        stats = dict(features.stats)
        receipt = dict(status='PASS', inputs=bind.inputs, arms=ARMS, steps_per_arm=600, total_steps=600,
            fits=fits, source_info=bundle['source_info'], primary_definition=definition, positive_pool='known_native_witness',
            ...
            mz55_calibration_rows_used=0, new_cutoffs=1, threshold_searches=0,
            ...
            backend='CUDA frozen RGB encoder and one native-query head; CPU source I/O', device=torch.cuda.get_device_name(),
