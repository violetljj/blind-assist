"""Source-only native labels, paired checks and lossless lightweight training source."""
import argparse
import hashlib
import itertools
import json
...
def build(capture, output, runtime, dependencies):
    ...
    with torch.inference_mode():
        for i, (case, row) in enumerate(zip(cases, verified_world['rows'])):
            ...
            role = 'HELDOUT_SITE' if case['site_id'] in HOLDOUT else 'TRAIN_CANDIDATE'
    ...
    write(package/'model/predictor.json', dict(schema='mz48-observable-input-v1', frames=len(cases),
    ...
    write(package/'evaluator/metadata.json', dict(schema='mz48-training-source-v1', records=records, pairs=pairs,
    ...
    result=dict(status='PASS_SOURCE_CHECKS',frames=len(cases),source_valid_frames=sum(r['source_valid'] for r in records),
        ...
        roles={role:sum(r['role']==role for r in records) for role in ('TRAIN_CANDIDATE','HELDOUT_SITE')},
