"""One fixed-budget 10k-B fit updating only query_readout."""
import os
...
def evaluate(a,out):
    ...
    for arm,ckpt in checkpoints.items():
        model=BodyQueryModel(a.pretrained,'B').cuda(); model.load_state_dict(torch.load(ckpt,map_location='cuda',weights_only=True))
...
def run(a):
    ...
    torch.manual_seed(17); torch.cuda.manual_seed_all(17); model=BodyQueryModel(a.pretrained,'B').cuda(); model.load_state_dict(torch.load(a.initial,map_location='cpu',weights_only=True)); initial=digest(model.state_dict())
    ...
    protocol=dict(seed=17,steps=2000,batch=32,lr=1e-5,weight_decay=1e-4,trainable='query_readout only',frozen='all other parameters and BN buffers',loss='original near + .25 support + .25 count',cache_sha256=sha(a.cache/'manifest.json'),initial_sha256=sha(a.initial),old_sha256=sha(a.old),initial_digest=initial,schedule_sha256=sha(out/'schedule.npy'),source_sha256=sha(Path(__file__)),dependency_sha256={n:sha(Path(__file__).with_name(n)) for n in ('body_query_model.py','body_query_data.py','body_query_train.py','city_data.py','city_dev_selection.py','decoupled_model.py','representation_model.py')},torch=torch.__version__,device=torch.cuda.get_device_name())
    ...
    try:
        for step,indices in enumerate(schedule,1):
            ids=torch.from_numpy(indices).cuda(); n,s,c=model(rgb[ids].permute(0,3,1,2).float()/255.); ln=F.binary_cross_entropy_with_logits(n,targets['near'][ids].float()); ls=pixel_support_bce(s,targets['support'][ids]); lc=-c.log_softmax(-1).gather(-1,targets['counts'][ids].long().unsqueeze(-1)).mean(); loss=ln+.25*ls+.25*lc
            ...
            if step%100==0: row=dict(step=step,near=float(ln.detach()),support=float(ls.detach()),query=float(lc.detach()),loss=float(loss.detach())); history.append(row); write(out/'progress.json',row); print(row,flush=True)
        ...
        torch.save(model.state_dict(),out/'NEW-step2000.pt'); write(out/'fit-complete.json',dict(steps=2000,seconds=time.perf_counter()-fit_start,history=history,gradient_history=gradient_history,initial_digest=initial,checkpoint_sha256=sha(out/'NEW-step2000.pt'),trainable='query_readout only',frozen_parameters_verified=True,buffers_verified=True))
