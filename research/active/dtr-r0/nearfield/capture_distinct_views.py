"""Unreal-only independent RGB/native-depth views; never save the frozen map."""
import array
import hashlib
import json
import math
...
def tick(delta):
    ...
    try:
        ...
        if stage == 2:
            ...
            frames.append({'sample_index': index, 'case_name': cases[index]['name'], 'time_s': index,
            ...
            if index == len(cases):
                write(OUT / 'model/sensor_manifest.json', {
                    'schema': 'nearfield-distinct-views-v1',
                    'calibration': {'width': 640, 'height': 360, 'horizontal_fov_degrees': 100., 'depth_max_m': 100.},
                    'sampling': 'INDEPENDENT_FIXED_VIEWS_NOT_TEMPORAL_SEQUENCE',
                    'authority': 'SENSOR_ONLY_RGB_FORWARD_DEPTH_AND_EGO_POSES', 'frames': frames})
                finish()
            else:
                after = 0
    except Exception:
