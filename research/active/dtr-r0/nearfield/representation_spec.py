"""G12 fresh TEST quartets; unchanged G10 narrow/diverse parameter ranges."""
import argparse
import json
...
def specification():
    preflight=False
    rng=random.Random(12080908);clips=[];cases=[];samples=[]
    count=32
    for i in range(count):
        split='test'
        stratum='narrow' if i%2==0 else 'diverse'
        if stratum=='narrow':
    ...
    return dict(schema='nf-g12-representation-spec-v1',map_sha256=MAP_SHA,clips=clips,cases=cases,samples=samples,
