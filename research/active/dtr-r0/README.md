...
- **JRDB/public line:** X21 is the frozen Development reference:
  ...
  is not source-disjoint confirmation; the unchanged arm still requires a new
  admissible source.
- **CARLA algorithm line:** X24 met its same-source C2 Development gate. X26
  and the X27--X30 occupancy-lineage successor did not meet their frozen gates;
  X31 is the frozen next algorithm representation.
- **CARLA source line:** C8, C9, and C10 are terminal `SOURCE_NOT_EVALUABLE`
  results and contain no X31 metric result. C11 is frozen for one fresh
  solid-body source capture but is `NOT_RUN`; no result is claimed.
