"""Replay X7 with track-carried X13 component ancestry.

Raw X13 birth cells inside a live YOLO11 mask are the only state origin.  A
state retains its last cell row and moves it one frame with frozen X14
ego/velocity transport.  The transported row survives only where its anchor
is still inside the same live current track mask.  It never absorbs a current
X7 cell.  Raw X13 births may refresh state; image, pose, track-ID, or mask loss
drops it immediately.  Emitted rows use unchanged X14 0.50-second transport
and unchanged X3 route lifecycle/scoring.
"""
...
import dtr_x20_component_track_gap_closure_replay as x20  # noqa: E402
...
LEDGER_SCHEMA = "blindassist-dtr-x21-track-carried-component-ancestry-ledger-v1"
PREDICTION_SCHEMA = "blindassist-dtr-x21-track-carried-component-ancestry-predictions-v1"
FREEZE_SCHEMA = "blindassist-dtr-x21-track-carried-component-ancestry-freeze-v1"
MATERIALIZATION_SCHEMA = "blindassist-dtr-x21-track-carried-component-ancestry-materialization-v1"
SEQUENCE_SCHEMA = "blindassist-dtr-x21-sequence-materialization-v1"
PROGRESS_SCHEMA = "blindassist-dtr-x21-progress-v1"
TIMELINE_FRAMES = x20.TIMELINE_FRAMES
ARM = "X21_TRACK_CARRIED_COMPONENT_ANCESTRY"
StateKey = tuple[int, str, int]
CellRow = tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]
...
def _baseline_rows(args: argparse.Namespace) -> dict[str, Any]:
    return x20._baseline_rows(args)


def _sealed_x7(args: argparse.Namespace, sequence: str) -> tuple[Path, Path, dict[str, Any]]:
    return x20._sealed_x7(args, sequence)


def _fingerprint(args: argparse.Namespace) -> dict[str, Any]:
    require(sha256_file(args.model.resolve(strict=True)) == x17.MODEL_SHA256, "x21_model_hash")
    sources = {}
    for sequence in sorted(_baseline_rows(args)):
        ...
        require(value["source"]["bag_sha256"] == sha256_file(bag), f"x21_bag_hash:{sequence}")
        sources[sequence] = {
    ...
    return {
        ...
        "algorithm_files": [
            ...
            for path in (__file__, x20.__file__, x19.__file__, x13.__file__, x17.__file__, x18.__file__, x14.__file__, x3.__file__)
        ],
        "source_config": {
            "candidate_base": "SEALED_X7_RAW_X13_BIRTHS_ONLY",
            "ancestry_origin": "RAW_X13_BIRTH_CELL_INSIDE_CURRENT_TRACK_MASK",
            "ancestry_key": "CLASS_ID_TRACK_ID_ORIGIN_X7_COMPONENT_ID",
            "state_payload": "LAST_FRAME_CELL_ROW",
            "one_step_rule": "UNCHANGED_X14_EGO_AND_CELL_VELOCITY_TRANSPORT",
            "survival_rule": "TRANSPORTED_ANCHOR_INSIDE_SAME_LIVE_CURRENT_TRACK_MASK",
            "forbidden": "NO_CURRENT_X7_CELL_ABSORPTION",
            "drop_policy": "IMMEDIATE_ON_IMAGE_POSE_TRACK_ID_OR_CURRENT_MASK_BREAK",
            "provider": "UNCHANGED_X20_X19_X17_YOLO11_FIVE_TILES_PER_CLASS_TRACKERS",
        ...
        "inputs": {
            ...
            "x20_result_sha256": sha256_file(args.x20_result.resolve(strict=True)),
...
def prepare(args: argparse.Namespace) -> dict[str, Any]:
    ...
    if freeze.exists():
        require(json.loads(freeze.read_text(encoding="utf-8")) == value, "x21_freeze_drift")
    else:
...
def _select(row: CellRow, keep: np.ndarray) -> CellRow:
    return tuple(column[keep] for column in row)  # type: ignore[return-value]


def _merge(rows: Sequence[CellRow]) -> CellRow:
    if not rows:
        return x20._empty_row()
    return tuple(np.concatenate([row[column] for row in rows], axis=0) for column in range(4))  # type: ignore[return-value]
...
def _stream_births(
    *,
    ...
    progress_path: Path,
) -> tuple[list[CellRow], list[dict[str, Any]], dict[str, Any]]:
    try:
        from rosbags.rosbag1 import Reader
        from rosbags.typesys import Stores, get_types_from_msg, get_typestore
    except ImportError as error:
        raise RuntimeError("dtr_x21 requires rosbags") from error
    ...
    require(len(by_stamp) == len(target_ns), "x21_duplicate_frame_timestamp")
    ordered = sorted(by_stamp)
    rows = [x5._frame_arrays(source, frame) for frame in frames]
    component_rows = [x20._component_ids(source, frame) for frame in frames]
    ...
    for frame, row, component_ids in zip(frames, rows, component_rows):
        require(len(row[0]) == len(component_ids), f"x21_component_alignment:{frame}")

    births = [x20._empty_row() for _ in frames]
    diagnostics = [
        {
            "frame": frame,
    ...
    states: dict[StateKey, CellRow] = {}
    previous: tuple[int, np.ndarray] | None = None
    seen: set[int] = set()
    matched = inferred = invalid = gaps = pose_breaks = track_or_mask_drops = 0

    with Reader(bag_path) as reader:
        connections = [item for item in reader.connections if item.topic.lstrip("/") == x8.STITCHED_TOPIC]
        require(len(connections) == 1, "x21_stitched_topic_missing")
        ...
        for connection, _bag_time, raw in reader.messages(connections=connections):
            ...
            refreshed: dict[StateKey, CellRow] = {}
            for track_key, mask in mask_by_track.items():
                ...
                for component_id in np.unique(component_ids[birth_indices]):
                    component_births = birth_indices[component_ids[birth_indices] == component_id]
                    key = (track_key[0], track_key[1], int(component_id))
                    refreshed[key] = _select(current, component_births)

            next_states = dict(refreshed)
            emitted: list[CellRow] = list(refreshed.values())
            transported_states = transported_cells = 0
            for key, last_row in states.items():
                if key in refreshed:
                    continue
                mask = mask_by_track.get(key[:2])
                if mask is None:
                    track_or_mask_drops += 1
                    continue
                propagated = x14._transport(
                    last_row,
                    source_pose=poses[frame - 1],
                    target_pose=poses[frame],
                    delta_s=timestamps[frame] - timestamps[frame - 1],
                )
                membership, _hits = x18._cell_membership(propagated, pose=poses[frame], masks=[mask], calibration=calibration)
                keep = np.flatnonzero(membership)
                if not len(keep):
                    track_or_mask_drops += 1
                    continue
                carried = _select(propagated, keep)
                next_states[key] = carried
                emitted.append(carried)
                transported_states += 1
                transported_cells += len(carried[0])

            states = next_states
            output = _merge(emitted)
            births[index] = output
            diagnostics[index].update(
                {
                    "x13_birth_cells": int(np.count_nonzero(raw_birth)),
                    "visible_tracks": len(masks),
                    "ancestry_states": len(states),
                    "created_or_refreshed_states": len(refreshed),
                    "transported_states": transported_states,
                    "transported_cells": transported_cells,
                    "authorized_cells": len(output[0]),
                }
            )
            previous = (frame, image)
            if inferred % 100 == 0:
                write_json(
                    progress_path,
                    {
                        ...
                        "stage": "TRACK_CARRIED_COMPONENT_ANCESTRY",
    ...
    return births, diagnostics, {
        ...
        "track_or_mask_drops": track_or_mask_drops,
    }


def materialize_sequence(args: argparse.Namespace) -> dict[str, Any]:
    require(args.sequence is not None, "x21_sequence_required")
    root = args.root.resolve()
    freeze = _paths(root)["freeze"].resolve(strict=True)
    require(json.loads(freeze.read_text(encoding="utf-8")) == _fingerprint(args), "x21_freeze_drift")
    require(args.sequence in _baseline_rows(args), f"x21_unknown_sequence:{args.sequence}")
    ...
    try:
        ...
        manifest = {
            ...
            "state_rule": "LAST_CELL_ROW_ONE_STEP_X14_TRANSPORT",
            "survival_rule": "TRANSPORTED_ANCHOR_INSIDE_SAME_LIVE_CURRENT_TRACK_MASK",
            "forbidden": "NO_CURRENT_X7_CELL_ABSORPTION",
            "drop_rule": "IMMEDIATE_ON_IMAGE_POSE_TRACK_ID_OR_MASK_BREAK",
            ...
            "diagnostics": {
                ...
                "transported_cells": int(sum(row["transported_cells"] for row in diagnostics)),
                ...
                "transported_states": int(sum(row["transported_states"] for row in diagnostics)),
                "frames": diagnostics,
            },
...
def assemble(args: argparse.Namespace) -> dict[str, Any]:
    ...
    require(json.loads(freeze.read_text(encoding="utf-8")) == _fingerprint(args), "x21_freeze_drift")
    receipts = []
    frames = inputs = x13_births = authorized = transported = continued = 0
    for sequence in sorted(_baseline_rows(args)):
        paths = _paths(root, sequence)
        manifest = json.loads(paths["manifest"].resolve(strict=True).read_text(encoding="utf-8"))
        require(manifest.get("schema") == LEDGER_SCHEMA and manifest.get("ledger_sha256") == sha256_file(paths["ledger"].resolve(strict=True)), f"x21_manifest:{sequence}")
...
def predict(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result["prediction_boundary"] = "sealed X21 raw-X13-origin track-carried cell ancestry plus unchanged X14/X3 lifecycle; no current-X7 absorption or labels"
...
def score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result["status"] = "DTR_X21_TRACK_CARRIED_COMPONENT_ANCESTRY_GATE_MET" if met else "DTR_X21_TRACK_CARRIED_COMPONENT_ANCESTRY_GATE_NOT_MET"
    result["metrics"][ARM] = result["metrics"].pop("X3_LAG_FLOXEL")
    result["decision"]["next"] = "FREEZE_X21_AND_CONFIRM_SOURCE_DISJOINT" if met else "CLOSE_TRACK_CARRIED_COMPONENT_ANCESTRY_WITHOUT_SWEEP"
    result["evidence_boundary"] = [
        "Raw X13 birth cells inside a live track mask are the only X21 ancestry origin or refresh.",
        "Gap continuation transports only the state's prior cell row and retains cells whose transported anchors remain in the same current track mask; no current X7 cell is absorbed.",
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--root", type=Path, default=REPO / "artifacts.local" / "evidence" / "dtr-x21" / "track-carried-component-ancestry-replay-20260829")
    ...
    parser.add_argument("--x20-result", type=Path, default=REPO / "artifacts.local" / "evidence" / "dtr-x20" / "component-track-gap-closure-replay-20260829" / "result.json")
