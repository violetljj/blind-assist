"""Replay X72 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x71_consumed_development as base71  # noqa: E402


runner = base71.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x72-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x72.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X72],
        "episodes": episodes,
        "fixed_constants": x72.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X72]
    return value
...
def main() -> int:
    ...
    parser.add_argument("--x71-predictions", type=Path, required=True)
    ...
    x71_path = args.x71_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    rigid_path = output_dir / "predictions-x25-rigid.json"
    predictions_path = output_dir / "predictions-x72.json"
    ...
    runner.base.require(
        ...
        "x72_consumed_source_protocol",
    )
    _freeze, contract, candidate_values = x24.require_freeze(run_root)
    x71_predictions = runner.base.read_json(x71_path)
    runner.base.require(
        x71_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x72_x71_prediction_contract",
    )
    ...
    for episode in contract.episodes:
        ...
        cursor += count
    runner.base.require(cursor == len(candidate_values), "x72_candidate_cursor")
    runner.base.write_json_exclusive(
        rigid_path, base71.base70.rigid_envelope(args.cohort_id, rigid_episodes)
    )

    episodes = {
        episode.episode_id: x72.apply_credentialed_surface_boundary_completion_episode(
            x71_predictions["episodes"][episode.episode_id],
            rigid_episodes[episode.episode_id],
        )
        for episode in contract.episodes
    }
    runner.base.require(set(episodes) == set(runner.EPISODES), "x72_episode_set")
    ...
    envelopes = {ARM_X71: x71_predictions, ARM_X72: predictions}
    predictions_full = {
        arm: {
    ...
    x71_metrics = aggregate[ARM_X71]
    x72_metrics = aggregate[ARM_X72]
    delta = base71.base70.effect(x72_metrics, x71_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X72]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X72]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    diagnostic_keys = (
        "x72_surface_credential_births",
        "x72_surface_credential_release_clears",
        "x72_boundary_completion_frames",
        "x72_boundary_completion_tracks",
        "x72_center_contained_rejections",
        "x72_overlap_absent_rejections",
    )
    mechanism_counts = {
        key.removeprefix("x72_"): sum(
            int(value["diagnostics"].get(key, 0)) for value in episodes.values()
        )
        for key in diagnostic_keys
    }
    reference_checks = {
        "precision_at_least_0_85": float(x72_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x72_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x72_metrics["f1"]) >= 0.78,
    ...
    exercised = mechanism_counts["boundary_completion_frames"] > 0
    if exercised and delta["tp_delta"] > 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X72_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X72_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X72_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X72_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x72-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x71_predictions_sha256": runner.base.sha256_file(x71_path),
            "x72_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x25_predictor_sha256": runner.base.sha256_file(Path(x25.__file__).resolve()),
            "x72_predictor_sha256": runner.base.sha256_file(Path(x72.__file__).resolve()),
