"""Replay X92 against X91 on one consumed CARLA cohort."""
...
import run_dtr_carla_x91_consumed_development as base91  # noqa: E402


runner = base91.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x92-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x92.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X92],
        "episodes": episodes,
        "fixed_constants": x92.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x91-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x91_path = args.x91_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x92.json"
    ...
    runner.base.require(
        ...
        "x92_consumed_source_protocol",
    )
    x91_predictions = runner.base.read_json(x91_path)
    runner.base.require(
        str(x91_predictions.get("status", "")).startswith("SEALED_")
        and set(x91_predictions.get("arms", [])) == {ARM_X91},
        "x92_x91_prediction_contract",
    )
    runner.base.require(
        set(x91_predictions["episodes"]) == set(runner.EPISODES),
        "x92_episode_set",
    )

    episodes = {
        episode_id: x92.apply_held_risk_birth_horizon_latch_episode(
            x91_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X91: x91_predictions, ARM_X92: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x91_predictions, ARM_X91)
    compatible = alias(predictions, ARM_X92)
    ...
    delta = effect(x92_metrics, x91_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X92]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X92]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x92_held_risk_birth_horizon_latch_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x92_held_risk_birth_horizon_latched_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x92_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x92_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x92_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X92_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X92_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X92_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x92-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x91_predictions_sha256": runner.base.sha256_file(x91_path),
            "x92_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x92_predictor_sha256": runner.base.sha256_file(Path(x92.__file__).resolve()),
