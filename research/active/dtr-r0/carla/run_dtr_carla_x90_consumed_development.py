"""Replay X90 against X89 on one consumed CARLA cohort."""
...
import run_dtr_carla_x89_consumed_development as base89  # noqa: E402


runner = base89.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x90-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x90.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X90],
        "episodes": episodes,
        "fixed_constants": x90.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x89-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x89_path = args.x89_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x90.json"
    ...
    runner.base.require(
        ...
        "x90_consumed_source_protocol",
    )
    x89_predictions = runner.base.read_json(x89_path)
    runner.base.require(
        str(x89_predictions.get("status", "")).startswith("SEALED_")
        and set(x89_predictions.get("arms", [])) == {ARM_X89},
        "x90_x89_prediction_contract",
    )
    runner.base.require(
        set(x89_predictions["episodes"]) == set(runner.EPISODES),
        "x90_episode_set",
    )

    episodes = {
        episode_id: x90.apply_collision_credentialed_lateral_dominant_release_episode(
            x89_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X89: x89_predictions, ARM_X90: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x89_predictions, ARM_X89)
    compatible = alias(predictions, ARM_X90)
    ...
    x89_metrics = aggregate[ARM_X89]
    x90_metrics = aggregate[ARM_X90]
    delta = effect(x90_metrics, x89_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X90]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X90]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x90_collision_credentialed_lateral_dominant_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x90_collision_credentialed_lateral_dominant_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x90_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x90_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x90_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X90_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X90_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X90_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x90-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x89_predictions_sha256": runner.base.sha256_file(x89_path),
            "x90_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x90_predictor_sha256": runner.base.sha256_file(Path(x90.__file__).resolve()),
