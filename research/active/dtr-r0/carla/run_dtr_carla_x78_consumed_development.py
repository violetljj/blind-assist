"""Replay X78 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x77_consumed_development as base77  # noqa: E402


runner = base77.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x78-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x78.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X78],
        "episodes": episodes,
        "fixed_constants": x78.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X78]
    return value
...
def main() -> int:
    ...
    parser.add_argument("--x77-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x77_path = args.x77_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x78.json"
    ...
    runner.base.require(
        ...
        "x78_consumed_source_protocol",
    )
    x77_predictions = runner.base.read_json(x77_path)
    runner.base.require(
        x77_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x78_x77_prediction_contract",
    )
    runner.base.require(
        set(x77_predictions["episodes"]) == set(runner.EPISODES),
        "x78_episode_set",
    )

    episodes = {
        episode_id: x78.apply_nonclosing_zero_shift_permanence_release_episode(
            x77_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X77: x77_predictions, ARM_X78: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base77.base76.base75.base74.base73.base72.base71.base70.effect(
        x78_metrics, x77_metrics
    )
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X78]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X78]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(value["diagnostics"].get(
                "x78_nonclosing_zero_shift_permanence_release_frames", 0
            ))
            for value in episodes.values()
        ),
        "released_tracks": sum(
            int(value["diagnostics"].get(
                "x78_nonclosing_zero_shift_permanence_released_tracks", 0
            ))
            for value in episodes.values()
        ),
    }
    reference_checks = {
        "precision_at_least_0_85": float(x78_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x78_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x78_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X78_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X78_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X78_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X78_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x78-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x78_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x78_predictor_sha256": runner.base.sha256_file(Path(x78.__file__).resolve()),
