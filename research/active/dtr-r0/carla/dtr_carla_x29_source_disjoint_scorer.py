"""Score the frozen C6 source-disjoint X24/X29 confirmation replay.

The protocol, X29 predictor identity, prediction envelopes, and all decision
...
X29_SCHEMA = "blindassist-dtr-carla-x29-temporal-lineage-predictions-v1"
X29_FREEZE_SCHEMA = "blindassist-dtr-carla-x29-temporal-lineage-freeze-v1"
ARM_X24 = "X24_ISSUED_PLAN_ADHERENCE"
ARM_X29 = "X29_ISSUED_PLAN_TEMPORAL_OCCUPANCY_LINEAGE"
EXPECTED_PROTOCOL_SCHEMA = 6
EXPECTED_COHORT_ID = "DTR_CARLA_C6_X29_SOURCE_DISJOINT_CONFIRMATION_V1"
EXPECTED_PROTOCOL_SHA256 = (
    "0968D746804BA5E99C16CE676B77682C9A0B1A3B08E863EE320C9F31A529FCB1"
)
EXPECTED_PARENT_COHORT_ID = "DTR_CARLA_C5_DYNAMIC_OCCUPANCY_FRESH_DEVELOPMENT_V1"
EXPECTED_PARENT_PROTOCOL_CANONICAL_SHA256 = (
    "878EEED5E79BDB0ADB4A7E6E93A51E6EC5AAEA6FDA11AF80E2B3F98CA583535A"
)
EXPECTED_X29_PREDICTOR_SHA256 = (
    "F04D0431FF3FE86589411838C28DE0E94E37AC64B6FF45A56D80D35B244C59F5"
)
PRE_SOURCE_SCORER_SHA256 = (
    "FD710EEA015E5904133656CA347C4EED801CF51B6641446396DB668184B729A3"
)
BINDING_CORRECTION = (
    "COMPARE_FREEZE_TO_MODEL_MANIFEST_FILE_HASH_NOT_SEALED_MODEL_FILE_LIST_HASH"
)
...
EXPECTED_DYNAMIC_PAIRS = [
    {
        ...
        "target_blueprint": "vehicle.lincoln.mkz_2020",
    ...
    },
    {
        ...
        "target_blueprint": "walker.pedestrian.0031",
        "pair_difference": "TARGET_LATERAL_OFFSET_ONLY",
    },
    {
        "kind": "motorcycle",
        ...
        "target_blueprint": "vehicle.yamaha.yzf",
        "pair_difference": "TARGET_LATERAL_OFFSET_ONLY",
    },
]
EXPECTED_CAPTURE_SEED = 96317
...
def validate_protocol(protocol: Mapping[str, Any], protocol_path: Path) -> None:
    base.require(
        ...
        "c6_protocol_hash_not_frozen",
    )
    base.require(
        protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA,
        "c6_protocol_schema",
    )
    base.require(protocol.get("cohort_id") == EXPECTED_COHORT_ID, "c6_cohort")
    base.require(
        int(protocol["capture"]["seed"]) == EXPECTED_CAPTURE_SEED,
        "c6_capture_seed",
    )
    layouts = protocol["layouts"]
    base.require(tuple(layouts) == tuple(EXPECTED_WEATHER_BY_LAYOUT), "c6_layouts")
    base.require(
    ...
    base.require(tuple(scenarios) == EPISODES, "c6_episode_order")
    base.require(
        tuple(key for key in EPISODES if scenarios[key]["expected_outcome"] == "CONTACT")
        == CONTACT_EPISODES,
        "c6_contact_scenarios",
    )
    base.require(
    ...
    base.require(
        ...
        "c6_future_truth_rule",
    )
    base.require(
        contract["score_window_end_seconds"] == SCORE_WINDOW_END_SECONDS,
        "c6_score_windows",
    )
    base.require(tuple(contract["contact_episodes"]) == CONTACT_EPISODES, "c6_contacts")
    base.require(tuple(contract["safe_episodes"]) == SAFE_EPISODES, "c6_safe")
    base.require(
    ...
    base.require(
        ...
        "c6_retained_occlusion",
    )
    base.require(
        contract["safe_segment_start_seconds"] == SAFE_SEGMENT_START_SECONDS,
        "c6_safe_boundaries",
    )
    base.require(contract["dynamic_pairs"] == EXPECTED_DYNAMIC_PAIRS, "c6_dynamic_pairs")
    base.require(
    ...
    base.require(
        ...
        == {"vehicle.mercedes.sprinter", "walker.pedestrian.0024"},
        "c6_parent_targets_excluded",
    )
...
def validate_prediction_envelopes(
    ...
    freeze_x29: Mapping[str, Any],
    run_root: Path,
) -> None:
    base.require(
        x24.get("schema") == X24_SCHEMA and x29.get("schema") == X29_SCHEMA,
        "prediction_schema",
    )
    base.require(
        x24.get("status") == PREDICTION_STATUS
        and x29.get("status") == PREDICTION_STATUS,
        "prediction_status",
    )
    base.require(
        x24.get("truth_blind") is True and x29.get("truth_blind") is True,
        "prediction_truth_blind",
    )
    base.require(
        x24.get("claim_boundary", {}).get("evaluator_opened") is False
        and x29.get("claim_boundary", {}).get("evaluator_opened") is False,
        "prediction_evaluator_opened",
    )
    base.require(
        x24.get("claim_boundary", {}).get("current_actor_oracle_used") is False
        and x29.get("claim_boundary", {}).get("current_actor_oracle_used") is False,
        "prediction_actor_oracle",
    )
    base.require(
        ARM_X24 in x24["arms"] and x29["arms"] == [ARM_X29],
        "prediction_arms",
    )
    base.require(
        tuple(x24["episodes"]) == EPISODES and tuple(x29["episodes"]) == EPISODES,
        "prediction_episodes",
    )

    base.require(freeze_x24.get("schema") == X24_FREEZE_SCHEMA, "x24_freeze_schema")
    base.require(freeze_x29.get("schema") == X29_FREEZE_SCHEMA, "x29_freeze_schema")
    base.require(
        freeze_x24.get("status") == FREEZE_STATUS
        and freeze_x29.get("status") == FREEZE_STATUS,
        "prediction_freeze_status",
    )
    base.require(
        freeze_x24.get("truth_blind") is True
        and freeze_x29.get("truth_blind") is True,
    ...
    x29_freeze_hash = base.sha256_file(run_root / "freeze-x29.json")
    base.require(
        x24["source"]["freeze_sha256"] == x24_freeze_hash,
        "x24_prediction_freeze_drift",
    )
    base.require(
        x29["source"]["freeze_sha256"] == x29_freeze_hash,
        "x29_prediction_freeze_drift",
    )
    base.require(
        freeze_x29["source"]["x24_freeze_sha256"] == x24_freeze_hash,
        "x29_x24_freeze_drift",
    )
    base.require(
        freeze_x29["source"]["x24_predictions_sha256"] == x24_predictions_hash
        and x29["source"]["x24_predictions_sha256"] == x24_predictions_hash,
        "x29_x24_prediction_drift",
    )
    base.require(
        freeze_x29["algorithm_files"]["x29_predictor"]["sha256"]
        == EXPECTED_X29_PREDICTOR_SHA256,
        "x29_predictor_not_frozen",
    )
    base.require(
        freeze_x29["fixed_constants"] == x29["fixed_constants"],
        "x29_constants_drift",
    )
    base.require(
        int(freeze_x24["episodes"]) == len(EPISODES)
        and int(freeze_x29["episodes"]) == len(EPISODES),
...
def render_svg(result: Mapping[str, Any]) -> str:
    ...
    x29 = aggregate[ARM_X29]
    contacts = result["contacts"]
    safe_segments = sum(
        result["safe"][key][ARM_X29]["false_alert_segment_count"]
    ...
    for y, episode_id, label in (
        ...
        metric = contacts[episode_id][ARM_X29]
...
<text x="75" y="105" font-family="Segoe UI,sans-serif" font-size="42" font-weight="700" fill="#f8fafc">X29 temporal occupancy lineage · CARLA C6</text>
<text x="75" y="158" font-family="Segoe UI,sans-serif" font-size="23" fill="{accent}">{decision}</text>
<text x="75" y="230" font-family="Segoe UI,sans-serif" font-size="24" fill="#93c5fd">Source-disjoint confirmation · uncensored scored-window metrics</text>
...
<text x="75" y="334" font-family="Segoe UI,sans-serif" font-size="35" font-weight="700" fill="{accent}">X29 F1 {x29['f1']:.3f} · precision {x29['precision']:.3f} · recall {x29['recall']:.3f}</text>
...
<text x="75" y="745" font-family="Segoe UI,sans-serif" font-size="31" fill="#f8fafc">Original occlusion coverage: X24 {result['occlusion'][ARM_X24]['coverage']:.3f} · X29 {result['occlusion'][ARM_X29]['coverage']:.3f}</text>
...
def score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    result_path = run_root / "result-x29.json"
    svg_path = run_root / "result-x29.svg"
    base.require(
        not result_path.exists() and not svg_path.exists(),
        "x29_score_outputs_exist",
    )

    protocol = base.read_json(protocol_path)
    validate_protocol(protocol, protocol_path)
    predictor_path = HERE / "dtr_carla_x29_temporal_occupancy_lineage_predictor.py"
    base.require(
        base.sha256_file(predictor_path) == EXPECTED_X29_PREDICTOR_SHA256,
        "x29_predictor_source_drift",
    )

    x24 = base.read_json(run_root / "predictions-x24.json")
    x29 = base.read_json(run_root / "predictions-x29.json")
    freeze_x24 = base.read_json(run_root / "freeze-x24.json")
    freeze_x29 = base.read_json(run_root / "freeze-x29.json")
    validate_prediction_envelopes(x24, x29, freeze_x24, freeze_x29, run_root)
    ...
    base.require(
        ...
        == freeze_x29["source"]["model_manifest_sha256"],
    ...
    predictions_full = {
        ...
        ARM_X29: {
            episode_id: arm_frames_full(x29, episode_id, ARM_X29)
            for episode_id in EPISODES
        },
    }
    ...
    invariants = authority_invariants(x29, score_end)
    safe_segments = sum(
        safe[key][ARM_X29]["false_alert_segment_count"] for key in SAFE_EPISODES
    )
    dynamic_safe_segments = sum(
        safe[key][ARM_X29]["false_alert_segment_count"]
    ...
    gate_checks = {
        ...
        "x29_detects_all_three_dynamic_contacts": all(
            contacts[key][ARM_X29]["event_detected_before_contact"]
            for key in DYNAMIC_CONTACT_EPISODES
        ),
        "each_dynamic_contact_has_at_least_2s_lead": all(
            contacts[key][ARM_X29]["first_alert_lead_seconds"] is not None
            and float(contacts[key][ARM_X29]["first_alert_lead_seconds"])
            + EPSILON
            >= MINIMUM_DYNAMIC_LEAD_SECONDS
            for key in DYNAMIC_CONTACT_EPISODES
        ),
        "each_dynamic_contact_future_positive_recall_at_least_0_80": all(
            contacts[key][ARM_X29]["future_positive_recall"] + EPSILON
            >= MINIMUM_DYNAMIC_CONTACT_RECALL
            for key in DYNAMIC_CONTACT_EPISODES
        ),
        "x29_has_zero_adjudicated_safe_risk_segments": safe_segments == 0,
        "x29_has_zero_dynamic_safe_risk_segments": dynamic_safe_segments == 0,
        "x29_aggregate_precision_at_least_0_95": (
            aggregate[ARM_X29]["precision"] + EPSILON
            >= MINIMUM_AGGREGATE_PRECISION
        ),
        "x29_aggregate_f1_at_least_0_80": (
            aggregate[ARM_X29]["f1"] + EPSILON >= MINIMUM_AGGREGATE_F1
        ),
        "x29_frame_f1_exceeds_x24": (
            aggregate[ARM_X29]["f1"] > aggregate[ARM_X24]["f1"] + EPSILON
        ),
        "x29_retains_original_occlusion_coverage": (
            occlusion[ARM_X29]["coverage"] + EPSILON
    ...
    result = {
        "schema": "blindassist-dtr-carla-c6-x29-source-disjoint-score-result-v1",
        "status": "COMPLETE",
        "decision": (
            "DTR_CARLA_X29_TEMPORAL_OCCUPANCY_LINEAGE_SOURCE_DISJOINT_REPLAY_GATE_MET_POST_SOURCE_BINDING_CORRECTION"
            if gate_met
            else "DTR_CARLA_X29_TEMPORAL_OCCUPANCY_LINEAGE_SOURCE_DISJOINT_REPLAY_GATE_NOT_MET_POST_SOURCE_BINDING_CORRECTION"
        ),
        "gate_met": gate_met,
        "gate_checks": gate_checks,
        "scorer_binding_correction": {
            "pre_source_scorer_sha256": PRE_SOURCE_SCORER_SHA256,
            "correction": BINDING_CORRECTION,
            "source_result_opened_before_correction": True,
            "per_frame_evaluator_opened_before_correction": False,
            "threshold_or_gate_logic_changed": False,
        },
        "thresholds": {
            ...
            "x29_f1_must_strictly_exceed_x24": True,
        ...
        "deltas": {
            "frame_f1": aggregate[ARM_X29]["f1"] - aggregate[ARM_X24]["f1"],
            "frame_precision": (
                aggregate[ARM_X29]["precision"]
                - aggregate[ARM_X24]["precision"]
            ),
            "frame_recall": (
                aggregate[ARM_X29]["recall"] - aggregate[ARM_X24]["recall"]
            ),
        ...
        "source": {
            ...
            "x29_predictions_sha256": base.sha256_file(
                run_root / "predictions-x29.json"
            ),
            "x24_freeze_sha256": base.sha256_file(run_root / "freeze-x24.json"),
            "x29_freeze_sha256": base.sha256_file(run_root / "freeze-x29.json"),
            "x29_predictor_sha256": base.sha256_file(predictor_path),
            "scorer_sha256": base.sha256_file(Path(__file__).resolve()),
        },
        "claim_boundary": {
            "fresh_scripted_carla_source_disjoint_confirmation": False,
            "fresh_scripted_carla_source_disjoint_replay": True,
            "pristine_preopened_scorer_identity": False,
            "post_source_binding_correction": BINDING_CORRECTION,
            "retained_occlusion_is_regression_only": True,
            "full_horizon_truth_tail": True,
            "frozen_x29_predictor_sha256": EXPECTED_X29_PREDICTOR_SHA256,
