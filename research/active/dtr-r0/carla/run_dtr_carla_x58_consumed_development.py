"""Replay frozen X58 composition on one consumed CARLA cohort."""
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in aliased["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X58]
    return aliased
...
def main() -> int:
    ...
    predictions_path = output_dir / "predictions-x58.json"
    summary_path = output_dir / "summary.json"

    episodes = {
        episode_id: x58.apply_bidirectional_metric_reliability_router_episode(
    ...
    measured_handbacks = sum(
        int(value["diagnostics"].get("x58_current_measured_metric_handback_frames", 0))
        for value in episodes.values()
    )
    receding_releases = sum(
        int(value["diagnostics"].get("x58_receding_permanence_release_frames", 0))
        for value in episodes.values()
    )
    x24_metrics = aggregate[ARM_X24]
    x54_metrics = aggregate[ARM_X54]
    x58_metrics = aggregate[ARM_X58]
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X58]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    contact_recall = {
        episode_id: float(
            contacts[episode_id][ARM_X58]["future_positive_recall"]
        )
        for episode_id in runner.CONTACT
    }
    ...
    reference_checks = {
        "precision_at_least_0_85": float(x58_metrics["precision"]) + epsilon
        >= 0.85,
        "recall_at_least_0_70": float(x58_metrics["recall"]) + epsilon >= 0.70,
        "f1_at_least_0_78": float(x58_metrics["f1"]) + epsilon >= 0.78,
    ...
    decision = (
        "DTR_CARLA_X58_CONSUMED_DEVELOPMENT_REFERENCE_TARGET_MET"
        if reference_target_met
        else "DTR_CARLA_X58_CONSUMED_DEVELOPMENT_REFERENCE_TARGET_NOT_MET"
    )
    summary = {
        "schema": "blindassist-dtr-carla-x58-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "mechanism_counts": {
            "x58_current_measured_metric_handback_frames": measured_handbacks,
            "x58_receding_permanence_release_frames": receding_releases,
        },
        "x58_effect_vs_x54": effect(x58_metrics, x54_metrics),
        "x58_effect_vs_x24": effect(x58_metrics, x24_metrics),
        ...
        "source": {
            ...
            "x58_predictor_sha256": runner.base.sha256_file(Path(x58.__file__).resolve()),
            "x58_predictions_sha256": runner.base.sha256_file(predictions_path),
            "development_runner_sha256": runner.base.sha256_file(Path(__file__).resolve()),
        },
        "claim_boundary": {
            "consumed_posthoc_development": True,
            "parent_frozen_decision_unchanged": True,
            "reference_thresholds_not_preregistered_for_x58": True,
            "fresh_confirmation": False,
        ...
        json.dumps(
            {
                ...
                "effect_vs_x54": summary["x58_effect_vs_x54"],
                "reference_checks": reference_checks,
