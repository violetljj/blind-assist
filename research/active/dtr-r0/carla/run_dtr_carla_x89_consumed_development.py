"""Replay X89 against X88 on one consumed CARLA cohort."""
...
import run_dtr_carla_x88_consumed_development as base88  # noqa: E402


runner = base88.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x89-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x89.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X89],
        "episodes": episodes,
        "fixed_constants": x89.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x88-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x88_path = args.x88_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x89.json"
    ...
    runner.base.require(
        ...
        "x89_consumed_source_protocol",
    )
    x88_predictions = runner.base.read_json(x88_path)
    runner.base.require(
        str(x88_predictions.get("status", "")).startswith("SEALED_")
        and set(x88_predictions.get("arms", [])) == {ARM_X88},
        "x89_x88_prediction_contract",
    )
    runner.base.require(
        set(x88_predictions["episodes"]) == set(runner.EPISODES),
        "x89_episode_set",
    )

    episodes = {
        episode_id: x89.apply_branch_overloaded_receding_release_episode(
            x88_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X88: x88_predictions, ARM_X89: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x88_predictions, ARM_X88)
    compatible = alias(predictions, ARM_X89)
    ...
    delta = effect(x89_metrics, x88_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X89]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X89]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x89_branch_overloaded_receding_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x89_branch_overloaded_receding_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x89_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x89_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x89_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X89_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X89_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X89_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x89-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x88_predictions_sha256": runner.base.sha256_file(x88_path),
            "x89_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x89_predictor_sha256": runner.base.sha256_file(Path(x89.__file__).resolve()),
