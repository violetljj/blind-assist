"""Replay X81 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x80_consumed_development as base80  # noqa: E402


runner = base80.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x81-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x81.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X81],
        "episodes": episodes,
        "fixed_constants": x81.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x80-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x80_path = args.x80_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x81.json"
    ...
    runner.base.require(
        ...
        "x81_consumed_source_protocol",
    )
    x80_predictions = runner.base.read_json(x80_path)
    runner.base.require(
        x80_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x81_x80_prediction_contract",
    )
    runner.base.require(
        set(x80_predictions["episodes"]) == set(runner.EPISODES),
        "x81_episode_set",
    )

    episodes = {
        episode_id: x81.apply_zero_shift_cross_route_shape_release_episode(
            x80_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X80: x80_predictions, ARM_X81: predictions}
    predictions_full = {
        arm: {
    ...
    delta = effect(x81_metrics, x80_metrics)
    contact_recall = {
    ...
    reference_checks = {
        "precision_at_least_0_85": float(x81_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x81_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x81_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X81_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X81_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X81_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X81_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x81-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x81_predictions_sha256": runner.base.sha256_file(predictions_path),
