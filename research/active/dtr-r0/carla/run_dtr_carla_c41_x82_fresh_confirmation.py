"""Run frozen X81/X82 once on fresh C41, then open and score truth."""
...
import run_dtr_carla_c40_x81_fresh_confirmation as base40  # noqa: E402
...
def prediction_envelope(
    ...
    return {
        "schema": f"blindassist-dtr-carla-c41-{arm.lower()}-fresh-predictions-v1",
...
def main() -> int:
    ...
    runner.base.require(
        ...
        "c41_protocol_hash_drift",
    )
    prereg = protocol["c41_x82_preregistration"]
    runner.base.require(
        prereg["single_x82_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X81", "X82"],
        "c41_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c41_frozen_component_hash_drift:{file_name}",
        )
    ...
    x82_path = output_dir / "predictions-x82.json"
    ...
    for episode in contract.episodes:
        ...
        x82_episodes[episode.episode_id] = core82
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X81 X82", flush=True)
    runner.base.require(cursor == len(candidate_values), "c41_candidate_cursor")

    rigid_predictions = base40.base39.base38.base37.base36.base35.base70.rigid_envelope(
        "C41", rigid_episodes
    )
    x81_predictions = prediction_envelope(
        ARM_X81, x81.EXPERIMENT_ID, x81.fixed_constants(), x81_episodes
    )
    x82_predictions = prediction_envelope(
        ARM_X82, x82.EXPERIMENT_ID, x82.fixed_constants(), x82_episodes
    )
    runner.base.write_json_exclusive(rigid_path, rigid_predictions)
    runner.base.write_json_exclusive(x81_path, x81_predictions)
    runner.base.write_json_exclusive(x82_path, x82_predictions)
    print("sealed_truth_blind X25 X81 X82", flush=True)
    ...
    envelopes = {ARM_X24: x24_predictions, ARM_X81: x81_predictions, ARM_X82: x82_predictions}
    predictions_full = {
        arm: {
    ...
    compatible = alias_for_transport_score(x82_predictions)
    ...
    effect_vs_x81 = effect(x82_metrics, x81_metrics)
    contact_recall = {
    ...
    mechanism_counts = {
        "held_proxy_consensus_release_frames": sum(
            int(value["diagnostics"].get("x82_held_proxy_consensus_release_frames", 0))
            for value in x82_episodes.values()
        ),
        "held_proxy_consensus_released_tracks": sum(
    ...
    gate_checks = {
        "x82_precision_at_least_0_85": float(x82_metrics["precision"]) + epsilon >= float(gates["minimum_precision"]),
        "x82_recall_at_least_0_70": float(x82_metrics["recall"]) + epsilon >= float(gates["minimum_recall"]),
        "x82_f1_at_least_0_78": float(x82_metrics["f1"]) + epsilon >= float(gates["minimum_f1"]),
        ...
        "x82_held_proxy_consensus_release_at_least_1": (
            mechanism_counts["held_proxy_consensus_release_frames"]
            >= int(gates["minimum_x82_held_proxy_consensus_release_frames"])
        ),
        "x82_tp_delta_vs_x81_at_least_0": effect_vs_x81["tp_delta"] >= int(gates["minimum_x82_tp_delta_vs_x81"]),
        "x82_fp_delta_vs_x81_at_most_minus_1": effect_vs_x81["fp_delta"] <= int(gates["maximum_x82_fp_delta_vs_x81"]),
        "required_authority_invariants_are_zero": all(
            int(invariants[key]) == 0 for key in gates["required_zero_authority_invariants"]
        ),
    }
    mechanism_exercised = gate_checks["x82_held_proxy_consensus_release_at_least_1"]
    ...
    if not mechanism_exercised:
        ...
        decision = "DTR_CARLA_C41_X82_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C41_X82_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c41-x82-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x82_effect_vs_x81": effect_vs_x81,
        "stretch_target": {
            **stretch,
            "met": float(x82_metrics["precision"]) + epsilon >= float(stretch["precision"])
            and float(x82_metrics["recall"]) + epsilon >= float(stretch["recall"])
            and float(x82_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        "source": {
            ...
            "x81_predictor_sha256": runner.base.sha256_file(Path(x81.__file__).resolve()),
        ...
        "claim_boundary": {
            ...
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x24_x25_x81_x82_predictions_were_sealed": True,
