"""Run frozen X54 and X56 on fresh C26, then open and score truth once."""
...
def prediction_envelope(
    *, arm: str, experiment_id: str, constants: dict[str, Any], episodes: dict[str, Any]
    ...
    return {
        "schema": f"blindassist-dtr-carla-c26-{arm.lower()}-fresh-predictions-v1",
        ...
        "claim_boundary": {
            "fresh_c26_source_corrected_generalization": True,
...
def require_frozen_inputs(
    ...
    prereg = protocol["c26_x56_preregistration"]
    runner.base.require(
        prereg["single_x56_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X54", "X56"],
        "c26_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c26_frozen_component_hash_drift:{file_name}",
        )
...
def main() -> int:
    ...
    x56_predictions_path = output_dir / "predictions-x56.json"
    ...
    x56_episodes: dict[str, Any] = {}
    cursor = 0
    for episode in contract.episodes:
        count = len(episode.observations)
        values = candidate_values[cursor : cursor + count]
        x54_episodes[episode.episode_id] = x54.predict_episode(
            episode, values, contract.calibration
        )
        x56_episodes[episode.episode_id] = x56.predict_episode(
            episode, values, contract.calibration
        )
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X54 X56", flush=True)
    runner.base.require(cursor == len(candidate_values), "c26_candidate_cursor")
    ...
    x56_predictions = prediction_envelope(
        arm=ARM_X56,
        experiment_id=x56.EXPERIMENT_ID,
        constants=x56.fixed_constants(),
        episodes=x56_episodes,
    )
    runner.base.write_json_exclusive(x54_predictions_path, x54_predictions)
    runner.base.write_json_exclusive(x56_predictions_path, x56_predictions)
    print("sealed_truth_blind X54 X56", flush=True)
    ...
    scoring_envelope = alias_for_transport_score(x56_predictions)
    ...
    diagnostics = {
        episode_id: {
            key: value
            for key, value in x56_episodes[episode_id]["diagnostics"].items()
    ...
    gate_checks = {
        "x56_precision_at_least_0_85": float(x56_metrics["precision"]) + epsilon
        >= float(gates["minimum_precision"]),
        "x56_recall_at_least_0_70": float(x56_metrics["recall"]) + epsilon
        >= float(gates["minimum_recall"]),
        "x56_f1_at_least_0_78": float(x56_metrics["f1"]) + epsilon
        ...
        >= int(gates["minimum_x55_parent_sibling_consensus_frames"]),
        "x56_handback_frames_at_least_1": mechanism_counts[
            "x56_zero_eligible_metric_handback_frames"
        ]
        >= int(gates["minimum_x56_zero_eligible_metric_handback_frames"]),
    ...
    mechanism_exercised = all(
        gate_checks[key]
        for key in (
            "x53_suppressions_at_least_1",
            "x54_continuations_at_least_1",
            "x55_consensus_frames_at_least_1",
            "x56_handback_frames_at_least_1",
        )
    )
    gate_met = mechanism_exercised and all(gate_checks.values())
    if not mechanism_exercised:
        decision = "DTR_CARLA_C26_X56_MECHANISM_NOT_EXERCISED"
    elif gate_met:
        decision = "DTR_CARLA_C26_X56_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C26_X56_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c26-x56-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x56_effect_vs_x54": effect(x56_metrics, x54_metrics),
        "x56_effect_vs_x24": effect(x56_metrics, x24_metrics),
        "stretch_target": {
            **stretch,
            "met": float(x56_metrics["precision"]) + epsilon
            >= float(stretch["precision"])
            and float(x56_metrics["recall"]) + epsilon
            >= float(stretch["recall"])
            and float(x56_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        "source": {
            ...
            "x56_predictions_sha256": runner.base.sha256_file(
                x56_predictions_path
            ),
        ...
        "claim_boundary": {
            ...
            "one_unseen_target_motion_profile": True,
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x54_and_x56_predictions_were_sealed": True,
        ...
        json.dumps(
            {
                ...
                "effect_vs_x24": summary["x56_effect_vs_x24"],
                "mechanism_counts": mechanism_counts,
                "authority_invariants": invariants,
                "gate_checks": gate_checks,
