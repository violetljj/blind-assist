"""Replay X76 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x75_consumed_development as base75  # noqa: E402


runner = base75.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x76-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x76.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X76],
        "episodes": episodes,
        "fixed_constants": x76.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X76]
    return value
...
def main() -> int:
    ...
    parser.add_argument("--x75-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x75_path = args.x75_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x76.json"
    ...
    runner.base.require(
        ...
        "x76_consumed_source_protocol",
    )
    x75_predictions = runner.base.read_json(x75_path)
    runner.base.require(
        x75_predictions.get("status")
        in {
            "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
            "SEALED_TRUTH_BLIND_PENDING_SINGLE_SCORE",
        },
        "x76_x75_prediction_contract",
    )
    runner.base.require(
        set(x75_predictions["episodes"]) == set(runner.EPISODES),
        "x76_episode_set",
    )

    episodes = {
        episode_id: x76.apply_zero_shift_parent_hull_motion_rejection_episode(
            x75_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X75: x75_predictions, ARM_X76: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base75.base74.base73.base72.base71.base70.effect(x76_metrics, x75_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X76]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X76]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(value["diagnostics"].get("x76_zero_shift_parent_hull_motion_release_frames", 0))
            for value in episodes.values()
        ),
        "released_tracks": sum(
            int(value["diagnostics"].get("x76_zero_shift_parent_hull_motion_released_tracks", 0))
            for value in episodes.values()
        ),
    }
    reference_checks = {
        "precision_at_least_0_85": float(x76_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x76_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x76_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X76_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X76_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X76_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X76_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x76-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x76_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x76_predictor_sha256": runner.base.sha256_file(Path(x76.__file__).resolve()),
