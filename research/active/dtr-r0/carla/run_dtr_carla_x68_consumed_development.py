"""Replay X68 on one consumed CARLA cohort for Development evidence."""
...
def envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x68-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x68.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X68],
        "episodes": episodes,
        "fixed_constants": x68.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X68]
    return value
...
def main() -> int:
    ...
    parser.add_argument("--x67-predictions", type=Path, required=True)
    ...
    x67_path = args.x67_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x68.json"
    ...
    runner.base.require(
        ...
        "x68_consumed_source_protocol",
    )
    _freeze, contract, _candidate_values = x24.require_freeze(run_root)
    x24_predictions = runner.base.read_json(run_root / "predictions-x24.json")
    x67_predictions = runner.base.read_json(x67_path)
    runner.base.require(
        x24_predictions.get("status") == "SEALED_TRUTH_BLIND_PENDING_SCORE",
        "x68_x24_prediction_contract",
    )
    runner.base.require(
        x67_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x68_x67_prediction_contract",
    )

    episodes = {
        episode.episode_id: x68.apply_object_local_lateral_dequantization_episode(
            x67_predictions["episodes"][episode.episode_id],
            x24_predictions["episodes"][episode.episode_id],
            episode,
        )
        for episode in contract.episodes
    }
    runner.base.require(set(episodes) == set(runner.EPISODES), "x68_episode_set")
    ...
    envelopes = {ARM_X67: x67_predictions, ARM_X68: predictions}
    predictions_full = {
        arm: {
    ...
    delta = effect(x68_metrics, x67_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X68]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X68]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "object_local_lateral_dequantization_frames": sum(
            int(value["diagnostics"].get(
                "x68_object_local_lateral_dequantization_frames", 0
            ))
            for value in episodes.values()
        ),
        "object_local_lateral_release_frames": sum(
            int(value["diagnostics"].get(
                "x68_object_local_lateral_release_frames", 0
            ))
            for value in episodes.values()
        ),
        "object_local_lateral_released_tracks": sum(
            int(value["diagnostics"].get(
                "x68_object_local_lateral_released_tracks", 0
            ))
            for value in episodes.values()
        ),
    }
    reference_checks = {
        "precision_at_least_0_85": float(x68_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x68_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x68_metrics["f1"]) >= 0.78,
        ...
        "object_local_lateral_dequantization_exercised": mechanism_counts[
            "object_local_lateral_dequantization_frames"
        ]
        > 0,
    }
    if not reference_checks["object_local_lateral_dequantization_exercised"]:
        decision = "DTR_CARLA_X68_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X68_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X68_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X68_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x68-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x67_predictions_sha256": runner.base.sha256_file(x67_path),
            "x68_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x68_predictor_sha256": runner.base.sha256_file(Path(x68.__file__).resolve()),
