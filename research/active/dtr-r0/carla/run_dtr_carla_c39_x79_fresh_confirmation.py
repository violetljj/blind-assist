"""Run frozen X78/X79 once on fresh C39, then open and score truth."""
...
import run_dtr_carla_c38_x77_fresh_confirmation as base38  # noqa: E402
...
def prediction_envelope(
    ...
    return {
        "schema": f"blindassist-dtr-carla-c39-{arm.lower()}-fresh-predictions-v1",
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X79]
    return value
...
def main() -> int:
    ...
    runner.base.require(
        ...
        "c39_protocol_hash_drift",
    )
    prereg = protocol["c39_x79_preregistration"]
    runner.base.require(
        prereg["single_x79_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X78", "X79"],
        "c39_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c39_frozen_component_hash_drift:{file_name}",
        )

    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    rigid_path = output_dir / "predictions-x25-rigid.json"
    x78_path = output_dir / "predictions-x78.json"
    x79_path = output_dir / "predictions-x79.json"
    ...
    x78_episodes: dict[str, Any] = {}
    x79_episodes: dict[str, Any] = {}
    cursor = 0
    for episode in contract.episodes:
        ...
        x78_episodes[episode.episode_id] = core78
        x79_episodes[episode.episode_id] = core79
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X78 X79", flush=True)
    runner.base.require(cursor == len(candidate_values), "c39_candidate_cursor")

    rigid_predictions = base38.base37.base36.base35.base70.rigid_envelope(
        "C39", rigid_episodes
    )
    x78_predictions = prediction_envelope(
        ARM_X78, x78.EXPERIMENT_ID, x78.fixed_constants(), x78_episodes
    )
    x79_predictions = prediction_envelope(
        ARM_X79, x79.EXPERIMENT_ID, x79.fixed_constants(), x79_episodes
    )
    runner.base.write_json_exclusive(rigid_path, rigid_predictions)
    runner.base.write_json_exclusive(x78_path, x78_predictions)
    runner.base.write_json_exclusive(x79_path, x79_predictions)
    print("sealed_truth_blind X25 X78 X79", flush=True)
    ...
    envelopes = {
        ...
        ARM_X78: x78_predictions,
        ARM_X79: x79_predictions,
    }
    predictions_full = {
        arm: {
    ...
    compatible = alias_for_transport_score(x79_predictions)
    ...
    x78_metrics = aggregate[ARM_X78]
    x79_metrics = aggregate[ARM_X79]
    effect_vs_x78 = base38.base37.base36.base35.base70.effect(
        x79_metrics, x78_metrics
    )
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X79]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X79]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "lateral_only_release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x79_uncredentialed_lateral_only_release_frames", 0
                )
            )
            for value in x79_episodes.values()
        ),
        "lateral_only_released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x79_uncredentialed_lateral_only_released_tracks", 0
                )
            )
            for value in x79_episodes.values()
        ),
    ...
    gate_checks = {
        "x79_precision_at_least_0_85": float(x79_metrics["precision"]) + epsilon
        >= float(gates["minimum_precision"]),
        "x79_recall_at_least_0_70": float(x79_metrics["recall"]) + epsilon
        >= float(gates["minimum_recall"]),
        "x79_f1_at_least_0_78": float(x79_metrics["f1"]) + epsilon
        ...
        "x79_lateral_only_release_at_least_1": (
            mechanism_counts["lateral_only_release_frames"]
            >= int(gates["minimum_x79_lateral_only_release_frames"])
        ),
        "x79_tp_delta_vs_x78_at_least_0": effect_vs_x78["tp_delta"]
        >= int(gates["minimum_x79_tp_delta_vs_x78"]),
        "x79_fp_delta_vs_x78_at_most_minus_1": effect_vs_x78["fp_delta"]
        <= int(gates["maximum_x79_fp_delta_vs_x78"]),
        "required_authority_invariants_are_zero": all(
            int(invariants[key]) == 0
            for key in gates["required_zero_authority_invariants"]
        ),
    }
    mechanism_exercised = gate_checks["x79_lateral_only_release_at_least_1"]
    ...
    if not mechanism_exercised:
        ...
        decision = "DTR_CARLA_C39_X79_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C39_X79_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c39-x79-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x79_effect_vs_x78": effect_vs_x78,
        "stretch_target": {
            **stretch,
            "met": float(x79_metrics["precision"]) + epsilon
            >= float(stretch["precision"])
            and float(x79_metrics["recall"]) + epsilon >= float(stretch["recall"])
            and float(x79_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        "source": {
            ...
            "x78_predictions_sha256": runner.base.sha256_file(x78_path),
            "x79_predictions_sha256": runner.base.sha256_file(x79_path),
            "x79_predictor_sha256": runner.base.sha256_file(
                Path(x79.__file__).resolve()
            ),
        ...
        "claim_boundary": {
            ...
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x24_x25_x78_x79_predictions_were_sealed": True,
