"""Replay X85 against X84 on one consumed CARLA cohort."""
...
import run_dtr_carla_x84_consumed_development as base84  # noqa: E402


runner = base84.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x85-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x85.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X85],
        "episodes": episodes,
        "fixed_constants": x85.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x84-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x84_path = args.x84_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x85.json"
    ...
    runner.base.require(
        ...
        "x85_consumed_source_protocol",
    )
    x84_predictions = runner.base.read_json(x84_path)
    runner.base.require(
        str(x84_predictions.get("status", "")).startswith("SEALED_")
        and set(x84_predictions.get("arms", [])) == {ARM_X84},
        "x85_x84_prediction_contract",
    )
    runner.base.require(
        set(x84_predictions["episodes"]) == set(runner.EPISODES),
        "x85_episode_set",
    )

    episodes = {
        episode_id: x85.apply_dequantization_completion_precedence_release_episode(
            x84_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X84: x84_predictions, ARM_X85: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x84_predictions, ARM_X84)
    compatible = alias(predictions, ARM_X85)
    ...
    delta = effect(x85_metrics, x84_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X85]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X85]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x85_dequantization_completion_precedence_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x85_dequantization_completion_precedence_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x85_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x85_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x85_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X85_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X85_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X85_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x85-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x84_predictions_sha256": runner.base.sha256_file(x84_path),
            "x85_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x85_predictor_sha256": runner.base.sha256_file(Path(x85.__file__).resolve()),
