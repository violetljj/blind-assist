"""Replay X71 on one consumed CARLA cohort for Development evidence."""
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x71-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x71.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X71],
        "episodes": episodes,
        "fixed_constants": x71.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X71]
    return value
...
def main() -> int:
    ...
    parser.add_argument("--x70-predictions", type=Path, required=True)
    ...
    x70_path = args.x70_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    rigid_path = output_dir / "predictions-x25-rigid.json"
    predictions_path = output_dir / "predictions-x71.json"
    ...
    x70_predictions = runner.base.read_json(x70_path)
    runner.base.require(
        metric_predictions.get("status") == "SEALED_TRUTH_BLIND_PENDING_SCORE",
        "x71_x24_prediction_contract",
    )
    runner.base.require(
        x70_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x71_x70_prediction_contract",
    )
    ...
    for episode in contract.episodes:
        ...
        cursor += count
    runner.base.require(cursor == len(candidate_values), "x71_candidate_cursor")
    runner.base.write_json_exclusive(
        rigid_path, base70.rigid_envelope(args.cohort_id, rigid_episodes)
    )

    episodes = {
        episode.episode_id: x71.apply_entry_cotransport_occupancy_birth_episode(
            x70_predictions["episodes"][episode.episode_id],
            rigid_episodes[episode.episode_id],
            metric_predictions["episodes"][episode.episode_id],
        )
        for episode in contract.episodes
    }
    runner.base.require(set(episodes) == set(runner.EPISODES), "x71_episode_set")
    ...
    envelopes = {ARM_X70: x70_predictions, ARM_X71: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base70.effect(x71_metrics, x70_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X71]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X71]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    diagnostic_keys = (
        "x71_entry_cotransport_birth_frames",
        "x71_entry_cotransport_birth_tracks",
        "x71_release_rejections",
        "x71_class_rejections",
        "x71_forward_direction_rejections",
        "x71_current_containment_rejections",
        "x71_future_cotransport_rejections",
    )
    mechanism_counts = {
        key.removeprefix("x71_"): sum(
            int(value["diagnostics"].get(key, 0)) for value in episodes.values()
        )
        for key in diagnostic_keys
    }
    reference_checks = {
        "precision_at_least_0_85": float(x71_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x71_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x71_metrics["f1"]) >= 0.78,
    ...
    exercised = mechanism_counts["entry_cotransport_birth_frames"] > 0
    if exercised and delta["tp_delta"] > 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X71_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X71_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X71_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X71_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x71-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x70_predictions_sha256": runner.base.sha256_file(x70_path),
            "x71_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x25_predictor_sha256": runner.base.sha256_file(Path(x25.__file__).resolve()),
            "x71_predictor_sha256": runner.base.sha256_file(Path(x71.__file__).resolve()),
