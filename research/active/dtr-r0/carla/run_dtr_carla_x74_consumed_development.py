"""Replay X74 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x73_consumed_development as base73  # noqa: E402


runner = base73.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x74-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x74.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X74],
        "episodes": episodes,
        "fixed_constants": x74.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x73-predictions", type=Path, required=True)
    ...
    x73_path = args.x73_predictions.resolve(strict=True)
    rigid_path = args.x25_rigid_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x74.json"
    ...
    runner.base.require(
        ...
        "x74_consumed_source_protocol",
    )
    x73_predictions = runner.base.read_json(x73_path)
    rigid_predictions = runner.base.read_json(rigid_path)
    runner.base.require(
        x73_predictions.get("status")
        in {
            "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
            "SEALED_TRUTH_BLIND_PENDING_SINGLE_SCORE",
        },
        "x74_x73_prediction_contract",
    )
    runner.base.require(
        set(x73_predictions["episodes"]) == set(runner.EPISODES)
        and set(rigid_predictions["episodes"]) == set(runner.EPISODES),
        "x74_episode_set",
    )

    episodes = {
        episode_id: x74.apply_metric_handback_class_contradiction_episode(
            x73_predictions["episodes"][episode_id],
    ...
    envelopes = {ARM_X73: x73_predictions, ARM_X74: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base73.base72.base71.base70.effect(x74_metrics, x73_metrics)
    contact_recall = {
    ...
    mechanism_counts = {
        "metric_handback_class_contradiction_release_frames": sum(
            int(value["diagnostics"].get("x74_metric_handback_class_contradiction_release_frames", 0))
            for value in episodes.values()
        ),
        "metric_handback_class_contradicted_tracks": sum(
            int(value["diagnostics"].get("x74_metric_handback_class_contradicted_tracks", 0))
            for value in episodes.values()
        ),
    }
    reference_checks = {
        "precision_at_least_0_85": float(x74_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x74_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x74_metrics["f1"]) >= 0.78,
    ...
    exercised = mechanism_counts["metric_handback_class_contradiction_release_frames"] > 0
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X74_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X74_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X74_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X74_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x74-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x74_predictions_sha256": runner.base.sha256_file(predictions_path),
