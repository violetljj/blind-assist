"""Replay X91 against X90 on one consumed CARLA cohort."""
...
import run_dtr_carla_x90_consumed_development as base90  # noqa: E402


runner = base90.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x91-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x91.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X91],
        "episodes": episodes,
        "fixed_constants": x91.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x90-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x90_path = args.x90_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x91.json"
    ...
    runner.base.require(
        ...
        "x91_consumed_source_protocol",
    )
    x90_predictions = runner.base.read_json(x90_path)
    runner.base.require(
        str(x90_predictions.get("status", "")).startswith("SEALED_")
        and set(x90_predictions.get("arms", [])) == {ARM_X90},
        "x91_x90_prediction_contract",
    )
    runner.base.require(
        set(x90_predictions["episodes"]) == set(runner.EPISODES),
        "x91_episode_set",
    )

    episodes = {
        episode_id: x91.apply_held_risk_birth_horizon_release_episode(
            x90_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X90: x90_predictions, ARM_X91: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x90_predictions, ARM_X90)
    compatible = alias(predictions, ARM_X91)
    ...
    x91_metrics = aggregate[ARM_X91]
    delta = effect(x91_metrics, x90_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X91]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X91]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x91_held_risk_birth_horizon_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x91_held_risk_birth_horizon_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x91_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x91_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x91_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X91_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X91_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X91_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x91-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x90_predictions_sha256": runner.base.sha256_file(x90_path),
            "x91_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x91_predictor_sha256": runner.base.sha256_file(Path(x91.__file__).resolve()),
