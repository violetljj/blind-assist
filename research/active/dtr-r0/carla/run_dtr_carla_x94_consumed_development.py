"""Replay X94 against X93 on one consumed CARLA cohort."""
...
import run_dtr_carla_x93_consumed_development as base93  # noqa: E402


runner = base93.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x94-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x94.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X94],
        "episodes": episodes,
        "fixed_constants": x94.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x93-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x93_path = args.x93_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x94.json"
    ...
    runner.base.require(
        ...
        "x94_consumed_source_protocol",
    )
    x93_predictions = runner.base.read_json(x93_path)
    runner.base.require(
        str(x93_predictions.get("status", "")).startswith("SEALED_")
        and set(x93_predictions.get("arms", [])) == {ARM_X93},
        "x94_x93_prediction_contract",
    )
    runner.base.require(
        set(x93_predictions["episodes"]) == set(runner.EPISODES),
        "x94_episode_set",
    )

    episodes = {
        episode_id: x94.apply_one_frame_full_dropout_continuity_episode(
            x93_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X93: x93_predictions, ARM_X94: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x93_predictions, ARM_X93)
    compatible = alias(predictions, ARM_X94)
    ...
    baseline_metrics = aggregate[ARM_X93]
    candidate_metrics = aggregate[ARM_X94]
    delta = effect(candidate_metrics, baseline_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X94]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X94]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "continuity_frames": sum(
            int(value["diagnostics"].get(
                "x94_one_frame_full_dropout_continuity_frames", 0
            ))
            for value in episodes.values()
        ),
        "continuity_tracks": sum(
            int(value["diagnostics"].get(
                "x94_one_frame_full_dropout_continuity_tracks", 0
            ))
            for value in episodes.values()
        ),
        "parent_witness_rejections": sum(
            int(value["diagnostics"].get("x94_parent_witness_rejections", 0))
            for value in episodes.values()
        ),
    }
    required_invariants = (
    ...
    reference_checks = {
        "true_positives_non_decreasing": delta["tp_delta"] >= 0,
        "false_positives_nonincreasing": delta["fp_delta"] <= 0,
        "precision_at_least_0_85": float(candidate_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(candidate_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(candidate_metrics["f1"]) >= 0.78,
    ...
    exercised = mechanism_counts["continuity_frames"] > 0
    if exercised and delta["tp_delta"] > 0 and delta["fp_delta"] <= 0 and all(
        reference_checks.values()
    ):
        decision = "DTR_CARLA_X94_CONSUMED_DEVELOPMENT_RECALL_EFFECT_POSITIVE"
    elif not exercised and all(
        delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")
    ):
        decision = "DTR_CARLA_X94_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X94_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x94-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x93_predictions_sha256": runner.base.sha256_file(x93_path),
            "x94_predictions_sha256": runner.base.sha256_file(predictions_path),
