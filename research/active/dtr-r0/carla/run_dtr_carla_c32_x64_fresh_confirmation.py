"""Run frozen X54/X64 on fresh C32, then open and score truth once."""
...
def prediction_envelope(
    *, arm: str, experiment_id: str, constants: dict[str, Any], episodes: dict[str, Any]
    ...
    return {
        "schema": f"blindassist-dtr-carla-c32-{arm.lower()}-fresh-predictions-v1",
        ...
        "claim_boundary": {
            "fresh_c32_l03_restored_transfer": True,
...
def require_frozen_inputs(
    ...
    runner.base.require(
        ...
        "c32_protocol_hash_drift",
    )
    prereg = protocol["c32_x64_preregistration"]
    runner.base.require(
        prereg["single_x64_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X54", "X64"],
        "c32_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c32_frozen_component_hash_drift:{file_name}",
        )
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in aliased["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X64]
    return aliased
...
def main() -> int:
    ...
    x64_predictions_path = output_dir / "predictions-x64.json"
    ...
    for episode in contract.episodes:
        ...
        x64_episodes[episode.episode_id] = (
            x64.apply_unanchored_crossing_release_episode(
                core, x24_predictions["episodes"][episode.episode_id]
            )
        )
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X54 X64", flush=True)
    runner.base.require(cursor == len(candidate_values), "c32_candidate_cursor")
    ...
    x64_predictions = prediction_envelope(
        arm=ARM_X64,
        experiment_id=x64.EXPERIMENT_ID,
        constants=x64.fixed_constants(),
        episodes=x64_episodes,
    )
    runner.base.write_json_exclusive(x54_predictions_path, x54_predictions)
    runner.base.write_json_exclusive(x64_predictions_path, x64_predictions)
    print("sealed_truth_blind X54 X64", flush=True)
    ...
    scoring_envelope = alias_for_transport_score(x64_predictions)
    ...
    diagnostics = {
        episode_id: {
            key: value
            for key, value in x64_episodes[episode_id]["diagnostics"].items()
            if key.startswith(("x52_", "x53_", "x54_", "x57_", "x59_", "x60_", "x61_", "x62_", "x64_"))
        }
        for episode_id in runner.EPISODES
    }
    x62_handbacks = sum(
    ...
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X64]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X64]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    ...
    gate_checks = {
        "x64_precision_at_least_0_85": float(x64_metrics["precision"]) + epsilon
        >= float(gates["minimum_precision"]),
        "x64_recall_at_least_0_70": float(x64_metrics["recall"]) + epsilon
        >= float(gates["minimum_recall"]),
        "x64_f1_at_least_0_78": float(x64_metrics["f1"]) + epsilon
        ...
        "x62_synchronized_conflict_handbacks_at_least_1": x62_handbacks
        >= int(gates["minimum_x62_synchronized_conflict_handbacks"]),
        "x64_unanchored_crossing_releases_at_least_1": x64_releases
        >= int(gates["minimum_x64_unanchored_crossing_releases"]),
    ...
    mechanism_exercised = (
        gate_checks["x62_synchronized_conflict_handbacks_at_least_1"]
        and gate_checks["x64_unanchored_crossing_releases_at_least_1"]
    )
    ...
    if not mechanism_exercised:
        ...
        decision = "DTR_CARLA_C32_X64_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C32_X64_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c32-x64-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x62_synchronized_conflict_handbacks": x62_handbacks,
        "x64_unanchored_crossing_releases": x64_releases,
        "mechanism_diagnostics_by_episode": diagnostics,
        "x64_effect_vs_x54": effect(x64_metrics, x54_metrics),
        "x64_effect_vs_x24": effect(x64_metrics, x24_metrics),
        "stretch_target": {
            **stretch,
            "met": float(x64_metrics["precision"]) + epsilon
            >= float(stretch["precision"])
            and float(x64_metrics["recall"]) + epsilon
            >= float(stretch["recall"])
            and float(x64_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        ...
        "source": {
            ...
            "x64_predictions_sha256": runner.base.sha256_file(x64_predictions_path),
        ...
        "claim_boundary": {
            ...
            "single_x64_scored_invocation": True,
            "same_map_route_layout_detector_and_most_motion_profiles": True,
            "fresh_seed_weather_and_l03_restored_motion": True,
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x54_and_x64_predictions_were_sealed": True,
        ...
        json.dumps(
            {
                ...
                "effect_vs_x54": summary["x64_effect_vs_x54"],
                "effect_vs_x24": summary["x64_effect_vs_x24"],
                "x62_synchronized_conflict_handbacks": x62_handbacks,
                "x64_unanchored_crossing_releases": x64_releases,
                "authority_invariants": invariants,
                "gate_checks": gate_checks,
