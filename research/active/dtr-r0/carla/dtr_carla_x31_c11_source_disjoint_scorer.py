"""Score the frozen one-shot C11 solid-body Development cohort.

Static validation opens only frozen code and protocol files.  It does not open
fresh source, predictions, evaluator truth, or a model result.  C10 failure
metadata was consumed only to replace the raster-permeable bus representation;
no C10 algorithm model/scorer/X24/X31 outcome existed or is used.
"""
...
import materialize_dtr_carla_c11_x31_solid_body_protocol as c11  # noqa: E402


EXPECTED_PROTOCOL_SCHEMA = 11
EXPECTED_COHORT_ID = "DTR_CARLA_C11_X31_SOLID_BODY_SOURCE_DISJOINT_V1"
EXPECTED_PROTOCOL_SHA256 = (
    "744320C57ABBCD25835155760C15E83A421D66FC5DEDC35FB07F5F30BE0E5ACD"
)
EXPECTED_CAPTURE_SEED = 131365
...
def protocol_default_path() -> Path:
    return HERE / "dtr_carla_c11_x31_solid_body_protocol.json"
...
def validate_protocol(
    ...
    base.require(
        ...
        "c11_protocol_hash_not_frozen",
    )
    base.require(
        protocol.get("schema_version") == EXPECTED_PROTOCOL_SCHEMA,
        "c11_protocol_schema",
    )
    base.require(protocol.get("cohort_id") == EXPECTED_COHORT_ID, "c11_cohort")
    base.require(
    ...
    base.require(protocol["environment"]["map"] == shared.EXPECTED_MAP, "c11_map")
    base.require(tuple(protocol["layouts"]) == LAYOUTS, "c11_layout_order")
    base.require(
    ...
    base.require(tuple(scenarios) == EPISODES, "c11_episode_order")
    base.require(
    ...
    base.require(
        ...
        "c11_frozen_thresholds",
    )
    base.require(
        evaluation["transport_cone_falsification"]
        == shared.EXPECTED_TRANSPORT_FALSIFICATION,
        "c11_transport_falsification",
    )
    base.require(
        evaluation["dynamic_pairs"] == shared.expected_dynamic_pairs(),
        "c11_dynamic_pairs",
    )
    base.require(len(protocol["occlusion_contracts"]) == len(EPISODES), "c11_occlusions")
    base.require(protocol["twin_contracts"] == [], "c11_twin_contracts")
    for episode_id, contract in zip(EPISODES, protocol["occlusion_contracts"]):
        base.require(contract["episodes"] == [episode_id], f"c11_occlusion_singleton:{episode_id}")

    parent_path = c11.DEFAULT_BASE.resolve(strict=True)
    base.require(
        base.sha256_file(parent_path) == c11.PARENT_PROTOCOL_SHA256,
        "c11_parent_protocol_hash",
    )
    static_receipt = c11.validate_c11(protocol, c11.read_json(parent_path))
    source = protocol["source_disjoint_contract"]
    base.require(source["predecessor_c10_disposition"] == c11.PREDECESSOR_DISPOSITION, "c11_c10_disposition")
    base.require(source["predecessor_instance_source_outcome_accessed_for_geometry_reachability"] is True, "c11_c10_source_access_not_recorded")
    base.require(source["predecessor_instance_jsonl_and_manifests_accessed_for_geometry_reachability"] is True, "c11_c10_jsonl_access_not_recorded")
    base.require(source["predecessor_algorithm_model_scorer_x24_or_x31_outcome_existed"] is False, "c11_c10_algorithm_outcome_existed")
    base.require(source["predecessor_algorithm_model_scorer_x24_or_x31_outcome_accessed"] is False, "c11_c10_algorithm_outcome_accessed")
    base.require(source["c9_firetruck_instance_jsonl_accessed_for_solid_body_reachability"] is True, "c11_c9_firetruck_source_not_recorded")
    base.require(source["c9_algorithm_model_evaluator_scorer_x24_or_x31_outcome_existed"] is False, "c11_c9_algorithm_outcome_existed")
    base.require(source["c9_algorithm_model_evaluator_scorer_x24_or_x31_outcome_accessed"] is False, "c11_c9_algorithm_outcome_accessed")
    base.require(source["development_only_never_confirmation"] is True, "c11_confirmation_leak")
    base.require(source["one_fresh_capture_only"] is True and source["resume_replay_reseed_substitution_or_partial_selection_allowed"] is False, "c11_one_shot_policy")
...
def validate_static(protocol_path: Path) -> dict[str, Any]:
    ...
    base.require(
        ...
        "c11_scorer_gate_constants",
    )
    return {
        "status": "C11_X31_SCORER_STATIC_VALID",
        ...
        "predecessor_c10_status": "DTR_CARLA_C2_RICH_MULTILAYOUT_SOURCE_NOT_EVALUABLE",
        "predecessor_c10_run_id": c11.C10_RUN_ID,
        "predecessor_c10_result_sha256": c11.C10_RESULT_SHA256,
        "predecessor_c10_instance_source_outcome_accessed_for_geometry_design": True,
        "predecessor_c10_algorithm_model_scorer_x24_or_x31_outcome_existed": False,
...
def render_svg(result: Mapping[str, Any]) -> str:
    ...
    return "\n".join(
        [
            ...
            '<text x="30" y="45" font-size="25" font-family="sans-serif" fill="#0f172a">C11 X31 solid-body Development score</text>',
            ...
            f'<text x="30" y="162" font-size="15" fill="#111827">ambiguity selected-loss frames={result["transport_ambiguity"]["selected_contact_loss_ambiguity_frames"]}; standardized solid-body occluders</text>',
            '<text x="30" y="202" font-size="15" fill="#111827">C10 predecessor: SOURCE_NOT_EVALUABLE; source representation diagnostic only, no algorithm outcome.</text>',
...
def score(args: argparse.Namespace) -> dict[str, Any]:
    ...
    base.require(not result_path.exists() and not svg_path.exists(), "c11_x31_score_outputs_exist")
    protocol = base.read_json(protocol_path)
    validate_protocol(protocol, protocol_path)
    predictor_path = shared.predictor_path().resolve(strict=True)
    shared.validate_predictor_source(predictor_path)
    base.require(base.sha256_file(predictor_path) == EXPECTED_X31_PREDICTOR_SHA256, "c11_x31_predictor_hash")
    base.require(base.sha256_file(Path(shared.__file__).resolve(strict=True)) == EXPECTED_SHARED_C8_SCORER_SHA256, "c11_shared_scorer_utility_hash")
    ...
    result = {
        "schema": "blindassist-dtr-carla-c11-x31-source-disjoint-score-result-v1",
        "status": "COMPLETE",
        "decision": "DTR_CARLA_X31_SOLID_BODY_SOURCE_DISJOINT_PRETRUTH_DEVELOPMENT_GATE_MET" if gate_met else "DTR_CARLA_X31_SOLID_BODY_SOURCE_DISJOINT_PRETRUTH_DEVELOPMENT_GATE_NOT_MET",
        "gate_met": gate_met,
        ...
        "claim_boundary": {
            ...
            "development_only_never_confirmation": True,
            "one_fresh_capture_only": True,
            "standardized_solid_body_single_pass_physical_occluders": True,
            "scripted_pose_collision_response_decoupled": True,
            "c10_predecessor_status": "DTR_CARLA_C2_RICH_MULTILAYOUT_SOURCE_NOT_EVALUABLE",
            "c10_instance_source_outcome_accessed_for_geometry_design": True,
            "c10_algorithm_model_scorer_x24_or_x31_outcome_existed": False,
            "c10_algorithm_model_scorer_x24_or_x31_outcome_accessed": False,
            "c9_firetruck_instance_source_accessed_for_solid_body_reachability": True,
            "c9_algorithm_model_evaluator_scorer_x24_or_x31_outcome_accessed": False,
            "c10_resume_replay_reseed_substitution_or_partial_selection_authorized": False,
