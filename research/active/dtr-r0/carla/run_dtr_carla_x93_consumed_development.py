"""Replay X93 against X92 on one consumed CARLA cohort."""
...
import run_dtr_carla_x92_consumed_development as base92  # noqa: E402


runner = base92.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x93-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x93.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X93],
        "episodes": episodes,
        "fixed_constants": x93.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x92-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x92_path = args.x92_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x93.json"
    ...
    runner.base.require(
        ...
        "x93_consumed_source_protocol",
    )
    x92_predictions = runner.base.read_json(x92_path)
    runner.base.require(
        str(x92_predictions.get("status", "")).startswith("SEALED_")
        and set(x92_predictions.get("arms", [])) == {ARM_X92},
        "x93_x92_prediction_contract",
    )
    runner.base.require(
        set(x92_predictions["episodes"]) == set(runner.EPISODES),
        "x93_episode_set",
    )

    episodes = {
        episode_id: x93.apply_conflicted_nonclosing_future_release_episode(
            x92_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X92: x92_predictions, ARM_X93: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x92_predictions, ARM_X92)
    compatible = alias(predictions, ARM_X93)
    ...
    x92_metrics = aggregate[ARM_X92]
    x93_metrics = aggregate[ARM_X93]
    delta = effect(x93_metrics, x92_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X93]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X93]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x93_conflicted_nonclosing_future_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x93_conflicted_nonclosing_future_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x93_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x93_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x93_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X93_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X93_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X93_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x93-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x92_predictions_sha256": runner.base.sha256_file(x92_path),
            "x93_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x93_predictor_sha256": runner.base.sha256_file(Path(x93.__file__).resolve()),
