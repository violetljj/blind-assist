"""Freeze a new C35 pixel source for unchanged X73 confirmation."""
...
PARENT_PROTOCOL = HERE / "dtr_carla_c34_x65_fresh_source_protocol.json"
...
COHORT_ID = "DTR_CARLA_C35_X73_FRESH_CONFIRMATION_V1"
CAPTURE_SEED = 351073
WEATHERS = {
    "c8_l01": "HardRainNoon",
    "c8_l02": "WetSunset",
...
def materialize(parent: dict) -> dict:
    ...
    protocol["schema_version"] = 35
    protocol["cohort_id"] = COHORT_ID
    protocol["capture"]["seed"] = CAPTURE_SEED
    protocol["objective"] = (
        "Confirm byte-frozen X73 once on genuinely new seed- and render-domain-"
        "disjoint CARLA pixels after its four-cohort consumed Development gain, "
    ...
    protocol.pop("c34_x65_preregistration")
    protocol["c35_x73_preregistration"] = {
        "schema": "dtr-carla-c35-x73-preregistration-v1",
        "baselines": ["X24", "X72", "X73"],
        ...
        "primary_transfer_gate": {
            ...
            "minimum_x73_parent_hull_reconstruction_frames": 1,
            "minimum_x73_tp_delta_vs_x72": 1,
            "maximum_x73_fp_delta_vs_x72": 0,
    ...
    protocol["source_disjoint_contract"] = {
        "schema": "dtr-carla-c35-x73-fresh-source-contract-v1",
        ...
        "c35_is_new_cohort_not_c34_retry": True,
        ...
        "camera_geometry_unchanged_from_c34": True,
        "target_and_occluder_trajectories_unchanged_from_c34": True,
        "route_and_truth_contract_unchanged_from_c34": True,
    ...
    protocol["claim_boundary"] = [
        "C35 uses new seed, render-domain assignment, and pixels; it does not reuse C34 payloads.",
        "X73 and every imported algorithm component are byte-frozen before C35 capture.",
        "The sole C35 score confirms X73 only if parent-hull reconstruction is exercised, adds TP over X72, and adds no FP.",
        "A mechanism-not-exercised result is not positive or negative incremental confirmation.",
        "C35 is source-disjoint synthetic Development, not unseen-map, open-world traffic, real-sensor, deployment, reliability, user-benefit, or safety evidence.",
        "A failed source gate is NOT_EVALUABLE; frozen X73 cannot be changed or rerun on C35 after pixels are opened.",
    ]
...
def main() -> int:
    ...
    parser.add_argument("--c34-source-result", type=Path, required=True)
    parser.add_argument("--output", type=Path, required=True)
    args = parser.parse_args()
    if c2.sha256_file(PARENT_PROTOCOL) != PARENT_PROTOCOL_SHA256:
        raise RuntimeError("frozen C34 parent protocol drift")
    source_result_path = args.c34_source_result.resolve(strict=True)
    if c2.sha256_file(source_result_path) != PARENT_SOURCE_RESULT_SHA256:
        raise RuntimeError("frozen C34 source result drift")
    source_result = read_json(source_result_path)
    if source_result.get("status") != PARENT_SOURCE_STATUS:
        raise RuntimeError("C34 source result status drift")
    for file_name, expected in FROZEN_COMPONENTS.items():
        path = HERE / file_name
        if not path.is_file() or c2.sha256_file(path) != expected:
            raise RuntimeError(f"frozen C35 component drift: {file_name}")
    for path, expected in {
        ...
        if c2.sha256_file(path) != expected:
            raise RuntimeError(f"frozen C35 source script drift: {path.name}")
    ...
    print(
        json.dumps(
            {
                "status": "C35_X73_PROTOCOL_STATIC_VALID_PREREGISTERED",
                "output": str(output),
                "sha256": c2.sha256_file(output),
                "cohort_id": COHORT_ID,
                "capture_seed": CAPTURE_SEED,
                "weather_domains": WEATHERS,
                "x73_sha256": FROZEN_COMPONENTS[
                    "dtr_carla_x73_credentialed_parent_hull_reconstruction.py"
                ],
            },
            sort_keys=True,
        )
