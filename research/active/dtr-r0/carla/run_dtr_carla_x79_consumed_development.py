"""Replay X79 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x78_consumed_development as base78  # noqa: E402


runner = base78.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x79-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x79.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X79],
        "episodes": episodes,
        "fixed_constants": x79.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x78-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x78_path = args.x78_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x79.json"
    ...
    runner.base.require(
        ...
        "x79_consumed_source_protocol",
    )
    x78_predictions = runner.base.read_json(x78_path)
    runner.base.require(
        x78_predictions.get("status") == "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "x79_x78_prediction_contract",
    )
    runner.base.require(
        set(x78_predictions["episodes"]) == set(runner.EPISODES),
        "x79_episode_set",
    )

    episodes = {
        episode_id: x79.apply_collision_credentialed_lateral_only_release_episode(
            x78_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X78: x78_predictions, ARM_X79: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base78.base77.base76.base75.base74.base73.base72.base71.base70.effect(
    ...
    reference_checks = {
        "precision_at_least_0_85": float(x79_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x79_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x79_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X79_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X79_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X79_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X79_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x79-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x79_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x79_predictor_sha256": runner.base.sha256_file(Path(x79.__file__).resolve()),
