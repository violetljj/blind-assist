"""Replay X88 against X87 on one consumed CARLA cohort."""
...
import run_dtr_carla_x87_consumed_development as base87  # noqa: E402


runner = base87.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x88-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x88.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X88],
        "episodes": episodes,
        "fixed_constants": x88.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x87-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x87_path = args.x87_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x88.json"
    ...
    runner.base.require(
        ...
        "x88_consumed_source_protocol",
    )
    x87_predictions = runner.base.read_json(x87_path)
    runner.base.require(
        str(x87_predictions.get("status", "")).startswith("SEALED_")
        and set(x87_predictions.get("arms", [])) == {ARM_X87},
        "x88_x87_prediction_contract",
    )
    runner.base.require(
        set(x87_predictions["episodes"]) == set(runner.EPISODES),
        "x88_episode_set",
    )

    episodes = {
        episode_id: x88.apply_motion_epoch_contradiction_release_episode(
            x87_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X87: x87_predictions, ARM_X88: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x87_predictions, ARM_X87)
    compatible = alias(predictions, ARM_X88)
    ...
    x87_metrics = aggregate[ARM_X87]
    x88_metrics = aggregate[ARM_X88]
    delta = effect(x88_metrics, x87_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X88]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X88]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x88_motion_epoch_contradiction_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x88_motion_epoch_contradiction_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x88_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x88_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x88_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X88_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X88_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X88_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x88-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x87_predictions_sha256": runner.base.sha256_file(x87_path),
            "x88_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x88_predictor_sha256": runner.base.sha256_file(Path(x88.__file__).resolve()),
