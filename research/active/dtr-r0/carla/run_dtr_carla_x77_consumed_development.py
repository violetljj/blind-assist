"""Replay X77 on one consumed CARLA cohort for Development evidence."""
...
import run_dtr_carla_x76_consumed_development as base76  # noqa: E402


runner = base76.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x77-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x77.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X77],
        "episodes": episodes,
        "fixed_constants": x77.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x76-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x76_path = args.x76_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x77.json"
    ...
    runner.base.require(
        ...
        "x77_consumed_source_protocol",
    )
    x76_predictions = runner.base.read_json(x76_path)
    runner.base.require(
        x76_predictions.get("status")
        in {
            "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
            "SEALED_TRUTH_BLIND_PENDING_SINGLE_SCORE",
        },
        "x77_x76_prediction_contract",
    )
    runner.base.require(
        set(x76_predictions["episodes"]) == set(runner.EPISODES),
        "x77_episode_set",
    )

    episodes = {
        episode_id: x77.apply_receding_metric_temporal_handoff_rejection_episode(
            x76_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X76: x76_predictions, ARM_X77: predictions}
    predictions_full = {
        arm: {
    ...
    delta = base76.base75.base74.base73.base72.base71.base70.effect(
        x77_metrics, x76_metrics
    )
    contact_recall = {
    ...
    mechanism_counts = {
        "release_frames": sum(
            int(value["diagnostics"].get("x77_receding_metric_temporal_handoff_release_frames", 0))
            for value in episodes.values()
        ),
        "released_tracks": sum(
            int(value["diagnostics"].get("x77_receding_metric_temporal_handoff_released_tracks", 0))
            for value in episodes.values()
        ),
    }
    reference_checks = {
        "precision_at_least_0_85": float(x77_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x77_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x77_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["tp_delta"] == 0 and delta["fp_delta"] < 0:
        decision = "DTR_CARLA_X77_CONSUMED_DEVELOPMENT_EFFECT_POSITIVE"
    elif not exercised and delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X77_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    elif delta["tp_delta"] == 0 and delta["fp_delta"] == 0:
        decision = "DTR_CARLA_X77_CONSUMED_DEVELOPMENT_EFFECT_NEUTRAL"
    else:
        decision = "DTR_CARLA_X77_CONSUMED_DEVELOPMENT_EFFECT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x77-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x77_predictions_sha256": runner.base.sha256_file(predictions_path),
