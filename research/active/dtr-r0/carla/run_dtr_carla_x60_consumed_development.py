"""Replay X60 route-entry credential memory on one consumed CARLA cohort."""
...
ARM_X54 = x60.x59.x54.ARM_X54
ARM_X60 = x60.ARM_X60


def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x60-consumed-development-predictions-v1",
        "status": "SEALED_BEFORE_CONSUMED_SCORE",
        "cohort_id": cohort_id,
        "experiment_id": x60.EXPERIMENT_ID,
        "truth_blind_component_inputs": True,
        "arms": [ARM_X60],
        "episodes": episodes,
        "fixed_constants": x60.fixed_constants(),
        "claim_boundary": {
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in aliased["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X60]
    return aliased
...
def gates_from(protocol: dict[str, Any]) -> dict[str, Any]:
    for key in ("c28_x59_preregistration", "c27_x57_preregistration", "c26_x56_preregistration"):
        if key in protocol:
...
def main() -> int:
    ...
    predictions_path = output_dir / "predictions-x60.json"
    summary_path = output_dir / "summary.json"

    episodes = {
        episode_id: x60.apply_route_entry_credential_memory_episode(
    ...
    measured_handbacks = sum(
        int(value["diagnostics"].get("x60_measured_route_entry_handback_frames", 0))
        for value in episodes.values()
    )
    held_handbacks = sum(
        int(
            value["diagnostics"].get(
                "x60_held_route_entry_credential_handback_frames", 0
            )
        )
        for value in episodes.values()
    )
    ...
    x60_metrics = aggregate[ARM_X60]
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X60]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    contact_recall = {
        episode_id: float(
            contacts[episode_id][ARM_X60]["future_positive_recall"]
        )
        for episode_id in runner.CONTACT
    }
    ...
    reference_checks = {
        "precision_at_least_0_85": float(x60_metrics["precision"]) + epsilon
        >= 0.85,
        "recall_at_least_0_70": float(x60_metrics["recall"]) + epsilon >= 0.70,
        "f1_at_least_0_78": float(x60_metrics["f1"]) + epsilon >= 0.78,
        ...
        "route_entry_credential_action_exercised": (
            measured_handbacks + held_handbacks >= 1
        ),
    ...
    decision = (
        "DTR_CARLA_X60_CONSUMED_DEVELOPMENT_REFERENCE_TARGET_MET"
        if reference_target_met
        else "DTR_CARLA_X60_CONSUMED_DEVELOPMENT_REFERENCE_TARGET_NOT_MET"
    )
    summary = {
        "schema": "blindassist-dtr-carla-x60-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "mechanism_counts": {
            "x60_measured_route_entry_handback_frames": measured_handbacks,
            "x60_held_route_entry_credential_handback_frames": held_handbacks,
        },
        "x60_effect_vs_x54": effect(x60_metrics, x54_metrics),
        "x60_effect_vs_x24": effect(x60_metrics, x24_metrics),
        ...
        "source": {
            ...
            "x60_predictor_sha256": runner.base.sha256_file(Path(x60.__file__).resolve()),
            "x60_predictions_sha256": runner.base.sha256_file(predictions_path),
            "development_runner_sha256": runner.base.sha256_file(Path(__file__).resolve()),
        },
        "claim_boundary": {
            "consumed_posthoc_development": True,
            "parent_frozen_decision_unchanged": True,
            "reference_thresholds_not_preregistered_for_x60": True,
            "fresh_confirmation": False,
        ...
        json.dumps(
            {
                ...
                "effect_vs_x54": summary["x60_effect_vs_x54"],
                "reference_checks": reference_checks,
