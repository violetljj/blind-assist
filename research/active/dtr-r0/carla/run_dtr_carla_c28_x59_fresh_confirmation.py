"""Run frozen X54 and X59 on fresh C28, then open and score truth once."""
...
def prediction_envelope(
    *, arm: str, experiment_id: str, constants: dict[str, Any], episodes: dict[str, Any]
    ...
    return {
        "schema": f"blindassist-dtr-carla-c28-{arm.lower()}-fresh-predictions-v1",
        ...
        "claim_boundary": {
            "fresh_c28_mixed_lighting_transfer": True,
...
def require_frozen_inputs(
    ...
    runner.base.require(
        ...
        "c28_protocol_hash_drift",
    )
    prereg = protocol["c28_x59_preregistration"]
    runner.base.require(
        prereg["single_x59_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X54", "X59"],
        "c28_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c28_frozen_component_hash_drift:{file_name}",
        )
...
def main() -> int:
    ...
    x59_predictions_path = output_dir / "predictions-x59.json"
    ...
    for episode in contract.episodes:
        ...
        x59_episodes[episode.episode_id] = (
            x59.apply_modality_evidence_reliability_router_episode(
                core, x24_predictions["episodes"][episode.episode_id]
            )
        )
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X54 X59", flush=True)
    runner.base.require(cursor == len(candidate_values), "c28_candidate_cursor")
    ...
    x59_predictions = prediction_envelope(
        arm=ARM_X59,
        experiment_id=x59.EXPERIMENT_ID,
        constants=x59.fixed_constants(),
        episodes=x59_episodes,
    )
    runner.base.write_json_exclusive(x54_predictions_path, x54_predictions)
    runner.base.write_json_exclusive(x59_predictions_path, x59_predictions)
    print("sealed_truth_blind X54 X59", flush=True)
    ...
    scoring_envelope = alias_for_transport_score(x59_predictions)
    ...
    diagnostics = {
        episode_id: {
            key: value
            for key, value in x59_episodes[episode_id]["diagnostics"].items()
            if key.startswith(("x52_", "x53_", "x54_", "x57_", "x59_"))
        }
        for episode_id in runner.EPISODES
    }
    modality_actions = sum(
        int(value.get("x59_current_measured_closing_handback_frames", 0))
        + int(value.get("x59_evidence_supported_receding_release_frames", 0))
    ...
    gate_checks = {
        "x59_precision_at_least_0_85": float(x59_metrics["precision"]) + epsilon
        >= float(gates["minimum_precision"]),
        "x59_recall_at_least_0_70": float(x59_metrics["recall"]) + epsilon
        >= float(gates["minimum_recall"]),
        "x59_f1_at_least_0_78": float(x59_metrics["f1"]) + epsilon
        ...
        "x59_modality_actions_at_least_1": modality_actions
        >= int(gates["minimum_x59_modality_reliability_actions"]),
        "required_authority_invariants_are_zero": all(
            int(invariants[key]) == 0
            for key in gates["required_zero_authority_invariants"]
        ),
    }
    mechanism_exercised = gate_checks["x59_modality_actions_at_least_1"]
    gate_met = mechanism_exercised and all(gate_checks.values())
    if not mechanism_exercised:
        decision = "DTR_CARLA_C28_X59_MECHANISM_NOT_EXERCISED"
    elif gate_met:
        decision = "DTR_CARLA_C28_X59_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C28_X59_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c28-x59-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x59_modality_actions": modality_actions,
        ...
        "stretch_target": {
            **stretch,
            "met": float(x59_metrics["precision"]) + epsilon
            >= float(stretch["precision"])
            and float(x59_metrics["recall"]) + epsilon
            >= float(stretch["recall"])
            and float(x59_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        ...
        "source": {
            ...
            "x59_predictions_sha256": runner.base.sha256_file(
                x59_predictions_path
            ),
            "x54_predictor_sha256": runner.base.sha256_file(
                Path(x54.__file__).resolve()
            ),
            "x59_predictor_sha256": runner.base.sha256_file(
                Path(x59.__file__).resolve()
            ),
        ...
        "claim_boundary": {
            ...
            "fresh_seed_and_four_mixed_weather_lighting_domains": True,
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x54_and_x59_predictions_were_sealed": True,
        ...
        json.dumps(
            {
                ...
                "effect_vs_x24": summary["x59_effect_vs_x24"],
                "x59_modality_actions": modality_actions,
                "authority_invariants": invariants,
                "gate_checks": gate_checks,
