"""Replay X87 against X86 on one consumed CARLA cohort."""
...
import run_dtr_carla_x86_consumed_development as base86  # noqa: E402


runner = base86.runner
...
def prediction_envelope(cohort_id: str, episodes: dict[str, Any]) -> dict[str, Any]:
    return {
        "schema": "blindassist-dtr-carla-x87-consumed-development-predictions-v1",
        "status": "SEALED_CONSUMED_DEVELOPMENT_PENDING_SCORE",
        "experiment_id": x87.EXPERIMENT_ID,
        "cohort_id": cohort_id,
        "arms": [ARM_X87],
        "episodes": episodes,
        "fixed_constants": x87.fixed_constants(),
        "claim_boundary": {
...
def main() -> int:
    ...
    parser.add_argument("--x86-predictions", type=Path, required=True)
    parser.add_argument("--output-dir", type=Path, required=True)
    args = parser.parse_args()

    source_root = args.source_root.resolve(strict=True)
    protocol_path = args.protocol.resolve(strict=True)
    x86_path = args.x86_predictions.resolve(strict=True)
    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    predictions_path = output_dir / "predictions-x87.json"
    ...
    runner.base.require(
        ...
        "x87_consumed_source_protocol",
    )
    x86_predictions = runner.base.read_json(x86_path)
    runner.base.require(
        str(x86_predictions.get("status", "")).startswith("SEALED_")
        and set(x86_predictions.get("arms", [])) == {ARM_X86},
        "x87_x86_prediction_contract",
    )
    runner.base.require(
        set(x86_predictions["episodes"]) == set(runner.EPISODES),
        "x87_episode_set",
    )

    episodes = {
        episode_id: x87.apply_solo_completion_horizon_release_episode(
            x86_predictions["episodes"][episode_id]
        )
    ...
    envelopes = {ARM_X86: x86_predictions, ARM_X87: predictions}
    predictions_full = {
        arm: {
    ...
    baseline_compatible = alias(x86_predictions, ARM_X86)
    compatible = alias(predictions, ARM_X87)
    ...
    delta = effect(x87_metrics, x86_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X87]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X87]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "release_frames": sum(
            int(
                value["diagnostics"].get(
                    "x87_solo_completion_horizon_release_frames", 0
                )
            )
        ...
        "released_tracks": sum(
            int(
                value["diagnostics"].get(
                    "x87_solo_completion_horizon_released_tracks", 0
                )
            )
            for value in episodes.values()
        ),
    ...
    reference_checks = {
        ...
        "precision_at_least_0_85": float(x87_metrics["precision"]) >= 0.85,
        "recall_at_least_0_70": float(x87_metrics["recall"]) >= 0.70,
        "f1_at_least_0_78": float(x87_metrics["f1"]) >= 0.78,
    ...
    if exercised and delta["fp_delta"] < 0 and all(reference_checks.values()):
        decision = "DTR_CARLA_X87_CONSUMED_DEVELOPMENT_PRECISION_EFFECT_POSITIVE"
    elif not exercised and all(delta[key] == 0 for key in ("tp_delta", "fp_delta", "fn_delta")):
        decision = "DTR_CARLA_X87_CONSUMED_DEVELOPMENT_MECHANISM_NOT_EXERCISED"
    else:
        decision = "DTR_CARLA_X87_CONSUMED_DEVELOPMENT_REGRESSION"

    summary = {
        "schema": "blindassist-dtr-carla-x87-consumed-development-v1",
        "status": "COMPLETE",
        ...
        "source": {
            ...
            "x86_predictions_sha256": runner.base.sha256_file(x86_path),
            "x87_predictions_sha256": runner.base.sha256_file(predictions_path),
            "x87_predictor_sha256": runner.base.sha256_file(Path(x87.__file__).resolve()),
