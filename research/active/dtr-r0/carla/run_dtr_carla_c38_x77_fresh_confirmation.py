"""Run frozen X76/X77 once on fresh C38, then open and score truth."""
...
import run_dtr_carla_c37_x75_fresh_confirmation as base37  # noqa: E402
...
def prediction_envelope(
    ...
    return {
        "schema": f"blindassist-dtr-carla-c38-{arm.lower()}-fresh-predictions-v1",
...
def alias_for_transport_score(predictions: dict[str, Any]) -> dict[str, Any]:
    ...
    for episode in value["episodes"].values():
        for frame in episode["frames"]:
            frame["arms"][ARM_X31] = frame["arms"][ARM_X77]
    return value
...
def main() -> int:
    ...
    runner.base.require(
        ...
        "c38_protocol_hash_drift",
    )
    prereg = protocol["c38_x77_preregistration"]
    runner.base.require(
        prereg["single_x77_scored_invocation"] is True
        and prereg["no_post_capture_algorithm_threshold_or_scenario_tuning"] is True
        and prereg["baselines"] == ["X24", "X76", "X77"],
        "c38_preregistration_contract",
    )
    for file_name, expected_sha256 in prereg["frozen_component_sha256"].items():
        ...
        runner.base.require(
            ...
            f"c38_frozen_component_hash_drift:{file_name}",
        )

    args.output_dir.mkdir(parents=True, exist_ok=False)
    output_dir = args.output_dir.resolve(strict=True)
    rigid_path = output_dir / "predictions-x25-rigid.json"
    x76_path = output_dir / "predictions-x76.json"
    x77_path = output_dir / "predictions-x77.json"
    ...
    x76_episodes: dict[str, Any] = {}
    x77_episodes: dict[str, Any] = {}
    cursor = 0
    for episode in contract.episodes:
        ...
        x76_episodes[episode.episode_id] = core76
        x77_episodes[episode.episode_id] = core77
        cursor += count
        print(f"predicted_truth_blind {episode.episode_id} X76 X77", flush=True)
    runner.base.require(cursor == len(candidate_values), "c38_candidate_cursor")

    rigid_predictions = base37.base36.base35.base70.rigid_envelope("C38", rigid_episodes)
    x76_predictions = prediction_envelope(
        ARM_X76, x76.EXPERIMENT_ID, x76.fixed_constants(), x76_episodes
    )
    x77_predictions = prediction_envelope(
        ARM_X77, x77.EXPERIMENT_ID, x77.fixed_constants(), x77_episodes
    )
    runner.base.write_json_exclusive(rigid_path, rigid_predictions)
    runner.base.write_json_exclusive(x76_path, x76_predictions)
    runner.base.write_json_exclusive(x77_path, x77_predictions)
    print("sealed_truth_blind X25 X76 X77", flush=True)
    ...
    envelopes = {
        ...
        ARM_X76: x76_predictions,
        ARM_X77: x77_predictions,
    }
    predictions_full = {
        arm: {
    ...
    compatible = alias_for_transport_score(x77_predictions)
    ...
    x76_metrics = aggregate[ARM_X76]
    x77_metrics = aggregate[ARM_X77]
    effect_vs_x76 = base37.base36.base35.base70.effect(x77_metrics, x76_metrics)
    contact_recall = {
        episode_id: float(contacts[episode_id][ARM_X77]["future_positive_recall"])
        for episode_id in runner.CONTACT
    }
    safe_counts = {
        episode_id: int(safe[episode_id][ARM_X77]["false_alert_segment_count"])
        for episode_id in runner.SAFE
    }
    mechanism_counts = {
        "receding_metric_temporal_handoff_release_frames": sum(
            int(value["diagnostics"].get(
                "x77_receding_metric_temporal_handoff_release_frames", 0
            ))
            for value in x77_episodes.values()
        ),
        "receding_metric_temporal_handoff_released_tracks": sum(
            int(value["diagnostics"].get(
                "x77_receding_metric_temporal_handoff_released_tracks", 0
            ))
            for value in x77_episodes.values()
        ),
    }
    ...
    gate_checks = {
        "x77_precision_at_least_0_85": float(x77_metrics["precision"]) + epsilon
        >= float(gates["minimum_precision"]),
        "x77_recall_at_least_0_70": float(x77_metrics["recall"]) + epsilon
        >= float(gates["minimum_recall"]),
        "x77_f1_at_least_0_78": float(x77_metrics["f1"]) + epsilon
        ...
        "x77_receding_handoff_release_at_least_1": (
            mechanism_counts["receding_metric_temporal_handoff_release_frames"]
            >= int(gates["minimum_x77_receding_handoff_release_frames"])
        ),
        "x77_tp_delta_vs_x76_at_least_0": effect_vs_x76["tp_delta"]
        >= int(gates["minimum_x77_tp_delta_vs_x76"]),
        "x77_fp_delta_vs_x76_at_most_minus_1": effect_vs_x76["fp_delta"]
        <= int(gates["maximum_x77_fp_delta_vs_x76"]),
        "required_authority_invariants_are_zero": all(
            int(invariants[key]) == 0
            for key in gates["required_zero_authority_invariants"]
        ),
    }
    mechanism_exercised = gate_checks["x77_receding_handoff_release_at_least_1"]
    ...
    if not mechanism_exercised:
        ...
        decision = "DTR_CARLA_C38_X77_GENERALIZATION_GATE_MET"
    else:
        decision = "DTR_CARLA_C38_X77_GENERALIZATION_GATE_NOT_MET"

    stretch = prereg["stretch_target"]
    summary = {
        "schema": "blindassist-dtr-carla-c38-x77-fresh-confirmation-v1",
        "status": "COMPLETE",
        "decision": decision,
        "gate_met": gate_met,
        ...
        "x77_effect_vs_x76": effect_vs_x76,
        "stretch_target": {
            **stretch,
            "met": float(x77_metrics["precision"]) + epsilon >= float(stretch["precision"])
            and float(x77_metrics["recall"]) + epsilon >= float(stretch["recall"])
            and float(x77_metrics["f1"]) + epsilon >= float(stretch["f1"]),
        },
        "source": {
            ...
            "x76_predictions_sha256": runner.base.sha256_file(x76_path),
            "x77_predictions_sha256": runner.base.sha256_file(x77_path),
            "x77_predictor_sha256": runner.base.sha256_file(Path(x77.__file__).resolve()),
        ...
        "claim_boundary": {
            ...
            "truth_blind_prediction_inputs": True,
            "evaluator_opened_only_after_x24_x25_x76_x77_predictions_were_sealed": True,
