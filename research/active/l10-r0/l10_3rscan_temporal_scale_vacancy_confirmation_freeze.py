#!/usr/bin/env python3
"""Freeze a fresh three-reference/two-adjacent-query confirmation cohort."""
...
PROTOCOL_SCHEMA = (
    "blindassist-l10-3rscan-temporal-scale-vacancy-confirmation-freeze-protocol-v1"
)
...
def run(protocol_path: Path, output_path: Path) -> None:
    ...
    pixel.require(pixel.sha256(HERE / row["path"]) == row["sha256"], "SOURCE_RESULT_HASH")
    ...
    for manifest in protocol["source"]["sequence_zips"].values():
        ...
        pixel.require(path.stat().st_size == int(manifest["bytes"]), f"SOURCE_BYTES:{manifest['path']}")
        pixel.require(pixel.sha256(path) == manifest["sha256"], f"SOURCE_HASH:{manifest['path']}")
    ...
    output = {
        "schema": "blindassist-l10-3rscan-temporal-scale-vacancy-confirmation-cohort-v1",
        "authority": "FROZEN_FRESH_FAMILY_PRE_RGB_PRE_MODEL_TEMPORAL_SCALE_VACANCY_CONFIRMATION_COHORT",
        ...
        "selection": {
            **protocol["selection"],
            ...
            "query_frame_gap": abs(int(queries[0]["frame"]) - int(queries[1]["frame"])),
