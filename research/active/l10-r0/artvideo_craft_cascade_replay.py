...
class L10SC2CraftFallbackCascade:
    """Arbitrate a unique CRAFT semantic proposal without mixing tracker memory.
    ...
    semantic identity for the current frame, but its embedding never updates
    the RapidOCR anchor.  This prevents a detector/source switch from silently
    turning into a cross-source appearance association.
    """

    name = "L10_SC2_craft_fallback_cascade"
    ...
    def _secondary_scope_match(self, candidate: Any) -> bool:
        """CRAFT has no sub-word carrier here, so only its full box owns identity."""
    ...
    def step(self, primary: list[Any], secondary: list[Any]) -> CascadeDecision:
        ...
        if primary_matches:
            ...
            source_mode = "SECONDARY_CRAFT_UNIQUE"
...
def run_episode(
    ...
    controller = L10SC2CraftFallbackCascade(goal)
    ...
    for position, frame_id in enumerate(frame_ids[1:], start=1):
        ...
        if cascade.source_mode == "SECONDARY_CRAFT_UNIQUE":
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run L10-SC2 RapidOCR -> unique CRAFT fallback cascade.")
    ...
    result = {
        "schema": "l10_sc2_rapidocr_unique_craft_fallback_replay_v0",
        "status": "SC2_CRAFT_CASCADE_EFFECT" if all(checks.values()) else "SC2_CRAFT_CASCADE_GATE_NOT_MET",
        "videos": args.videos,
        "algorithm": {
            ...
            "fallback_admission": "Exactly one EasyOCR/CRAFT full-box text exactly matches the normalized goal. Multiple matches or token-only matches inside a larger CRAFT box abstain because this source has no sub-word carrier.",
