#!/usr/bin/env python3
"""Evaluate the frozen temporal appearance router on new-city federated targets."""
...
PROTOCOL_SCHEMA = "blindassist-l10-panolab-federated-router-confirmation-appearance-protocol-v1"
RESULT_SCHEMA = "blindassist-l10-panolab-federated-router-confirmation-appearance-result-v1"
...
def main() -> None:
    ...
    prior_result = appearance.load(appearance.verify(inputs["prior_six_target_result"]))
    selection_path = appearance.verify(inputs["confirmation_selection"])
    selection = appearance.load(selection_path)
    manifest_path = appearance.verify(inputs["confirmation_materialization"])
    manifest = appearance.load(manifest_path)
    truth = appearance.load(appearance.verify(inputs["confirmation_truth"]))
    ...
    appearance.require(
        selection["router_calls_on_all_selected_pixels_before_freeze"] == 0
        and selection["new_neighbor_router_calls_before_freeze"] == 0
        and truth["router_calls_on_selected_images_before_truth_freeze"] == 0,
        "SELECTED_PIXEL_NOT_ROUTER_UNSEEN_BEFORE_TRUTH",
    )

    thresholds = protocol["accept_contract"]
    prior_thresholds = prior_result["frozen_thresholds"]
    appearance.require(
    ...
    appearance.require(len(target_ids) == len(set(target_ids)) == 8, "TARGET_ROSTER_NOT_EIGHT")
    appearance.require(len(evaluated_ids) == len(set(evaluated_ids)) == 2, "EVALUATED_EPISODES_NOT_TWO")
    appearance.require(target_ids[-2:] == evaluated_ids, "CONFIRMATION_TARGET_ORDER_MISMATCH")

    reference_rows: dict[str, list[fingerprint.ImageRow]] = {target_id: [] for target_id in target_ids}
    prior_reference_receipts = [
        row for row in prior_result["crop_receipts"] if row["role"] == "reference"
    ]
    appearance.require(len(prior_reference_receipts) == 12, "PRIOR_REFERENCE_CROP_COUNT_NOT_TWELVE")
    all_rows: list[fingerprint.ImageRow] = []
    for receipt in prior_reference_receipts:
        ...
        all_rows.append(row)
    appearance.require(
        all(len(reference_rows[target_id]) == 2 for target_id in target_ids[:-2]),
    ...
    appearance.require(
        ...
        "CONFIRMATION_EPISODE_MISMATCH",
    )
    appearance.require(
        all(row["valid_for_fixed_replay"] for row in truth_index.values()),
        "INVALID_CONFIRMATION_TRUTH_ROW",
    )
    image_index = {row["item_id"]: row for row in manifest["images"]}
    appearance.require(len(image_index) == 10, "CONFIRMATION_IMAGE_COUNT_NOT_TEN")
    ...
    for episode_id in evaluated_ids:
            ...
            reference_metadata.append(
                {
                    ...
                    "relation": member["relation_to_anchor"],
        ...
        appearance.require(len(reference_rows[episode_id]) == 2, f"REFERENCE_COUNT:{episode_id}")
    ...
    for episode_id in evaluated_ids:
        ...
        predicted_margin = top_score - max(
    ...
    metrics = {
        ...
        "confirmation_cities": len({episode_index[row]["source_city"] for row in evaluated_ids}),
        "query_frames": sum(len(value) for value in query_rows.values()),
        "new_neighbor_frames": sum(
            not image_index[member["item_id"]]["reused_federated_anchor"]
            for episode_id in evaluated_ids
            for member in episode_index[episode_id]["queries"]
        ),
        "combined_target_roster": len(target_ids),
        "combined_reference_views": sum(len(value) for value in reference_rows.values()),
        "confirmation_reference_views": sum(len(reference_rows[row]) for row in evaluated_ids),
        "collection_disjoint_confirmation_reference_views": sum(
            row["collection_disjoint"] for row in reference_metadata
        ),
    ...
    gate = {
        "two_valid_router_unseen_truth_rows": len(evaluated_ids) == 2,
        "four_cross_collection_reference_views": metrics[
            "collection_disjoint_confirmation_reference_views"
        ] == 4,
        "six_reciprocal_query_frames": metrics["query_frames"] == 6,
    ...
    result = {
        ...
        "authority": "NEW_CITY_TARGET_WAY_ITEM_DISJOINT_FEDERATED_SAME_PROVIDER_DEVELOPMENT_CANDIDATE_ROUTING_ONLY",
