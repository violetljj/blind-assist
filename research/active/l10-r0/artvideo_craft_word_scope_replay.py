...
class L10SC3CraftWordScopeCascade:
    """Arbitrate a unique CRAFT word proposal without mixing tracker memory.
    ...
    name = "L10_SC3_craft_word_scope_cascade"
...
def run_episode(
    ...
    controller = L10SC3CraftWordScopeCascade(goal)
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Run L10-SC3 source-isolated CRAFT word-scope cascade.")
    ...
    result = {
        "schema": "l10_sc3_rapidocr_craft_word_scope_replay_v0",
        "status": "SC3_CRAFT_WORD_SCOPE_EFFECT" if all(checks.values()) else "SC3_CRAFT_WORD_SCOPE_GATE_NOT_MET",
        "videos": args.videos,
        "algorithm": {
