#!/usr/bin/env python3
"""Consumed SceneNN posthoc repair: temporally local active RoMa support."""
...
PROTOCOL_SCHEMA = "blindassist-l10-scenenn-roma-temporal-local-posthoc-protocol-v1"
ADMISSION_SCHEMA = "blindassist-l10-scenenn-roma-temporal-local-posthoc-source-admission-v1"
COHORT_SCHEMA = "blindassist-l10-scenenn-roma-temporal-local-posthoc-cohort-v1"
RECEIPT_SCHEMA = "blindassist-l10-scenenn-roma-temporal-local-posthoc-rgb-receipt-v1"
RESULT_SCHEMA = "blindassist-l10-scenenn-roma-temporal-local-posthoc-result-v1"
...
def freeze(protocol_path: Path, admission_path: Path, source_root: Path, output_path: Path) -> None:
    ...
    cohort["authority"] = "CONSUMED_POSTHOC_PRE_RGBD_TEMPORALLY_LOCAL_ROMA_NONE_COHORT"
...
def seal(cohort_path: Path, source_root: Path, extraction_root: Path, extractor_exe: Path, output_path: Path) -> None:
    ...
    receipt["authority"] = "CONSUMED_POSTHOC_TEMPORALLY_LOCAL_SPARSE_RGB_ONLY_RECEIPT"
...
def replay(protocol_path: Path, cohort_path: Path, receipt_path: Path, source_root: Path, output_path: Path) -> None:
    ...
    result["authority"] = "CONSUMED_POSTHOC_TEMPORALLY_LOCAL_ROMA_CYCLE_NONE_DEVELOPMENT_RESULT"
    result["conclusion"] = (
        "L10_SCENENN_ROMA_TEMPORAL_LOCAL_POSTHOC_DEVELOPMENT_GATE_MET"
        if result["gate_met"]
        else "L10_SCENENN_ROMA_TEMPORAL_LOCAL_POSTHOC_DEVELOPMENT_GATE_NOT_MET"
    )
