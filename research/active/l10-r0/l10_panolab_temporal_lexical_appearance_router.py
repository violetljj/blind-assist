#!/usr/bin/env python3
"""Route reciprocal query sequences through frozen lexical evidence, then appearance."""
...
PROTOCOL_SCHEMA = "blindassist-l10-panolab-temporal-lexical-appearance-router-protocol-v1"
RESULT_SCHEMA = "blindassist-l10-panolab-temporal-lexical-appearance-router-result-v1"
...
def main() -> None:
    ...
    base_protocol = lexical.load(lexical.verify(inputs["base_component_protocol"]))
    selection_path = lexical.verify(inputs["temporal_selection"])
    selection = lexical.load(selection_path)
    manifest_path = lexical.verify(inputs["temporal_materialization"])
    manifest = lexical.load(manifest_path)
    truth = lexical.load(lexical.verify(inputs["temporal_truth"]))
    appearance_result = lexical.load(lexical.verify(inputs["temporal_appearance_result"]))
    projection_protocol = lexical.load(
        lexical.verify(inputs["orientation_projection_protocol"])
    )
    lexical.require(
    ...
    lexical.require(
        ...
        "EXPECTED_TEMPORAL_APPEARANCE_FAILURE_NOT_FROZEN",
    )
    lexical.require(
        selection["new_neighbor_model_calls_before_freeze"] == 0
        and truth["new_neighbor_model_calls_before_truth_freeze"] == 0,
        "NEIGHBOR_MODEL_CALLED_BEFORE_TRUTH",
    )
    ...
    base_selection = lexical.load(lexical.verify(base_protocol["inputs"]["fresh_selection"]))
    targets = [
        {"episode_id": row["episode_id"], "entity_name": row["target_name"]}
        for row in base_selection["episodes"]
        if row["episode_id"] in protocol["target_roster"]
    ]
        ...
        targets.append(
            {
                ...
                "entity_name": episode["target_way"]["tags"]["name"],
            }
        )
    target_ids = [row["episode_id"] for row in targets]
    lexical.require(target_ids == protocol["target_roster"], f"TARGET_ROSTER_MISMATCH:{target_ids}")
    ...
    lexical.require(
        set(protocol["evaluated_episodes"]) == set(episode_index) == set(truth_index) == set(appearance_index),
    ...
    lexical_correct = 0
    lexical_wrong = 0
    lexical_ambiguous_or_unknown = 0
    combined_correct = 0
    combined_wrong = 0
    combined_unknown = 0
    ...
    for episode_id in protocol["evaluated_episodes"]:
        ...
        if matched_targets == [episode_id]:
            ...
            lexical_state = "NO_MATCH" if not matched_targets else "AMBIGUOUS_MULTIPLE_MATCHES"
            lexical_ambiguous_or_unknown += 1
    ...
    metrics = {
        "consumed_posthoc_episodes": len(protocol["evaluated_episodes"]),
        "query_frames": sum(len(row["frames"]) for row in result_rows),
        "target_roster": len(target_ids),
        "lexical_correct_wrong_ambiguous_or_unknown": [
            lexical_correct,
            lexical_wrong,
            lexical_ambiguous_or_unknown,
        ],
        "appearance_fallback_correct_wrong_unknown": [
            appearance_correct,
            0,
            len(protocol["evaluated_episodes"]) - appearance_correct,
        ],
        ...
        "wrong_target_lexical_trials": len(protocol["evaluated_episodes"]) * (len(target_ids) - 1),
        "wrong_target_lexical_matches": wrong_target_lexical_matches,
        "portal_ownership_bindings_emitted": 0,
        "wall_seconds": round(time.perf_counter() - started, 6),
    }
    gate = {
        "two_consumed_sequences_evaluated": len(result_rows) == 2,
        "six_query_frames": metrics["query_frames"] == 6,
        "two_of_two_combined_correct": combined_correct == 2,
        "minimum_one_correct_gain_over_appearance": combined_correct - appearance_correct >= 1,
    ...
    result = {
        ...
        "authority": "CONSUMED_POSTHOC_ORTHOGONAL_EVIDENCE_ROUTING_DEVELOPMENT_ONLY",
