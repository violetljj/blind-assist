#!/usr/bin/env python3
"""Evaluate the frozen lexical-then-appearance router on producer-stratified targets."""
...
import l10_panolab_federated_lexical_appearance_confirmation as prior
import l10_panolab_track_lexical_ledger as ledger
import l10_panolab_track_token_bank_fresh as lexical


PROTOCOL_SCHEMA = "blindassist-l10-panolab-producer-stratified-lexical-appearance-router-protocol-v1"
RESULT_SCHEMA = "blindassist-l10-panolab-producer-stratified-lexical-appearance-router-result-v1"
...
def main() -> None:
    ...
    selection_path = lexical.verify(inputs["stratified_selection"])
    selection = lexical.load(selection_path)
    manifest_path = lexical.verify(inputs["stratified_materialization"])
    manifest = lexical.load(manifest_path)
    truth = lexical.load(lexical.verify(inputs["stratified_truth"]))
    appearance_result = lexical.load(lexical.verify(inputs["stratified_appearance_result"]))
    ...
    lexical.require(
        ...
        == "L10_PANOLAB_PRODUCER_STRATIFIED_PIXEL_UNSEEN_APPEARANCE_ROUTER_DEVELOPMENT_GATE_NOT_MET",
        "EXPECTED_STRATIFIED_APPEARANCE_FAILURE_NOT_FROZEN",
    )
    lexical.require(
    ...
    lexical.require(len(target_ids) == len(set(target_ids)) == 11, "TARGET_ROSTER_NOT_ELEVEN")
    lexical.require(target_ids[-3:] == evaluated_ids, "EVALUATED_TARGET_ORDER_MISMATCH")
    ...
    for episode_id in evaluated_ids:
        ...
        frames = [
            prior.evaluate_frame(
                pipeline,
                prior.build_frame(episode, member, image_index[member["item_id"]]),
                projection_protocol,
                lexical_protocol,
            )
    ...
    gate = {
        "three_pixel_unseen_sequences_evaluated": len(result_rows) == 3,
        "three_producer_strata": metrics["producer_strata"] == 3,
        "nine_query_frames": metrics["query_frames"] == 9,
    ...
    result = {
        ...
        "authority": "PRODUCER_STRATIFIED_PIXEL_AND_OCR_UNSEEN_TARGET_WAY_ITEM_DISJOINT_SAME_PROVIDER_DEVELOPMENT_ROUTING_ONLY",
