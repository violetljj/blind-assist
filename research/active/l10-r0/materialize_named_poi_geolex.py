"""Materialize the metadata-frozen PB3 GeoLex cohort from Wikimedia Commons."""
...
def materialize(source_path: Path, output_root: Path, excluded: list[Path]) -> dict[str, Any]:
    ...
    for entity in entities:
        ...
        reference = _download(entity["reference"], entity_root / "reference.jpg", reuse_allowed=False)
        queries = []
            ...
            queries.append(
                {
                    ...
                    "facet": query["facet"],
                    **_download(query["commons_file"], entity_root / f"query-{index:02d}.jpg", reuse_allowed=False),
                }
            )
        rows.append(
            {
    ...
    inventory = [
        {"entity": row["id"], "role": role, "facet": image.get("facet"), "commons_file": image["commons_file"], "sha256": image["sha256"]}
        for row in rows
    ...
    payload = {
        "schema": "l10-named-poi-geolex-manifest-v1",
        "created_at": "2026-08-29",
        "source_spec": str(source_path.resolve()),
        "source_spec_sha256": _sha256(source_path),
        "excluded_manifests": [{"path": str(path.resolve()), "sha256": _sha256(path)} for path in excluded],
...
def main() -> None:
    ...
    parser.add_argument("--source", type=Path, default=Path(__file__).with_name("named_poi_geolex_source_v1.json"))
    parser.add_argument("--output-root", type=Path, default=ROOT / "artifacts.local/knowledge/named-poi-geolex-v1")
    parser.add_argument("--exclude-manifest", type=Path, action="append", required=True)
    args = parser.parse_args()
    result = materialize(args.source.resolve(), args.output_root.resolve(), [path.resolve() for path in args.exclude_manifest])
    print(json.dumps({key: result[key] for key in ("entity_counts", "query_counts", "inventory_sha256", "model_calls")}, indent=2))
