...
/**
 * Outcome-unseen R2-L1E-R2 transport amendment.
 *
 * This exporter preserves the R1 Canvas/TFLite/model/labels/threshold/NMS contract
 * while using a distinct invocation and device namespace. It has input-export
 * authority only and cannot score, select, rank, promote, or run Android shadow.
 */
@RunWith(AndroidJUnit4::class)
class UstrfR2L1ExploratoryProfileR2DeviceTest {
    ...
    @Test
    fun exportOneFrozenMaskedLedgerR2() {
        if (arguments.getString(ARG_REQUIRED)?.toBooleanStrictOrNull() != true) {
            assumeTrue("R2-L1E-R2 export was not requested", false)
            return
        }
        val inputFile = privateFile(
            requireNotNull(arguments.getString(ARG_INPUT)),
            "R2-L1E-R2 manifest"
        )
        val input = JSONObject(inputFile.readText(Charsets.UTF_8))
        check(input.getString("schema") == INPUT_SCHEMA)
        check(input.getString("stage") == "R2-L1E-R2")
        check(input.getString("attempt_namespace") == "r2-l1e-r2")
        ...
        try {
                ...
                for (index in 0 until frames.length()) {
                    ...
                    rows.put(
                        JSONObject()
                            ...
                            .put(
                                "detector_processing_latency_ns",
                                consumedNs - startedNs
                            )
            ...
            val receipt = JSONObject()
                ...
                .put("stage", "R2-L1E-R2")
                .put("attempt_namespace", "r2-l1e-r2")
                .put(
                    "status",
                ...
                .put(
                    ...
                    JSONObject()
                        .put(
                            "encoding",
                            "gzip_concatenated_float32_native_little_endian"
                        )
                        .put(
                            "bytes_per_frame_uncompressed",
                            CANONICAL_RAW_BYTES_PER_FRAME
                        )
                ...
                .put(
                    ...
                    JSONObject()
                        ...
                        .put(
                            "maximum_battery_temperature_rise_c",
                            MAX_BATTERY_RISE_C
                        )
                        .put("start_battery_temperature_c", startBatteryC)
                        .put("final_battery_temperature_c", finalBatteryC)
                        .put(
                            "observed_maximum_battery_temperature_c",
                            maxBatteryC
                        )
                        .put(
                            "observed_maximum_thermal_status",
                            maxThermalStatus
                        )
                )
                .put("frames", rows)
        ...
        check(error == null && completed == frames.length()) {
            error ?: "incomplete R2-L1E-R2 device shard"
        }
    }
    ...
    private fun privateNewOutputFile(relative: String): File =
        privateOutputFile(relative).also {
            check(!it.exists()) {
                "refusing to overwrite R2-L1E-R2 raw shard: $relative"
            }
        }
    ...
    private companion object {
        ...
        const val ARG_REQUIRED = "ustrfR2L1eR2Required"
        const val ARG_INPUT = "ustrfR2L1eR2Input"
        const val ARG_RAW_OUTPUT = "ustrfR2L1eR2RawOutput"
        const val ARG_RECEIPT_OUTPUT = "ustrfR2L1eR2ReceiptOutput"
    }
}
